...
|---|---|---|---|
| **CVH** — control volume hydrodynamics | Lumped volumes with separate pool/atmosphere, two-phase non-equilibrium, water + non-condensable gases | `tuas_boussinesq_solver` (`single_control_vol`, `array_fluid_collections`) — but **single-phase, incompressible, Boussinesq**; `tampines/hem` homogeneous-equilibrium | **Major.** Architecture right, physics one-phase. |
