...
| `htr10_settled.data` | — | `htr10_settled.csv` converted for LIGGGHTS `read_data` by `csv2data.sh` beside it — how a bed settled by **either** code is handed back to LIGGGHTS as an initial condition | — | — | — |
| `htr10_conus_t2000_liggghts.csv`, `htr10_conus_t20000_liggghts.csv`, `htr10_conus_settled_liggghts.csv` | `in.htr10_conus` | **HTR-10 conus slump**: the settled core bed drains onto the published bottom conus + discharge tube (mesh wall) and valve plane, the flat floor removed. The deck dumps every 2000 steps; the `t6000`/`t12000` frames are **gitignored** as derived data — the two committed frames carry both asserted claims. Regenerate any frame with `dumpframe.sh` | `35 µs` | 20 000 | every 2000 |
| `htr10_conus_presettled_<label>.csv` | — | **this port's own output**: the bed settled into the conus at one friction setting, cached so a re-run of that setting skips ~10 min of settling | — | — | final |
| `htr10_phi_vs_step_<label>.csv` | — | **this port's own output**: **packing fraction vs DEM step** through the conus slump at one friction setting. Written by `examples/htr10_recirculation_sweep.rs --trace-only` | `35 µs` | to equilibrium | every 500 |
| `htr10_recirc_<label>_s8k.csv` | — | **this port's own output**: the **rate-independence control** — the same recirculation at a **4x longer settle window** (8 000 steps), which is the run that is genuinely quasi-static (worst core `KE/E_drop` 2.2e-4 / 4.8e-4). Its final beds and seeded pre-settle copies are gitignored as reproducible derived data | — | 10 batches | per batch |
| `htr10_recirc_<label>.csv`, `htr10_recirc_<label>_bed.csv` | — | **this port's own output**: per-batch solid fraction during slow defuelling at one friction setting, and the final bed. Written by `examples/htr10_recirculation_sweep.rs` | — | — | per batch / final |
| `htr10_rdf.csv` | — | **analysis output** (this port's `src/rdf.rs`, CPU path): radial distribution function `g(r)` vs pair separation for every HTR-10 bed above, one tidy row per (case, bin). Written by `tests/htr10_rdf.rs` | — | — | 250 bins to `5 d` |
...
./dumpframe.sh  dump.htr10_conus 6000 htr10_conus_t6000_liggghts.csv   # ONE frame of a running dump
```

`dump2csv.sh` handles the single frame a `write_dump` emits; `dumpframe.sh`
pulls one timestep out of a multi-frame running `dump`, so a trajectory can be
compared frame by frame rather than only at its endpoint. It is verified
against `dump2csv.sh`: extracting the last frame of `dump.htr10_conus` and
converting `dump.htr10_conus_settled` give **byte-identical** files.

The reverse direction — CSV back into LIGGGHTS — is `csv2data.sh`, which emits
a `read_data` file for `atom_style granular`:

```bash
./csv2data.sh htr10_settled.csv htr10_settled.data 0.06 1730 -0.70 6.00
```

This is what lets a case start **both** codes from the identical
configuration instead of from each code's own random insertion.
