use tuas_boussinesq_solver::array_control_vol_and_fluid_component_collections::one_d_fluid_array_with_lateral_coupling::fluid_component_calculation::DimensionlessDarcyLossCorrelations;
...
use tuas_boussinesq_solver::heat_transfer_correlations::nusselt_number_correlations::input_structs::GnielinskiData;
...
use tuas_boussinesq_solver::prelude::beta_testing::{FluidArray, HeatTransferEntity};
use tuas_boussinesq_solver::single_control_vol::SingleCVNode;
...
pub fn new_reactor_vessel_pipe_1(
    ...
    fn cp_graphite(t: ThermodynamicTemperature) -> SpecificHeatCapacity {
        ...
        let cp_cal_g_kelvin = 0.538657 + 9.11129e-6 * t_kelvin
            - 90.2725 * t_kelvin.powf(-1.0)
            - 43449.3 * t_kelvin.powf(-2.0)
            + 1.59309e7 * t_kelvin.powf(-3.0)
            - 1.43688e9 * t_kelvin.powf(-4.0);

        return SpecificHeatCapacity::new::<calorie_per_gram_kelvin>(cp_cal_g_kelvin);
    }
    ...
    fn thermal_cond_grpahite(_t: ThermodynamicTemperature) -> ThermalConductivity {
        ...
        return ThermalConductivity::new::<watt_per_meter_kelvin>(36.3);
    }
    ...
    fn density_graphite(_t: ThermodynamicTemperature) -> MassDensity {
        ...
        return MassDensity::new::<gram_per_cubic_centimeter>(2.20);
    }
    ...
    let graphite = SolidMaterial::CustomSolid(
...
pub fn new_fhr_pri_loop_pump_9(
    initial_temperature: ThermodynamicTemperature,
...
///
pub fn new_ihx_sthe_6_version_1(
    initial_temperature: ThermodynamicTemperature,
    ...
    let tube_side_flow_area_single_tube = flow_area / number_of_tubes as f64;
    ...
    let tube_side_length_to_diameter: Ratio = sthe_length / tube_side_hydraulic_diameter;
    let _tube_side_gnielinski_data: GnielinskiData = GnielinskiData {
    ...
    let tube_side_nusselt_correlation = NusseltCorrelation::IdealNusseltOneBillion;
    ...
    let shell_side_length_to_diameter: Ratio = sthe_length / shell_side_hydraulic_diameter;
    ...
    let shell_side_nusselt_correlation_to_tubes =
        NusseltCorrelation::PipeGnielinskiGeneric(shell_side_gnielinski_data);
...
pub fn new_fhr_intermediate_loop_pump_16(
    initial_temperature: ThermodynamicTemperature,
) -> NonInsulatedFluidComponent {
    let ambient_temperature = ThermodynamicTemperature::new::<degree_celsius>(20.0);
    let fluid_pressure = Pressure::new::<atmosphere>(1.0);
...
/// goes up by 1.0 m
pub fn new_fhr_intermediate_loop_steam_generator_shell_side_14(
    initial_temperature: ThermodynamicTemperature,
) -> NonInsulatedFluidComponent {
    let ambient_temperature = ThermodynamicTemperature::new::<degree_celsius>(20.0);
    let fluid_pressure = Pressure::new::<atmosphere>(1.0);
    ...
    let mut fluid_array_nusselt_adjust: FluidArray = non_insulated_component
        .pipe_fluid_array
        .clone()
        .try_into()
        .unwrap();

    fluid_array_nusselt_adjust.nusselt_correlation = ideal_nusselt;

    non_insulated_component.pipe_fluid_array = fluid_array_nusselt_adjust.into();
...
pub fn gfhr_bottom_mixing_node_pri_loop(
    initial_temperature: ThermodynamicTemperature,
) -> HeatTransferEntity {
...
pub fn gfhr_top_mixing_node_pri_loop(
    initial_temperature: ThermodynamicTemperature,
) -> HeatTransferEntity {
    let mixing_node_diameter = Length::new::<inch>(14.0);
    let mixing_node_material = LiquidMaterial::FLiBe;
...
pub fn gfhr_bottom_mixing_node_intrmd_loop(
    initial_temperature: ThermodynamicTemperature,
) -> HeatTransferEntity {
    let mixing_node_diameter = Length::new::<inch>(14.0);
...
pub fn gfhr_top_mixing_node_intrmd_loop(
    initial_temperature: ThermodynamicTemperature,
) -> HeatTransferEntity {
    let mixing_node_diameter = Length::new::<inch>(14.0);
    let mixing_node_material = LiquidMaterial::HITEC;
