...
use tampines_steam_tables::interfaces::functional_programming::{
    ph_flash_eqm, ps_flash_eqm, pt_flash_eqm,
};
...
pub(crate) fn build_steam_generator_tube() -> TampinesSteamArray {
    ...
    tube.set_uniform_velocity_field(Velocity::new::<meter_per_second>(
        SG_TUBE_NOMINAL_VELOCITY_M_PER_S,
    ));
    tube
}
...
impl FHRSimulatorApp {
    ...
    pub(crate) fn secondary_loop_single_timestep(
        ...
        let heat_rate_to_steam_generator_tube =
            -fhr_th_state.heat_added_to_steam_generator_shell_side / timestep;
        ...
        let condenser_outlet_temperature = ThermodynamicTemperature::new::<degree_celsius>(35.0);
        ...
        let steam_quality_after_condenser =
            x_ps_flash(condenser_outlet_pressure, condenser_outlet_entropy);
        ...
        if user_specified_pump_outlet_pressure < one_bar {
            pump_outlet_temperature = ps_flash_eqm::t_ps_eqm(one_bar, condenser_outlet_entropy);
            sg_inlet_pressure = one_bar;
        } else {
            pump_outlet_temperature = ps_flash_eqm::t_ps_eqm(
        ...
        let steam_generator_tube_inlet_enthalpy: AvailableEnergy =
            pt_flash_eqm::h_tp_eqm_single_phase(pump_outlet_temperature, sg_inlet_pressure);

        let pressure_after_pump = sg_inlet_pressure;
        let enthalpy_after_pump = steam_generator_tube_inlet_enthalpy;

        let steam_quality_after_pump = x_ph_flash(pressure_after_pump, enthalpy_after_pump);

        // now, if mass flowrate is less than some value, crash the
        // system because the steam generator is too hot
        if *user_specified_secondary_loop_mass_flowrate < MassRate::new::<kilogram_per_second>(0.1)
        {
        ...
        // `build_steam_generator_tube`) and its outlet enthalpy is read back.
        {
            ...
            // outlet (T, p).
            let feed_rho =
                pt_flash_eqm::v_tp_eqm_single_phase(pump_outlet_temperature, sg_inlet_pressure)
                    .recip();
            let inlet_velocity: Velocity = *user_specified_secondary_loop_mass_flowrate
                / feed_rho
                / Area::new::<square_meter>(SG_TUBE_AREA_M2);
        ...
        let steam_generator_tube_outlet_enthalpy = steam_generator_tube.get_outlet_enthalpy();
        ...
        let steam_gen_tube_outlet_temperature =
            ph_flash_eqm::t_ph_eqm(sg_outlet_pressure, steam_generator_tube_outlet_enthalpy);
        ...
        let pressure_after_steam_generator_tube = turbine_inlet_pressure;
        let enthalpy_after_steam_generator_tube = turbine_inlet_enthalpy;
        let steam_quality_after_steam_generator_tube_side = x_ph_flash(
        ...
        let turbine_outlet_pressure = Pressure::new::<bar>(0.2);
        let turbine_outlet_entropy = turbine_inlet_entropy;

        let turbine_outlet_enthalpy =
            ps_flash_eqm::h_ps_eqm(turbine_outlet_pressure, turbine_outlet_entropy);

        let steam_quality_after_turbine =
            x_ph_flash(turbine_outlet_pressure, turbine_outlet_enthalpy);

        // now it goes into the condenser
        let condenser_inlet_enthalpy = turbine_outlet_enthalpy;
        let condenser_outlet_enthalpy = pt_flash_eqm::h_tp_eqm_single_phase(
            condenser_outlet_temperature,
            condenser_outlet_pressure,
        );

        // this is the required cooling rate for condenser at steady state
        let condenser_duty: Power = *user_specified_secondary_loop_mass_flowrate
            * (condenser_inlet_enthalpy - condenser_outlet_enthalpy);

        // now the steam turbine,
        let mut steam_turbine = ThreePhaseElectricGeneratorTurbine::new_250_megawatt_generator();
        ...
        let turbine_power_input: Power =
            *user_specified_secondary_loop_mass_flowrate * turbine_enthalpy_loss;

        let turbine_inlet_area = Length::new::<inch>(20.0) * Length::new::<inch>(20.0);

        let turbine_radius = Length::new::<meter>(5.0);
        let turbine_blade_speed: Velocity = turbine_radius * turbine_omega;
        ...
        turbine_steam_speed =
            *user_specified_secondary_loop_mass_flowrate / turbine_inlet_rho / turbine_inlet_area;

        // note: using relative velocity is kind of unstable
        // numerically
        let _turbine_steam_relative_velocity: Velocity = turbine_steam_speed - turbine_blade_speed;

        let turbine_force: Force = turbine_power_input / turbine_blade_speed;
        ...
        torque_source = (turbine_force * turbine_radius * efficiency_factor).into();
        ...
        let secondary_loop_state = SecondaryLoopState {
