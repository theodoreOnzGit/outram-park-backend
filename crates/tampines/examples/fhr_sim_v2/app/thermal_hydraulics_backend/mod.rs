...
use tuas_boussinesq_solver::prelude::beta_testing::{
    FluidArray, HeatTransferEntity, HeatTransferInteractionType,
};
...
impl FHRSimulatorApp {
    ...
    pub(crate) fn four_branch_pri_and_intermediate_loop_single_time_step(
        ...
        let average_flibe_density = LiquidMaterial::FLiBe
            .try_get_density(average_temperature_for_density_calcs_pri_loop)
            .unwrap();
        ...
        let average_hitec_density = LiquidMaterial::HITEC
            .try_get_density(average_temperature_for_density_calcs_intrmd_loop)
            .unwrap();
        ...
        let heat_added_to_steam_generator: Energy;
        {
            ...
            q_frac_arr[0] = 0.5;
            q_frac_arr[1] = 0.5;
            let mut sg_fluid_array_clone: FluidArray = fhr_steam_generator_shell_side_14
                .pipe_fluid_array
                .clone()
                .try_into()
                .unwrap();
            ...
            let temperature_diff = TemperatureInterval::new::<uom::si::temperature_interval::kelvin>(
                sg_fluid_array_clone
                    .try_get_bulk_temperature()
                    .unwrap()
                    .get::<degree_celsius>()
                    - steam_generator_tube_side_temperature.get::<degree_celsius>(),
            );
            ...
            heat_added_to_steam_generator = steam_gen_heat_change * timestep;

            sg_fluid_array_clone
                .lateral_link_new_power_vector(steam_gen_heat_change, q_frac_arr)
                .unwrap();

            fhr_steam_generator_shell_side_14.pipe_fluid_array = sg_fluid_array_clone.into();
        }
        ...
        // power input here
        {
            ...
            let mut q_frac_arr: Array1<f64> =
                Array::default(number_of_temperature_nodes_for_reactor);
            // we want the middle node to contain all the power
            q_frac_arr[0] = 0.0;
            q_frac_arr[1] = 0.0;
            q_frac_arr[2] = 1.0;
            ...
            let mut reactor_fluid_array_clone: FluidArray =
                reactor_pipe_1.pipe_fluid_array.clone().try_into().unwrap();

            reactor_fluid_array_clone
                .lateral_link_new_power_vector(reactor_power, q_frac_arr)
                .unwrap();

            reactor_pipe_1.pipe_fluid_array = reactor_fluid_array_clone.into();
        ...
        {
            // pri loop (with ihx)
            reactor_pipe_1.advance_timestep(timestep).unwrap();
            downcomer_pipe_2.advance_timestep(timestep).unwrap();
            downcomer_pipe_3.advance_timestep(timestep).unwrap();

            fhr_pipe_4.advance_timestep(timestep).unwrap();
            fhr_pipe_5.advance_timestep(timestep).unwrap();
            fhr_pipe_7.advance_timestep(timestep).unwrap();
            fhr_pipe_8.advance_timestep(timestep).unwrap();
            fhr_pri_loop_pump_9.advance_timestep(timestep).unwrap();
            fhr_pipe_10.advance_timestep(timestep).unwrap();
            fhr_pipe_11.advance_timestep(timestep).unwrap();

            // intermediate branch (less ihx)
            fhr_pipe_12.advance_timestep(timestep).unwrap();
            fhr_pipe_17.advance_timestep(timestep).unwrap();
            fhr_pipe_13.advance_timestep(timestep).unwrap();
            fhr_steam_generator_shell_side_14
                .advance_timestep(timestep)
                .unwrap();
            fhr_pipe_15.advance_timestep(timestep).unwrap();
            fhr_intrmd_loop_pump_16.advance_timestep(timestep).unwrap();
            ...
            ihx_sthe_6.advance_timestep(timestep).unwrap();
        }

        // now I want reactor temperature profile
        let reactor_temp_profile: Vec<ThermodynamicTemperature> =
            reactor_pipe_1.pipe_fluid_array_temperature().unwrap();
        let reactor_temp_profile_degc: Vec<f64> = reactor_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();

        // sthe temperature profile
        let ihx_shell_side_temp_profile: Vec<ThermodynamicTemperature> =
            ihx_sthe_6.shell_side_fluid_array_temperature().unwrap();

        let ihx_shell_side_temp_profile_degc: Vec<f64> = ihx_shell_side_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();

        let ihx_tube_side_temp_profile: Vec<ThermodynamicTemperature> =
            ihx_sthe_6.inner_tube_fluid_array_temperature().unwrap();

        let ihx_tube_side_temp_profile_degc: Vec<f64> = ihx_tube_side_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        ...
        let sg_shell_side_temp_profile_degc: Vec<f64> = sg_shell_side_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        ...
        let pipe_4_temp_profile: Vec<ThermodynamicTemperature> =
            fhr_pipe_4.pipe_fluid_array_temperature().unwrap();

        let pipe_4_temp_profile_degc: Vec<f64> = pipe_4_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        // pipe 5, just before STHE
        let pipe_5_temp_profile: Vec<ThermodynamicTemperature> =
            fhr_pipe_5.pipe_fluid_array_temperature().unwrap();

        let pipe_5_temp_profile_degc: Vec<f64> = pipe_5_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        // pipe 7, just after STHE
        let pipe_7_temp_profile: Vec<ThermodynamicTemperature> =
            fhr_pipe_7.pipe_fluid_array_temperature().unwrap();

        let pipe_7_temp_profile_degc: Vec<f64> = pipe_7_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        // pipe 8, just before pump
        let pipe_8_temp_profile: Vec<ThermodynamicTemperature> =
            fhr_pipe_8.pipe_fluid_array_temperature().unwrap();

        let pipe_8_temp_profile_degc: Vec<f64> = pipe_8_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        ...
        let pipe_10_temp_profile: Vec<ThermodynamicTemperature> =
            fhr_pipe_10.pipe_fluid_array_temperature().unwrap();

        let pipe_10_temp_profile_degc: Vec<f64> = pipe_10_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        ...
        let pipe_11_temp_profile: Vec<ThermodynamicTemperature> =
            fhr_pipe_11.pipe_fluid_array_temperature().unwrap();

        let pipe_11_temp_profile_degc: Vec<f64> = pipe_11_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        ...
        let pipe_12_temp_profile: Vec<ThermodynamicTemperature> =
            fhr_pipe_12.pipe_fluid_array_temperature().unwrap();

        let pipe_12_temp_profile_degc: Vec<f64> = pipe_12_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        ...
        let pipe_13_temp_profile: Vec<ThermodynamicTemperature> =
            fhr_pipe_13.pipe_fluid_array_temperature().unwrap();

        let pipe_13_temp_profile_degc: Vec<f64> = pipe_13_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        ...
        let pipe_15_temp_profile: Vec<ThermodynamicTemperature> =
            fhr_pipe_15.pipe_fluid_array_temperature().unwrap();

        let pipe_15_temp_profile_degc: Vec<f64> = pipe_15_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        ...
        let pipe_17_temp_profile: Vec<ThermodynamicTemperature> =
            fhr_pipe_17.pipe_fluid_array_temperature().unwrap();

        let pipe_17_temp_profile_degc: Vec<f64> = pipe_17_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        ...
        let pump_9_temp_profile: Vec<ThermodynamicTemperature> =
            fhr_pri_loop_pump_9.pipe_fluid_array_temperature().unwrap();

        let pump_9_temp_profile_degc: Vec<f64> = pump_9_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();

        // intrmd pump
        let pump_16_temp_profile: Vec<ThermodynamicTemperature> = fhr_intrmd_loop_pump_16
            .pipe_fluid_array_temperature()
            .unwrap();

        let pump_16_temp_profile_degc: Vec<f64> = pump_16_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
        ...
        let downcomer_2_temp_profile: Vec<ThermodynamicTemperature> =
            downcomer_pipe_2.pipe_fluid_array_temperature().unwrap();

        let downcomer_2_temp_profile_degc: Vec<f64> = downcomer_2_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();

        let downcomer_3_temp_profile: Vec<ThermodynamicTemperature> =
            downcomer_pipe_3.pipe_fluid_array_temperature().unwrap();

        let downcomer_3_temp_profile_degc: Vec<f64> = downcomer_3_temp_profile
            .into_iter()
            .map(|temperature| (temperature.get::<degree_celsius>() * 100.0).round() / 100.0)
            .collect();
    ...
    pub fn calculate_thermal_hydraulics_loop(fhr_state: Arc<Mutex<FHRState>>) {
        ...
        let initial_temperature_intrmd_loop = initial_temperature;
        // intermediate loop ihx side
        // (excluding sthe)
        ...
        // intermediate loop steam generator side
        let mut fhr_intrmd_loop_pump_16 =
            new_fhr_intermediate_loop_pump_16(initial_temperature_intrmd_loop);
        ...
        // mixing nodes for pri loop
        let mut bottom_mixing_node_pri_loop = gfhr_bottom_mixing_node_pri_loop(initial_temperature);
        let mut top_mixing_node_pri_loop = gfhr_top_mixing_node_pri_loop(initial_temperature);
        // mixing nodes for intermediate loop
        ...
        let mut current_fhr_steam_gen_state: SecondaryLoopState = SecondaryLoopState {
            steam_gen_tube_outlet_temperature: ThermodynamicTemperature::new::<degree_celsius>(
                35.0,
            ),
        ...
        loop {
            ...
            //
            let accumulated_energy_from_prke = Energy::new::<kilojoule>(
                fhr_state_clone
                    .lock()
                    .unwrap()
                    .prke_loop_accumulated_heat_removal_kilojoules,
            );
            ...
            let reactor_power = accumulated_energy_from_prke / thermal_hydraulics_timestep;

            // then primary loop pressure

            pri_loop_pump_pressure = Pressure::new::<kilopascal>(
                -fhr_state_clone
                    .lock()
                    .unwrap()
                    .fhr_pri_loop_pump_pressure_kilopascals,
            );
            intrmd_loop_pump_pressure = Pressure::new::<kilopascal>(
                -fhr_state_clone
                    .lock()
                    .unwrap()
                    .fhr_intermediate_loop_pump_pressure_kilopascals,
            );
            // steam generator settings
            //
            ...
            let num_of_nodes_in_sg_shell: f64 = fhr_state_clone
                .lock()
                .unwrap()
                .sg_shell_14_temperature_vector_degc
                .len() as f64;
            let sg_shell_14_sum_temp_degc: f64 = fhr_state_clone
                .lock()
                .unwrap()
                .sg_shell_14_temperature_vector_degc
                .iter()
            ...
            let sg_shell_14_avg_temp: f64 = sg_shell_14_sum_temp_degc / num_of_nodes_in_sg_shell;
            ...
            let degree_of_superheat: TemperatureInterval =
                TemperatureInterval::new::<uom::si::temperature_interval::kelvin>(
                    sat_temperature_in_sg_tube_degc - sg_shell_14_avg_temp,
                );

            let mut critical_heat_flux_ua_modifier: f64 =
                pool_boiling_improvised_correlation_as_fraction_of_maximum(degree_of_superheat);
            ...
            let sg_tube_steam_quality =
                current_fhr_steam_gen_state.steam_quality_after_steam_generator_tube_side;
            ...
            let mut steam_generator_overall_ua: ThermalConductance =
                ThermalConductance::new::<watt_per_kelvin>(
                    fhr_state_clone
                        .lock()
                        .unwrap()
                        .user_specified_secondary_loop_ua_watt_per_kelvin
                        * critical_heat_flux_ua_modifier,
                );
            if !chf_on {
                steam_generator_overall_ua = ThermalConductance::new::<watt_per_kelvin>(
                    fhr_state_clone
                        .lock()
                        .unwrap()
                        .user_specified_secondary_loop_ua_watt_per_kelvin,
            ...
            let user_specified_pump_outlet_pressure = Pressure::new::<bar>(
                fhr_state_clone
                    .lock()
                    .unwrap()
            ...
            let turbine_omega: AngularVelocity = AngularVelocity::new::<revolution_per_minute>(
            ...
            current_fhr_steam_gen_state = Self::secondary_loop_single_timestep(
            ...
            *&mut fhr_state_clone
                .lock()
                .unwrap()
                .thermal_hydraulics_simulation_time_seconds = elapsed_time_seconds;
            ...
            *&mut fhr_state_clone
                .lock()
                .unwrap()
                .thermal_hydraulics_timestep_microseconds =
                thermal_hydraulics_timestep.get::<microsecond>().round();
            *&mut fhr_state_clone
                .lock()
                .unwrap()
                .thermal_hydraulics_calc_time_microseconds =
            ...
            // update temperatures
            {
                ...
                fhr_state_lock.core_inlet_temp_degc =
                    current_fhr_thermal_hydraulics_state.reactor_temp_profile_degc[0];
                fhr_state_lock.core_bottom_temp_degc =
                    current_fhr_thermal_hydraulics_state.reactor_temp_profile_degc[1];
                fhr_state_lock.pebble_bed_coolant_temp_degc =
                    current_fhr_thermal_hydraulics_state.reactor_temp_profile_degc[2];
                fhr_state_lock.core_top_temp_degc =
                    current_fhr_thermal_hydraulics_state.reactor_temp_profile_degc[3];
                fhr_state_lock.core_outlet_temp_degc =
                    current_fhr_thermal_hydraulics_state.reactor_temp_profile_degc[4];
                ...
                fhr_state_lock.left_downcomer_lower_temp_degc =
                    current_fhr_thermal_hydraulics_state.downcomer_2_temp_profile_degc[0];
                fhr_state_lock.left_downcomer_mid_temp_degc =
                    current_fhr_thermal_hydraulics_state.downcomer_2_temp_profile_degc[2];
                fhr_state_lock.left_downcomer_upper_temp_degc =
                    current_fhr_thermal_hydraulics_state.downcomer_2_temp_profile_degc[4];

                fhr_state_lock.right_downcomer_lower_temp_degc =
                    current_fhr_thermal_hydraulics_state.downcomer_3_temp_profile_degc[0];
                fhr_state_lock.right_downcomer_mid_temp_degc =
                    current_fhr_thermal_hydraulics_state.downcomer_3_temp_profile_degc[2];
                fhr_state_lock.right_downcomer_upper_temp_degc =
                    current_fhr_thermal_hydraulics_state.downcomer_3_temp_profile_degc[4];
                ...
                fhr_state_lock.downcomer2_branch_flowrate_kg_per_s =
                    (current_fhr_thermal_hydraulics_state
                        ...
                        * 1000.0)
                        / 1000.0;
                fhr_state_lock.ihx_branch_flowrate_kg_per_s = (current_fhr_thermal_hydraulics_state
                    .intermediate_heat_exchanger_branch_flow
                    .get::<kilogram_per_second>()
                    * 1000.0)
                    / 1000.0;
                ...
                fhr_state_lock.pipe_4_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.pipe_4_temp_profile_degc;
                fhr_state_lock.pipe_5_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.pipe_5_temp_profile_degc;
                fhr_state_lock.ihx_shell_6_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.ihx_shell_side_temp_profile_degc;
                fhr_state_lock.pipe_7_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.pipe_7_temp_profile_degc;
                fhr_state_lock.pipe_8_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.pipe_8_temp_profile_degc;
                fhr_state_lock.pri_pump_9_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.pump_9_temp_profile_degc;
                fhr_state_lock.pipe_10_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.pipe_10_temp_profile_degc;
                fhr_state_lock.pipe_11_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.pipe_11_temp_profile_degc;
                // intermediate loop state
                fhr_state_lock.ihx_tube_6_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.ihx_tube_side_temp_profile_degc;
                fhr_state_lock.pipe_12_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.pipe_12_temp_profile_degc;
                fhr_state_lock.pipe_13_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.pipe_13_temp_profile_degc;
                // sg = steam generator
                fhr_state_lock.sg_shell_14_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.sg_shell_side_temp_profile_degc;
                fhr_state_lock.pipe_15_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.pipe_15_temp_profile_degc;
                fhr_state_lock.intrmd_pump_16_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.pump_16_temp_profile_degc;
                fhr_state_lock.pipe_17_temperature_vector_degc =
                    current_fhr_thermal_hydraulics_state.pipe_17_temp_profile_degc;

                // secondary loop state
                fhr_state_lock.user_specified_secondary_loop_mass_flowrate_kg_per_s =
                    (user_specified_secondary_loop_mass_flowrate.get::<kilogram_per_second>()
                        * 1000.0)
                        / 1000.0;

                fhr_state_lock.steam_generator_tube_outlet_temperature_degc =
                    (current_fhr_steam_gen_state
                        .steam_gen_tube_outlet_temperature
                        .get::<degree_celsius>()
                        * 1000.0)
                        / 1000.0;
                fhr_state_lock.steam_quality_after_condenser =
                    (current_fhr_steam_gen_state.steam_quality_after_condenser * 1000.0) / 1000.0;
                fhr_state_lock.steam_quality_after_pump =
                    (current_fhr_steam_gen_state.steam_quality_after_pump * 1000.0) / 1000.0;
                fhr_state_lock.steam_quality_after_steam_generator_tube_side =
                    (current_fhr_steam_gen_state.steam_quality_after_steam_generator_tube_side
                        * 1000.0)
                        / 1000.0;
                fhr_state_lock.steam_quality_after_turbine =
                    (current_fhr_steam_gen_state.steam_quality_after_turbine * 1000.0) / 1000.0;
                fhr_state_lock.turbine_rpm = (current_fhr_steam_gen_state
                    .steam_turbine
                    .get_omega()
                    .get::<revolution_per_minute>()
                    * 1000.0)
                    / 1000.0;
                fhr_state_lock.turbine_power_megawatts =
                    (current_fhr_steam_gen_state.turbine_power.get::<megawatt>() * 1000.0) / 1000.0;
                fhr_state_lock.condenser_duty_megawatts =
                    (current_fhr_steam_gen_state.condenser_duty.get::<megawatt>() * 1000.0)
                        / 1000.0;
            }

            let time_to_sleep_microseconds: u64 = (thermal_hydraulics_timestep.get::<microsecond>()
                - time_taken_for_calculation_loop_microseconds)
                .round()
                .abs() as u64;
