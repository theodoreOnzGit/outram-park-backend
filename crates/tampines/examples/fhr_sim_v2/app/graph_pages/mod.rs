...
impl FHRSimulatorApp {
    pub fn reactor_power_page_graph(&mut self, ui: &mut Ui) {
        ...
        egui::ScrollArea::both().show(ui, |ui| {
            ...
            reactor_power_plot = reactor_power_plot
                .x_axis_label("time (seconds), current time (seconds): ".to_owned());
            reactor_power_plot =
                reactor_power_plot.y_axis_label("Reactor Power (Megawatts)".to_owned());
            ...
            reactor_power_plot.show(ui, |plot_ui| {
                plot_ui.line(Line::new(
                    "Reactor power with decay heat Megawatts",
                    PlotPoints::from(time_reactor_power_vec.clone()),
                ));
                plot_ui.line(Line::new(
                    "Reactor power without decay heat Megawatts",
                    PlotPoints::from(time_reactor_power_no_decay_heat_vec.clone()),
                ));
