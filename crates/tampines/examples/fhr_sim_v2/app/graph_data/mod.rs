...
impl PagePlotData {
    ...
    pub fn insert_reactor_power_data(
        &mut self,
        ...
        let mut current_heater_data_vec: Vec<(Time, Power, Power)>;

        current_heater_data_vec = self
        ...
        let mut new_array_to_be_put_back: Vec<(Time, Power, Power)> =
            vec![(Time::ZERO, Power::ZERO, Power::ZERO); NUM_DATA_PTS_IN_PLOTS];
    ...
    pub fn get_reactor_power_megawatts_vs_time_secs_vec(&self) -> Vec<[f64; 2]> {
        let time_reactor_power_vec: Vec<[f64; 2]> = self
            .reactor_power_plot_data
            .iter()
            .map(|tuple| {
        ...
        let time_reactor_power_vec: Vec<[f64; 2]> = self
            ...
            .map(|tuple| {
                ...
                if reactor_power_less_decay_heat.get::<megawatt>() > 0.0 {
                    [
                        time.get::<second>(),
                        reactor_power_less_decay_heat.get::<megawatt>(),
                    ]
                } else {
                    // don't return anything, a default 0.0 will do
                    // this is the initial condition
