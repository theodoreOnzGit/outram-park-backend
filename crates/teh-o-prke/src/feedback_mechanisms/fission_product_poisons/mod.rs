...
impl Xenon135Poisoning {
    ...
    #[inline]
    fn calc_iodine_135_and_return_conc(
        &mut self,
        timestep: Time,
        ...
        let new_iodine_conc: VolumetricNumberDensity =
            (rhs / (Ratio::new::<ratio>(1.0) + timestep * Self::iodine_135_decay_const())).into();
    ...
    pub fn calc_xe_135_and_return_num_density(
        ...
        let iodine_conc =
            self.calc_iodine_135_and_return_conc(timestep, fission_rate, fissioning_nuclide);

        let xe135_addition_rate_from_iodine: VolumetricNumberRate = (iodine_conc * lambda_i).into();
        ...
        // neutron flux  = n(t) * v
        let thermal_neutron_flux = thermal_neutron_conc * Velocity::new::<meter_per_second>(2200.0);

        let micro_xs_abs_xe_135: Area = Self::xe135_thermal_abs_xs();

        let denominator = Ratio::new::<ratio>(1.0)
            + lambda_x * timestep
            + thermal_neutron_flux * micro_xs_abs_xe_135 * timestep;

        let xe_conc_next_timestep: VolumetricNumberDensity = (rhs / denominator).into();
    ...
    pub fn simplified_poison_concentration_feedback(poison_conc: MassConcentration) -> Ratio {
        ...
        let avogadro_constant = Ratio::new::<ratio>(6.022e23) / AmountOfSubstance::new::<mole>(1.0);

        let xe135_number_density: VolumetricNumberDensity =
            (xe135_molar_density * avogadro_constant).into();

        let xe135_macro_xs: ReciprocalLength = xe135_number_density * Self::xe135_thermal_abs_xs();

        let enrichment_fraction_u235: f64 = 0.199;

        let u235_macro_abs_xs: ReciprocalLength = enrichment_fraction_u235
            * Self::uranium_number_density_est_in_uo2()
            * Self::u235_thermal_abs_xs();

        let u238_macro_abs_xs: ReciprocalLength = (1.0 - enrichment_fraction_u235)
            * Self::uranium_number_density_est_in_uo2()
            * Self::u238_thermal_abs_xs();

        let change_in_thermal_utilisation_factor: Ratio = (u235_macro_abs_xs + u238_macro_abs_xs)
    ...
    pub fn get_current_xe135_conc(&self) -> MassConcentration {
        ...
        let xe135_mass_conc = xe135_molar_density * Self::gaseous_xe135_molar_mass();
