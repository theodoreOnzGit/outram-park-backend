...
impl SixGroupPRKE {
    ...
    pub fn solve_next_timestep_precursor_concentration_and_neutron_pop_vector_explicit(
        ...
        // use si units
        let prke_ode_system = |_t: f64, y: &Vec<f64>| -> Vec<f64> {
            ...
            let total_decay_rate: VolumetricNumberRate = decay_rate_1
                + decay_rate_2
                + decay_rate_3
                + decay_rate_4
                + decay_rate_5
        ...
        let current_neutron_pop_and_precursor_vector_si_units: Vec<f64> = vec![
        ...
        let precursor_and_neutron_pop_and_source_vector_next_timestep: Vec<
            VolumetricNumberDensity,
        > = next_timestep_neutron_pop_and_precursor_vector_si_units
            .into_iter()
            .map(|num_density_f64| VolumetricNumberDensity::new::<per_cubic_meter>(num_density_f64))
            .collect();
