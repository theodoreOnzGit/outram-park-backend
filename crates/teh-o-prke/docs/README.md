...
This folder already has `notes.md` (hand-maintained) and `prke_theory.typ`/`.bib`/`.pdf` — a theory document written in **Typst**, not LaTeX.

**Exception, confirmed 2026-07-10:** `prke_theory.typ` is left as-is, not converted to LaTeX or restructured into the bite-sized 4-part series — it predates this convention and conversion risked introducing transcription errors into a physics document for no clear benefit. Any genuinely *new* topic written under this crate's `docs/` going forward should use the bite-sized LaTeX convention (see `outram-park-fork-coolprop/docs/`); `prke_theory.typ` itself is not touched.
