# outram-foam-appbuilder-lib — C++ refs, planned modules & implementation order

Reference material: the OpenFOAM C++ source paths for each ported solver, the
planned module layout (`io::` case-file parsers and `solvers::` loops with the
equations each solves), and the implementation order. Consulted on demand when
building a module — not per-turn guidance. The crate's design rules and
conventions live in CLAUDE.md.

OpenFOAM reference C++ source tree: `/home/teddy0/Documents/research/openfoam/`
...
The flux splitting is in `outram-foam-basic-lib` (Kurganov-Tadmor fluxes);
