...
> **Named 2026-09-18** by the maintainer, replacing the provisional
> `outram-park-fork-cyclus` the porting branch carried (its own workspace entry
> said "naming provisional per maintainer"). KAKI BUKIT follows the Singapore
> place-name convention the ecosystem uses for first-party crates — Tampines,
> Bedok, Changi, Boon Lay, Farrer Park, Nee Soon, Kovan, Dhoby Ghaut — rather
> than the `outram-park-fork-*` prefix reserved for ports that stay close to
> their upstream's shape. See `docs/ecosystem-naming.md`.
...
> `kaki-bukit` follows the workspace's `outram-park-fork-<project>`
...
```bash
cargo build --release -p kaki-bukit
cargo test  --release -p kaki-bukit --lib --tests
```
