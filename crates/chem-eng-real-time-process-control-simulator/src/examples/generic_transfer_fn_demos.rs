...
pub(crate) fn stable_second_order_simulation_with_delay(){
    ...
    let a1: TimeSquared = one_second * Time::new::<second>(3.0);
    let b1: Time = -Time::new::<second>(2.0);
    let c1: Ratio = Ratio::new::<ratio>(4.0);

    let a2: TimeSquared = one_second * Time::new::<second>(3.0);
    let b2: Time = Time::new::<second>(4.0);
    let c2: Ratio = Ratio::new::<ratio>(5.0);
