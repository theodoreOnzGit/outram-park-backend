...
use crate::alpha_nightly::stable_transfer_functions::first_order_transfer_fn::FirstOrderStableTransferFnNoZeroes;
use crate::alpha_nightly::stable_transfer_functions::first_order_transfer_fn_with_zeroes::FirstOrderStableTransferFnForZeroes;
...
impl TransferFnTraits for TransferFnFirstOrder {
    fn set_dead_time(&mut self, dead_time: Time) {
        match self {
            TransferFnFirstOrder::Stable(transfer_fn_no_zeroes, transfer_fn_for_zeroes) => {
    ...
    fn set_user_input_and_calc(
        ...
        match self {
            TransferFnFirstOrder::Stable(first_order_no_zeroes, first_order_zeroes) => {
                let mut response: Ratio = Ratio::ZERO;

                let first_order_output_1 =
                    first_order_no_zeroes.set_user_input_and_calc_output(time, user_input)?;

                let first_order_output_2 =
                    first_order_zeroes.set_user_input_and_calc_output(time, user_input)?;
...
impl TransferFnFirstOrder {
    ...
    pub fn new(
        ...
        if tau_p_value < 0.0 {
            ...
            // stable system
            let first_ord_transfer_fn_no_zeroes = FirstOrderStableTransferFnNoZeroes::new(
                k_p,
                tau_p,
                Ratio::ZERO,
                Ratio::ZERO,
            ...
            let first_ord_transfer_fn_for_zeroes = FirstOrderStableTransferFnForZeroes::new(
                k_p_for_zero,
                tau_p,
                Ratio::ZERO,
                Ratio::ZERO,
...
impl TryFrom<TransferFn> for TransferFnFirstOrder {
    ...
    fn try_from(generic_transfer_function: TransferFn) -> Result<Self, Self::Error> {
        if let TransferFn::FirstOrder(second_order) = generic_transfer_function {
