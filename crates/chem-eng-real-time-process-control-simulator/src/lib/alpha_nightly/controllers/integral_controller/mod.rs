...
impl Default for IntegralController {
    /// returns 1/s
    fn default() -> Self {
        Self::new(Ratio::new::<ratio>(1.0), Time::new::<second>(1.0)).unwrap()
    }
}
...
impl TransferFnTraits for IntegralController {
    ...
    fn set_user_input_and_calc(
        ...
        let delay_fn_output: Ratio = self
            .delay_function
            .set_user_input_and_calc(user_input, time_of_input)?;

        // now pipe the delay fn output into the ramp function
        let integral_controller_output = self
            .ramp_function
            .set_user_input_and_calc(delay_fn_output, time_of_input)?;
...
impl IntegralController {
    ...
    pub fn new(
        ...
        let ramp_function = RampResponseRealTime::new(integral_time, controller_gain)?;
        ...
        let delay_function: TransferFnFirstOrder = TransferFnFirstOrder::new(a1, b1, a2, b2)?;
...
impl RampResponseRealTime {
    ...
    pub fn set_user_input_and_calc(
        &mut self,
        user_input: Ratio,
        time_of_input: Time,
    ) -> Result<Ratio, ChemEngProcessControlSimulatorError> {
        ...
        let input_changed: bool = (user_input.get::<ratio>() * 1e9).round()
...
impl RampResponse {
    ...
    pub fn calculate_response(&mut self, simulation_time: Time) -> Ratio {
        ...
        let response: Ratio = self.user_input * self.gradient_gain * time_elapsed;
        return response;
    }
    ...
    pub fn is_started_for_1s(&self) -> bool {
        ...
        let turned_on_for_1s: bool = time_elapsed >= Time::new::<second>(1.0);
...
impl Into<AnalogController> for IntegralController {
    fn into(self) -> AnalogController {
        AnalogController::IntegralStandalone(self)
    }
}
