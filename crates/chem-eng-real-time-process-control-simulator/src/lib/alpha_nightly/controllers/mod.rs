...
impl AnalogController {
    pub fn new_pi_controller(
        ...
        let i_controller = IntegralController::new(controller_gain, integral_time)?;

        Ok(Self::PI(p_controller, i_controller))
    }

    pub fn new_filtered_pid_controller(
        controller_gain: Ratio,
        integral_time: Time,
        ...
        let i_controller = IntegralController::new(controller_gain, integral_time)?;
        let d_controller =
            FilteredDerivativeController::new(controller_gain, derivative_time, alpha)?;

        Ok(Self::PIDFiltered(p_controller, i_controller, d_controller))
    }
    pub fn new_filtered_pd_controller(
        controller_gain: Ratio,
        derivative_time: Time,
        alpha: Ratio,
    ) -> Result<Self, ChemEngProcessControlSimulatorError> {
...
impl TransferFnTraits for AnalogController {
    fn set_dead_time(&mut self, dead_time: uom::si::f64::Time) {
        match self {
            ...
            AnalogController::P(ctrl) => ctrl.set_dead_time(dead_time),
            AnalogController::IntegralStandalone(ctrl) => ctrl.set_dead_time(dead_time),
            AnalogController::DerivativeFilteredStandalone(ctrl) => ctrl.set_dead_time(dead_time),
            AnalogController::PDFiltered(
                proportional_controller,
                filtered_derivative_controller,
            ) => {
                proportional_controller.set_dead_time(dead_time);
    ...
    fn set_user_input_and_calc(
        ...
        time_of_input: uom::si::f64::Time,
    ) -> Result<uom::si::f64::Ratio, super::errors::ChemEngProcessControlSimulatorError> {
        match self {
            AnalogController::PIDFiltered(
                proportional_controller,
                integral_controller,
                ...
                let d_output = filtered_derivative_controller
            ...
            AnalogController::P(ctrl) => ctrl.set_user_input_and_calc(user_input, time_of_input),
    ...
    fn spawn_writer(
        &mut self,
        name: String,
    ) -> Result<csv::Writer<std::fs::File>, super::errors::ChemEngProcessControlSimulatorError>
    {
        let mut title_string: String = name;
        match self {
            AnalogController::PIDFiltered(_, _, _) => title_string += "_PID_controller.csv",
            AnalogController::PI(_, _) => title_string += "_PI_controller.csv",
    ...
    fn csv_write_values(
        ...
        output: uom::si::f64::Ratio,
    ) -> Result<(), super::errors::ChemEngProcessControlSimulatorError> {
