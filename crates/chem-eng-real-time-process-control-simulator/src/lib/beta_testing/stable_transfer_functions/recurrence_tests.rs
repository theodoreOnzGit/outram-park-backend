...
//! `SecondOrderStableStepResponse`, `DecaySinusoidResponse`) over every
//! increment. That superposition is the *reference*: it is the same mathematics the pre-0.2.0
...
use crate::beta_testing::transfer_fn_wrapper_and_enums::{TransferFnFirstOrder, TransferFnTraits};
...
/// # Methodology
///
/// Each of the four affected block types in this tier is driven for 100,000
/// steps at
...
/// 99,999**, namely `[2, 1, 3, 2]` — 2 for the first-order lag, 1 for the
/// zero-bearing block, 3 for the second-order lag and 2 for the decaying
/// sinusoid. Under the pre-0.2.0
...
fn block_state_size_does_not_grow_with_step_index() {
    ...
    .unwrap();
    let mut early_sizes = [0usize; 4];
    let mut late_sizes = [0usize; 4];
...
/// it: the wall-clock cost of one first-order transfer-function step must not
/// grow with the step index.
///
/// The `beta_testing` tier has no controllers of its own — the PID equivalent
/// of this test lives in the `alpha_nightly` tier, which is the tier TUAS
/// imports. Here the same claim is checked on the composite
/// `TransferFnFirstOrder`, which is what a controller is built out of and
/// which carries three of the response accumulators that used to be vectors.
///
/// # Methodology
///
/// Two blocks, both stepped at `dt = 0.001 s` — the timestep at which the old
/// implementation degraded worst:
///
/// - a filtered-derivative block `G(s) = K_c tau_d s / (alpha tau_d s + 1)`
///   with `K_c = 1.75` (dimensionless), `tau_d = 80.8 s` and `alpha = 1.0`,
///   so its retirement horizon of `20 tau / dt` = 1.6 M steps is never
///   reached inside the run — this is the `teh-o-prke` shape;
/// - a pure gain `G(s) = K_c`, realised as `(K_c s + K_c)/(s + 1)`, which is
...
#[test]
fn first_order_step_cost_does_not_grow_with_step_index() {
    let timestep = 0.001_f64;
    let controller_gain = Ratio::new::<ratio>(1.75);

    // K_c tau_d s / (alpha tau_d s + 1), tau_d = 80.8 s, alpha = 1.0
    let derivative_time = Time::new::<second>(80.8);
    let mut filtered_derivative = TransferFnFirstOrder::new(
        derivative_time * controller_gain,
        Ratio::ZERO,
        derivative_time,
        Ratio::new::<ratio>(1.0),
    )
    ...
    // pure gain K_c, realised as (K_c s + K_c)/(s + 1)
    let mut pure_gain = TransferFnFirstOrder::new(
        Time::new::<second>(1.0) * controller_gain,
        controller_gain,
        Time::new::<second>(1.0),
        Ratio::new::<ratio>(1.0),
    )
    ...
    let mut early_derivative_seconds = 0.0_f64;
    let mut late_derivative_seconds = 0.0_f64;
    let mut early_gain_seconds = 0.0_f64;
    let mut late_gain_seconds = 0.0_f64;
    ...
    while step < late_checkpoint + window {
        ...
        if at_early || at_late {
            ...
            for _ in 0..repeats {
                ...
                derivative_seconds =
                    derivative_seconds.min(derivative_start.elapsed().as_secs_f64());
            ...
            if at_early {
                ...
                early_gain_seconds = gain_seconds;
            } else {
    ...
    let early_derivative_us = early_derivative_seconds * 1e6 / window as f64;
    let late_derivative_us = late_derivative_seconds * 1e6 / window as f64;
    let early_gain_us = early_gain_seconds * 1e6 / window as f64;
    let late_gain_us = late_gain_seconds * 1e6 / window as f64;

    println!(
        "filtered derivative us/step: {early_derivative_us:.4} @{early_checkpoint} -> \
         {late_derivative_us:.4} @{late_checkpoint} (ratio {:.2})",
        late_derivative_us / early_derivative_us
    );
    println!(
        "pure gain           us/step: {early_gain_us:.4} @{early_checkpoint} -> \
         {late_gain_us:.4} @{late_checkpoint} (ratio {:.2})",
        late_gain_us / early_gain_us
    );
    println!("(sink {sink:.3})");

    assert!(
        late_derivative_us < 5.0 * early_derivative_us,
        "filtered-derivative step cost grew from {early_derivative_us:.4} us to \
         {late_derivative_us:.4} us per step; this is the op-fm5 defect coming back"
    );
    assert!(
        late_gain_us < 5.0 * early_gain_us,
        "pure-gain step cost grew from {early_gain_us:.4} us to {late_gain_us:.4} us per step; \
