...
impl TransferFnTraits for TransferFn {
    fn set_dead_time(&mut self, dead_time: Time) {
        match self {
            TransferFn::FirstOrder(first_order) => first_order.set_dead_time(dead_time),
            TransferFn::SecondOrder(second_order) => second_order.set_dead_time(dead_time),
        }
    }
    ...
    fn spawn_writer(
        ...
        match self {
            TransferFn::FirstOrder(first_order) => first_order.spawn_writer(name),
            TransferFn::SecondOrder(second_order) => second_order.spawn_writer(name),
        }
    }
    ...
    fn csv_write_values(
        ...
        match self {
            ...
            TransferFn::SecondOrder(second_order) => {
                second_order.csv_write_values(wtr, time, input, output)
            }
        }
    }
}
