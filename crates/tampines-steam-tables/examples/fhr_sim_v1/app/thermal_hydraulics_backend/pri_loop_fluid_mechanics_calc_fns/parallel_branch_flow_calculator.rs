...
use tuas_boussinesq_solver::array_fluid_collections::fluid_component_collection::collection_series_and_parallel_functions::FluidComponentCollectionSeriesAssociatedFunctions;
use tuas_boussinesq_solver::array_fluid_collections::fluid_component_collection::fluid_component::FluidComponent;
use tuas_boussinesq_solver::array_fluid_collections::fluid_component_collection::fluid_component_collection::{FluidComponentCollection, FluidComponentCollectionMethods};
...
use tuas_boussinesq_solver::array_fluid_collections::fluid_component_collection::super_collection_series_and_parallel_functions::FluidComponentSuperCollectionParallelAssociatedFunctions;
...
pub fn calculate_pressure_change_using_guessed_branch_mass_flowrate_fhr_sim_v1_custom(
    ...
    let pressure_diff_at_guessed_average_flow =
        max_pressure_change_at_guessed_average_flow - min_pressure_change_at_guessed_average_flow;

    let mut static_pressure_variation_estimate = pressure_diff_at_guessed_average_flow;

    // do not expect this variation to go beyond 1 bar
    let max_static_pressure_variation = Pressure::new::<bar>(1.0);
    ...
    dbg!(&(
        user_specified_pressure_upper_bound,
...
pub fn calculate_mass_flowrate_from_pressure_change_for_parallel_branches(
    ...
    for (index, fluid_component_pointer) in fluid_component_collection_vector.iter().enumerate() {
        ...
        let fluid_component_vector = fluid_component.get_immutable_fluid_component_vector();
