...
fn main() {
    ...
    let water_initial_specific_volume: SpecificVolume =
        pt_flash_eqm::v_tp_eqm_two_phase(steam_temperature, steam_pressure, steam_quality);
    // we can find the mass of water inside the RPV
    let water_mass: Mass = pri_circuit_volume / water_initial_specific_volume;

    // given the mass of water, we can now
    // find the specific volume at the end

    let water_final_specific_volume_estimate: SpecificVolume = pri_containment_volume / water_mass;

    // the specific volume is around 0.403 m3/kg
    assert_eq!(
        water_final_specific_volume_estimate,
    ...
    let water_enthalpy: AvailableEnergy =
        h_tp_eqm_two_phase(steam_temperature, steam_pressure, steam_quality);
    ...
    fn bisection_algo(
        lower_bound_pressure: Pressure,
        upper_bound_pressure: Pressure,
        water_enthalpy: AvailableEnergy,
        target_specific_volume: SpecificVolume,
        ...
        let mut guessed_pressure = 0.5 * (upper_bound_pressure + lower_bound_pressure);
        let mut upper_pressure_bracket: Pressure = upper_bound_pressure;
        let mut lower_pressure_bracket: Pressure = lower_bound_pressure;

        let mut guess_specific_volume = v_ph_eqm(guessed_pressure, water_enthalpy);

        let mut error: Ratio =
            (guess_specific_volume - target_specific_volume) / target_specific_volume;
        ...
        while error.abs() > tolerance {
            ...
            guessed_pressure = 0.5 * (upper_pressure_bracket + lower_pressure_bracket);
            // and then the specific volume
            guess_specific_volume = v_ph_eqm(guessed_pressure, water_enthalpy);
            // recalculate the error
            error = (guess_specific_volume - target_specific_volume) / target_specific_volume;
    ...
    assert_eq!(final_quality, 0.38614681771539744);
    ...
    // pressurised water
    //
    let liquid_mass: Mass = final_quality * water_mass;
