...
impl SymmTensor {
    /// The zero symmetric tensor (all six independent components `0`).
    ...
    /// The identity symmetric tensor (unit diagonal, zero off-diagonal).
    ...
    /// Build a symmetric tensor from its six independent components
    /// (xx, xy, xz, yy, yz, zz; yx = xy, zx = xz, zy = yz by symmetry).
    #[inline]
    ...
    /// Second row as a vector, `(xy, yy, yz)` (symmetric: yx = xy).
    #[inline]
    pub fn row_y(self) -> Vector3 {
        Vector3::new(self.xy, self.yy, self.yz)
    }
    /// Third row as a vector, `(xz, yz, zz)` (symmetric: zx = xz, zy = yz).
    #[inline]
    ...
    pub fn sph(self) -> SphericalTensor {
        SphericalTensor::new(self.tr() / 3.0)
    }
    ...
    pub fn det(self) -> f64 {
        ...
        xx * yy * zz + xy * yz * xz + xz * xy * yz - xx * yz * yz - xy * xy * zz - xz * yy * xz
    }
    ...
    pub fn safe_inv(self) -> Self {
        ...
        let d = work.det();
        if d.abs() < super::scalar::ROOT_VSMALL {
            return Self::ZERO;
        }
...
impl Mul<SymmTensor> for f64 {
    ...
    #[inline]
    fn mul(self, st: SymmTensor) -> SymmTensor {
        st * self
    }
}
...
impl Add<SphericalTensor> for SymmTensor {
    type Output = Self;
    #[inline]
    fn add(self, spt: SphericalTensor) -> Self {
        spt + self
    }
}
...
impl Mul<SphericalTensor> for SymmTensor {
    type Output = Self;
    #[inline]
    fn mul(self, spt: SphericalTensor) -> Self {
        self * spt.ii
    }
}

// --- Free functions mirroring OpenFOAM globals: each is a thin wrapper
// around the identically-named inherent method above. ---

/// Free-function form of [`SymmTensor::tr`]: the trace `xx + yy + zz`.
#[inline]
pub fn tr(st: SymmTensor) -> f64 {
    st.tr()
}

/// Free-function form of [`SymmTensor::det`]: the determinant.
#[inline]
pub fn det(st: SymmTensor) -> f64 {
    st.det()
}

/// Free-function form of [`SymmTensor::inv`]: the matrix inverse.
#[inline]
pub fn inv(st: SymmTensor) -> SymmTensor {
    st.inv()
}

/// Free-function form of [`SymmTensor::dev`]: the deviatoric part.
#[inline]
pub fn dev(st: SymmTensor) -> SymmTensor {
    st.dev()
}

/// Free-function form of [`SymmTensor::dev2`]: the "2/3" deviatoric part.
#[inline]
...
#[inline]
pub fn sqr(v: Vector3) -> SymmTensor {
    SymmTensor::from_outer(v)
}

/// Free-function form of [`SymmTensor::mag_sqr`]: Frobenius norm squared.
#[inline]
pub fn mag_sqr(st: SymmTensor) -> f64 {
    st.mag_sqr()
}

/// Free-function form of [`SymmTensor::lerp`]: linear interpolation `(1-t)*a + t*b`.
#[inline]
...
mod tests {
    ...
    fn inv_roundtrip_is_identity() {
        ...
        use crate::openfoam_algorithms::openfoam_source::Tensor;
