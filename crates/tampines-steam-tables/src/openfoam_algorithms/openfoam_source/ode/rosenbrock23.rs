...
impl Rosenbrock23 {
    ...
    /// all other config fields take their `Default` values. Allocates the
    /// `n × n` Jacobian and LU-factorisation scratch matrices up front.
    ...
    /// Integrate from `x_start` to `x_end` using repeated adaptive Rosenbrock23
    /// steps. Requires `ode.jacobian` to be implemented. `dx_est` is used as
