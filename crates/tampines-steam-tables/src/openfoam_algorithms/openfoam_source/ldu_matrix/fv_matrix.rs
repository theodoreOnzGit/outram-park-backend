...
impl FvMatrix {
    ...
    ///
    /// Uses Gauss-Seidel — works on any (including asymmetric) matrix. For a
    /// symmetric SPD system (e.g. the pressure Poisson equation) prefer
    /// [`solve_cg`](Self::solve_cg) or [`solve_gamg`](Self::solve_gamg),
    /// which converge much faster.
    pub fn solve(
        &self,
