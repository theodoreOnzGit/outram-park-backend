...
impl LduMatrix {
    /// Create a zero-initialised LDU matrix over `n_cells` with the given
    /// face-addressing arrays (`owner[f]`/`neighbour[f]` give the two cells
    /// each internal face `f` connects). `diag`, `lower`, and `upper` start
    /// at zero and are filled in by `fvm::` assembly.
    ...
    /// L1-scaled norm of the residual: `||r||₁ / (0.5·(||A·x||₁ + ||b||₁))`,
    /// falling back to the unscaled `||r||₁` when that denominator is
    /// vanishingly small (near-zero system).
