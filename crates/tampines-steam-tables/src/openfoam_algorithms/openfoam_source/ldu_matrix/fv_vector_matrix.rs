...
impl FvVectorMatrix {
    /// Create a new zero-initialised `FvVectorMatrix` for the given mesh
    /// (zero diagonal/off-diagonal coefficients, zero source).
    ...
    /// Add `coeff[c]` to the diagonal at each cell (e.g. from a time
    /// derivative term). The same scalar coefficient applies to all three
    /// velocity components.
    ...
    /// Add `term[c]` to the source vector at each cell.
