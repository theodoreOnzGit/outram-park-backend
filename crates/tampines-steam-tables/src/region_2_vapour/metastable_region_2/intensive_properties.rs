...
pub fn v_tp_2_metastable(t: ThermodynamicTemperature, p: Pressure) -> SpecificVolume {
    ((specific_gas_constant_of_water()) * t / p)
        * pi_2(p)
        * (gamma_metastable_pi_2_ideal(t, p) + gamma_metastable_pi_2_res(t, p))
}
...
pub fn h_tp_2_metastable(t: ThermodynamicTemperature, p: Pressure) -> AvailableEnergy {
    specific_gas_constant_of_water()
        * t
        * tau_2(t)
        * (gamma_metastable_tau_2_ideal(t, p) + gamma_metastable_tau_2_res(t, p))
}
...
pub fn cp_tp_2_metastable(t: ThermodynamicTemperature, p: Pressure) -> SpecificHeatCapacity {
    ...
    -specific_gas_constant_of_water()
        * tau.powi(2)
        * (gamma_metastable_tau_tau_2_ideal(t, p) + gamma_metastable_tau_tau_2_res(t, p))
}
...
pub fn cv_tp_2_metastable(t: ThermodynamicTemperature, p: Pressure) -> SpecificHeatCapacity {
    let tau = tau_2(t);
    let pi = pi_2(p);
    let num = (1.0 + pi * gamma_metastable_pi_2_res(t, p)
        - tau * pi * gamma_metastable_pi_tau_2_res(t, p))
    .powi(2);
...
pub fn w_tp_2_metastable(t: ThermodynamicTemperature, p: Pressure) -> Velocity {
    ...
    let pi = pi_2(p);
    let num = 1.0
        + 2.0 * pi * gamma_metastable_pi_2_res(t, p)
        + pi.powi(2) * gamma_metastable_pi_2_res(t, p).powi(2);
    let subnum = (1.0 + pi * gamma_metastable_pi_2_res(t, p)
        - tau * pi * gamma_metastable_pi_tau_2_res(t, p))
    .powi(2);
    let subden = tau.powi(2)
        * (gamma_metastable_tau_tau_2_ideal(t, p) + gamma_metastable_tau_tau_2_res(t, p));
...
pub fn alpha_v_tp_2_metastable(t: ThermodynamicTemperature, p: Pressure) -> TemperatureCoefficient {
    ...
    let num =
        1.0 + pi * gamma_metastable_pi_2_res(t, p) - tau * pi * gamma_metastable_pi_tau_2_res(t, p);
