...
use crate::interfaces::functional_programming::pt_flash_eqm::{
    cp_tp_eqm_single_phase, v_tp_eqm_two_phase,
};
...
pub fn lambda_tp_eqm_two_phase(
...
pub(crate) fn lambda_1(rho: MassDensity, t: ThermodynamicTemperature) -> f64 {
    let t_c = t_crit_water();
    let theta_f64: f64 = (t / t_c).get::<ratio>();
...
pub(crate) fn lambda_2_crit_enhancement_term_tp_two_phase_estimate(
    ...
    let captial_a: f64 = captial_a(n2, n3, delta_f64, theta_f64, kappa_t, n4, n5, b);

    let gas_constant_r = SpecificHeatCapacity::new::<kilojoule_per_kilogram_kelvin>(0.461_518_05);

    let lambda_2 = n1 * delta_f64 * theta_f64 / psi * cp / gas_constant_r * captial_a;
...
fn captial_b(delta: f64, theta: f64, kappa_t: InversePressure, n5: f64) -> f64 {
    ...
    let mut captial_b =
        (p_c * delta * corrected_kappa_t).get::<ratio>() - n5 * theta.recip() * captial_c;
