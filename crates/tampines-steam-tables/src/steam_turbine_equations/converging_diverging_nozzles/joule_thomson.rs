...
pub fn get_outlet_velocity_and_state_joule_thomson(
    ...
    for i in 0..max_iterations {
        ...
        let (mass_rate_test, state_2_test) =
            calculate_mass_rate_and_state_at_outlet_ph_velocity(h0, p2, v2, a2);

        // check residual
        //
        relative_error = ((mass_rate_test - mass_flowrate_ref) / mass_flowrate_ref).get::<ratio>();
    ...
    let root_finder = |v_test: Velocity| -> (MassRate, TampinesSteamTableCV) {
        let (mass_flowrate_calc, outlet_state_with_shocks) =
            calculate_mass_rate_and_state_at_outlet_ph_velocity(h0, p2, v_test, a1);
...
mod joule_thomson_test {
    ...
    fn superheated_steam() {
        ...
        let mass_flowrate: MassRate = state_1.get_rho() * v1 * area;
        let h1 = state_1.get_specific_enthalpy();

        let (v2, state_2) =
            get_outlet_velocity_and_state_joule_thomson(p1, h1, p2, mass_flowrate, area);
        ...
        approx::assert_relative_eq!(v2.get::<meter_per_second>(), 1035.67, max_relative = 1e-4);
    }
}
