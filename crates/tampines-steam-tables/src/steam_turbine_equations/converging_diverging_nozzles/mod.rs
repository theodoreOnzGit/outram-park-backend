...
pub fn calculate_velocity_mass_flowrate_and_state_in_cd_nozzle(
    ...
    let s2_ideal = s1;
    let outlet_flow_state_ideal = TampinesSteamTableCV::new_from_ps(p2, s2_ideal, ref_vol);
    ...
    if !throat_has_choked_flow {
        ...
        let state_outlet = outlet_flow_state_ideal;
    ...
    let (v, state_outlet) = guess_velocity_and_state_for_diverge_nozzle_from_choked_throat(
        h0,
        s0,
        p2,
        a2,
