...
fn validate_zaloudek_curve_subcooled(
    ...
    for &(p_psia, g_expected_val, _h0_expected_val) in data {
        ...
        let g_expected =
            MassRate::new::<pound_per_second>(g_expected_val) / Area::new::<square_foot>(1.0);

        // throat state -> stagnation (backward map)
        let state_t =
            TampinesSteamTableCV::new_from_sat_pressure_quality(p_throat_ref, x_t, ref_vol);
        ...
        let (p_crit_calc, g_calc) = get_critical_pressure_and_mass_flux_subcooled_liquid_ph(p0, h0);
...
fn diagnose_bubble_point_artifact() {
    ...
    use crate::interfaces::functional_programming::pt_flash_eqm::{
        v_tp_eqm_two_phase, h_tp_eqm_two_phase,
    };
    ...
    for &(p_psia, g_expected_val, _h0) in &data {
        ...
        let branch = if (p_sub_kpa - p_bub_kpa).abs() / p_bub_kpa < 1e-3 {
            "bubble"
        } else {
            "2phase"
        };
        let sub_dp = (p_sub_kpa - p_throat_ref.get::<kilopascal>())
            / p_throat_ref.get::<kilopascal>()
            * 100.0;
        ...
        let dome_dp = (p_dome.get::<kilopascal>() - p_throat_ref.get::<kilopascal>())
            / p_throat_ref.get::<kilopascal>()
            * 100.0;
    ...
    eprintln!();
    eprintln!(
        "Legend: dPerr = (p_choke - p_throat)/p_throat; dGlg = log10(G_calc) - log10(G_exp)."
    );
