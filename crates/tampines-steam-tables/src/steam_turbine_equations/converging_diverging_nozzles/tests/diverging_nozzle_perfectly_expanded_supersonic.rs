...
use crate::prelude::{
    TampinesSteamTableCV, get_choked_flow_massrate_and_state_from_stagnation_properties_and_area,
};
...
fn diverging_nozzle_perfectly_expanded_supersonic_dry_steam() {
    ...
    let (choked_mass_flowrate, choked_state) =
        get_choked_flow_massrate_and_state_from_stagnation_properties_and_area(p0, h0, a_throat);
    ...
    let exit_mass_flowrate: MassRate = state_exit.get_rho() * v_exit * a2;
...
fn diverging_nozzle_perfectly_expanded_supersonic_wet_steam() {
    ...
    let p1 = Pressure::new::<bar>(20.0);
    let quality = 0.2;
    let inlet_state = TampinesSteamTableCV::new_from_tp_quality(temperature, p1, ref_vol, quality);
    ...
    let inlet_state_quality_1 =
        TampinesSteamTableCV::new_from_tp_quality_1(temperature, p1, ref_vol);

    dbg!(&inlet_state_quality_1);
    dbg!(&inlet_state);

    // assert quality of inlet state
    approx::assert_relative_eq!(inlet_state.get_quality(), 0.2, max_relative = 1e-6);

    // check the quality of the exit state
    // (it should be 1 as the steam is fully vapour)
    approx::assert_relative_eq!(state_exit.get_quality(), 1.0, max_relative = 1e-6);
}
