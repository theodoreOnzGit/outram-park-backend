...
pub fn guess_velocity_and_state_for_diverge_nozzle_from_choked_throat(
    ...
    if p2 > p_ideal_exp_supersonic && p2 < p_ideal_exp_subsonic {
        ...
        let root_finder = |v_test: Velocity| -> (MassRate, TampinesSteamTableCV) {
            let (mass_flowrate_calc, outlet_state_with_shocks) =
                calculate_mass_rate_and_state_at_outlet_ph_velocity(h0, p2, v_test, a_exit);
        ...
        let (mut error_lower_limit, _state_lower_limit) = root_finder(v_lower_limit);
        let (mut error_upper_limit, _state_upper_limit) = root_finder(v_upper_limit);
        ...
        while relative_error.abs() > TOLERANCE && iteration < max_iterations {
            ...
            v_test = v_upper_limit
                - (v_upper_limit - v_lower_limit) * error_upper_limit
                    / (error_upper_limit - error_lower_limit);
    ...
    if p2 < p_ideal_exp_supersonic {
        ...
        let (v2, state2) =
            get_outlet_velocity_and_state_joule_thomson(p1, h1, p2, mass_rate_throat, a_exit);
...
pub fn calculate_isentropic_exit_pressure_velocity_and_state_supersonic(
    ...
    while v_test <= v_upper_scan {
        let (state_exit_guess, mass_flowrate_calc, c_exit_guess) =
            guess_state_exit_and_mass_flow_based_on_velocity(v_test, h0, s0, ref_vol, a_exit);
    ...
    for _ in 0..max_iterations {
        ...
        if error.get::<ratio>().abs() < 1e-6 {
            assert!(
                v_exit > c_exit,
                "Sanity check failed: Converged to a subsonic velocity!"
            );
    ...
    assert!(
        v_exit > state_exit.get_speed_of_sound(),
        "Final result must be supersonic!"
    );
...
pub fn calculate_isentropic_exit_pressure_velocity_and_state_subsonic(
    ...
    for _ in 0..max_iterations {
        ...
        if error.get::<ratio>().abs() < 1e-6 {
            assert!(
                v_exit < c_exit,
                "Sanity check failed: Converged to a supersonic velocity!"
            );
            return (p_mid, v_exit, state_exit);
        }
