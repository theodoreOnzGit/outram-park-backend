...
use crate::interfaces::functional_programming::ph_flash_eqm::{
    cp_ph_eqm, cv_ph_eqm, s_ph_eqm, w_ph_wood_wallis,
};
...
pub fn get_choked_flow_state_for_nozzle_subsonic_to_sonic(
    ...
    let mass_flowrate_error: Ratio = (choked_mass_flowrate - mass_flowrate) / mass_flowrate;

    if mass_flowrate_error.abs() >= Ratio::new::<ratio>(0.05) {
        eprintln!(
            "Warning: Mass balance error = {:.2}%. \
    ...
    if momentum_balance_check_1d {
        ...
        if momentum_balance_error.abs() >= Ratio::new::<ratio>(0.05) {
            eprintln!(
                "Warning: Momentum balance error = {:.2}%. \
...
pub fn get_choked_flow_supersonic_nozzle_exit_area_and_state(
    ...
    let ref_vol = Volume::new::<cubic_meter>(1.0);
    let state_throat = TampinesSteamTableCV::new_from_ph(p_throat, h_throat, ref_vol);
    ...
    let state_exit = TampinesSteamTableCV::new_from_ps(p_exit, s0, ref_vol);
...
mod choked_flow_examples {
    ...
    use crate::prelude::TampinesSteamTableCV;
    use super::{
        get_choked_flow_supersonic_nozzle_exit_area_and_state, get_choked_flow_nozzle_area,
    ...
    pub fn steam_flow_cd_nozzle() {
        ...
        let state_0 = TampinesSteamTableCV::new_from_tp_quality_1(t1, p0, ref_vol);
        ...
        let critical_pressure_ratio = state_0.get_critical_pressure_ratio_pure_vapour();
        ...
        approx::assert_relative_eq!(p_throat.get::<megapascal>(), 1.09, max_relative = 5e-3,);

        // now for throat conditions

        let state_throat = TampinesSteamTableCV::new_from_ps(p_throat, s0, ref_vol);
        ...
        approx::assert_relative_eq!(c.get::<meter_per_second>(), 584.74, max_relative = 1e-3,);
        ...
        approx::assert_relative_eq!(v1.get::<meter_per_second>(), 3.659, max_relative = 1e-3,);

        // let's see whether given these areas, we get the correct
        // critical pressure predictions

        let (p_crit, h_crit, mass_flowrate) =
            get_choked_flow_state_for_nozzle_subsonic_to_sonic(p1, h0, a1, a2, v1);

        approx::assert_relative_eq!(p_crit.get::<megapascal>(), 1.092, max_relative = 1e-3,);
        approx::assert_relative_eq!(
        ...
        let state_exit = TampinesSteamTableCV::new_from_ph(p_exit, h_exit, ref_vol);

        let c_exit: Velocity = state_exit.get_speed_of_sound();

        // from Cengel's the exit speed of sound is 515.4 m/s
        approx::assert_relative_eq!(c_exit.get::<meter_per_second>(), 515.4, max_relative = 1e-3,);
        ...
        approx::assert_relative_eq!(v_exit.get::<meter_per_second>(), 929.8, max_relative = 1e-3,);

        // exit mach number is 1.804

        let mach_number_exit = state_exit.get_mach_number(v_exit);
        approx::assert_relative_eq!(mach_number_exit.get::<ratio>(), 1.804, max_relative = 1e-3,);
    ...
    fn steam_flow_cd_nozzle_2() {
        let inlet_temperature = ThermodynamicTemperature::new::<degree_celsius>(500.0);

        let inlet_pressure = Pressure::new::<megapascal>(1.0);

        let mass_flowrate = MassRate::new::<kilogram_per_second>(2.5);

        let ref_vol = Volume::new::<cubic_meter>(1.0);
        let state_0 =
            TampinesSteamTableCV::new_from_tp_quality_1(inlet_temperature, inlet_pressure, ref_vol);
        let h0 = state_0.get_specific_enthalpy();
        let p0 = inlet_pressure;

        // note, throat diameter is 22.4 cm^2
        let area = get_choked_flow_nozzle_area(p0, h0, mass_flowrate);
        approx::assert_relative_eq!(area.get::<square_centimeter>(), 22.399, max_relative = 1e-3,);
        let ref_vol = Volume::new::<cubic_meter>(1.0);
        let state_0 = TampinesSteamTableCV::new_from_ph(p0, h0, ref_vol);
        let s0 = state_0.get_specific_entropy();
        let s1 = s0;
        ...
        let (p_exit, h_exit, exit_area) = get_choked_flow_supersonic_nozzle_exit_area_and_state(
            p_throat,
            h_throat,
            p_exit,
            mass_flowrate,
        ...
        approx::assert_relative_eq!(mach_number.get::<ratio>(), 1.738, max_relative = 1e-2,);
        approx::assert_relative_eq!(mach_number.get::<ratio>(), 1.728, max_relative = 1e-3,);
    }
}
...
pub fn get_critical_pressure_pure_vapour_ph_stagnation_properties(
    p0: Pressure,
    h0: AvailableEnergy,
    s0_opt: Option<SpecificHeatCapacity>,
) -> Pressure {
    let s0 = match s0_opt {
        Some(s0) => s0,
        None => s_ph_eqm(p0, h0),
    ...
    let p_guess = get_critical_pressure_ratio_ideal_gas_using_throat_ph(p0, h0) * p0;
