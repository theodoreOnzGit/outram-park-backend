...
// formulae
//
/// Computes the differential pressure change `dp` (Pa) across an
/// infinitesimal area change from `a1` to `a2` in a quasi-1D isentropic
/// nozzle/diffuser, using the area-Mach-number influence-coefficient
/// relation `dp = (da/a1) * rho1 * v1^2 / (1 - M1^2)`, evaluated at the
/// inlet state `(p1, h1, v1)`. Singular as the inlet Mach number `M1`
/// approaches 1 (the throat).
...
// formulae
//
/// Computes the differential velocity change `dv` (m/s) across an
/// infinitesimal area change from `a1` to `a2` in a quasi-1D isentropic
/// nozzle/diffuser, using the area-Mach-number influence-coefficient
/// relation `dv = -(da/a1) * v1 / (1 - M1^2)`, evaluated at the inlet state
/// `(p1, h1, v1)`. Singular as the inlet Mach number `M1` approaches 1 (the
/// throat).
...
pub fn _get_dp_dv_isentropic_nozzle_diffuser(
    ...
    if m_value > 0.99 && m_value < 1.01 {
        ...
        let critical_pressure_ratio: Ratio = state_1.get_critical_pressure_ratio_pure_vapour();
...
pub fn _get_outlet_pressure_velocity_enthalpy_isentropic_nozzle_diffuser(
    ...
    v1: Velocity,
    da_by_a_max: Ratio,
) -> (Pressure, Velocity, AvailableEnergy) {
    ...
    while da_by_a_remaining > tolerance {
        ...
        // Apply sign to step
        let da_by_a_signed = Ratio::new::<ratio>(da_by_a_step.get::<ratio>() * area_change_sign);
