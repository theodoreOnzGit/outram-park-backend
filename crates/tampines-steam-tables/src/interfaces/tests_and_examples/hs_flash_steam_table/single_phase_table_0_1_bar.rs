...
use crate::interfaces::functional_programming::hs_flash_eqm::{
    cp_hs_eqm, kappa_hs_eqm, lambda_hs_eqm, mu_hs_eqm, tpvx_hs_flash_eqm, w_hs_eqm,
};
...
/// also hs equation fails for region 1  
/// up to saturation point at 0.1 bar
#[test]
...
fn assert_hs_flash(
    ...
    let s = SpecificHeatCapacity::new::<kilojoule_per_kilogram_kelvin>(s_kj_per_kg_k);

    // assert temp first to within 0.025 mk
    // based on table 2.8
    let temp_tol_millikelvin = 25.0;

    let (t_test, p_test, v_test, x_test) = tpvx_hs_flash_eqm(h, s);
    ...
    if p > Pressure::new::<megapascal>(2.5) {
        ...
        approx::assert_abs_diff_eq!(p_bar, p_test.get::<bar>(), epsilon = pressure_error_tol_bar);
    } else {
        dbg!(&(p_bar, t_deg_c));
        approx::assert_relative_eq!(p_bar, p_test.get::<bar>(), max_relative = 20e-2);
    }
    // the x for ph is quite reliable, i'll use that as reference
    let x_ref = x_ph_flash(p, h);

    approx::assert_relative_eq!(x_ref, x_test.get::<ratio>(), max_relative = 1e-3);
    ...
    // dynamic_viscosity
    //
    let eta_micropascal_second_test = mu_hs_eqm(h, s).get::<micropascal_second>();
    approx::assert_relative_eq!(
        eta_micropascal_second,
