...
use super::ph_flash_eqm::{
    cp_ph_eqm, kappa_ph_eqm, lambda_ph_eqm, mu_ph_eqm, t_ph_eqm, w_ph_wood_wallis,
};
...
pub fn tpvx_hs_flash_eqm(
    ...
    match region {
        ...
        BackwdEqnSubRegion::Region4 => {
            ...
            let max_sat_temp_for_backward = ThermodynamicTemperature::new::<kelvin>(623.15);
            let min_entropy_for_backward_eqn =
                SpecificHeatCapacity::new::<kilojoule_per_kilogram_kelvin>(5.210_887_825_f64);

            let sat_temp = tsat_hs_4(h, s);

            if s >= min_entropy_for_backward_eqn && sat_temp <= max_sat_temp_for_backward {
                // page 103
                let sat_temp = tsat_hs_4(h, s);
...
pub fn hs_flash_region(h: AvailableEnergy, s: SpecificHeatCapacity) -> BackwdEqnSubRegion {
    ...
    let s_b23_min = SpecificHeatCapacity::new::<kilojoule_per_kilogram_kelvin>(5.048_096_828);

    let s_b23_max = SpecificHeatCapacity::new::<kilojoule_per_kilogram_kelvin>(5.260_578_707);
    ...
    let s_2bc = SpecificHeatCapacity::new::<kilojoule_per_kilogram_kelvin>(5.85);
    ...
    let s_where_pure_2b_ends = SpecificHeatCapacity::new::<kilojoule_per_kilogram_kelvin>(6.070);
    ...
    // based on fig 2.19
    let s_where_pure_2a_or_4_starts =
        SpecificHeatCapacity::new::<kilojoule_per_kilogram_kelvin>(7.852);

    if s <= s_where_pure_2a_or_4_starts {
        return hs_region_high_entropy_region_2b_2a_and_4(h, s);
    };

    let s_where_2a_only_starts = SpecificHeatCapacity::new::<kilojoule_per_kilogram_kelvin>(9.156);
    // otherwise, should be in region 2a or 4

    if s < s_where_2a_only_starts {
        return hs_region_high_entropy_region_2a_or_4_only(h, s);
    } else {
...
fn hs_region_low_entropy_region_1_and_4(
    ...
    let region_1_and_4_h_boundary: AvailableEnergy =
        h1_prime_s_boundary_enthalpy(s) - AvailableEnergy::new::<kilojoule_per_kilogram>(0.0034);
...
fn hs_region_low_entropy_region_1_3a_and_4(
    ...
    let hb13_boundary =
        hb13_s_boundary_enthalpy(s) + AvailableEnergy::new::<kilojoule_per_kilogram>(0.018);
...
fn hs_region_near_crit_entropy_region_3a_and_4(
    ...
    let region_3a_and_4_h_boundary: AvailableEnergy =
        h3a_prime_s_boundary_enthalpy(s) - AvailableEnergy::new::<kilojoule_per_kilogram>(0.0045);
...
fn hs_region_near_crit_entropy_region_3b_and_4(
    ...
    // bound of region 2c, we correct with 0.0073 kJ/(kg )
    let h2c3b_boundary =
        h2c3b_prime_s_boundary_enthalpy(s) - AvailableEnergy::new::<kilojoule_per_kilogram>(0.0073);
...
fn hs_region_near_crit_entropy_region_2c_3b_and_4(
    ...
    let h2c3b_boundary = h2c3b_prime_s_boundary_enthalpy(s) - correction_factor;

    if h < h2c3b_boundary {
        return BackwdEqnSubRegion::Region4;
    };
    // now a few more guard clauses to bound maximum and minimum h
    let hb23_min = AvailableEnergy::new::<kilojoule_per_kilogram>(2.563_592_004e3);
    let hb23_max = AvailableEnergy::new::<kilojoule_per_kilogram>(2.812_942_061e3);
    if h >= hb23_max {
...
fn hs_region_high_entropy_region_2c_and_4(
    ...
    // using 0.0058 kJ/kg
    let h2c3b_boundary =
        h2c3b_prime_s_boundary_enthalpy(s) - AvailableEnergy::new::<kilojoule_per_kilogram>(0.0058);
...
fn hs_region_high_entropy_region_2a_or_4_only(
    h: AvailableEnergy,
    s: SpecificHeatCapacity,
) -> BackwdEqnSubRegion {
    let lower_bound_pressure = Pressure::new::<megapascal>(0.000_611_212_677 * 1.01);
...
fn additional_temperature_check_for_1073_15_k_isotherm(
    ...
    let low_bound_entropy_pressure = Pressure::new::<megapascal>(100.0 - 1.0e-3);
    let mid_bound_entropy_pressure = Pressure::new::<megapascal>(4.0);
    let high_bound_entropy_pressure =
        sat_pressure_4(ThermodynamicTemperature::new::<kelvin>(273.15));
    let low_bound_entropy = s_tp_eqm_single_phase(t_bound, low_bound_entropy_pressure);
    let mid_bound_entropy = s_tp_eqm_single_phase(t_bound, mid_bound_entropy_pressure);
    let high_bound_entropy = s_tp_eqm_single_phase(t_bound, high_bound_entropy_pressure);
...
pub fn find_pressure_from_hs_region_4(
    ...
    if f_low.get::<joule_per_kilogram_kelvin>() * f_high.get::<joule_per_kilogram_kelvin>() > 0.0 {
        panic!(
            "Solution not bracketed by pressure bounds. Check if (h,s) point is physically valid."
        );
    }
    let mut n_iter = 0;
    ...
    eprintln!(
        "Warning: Bisection did not converge after {} iterations. \
