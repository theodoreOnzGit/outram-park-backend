...
fn main() {
    ...
    let (domain_pts, domain_tris) = box_surface(Vec3::new(-2.0, -2.0, -2.0), Vec3::new(2.0, 2.0, 2.0));
    ...
    println!("  pebble volume      = {pebble_vol:.4}  (analytic (4/3)pi r^3 = {:.4})", 4.0 / 3.0 * PI);
    ...
    all_tris.extend(pebble_tris.iter().map(|t| [t[0] + offset, t[1] + offset, t[2] + offset]));
