...
| `krylov` | `bicgstab_prepared`, `gmres_prepared` | BiCGStab and GMRES taking a `HybridLdu` and a `ComputeBackend`, so the inner SpMV and vector operations run on the hybrid kernels. **`Serial` vs `CpuMulti` is bit-exact including every residual-history entry.** |
| `ldu_matrix` | `krylov_solve_prepared` | The `FvMatrix` bridge onto the above, with `KrylovOptions::backend`. |
| `krylov` | *(end-to-end speed-up — measured)* | 512 000 cells: Jacobi **2.48×**, ILU(0) **1.50×**. 32 768 cells: 1.70× / 1.29×. **At 4 096 cells the solve *loses* (0.67× / 0.60×) — and 4 096 is exactly `SPMV_MIN_CELLS`.** The kernel's crossover is not the solver's crossover; the solve only wins from roughly **13 000 cells**. |
| `krylov` | *(ILU(0) is the brake)* | Both ILU(0) phases have loop-carried dependence and are **not** parallelised. Its triangular solves are **29–46% of serial solve time**, which caps attainable speed-up at **1.7–2.1× on 4 cores** by Amdahl; the measured 1.5× sits below that cap, so ILU(0) dominates but is not the only limit. Factorisation costs a further ~24% of one serial solve per coefficient update. |
| `krylov` | *(reduction-order safety)* | The blocked `dot` is not bit-equal to `vecops`' flat sum, which could in principle change an iteration count near a tolerance. Swept **216 solve pairs** (3 sizes × 3 dominances × 3 seeds × 4 tolerances × 2 solvers): **zero iteration-count flips**, GMRES included. The worst case stopped at **0.86× its own tolerance** and still did not flip. This bounds the flip rate at ~1-in-216 for systems of this kind; it is not proof that none flips. |
| `math::differentiate` | `derivative`, `derivative_batch`, `DiffScheme` | Finite differences — forward / backward / central / central-4th — batched on `ComputeBackend`. Step size follows the workspace convention already set by `outram-park-fork-offbeat`'s `newton_perturbed` (cited to Code_Aster `NEWTON_PERT`): `h = rel_step · max(\|x\|, 1)`, which also answers `x = 0`. Divides by the **realised** step `xp − x`, not the requested `h`. |
| `math::differentiate` | `NumericalJacobian`, `jacobian`, `jacobian_batch` | Supplies `OdeSystem::jacobian` **numerically**, so `Rosenbrock23` integrates a system with no hand-coded Jacobian — that method's default impl panics. Verified end-to-end on stiff scalar and 2×2 systems; costs ~1.9–2.9× the analytic Jacobian on Van der Pol while reproducing it to all 8 printed decimals. Square Jacobians only. |
| `math::differentiate` | *(accuracy)* | **Measured** worst relative error over 6 points: forward `4.470348e-8`, central `6.640831e-11`, central-4th `1.963985e-13` — within 3× of theory, with observed convergence orders `1.0079 / 1.9998 / 3.9994`. Central beats forward **673×**. A step 6e6× below optimum is 1.04e7× *worse* — the round-off wall is measured, not asserted. |
| `math::differentiate` | *(what cannot be detected)* | A symmetric stencil **straddles a pole without noticing**: `1/x` at `x = 0` returns `1/h²` and reports success. Undetectable in principle, documented and pinned by a test — as are cancellation with no correct digits, and non-differentiable points. |
