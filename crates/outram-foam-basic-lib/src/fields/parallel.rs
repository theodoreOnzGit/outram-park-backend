...
///
/// Measured 2026-08-12, release profile, `f64` fields, on the development
/// machine with **4 logical cores** reported by
/// [`std::thread::available_parallelism`]; operation `c = a + b` (out-of-place
/// add), best of 5 repeats after a warm-up, by the `#[ignore]`d
/// `measure_crossover_add` test in this module. Absolute wall-clock per call:
///
/// | n | serial | CpuMulti (4 threads) | speedup |
/// |---|---|---|---|
/// | 1 024 | 0.44 us | 12.10 us | 0.04x |
/// | 4 096 | 1.72 us | 13.30 us | 0.13x |
/// | 16 384 | 7.71 us | 21.90 us | 0.35x |
/// | 65 536 | 47.02 us | 55.11 us | 0.85x |
/// | 262 144 | 258.86 us | 176.09 us | 1.47x |
/// | 1 048 576 | 1650.62 us | 1063.29 us | 1.55x |
///
/// The **measured crossover** — the smallest tested size at which the parallel
/// path wins — is **262 144 elements** on that machine. This constant is set to
/// `131_072`, one octave below it, because the two paths are already within ~15%
/// at 65 536 and the parallel path wins decisively above; a machine with more
/// cores or more memory bandwidth moves the crossover down, so erring low costs
/// microseconds while erring high forfeits the whole speed-up.
///
/// A mesh small enough to sit below this threshold takes microseconds per
/// operation either way, so the fallback costs nothing that matters.
pub const FIELD_PARALLEL_CROSSOVER: usize = 131_072;

/// The element count at or above which multi-threading is worth its overhead.
///
/// Returns [`FIELD_PARALLEL_CROSSOVER`]. It is a *function*, not a bare
/// constant, so the policy can later consult a runtime-configured value (or the
/// workspace-level policy in [`crate::compute`]) without touching a single
/// operator.
...
///   (there is no GPU field kernel yet, so `Gpu` takes the best CPU path);
/// - the crate was built with the **`parallel`** feature (rayon present);
...
pub fn should_parallelise(backend: ComputeBackend, n: usize) -> bool {
    match backend {
        ComputeBackend::Serial => false,
        // No GPU field kernel exists yet; `Gpu` takes the best CPU path. When
        // one lands it is dispatched here and every call site is unchanged.
        ComputeBackend::CpuMulti | ComputeBackend::Gpu => {
            cfg!(feature = "parallel") && n >= field_parallel_crossover()
        }
    }
}
...
{
    if should_parallelise(backend, a.len()) {
        a.par_iter().zip(b.par_iter()).map(|(p, q)| f(p, q)).collect()
    } else {
...
{
    if should_parallelise(backend, a.len()) {
        a.par_iter_mut().zip(b.par_iter()).for_each(|(p, q)| f(p, q));
    } else {
    ...
    F: Fn(f64, f64) -> f64 + Send + Sync,
{
    if should_parallelise(backend, x.len()) {
        let partials: Vec<f64> = x
            ...
            .map(|(cx, cy)| cx.iter().zip(cy.iter()).map(|(p, q)| f(*p, *q)).sum::<f64>())
            .collect();
        partials.iter().sum()
    } else {
...
pub fn dot_field(backend: ComputeBackend, a: &Field<Vector3>, b: &Field<Vector3>) -> Field<f64> {
    ...
    Field::new(zip_map(backend, a.as_slice(), b.as_slice(), |p, q| p.dot(*q)))
}
