...
fn sample(n: usize) -> Field<f64> {
    Field::from_fn(n, |i| {
        ...
        (t * 0.61803398874989).sin() * 1.0e6 + (t * 0.31830988618379).cos() * 1.0e-7
    })
}
...
fn vv_elementwise_scalar_kernels_are_bit_identical_to_serial() {
    for n in SIZES {
        ...
        for backend in BACKENDS {
            assert_eq!(add(backend, &a, &b).as_slice(), ref_add.as_slice(), "add n={n}");
            assert_eq!(sub(backend, &a, &b).as_slice(), ref_sub.as_slice(), "sub n={n}");
            assert_eq!(
...
fn vv_in_place_kernels_match_out_of_place() {
    for n in SIZES {
        ...
        for backend in BACKENDS {
            ...
            assert_eq!(y.as_slice(), scale(s, &a, 0.25).as_slice(), "scale_assign n={n}");

            let mut y = a.clone();
            axpy_assign(backend, &mut y, -3.5, &b);
            assert_eq!(y.as_slice(), axpy(s, &a, -3.5, &b).as_slice(), "axpy_assign n={n}");
        }
    }
}
...
fn vv_elementwise_vector_kernels_are_bit_identical_to_serial() {
    for n in SIZES {
        ...
        for backend in BACKENDS {
            assert_eq!(add(backend, &u, &v).as_slice(), ref_add.as_slice(), "vec add n={n}");
            assert_eq!(
...
/// Pass criterion: relative deviation `< 1e-12` (about four orders of magnitude
/// looser than the measured value, so the test is not brittle, while still
/// catching a genuine reassociation blow-up).
///
/// Results (measured 2026-08-12, `--features parallel`, 4 logical cores,
/// n = 200 003; the numbers below were printed by this test and transcribed):
///
/// | reduction | serial | parallel | relative deviation |
/// |---|---|---|---|
/// | `sum` | 8.335363733929351e5 | 8.335363733929366e5 | 1.75e-15 |
/// | `l2_norm` | 1.5811409718319444e8 | 1.5811409718319444e8 | 0.00e0 |
/// | `dot` | 1.9060861002357006e8 | 1.9060861002356970e8 | 1.88e-15 |
/// | `vol_integral` | 4.167681866964676e0 | 4.167681866964683e0 | 1.75e-15 |
///
/// Interpretation: worst-case relative deviation **1.88e-15**, i.e. ~8 units in
/// the last place of an `f64` — the expected size of a reassociation difference
/// over 2e5 terms, not a defect. Both results are equally correct; neither is
/// "the" right answer in exact arithmetic terms. In the default build (no
/// `parallel` feature) the deviation is identically zero because both paths are
/// the same code.
#[test]
fn vv_parallel_sum_matches_serial_within_tolerance() {
    ...
    let phi = VolScalarField::new(
        ...
        mesh.clone(),
        a.clone(),
        mesh.patches.iter().map(|q| crate::fields::boundary::bc::PatchField::zero_gradient(q.size)).collect(),
    );
    ...
    assert!(rel < TOL, "vol_integral relative deviation {rel:.3e} exceeds {TOL:.0e}");
    worst = worst.max(rel);
    ...
    for backend in BACKENDS {
        ...
        for patch in &c.boundary {
            for j in 0..patch.values.len() {
                assert!((patch.values[j] - 4.5).abs() < 1e-15, "boundary not updated");
            }
        }
...
fn name_does_not_grow_under_self_referential_reassignment_vol() {
    for backend in BACKENDS {
        ...
        assert!((rho.internal[0] - 17.0).abs() < 1e-12, "data must still be right");
...
/// times. **Measured wall clock for the whole test: 1.03 s** on the development
/// machine (2026-08-12, `--release --features parallel`, 4 logical cores).
///
/// Run with:
...
fn measure_crossover_add() {
    ...
    println!("field_parallel_crossover() = {}", field_parallel_crossover());
    println!("{:>10} {:>14} {:>14} {:>10}", "n", "serial [us]", "cpumulti [us]", "speedup");

    for n in [1024_usize, 4096, 16_384, 65_536, 262_144, 1_048_576] {
        let a = sample(n);
        let b = sample_shift(n);
...
/// absolute microseconds per call are printed alongside the speedup. No
/// correctness assertion beyond a sanity check that the two backends agree — the
/// correctness V&V lives in the tests above.
///
/// `#[ignore]`d. **Measured wall clock for the whole test: 2.4 s** on the
/// development machine (2026-08-12, `--release --features parallel`, 4 logical
/// cores). Results are transcribed into the agent hand-off and the crate docs.
///
/// Run with:
