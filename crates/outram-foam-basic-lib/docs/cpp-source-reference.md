# outram-foam-basic-lib — C++ source reference map, module plan & translation notes

Reference material mapping the OpenFOAM C++ source tree to the Rust port,
layer by layer, plus the planned Rust module layout and per-type translation
notes. Consulted on demand when porting a specific layer — not per-turn guidance.

Reference C++ source tree: `/home/teddy0/Documents/research/openfoam/`
...
- **⚠ Never build a field's `name` String compositionally inside an arithmetic
  ...
  - A solver repeatedly reassigns a persistent field from an expression that
    ...
    `outram-foam-appbuilder-lib`. Diagnosed by printing `field.name.len()` per
