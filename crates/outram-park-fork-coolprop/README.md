...
- **Bounded by data-porting scale, not implementation difficulty**
  (mixtures' 887 remaining binary pairs). The *engine* is done and verified —
  Nitrogen–Oxygen is proof it works. What's missing is coefficient data:
  `mixture_binary_pairs.json` is ~10,600 lines, plus a departure-function
  table for any pair with `F ≠ 0`. The one ported pair was chosen specifically
  because `F = 0` (no departure function needed) to prove the reducing-function
  engine in isolation first. Incompressibles had the identical shape of gap —
  a single hand-transcribed fluid (T66) proving the engine — and going from
  "one fluid by hand" to "all 126 via `dev/gen_incompressible.py`/
  `dev/regen_incompressible_all.py`" is exactly the codegen pattern mixtures'
  binary pairs still need (`dev/gen_mixture.py`, unwritten). That codegen pass
  also caught a second real transcription bug (below) purely by *changing* the
  fitted range from one hand-picked point to all 126 fluids' own midpoints in
  a smoke test — evidence for doing this via codegen + broad smoke-testing
  rather than more hand-transcription, wherever that's still the plan (mixture
  binary pairs, departure functions).
