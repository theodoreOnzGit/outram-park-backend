...
fn check(label: &str, t: f64, rho: f64, p_ref: f64, tol: f64) {
    ...
    println!(
        "{label:14} p = {:.6} Pa  (ref {:.6})  rel = {:.3e}",
        s.pressure, p_ref, rel
    );
    assert!(
        rel < tol,
        "{label}: pressure {} Pa vs {} Pa (rel {:.3e} > {:.1e})",
        s.pressure,
        p_ref,
        rel,
        tol
...
fn helium_state_is_physical() {
    ...
    assert!(
        s.cp > 0.0 && s.cv > 0.0 && s.cp >= s.cv,
        "cp={} cv={}",
        s.cp,
        s.cv
    );
    assert!(
        s.speed_of_sound > 0.0 && s.speed_of_sound.is_finite(),
        "w={}",
        s.speed_of_sound
    );
}
