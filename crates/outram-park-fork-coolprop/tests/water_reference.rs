...
fn water_triple_vapor_pressure() {
    ...
    println!(
        "triple_vapor  p = {:.6} Pa  (ref {:.6})  rel = {:.3e}",
        s.pressure, P_TRIPLE, rel
    );
    assert!(
        rel < 1e-3,
        "triple-vapour pressure {} Pa vs {} Pa (rel {:.3e})",
        s.pressure,
        P_TRIPLE,
        rel
    );
...
fn water_triple_liquid_pressure() {
    ...
    let rel = (s.pressure - P_TRIPLE).abs() / P_TRIPLE;
    println!(
        "triple_liquid p = {:.6} Pa  (ref {:.6})  rel = {:.3e}",
        s.pressure, P_TRIPLE, rel
    );
    ...
    assert!(
        rel < 1e-3,
        "triple-liquid pressure {} Pa vs {} Pa (rel {:.3e})",
        s.pressure,
        P_TRIPLE,
        rel
    );
...
fn water_state_is_physical() {
    ...
    assert!(
        s.speed_of_sound.is_finite() && s.speed_of_sound > 0.0,
        "w={}",
        s.speed_of_sound
    );
}
