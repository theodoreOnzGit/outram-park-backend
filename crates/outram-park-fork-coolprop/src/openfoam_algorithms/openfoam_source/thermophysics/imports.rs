...
/// type/unit import block.  These primitives are **crate-internal**
/// (`pub(crate)`), so the block below is illustrative only (marked `ignore`):
///
/// ```rust,ignore
/// use crate::openfoam_algorithms::openfoam_source::thermophysics::imports::*;
