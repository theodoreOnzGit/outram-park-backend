...
fn diffusion_fourier_number_calc() {
    ...
    let sic_diffusion_coeff =
        try_get_diffusion_coeff_jiang(sic, nuclide, temperature, gamma_neutron_fluence).unwrap();

    let fuel_diffusion_coeff =
        try_get_diffusion_coeff_jiang(fuel, nuclide, temperature, gamma_neutron_fluence).unwrap();

    let pyc_diffusion_coeff =
        try_get_diffusion_coeff_jiang(pyc, nuclide, temperature, gamma_neutron_fluence).unwrap();

    let buffer_diffusion_coeff =
        try_get_diffusion_coeff_jiang(buffer, nuclide, temperature, gamma_neutron_fluence).unwrap();

    // then lengthscales
    // radius and layer thickness

    let fuel_radius = Length::new::<micrometer>(425.0 * 0.5);
    let buffer_thickness = Length::new::<micrometer>(100.0);
    let ipyc_thickness = Length::new::<micrometer>(40.0);
    let sic_thickness = Length::new::<micrometer>(35.0);
    ...
    let fourier_number_fuel_1s: Ratio =
        fuel_diffusion_coeff * timestep_1s / fuel_radius / fuel_radius;

    let fourier_number_buffer_1s: Ratio =
        buffer_diffusion_coeff * timestep_1s / buffer_thickness / buffer_thickness;

    let fourier_number_sic_1s: Ratio =
        sic_diffusion_coeff * timestep_1s / sic_thickness / sic_thickness;

    let fourier_number_ipyc_1s: Ratio =
        pyc_diffusion_coeff * timestep_1s / ipyc_thickness / ipyc_thickness;

    // 10s
    let fourier_number_fuel_10s: Ratio =
        fuel_diffusion_coeff * timestep_10s / fuel_radius / fuel_radius;

    let fourier_number_buffer_10s: Ratio =
        buffer_diffusion_coeff * timestep_10s / buffer_thickness / buffer_thickness;

    let fourier_number_sic_10s: Ratio =
        sic_diffusion_coeff * timestep_10s / sic_thickness / sic_thickness;

    let fourier_number_ipyc_10s: Ratio =
        pyc_diffusion_coeff * timestep_10s / ipyc_thickness / ipyc_thickness;
