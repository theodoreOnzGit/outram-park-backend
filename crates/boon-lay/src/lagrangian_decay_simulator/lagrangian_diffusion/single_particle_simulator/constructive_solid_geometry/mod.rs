...
impl Region {
    ...
    pub fn try_return_center_and_radius_of_sphere(&self) -> Option<([Length; 3], Length)> {
        match self {
...
impl TrisoCell {
    ...
    pub fn new(
        ...
        let buffer_region = Region::new_sphere(center, buffer_radius);
        let ipyc_region = Region::new_sphere(center, ipyc_radius);
        let sic_region = Region::new_sphere(center, sic_radius);
        let opyc_region = Region::new_sphere(center, opyc_radius);
    ...
    #[inline]
    pub fn get_lengthscale_for_fourier_number(&self, coordinates: [Length; 3]) -> Length {
        let triso_cell_region: TrisoRegion = self.get_triso_region(coordinates);
