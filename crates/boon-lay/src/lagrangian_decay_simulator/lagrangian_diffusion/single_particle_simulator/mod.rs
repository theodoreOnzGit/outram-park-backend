use uom::{
    si::{f64::*, length::meter, linear_number_density::per_meter, ratio::ratio, time::second},
    ConstZero,
};
...
impl SingleParticleDiffusionSimulatorMC {
    ...
    pub fn move_particle_gaussian_sampling_u64(
        &mut self,
        mean_free_path: Length,
        no_of_collisions: u64,
    ) {
        let per_component_variance =
            per_component_variance_exponential_for_3d_vector_u64(no_of_collisions, mean_free_path);

        let gaussian_length_array =
            sample_dimensioned_gaussian_vector(&mut self.rng.0, per_component_variance);

        self.move_particle_using_array(gaussian_length_array);
    }
    /// move particle assuming normal distribution
    /// given mean free path and number of collisions
    pub fn move_particle_gaussian_sampling_f64(
        &mut self,
        mean_free_path: Length,
        no_of_collisions: f64,
    ) {
        let per_component_variance =
            per_component_variance_exponential_for_3d_vector(no_of_collisions, mean_free_path);
    ...
    pub fn get_gaussian_velocity_vector(
        ...
        let per_component_variance = per_component_variance_exponential_for_3d_vector(
    ...
    pub fn sample_mean_free_path_given_sigma_s(&mut self, sigma_s: LinearNumberDensity) -> Length {
    ...
    #[inline]
    pub fn scatter_isotropically_using_macro_xs(&mut self, sigma_s: LinearNumberDensity) {
