...
impl SingleNuclideSimulatorMC {
    ...
    pub fn new_decay_chain_simulation(
        current_nuclide: Nuclide,
        ...
        for (_nuclide, half_life_info) in decay_chain_for_new_nuclide.iter() {
            match half_life_info {
                HalfLifeAndDecayEnergyInfo::Stable => {
                    ...
                    time_to_live_vec.push(Time::new::<second>(f64::INFINITY));
                }
                HalfLifeAndDecayEnergyInfo::Unstable(half_life, _decay_energy) => {
                    let time_to_live = Self::get_time_to_decay_stochastic(
        ...
        let nuclide_decay_struct: NuclideReactionAndDecayData = decay_library
            .try_match_nuclides_to_decay_data(current_nuclide)
            .unwrap();
    ...
    pub fn transmute_nuclide(&mut self, nuclide: Nuclide, decay_library: &mut DecayLibrary) {
        // i'm going to return a blank decay simulation first

        let mut fresh_simulation = Self::new_decay_chain_simulation(nuclide, decay_library);
    ...
    #[inline]
    pub fn advance_timestep(&mut self, timestep: Time) -> (Nuclide, HalfLifeAndDecayEnergyInfo) {
        // we do loop timing
        ...
        // given a timestep, the job is to then find the next nuclide
        {
            ...
            for (i, (nuclide, half_life_info)) in self.stochastic_decay_chain.iter().enumerate() {
        ...
        self.calculation_time +=
            Time::new::<millisecond>(time_taken_for_calculation_loop_milliseconds);
    ...
    pub fn get_current_half_life(&self) -> Time {
        match self.current_half_life_info {
            HalfLifeAndDecayEnergyInfo::Stable => Time::new::<second>(f64::INFINITY),
            HalfLifeAndDecayEnergyInfo::Unstable(half_life, _decay_energy) => half_life,
        }
    }
    ...
    pub fn get_decay_constant(&self) -> Radioactivity {
        match self.current_half_life_info {
            HalfLifeAndDecayEnergyInfo::Stable => Radioactivity::new::<becquerel>(0.0),
            HalfLifeAndDecayEnergyInfo::Unstable(half_life, _decay_energy) => {
