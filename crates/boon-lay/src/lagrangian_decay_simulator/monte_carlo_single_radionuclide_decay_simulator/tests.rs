...
fn stochastic_half_life_calculator() {
    ...
    for _i in 1..number_of_particles {
        let time_to_live =
            SingleNuclideSimulatorMC::get_time_to_decay_stochastic(&mut rng, half_life);

        time_to_live_vec.push(time_to_live);
    }

    // now let's determine a function to see how many particles remain
    fn determine_surviving_fraction(simulated_time: Time, time_to_live_vec: &Vec<Time>) -> f64 {
        ...
        let surviving_fraction: f64 = survivor_counter as f64 / number_of_particles as f64;
    ...
    let decay_vec_30s_halflife_reference: Vec<(f64, f64)> = vec![
        (0_f64, 1_f64),
...
fn decay_chain_th232() {
    ...
    let mut th232_decay_simulation =
        SingleNuclideSimulatorMC::new_decay_chain_simulation(th232, &mut decay_library);
    ...
    assert_eq!(th232_decay_simulation.get_current_nuclide(), th232);
    ...
    let (ra228, _half_life_info) = th232_decay_simulation.advance_timestep(timestep);
    assert_eq!(ra228, Nuclide::Ra228);

    // the next nuclide in the decay chain is Ra228
    // this is the other way to get the current nuclide
    assert_eq!(th232_decay_simulation.current_nuclide, Nuclide::Ra228);
    ...
    assert!(
        ...
        "expected Po212 or Tl208 after Bi212, got {:?}",
        bi212_daughter,
    );
