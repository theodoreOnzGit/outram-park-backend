...
impl DecayLibrary {
    #[inline]
    pub fn try_match_nuclides_to_decay_data(
        &self,
        nuclide: Nuclide,
        ...
        let decay_data_vec_for_element: Vec<NuclideReactionAndDecayData> = match proton_number {
        ...
        let mut nuclide_reaction_and_decay_data_result_vec: Vec<NuclideReactionAndDecayData> =
            vec![];

        for nuclide_decay_data in decay_data_vec_for_element {
            if nuclide == nuclide_decay_data.nuclide {
                nuclide_reaction_and_decay_data_result_vec.push(nuclide_decay_data);
            }
        }
