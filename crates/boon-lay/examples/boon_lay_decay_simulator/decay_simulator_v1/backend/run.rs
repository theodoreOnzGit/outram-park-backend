        ...
        loop {
            ...
            if simulator_state_clone.is_restart_button_pressed() {
                ...
                for simulation in simulation_vector.iter_mut() {
                    let new_simulation = SingleNuclideSimulatorMC::new_decay_chain_simulation(
                        user_set_nuclide,
                        &mut decay_library,
                    );
                ...
                // nuclide
                let nuclide_info: boon_lay::prelude::NuclideReactionAndDecayData = decay_library
                    .try_match_nuclides_to_decay_data(user_set_nuclide)
                    .unwrap();

                // get half life
                let nuclide_half_life: Time = nuclide_info.try_get_half_life().unwrap();
                ...
                *thread_ptr.lock().unwrap() = (simulation_vector, decay_library);
                if thread_number == 1 {
                    ...
                    simulator_state_ptr
                        .lock()
                        .unwrap()
                        .set_timestep(timestep_based_on_hl);
            ...
            *thread_ptr.lock().unwrap() = (simulation_vector, decay_library);
            ...
            if realtime {
                ...
                let time_to_sleep_milliseconds: u64 = 50;
