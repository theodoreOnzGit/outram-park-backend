...
impl TRISOSimApp {
    pub fn graph_page(&self, ui: &mut Ui) {
        let mut simulator_state_clone = self.simulator_state.lock().unwrap().clone();
        ...
        let plot_width_slider =
            egui::Slider::new(&mut simulator_state_clone.plot_width_pixels, 800.0..=1800.0)
                .logarithmic(false)
                .text("Plot Width (Pixels)")
        ...
        nuclide_plot =
            nuclide_plot.x_axis_label("time (seconds), current time (seconds): ".to_owned());
        nuclide_plot = nuclide_plot.y_axis_label("nuclide fractions".to_owned());
        ...
        nuclide_plot.show(ui, |plot_ui| {
            for (nuclide_index, nuclide) in nuclides_to_plot.iter().enumerate() {
                    ...
                    nuclide_fractions_over_time.iter().enumerate()
                {
                    ...
                    plot_vector_time_and_nuclide_fraction.push([time_seconds, nuclide_fraction]);
    ...
    pub fn graph_page_side_panel(&mut self, ui: &mut Ui) {
        ...
        let csv_simulator_state_clone = self.csv_simulator_state.clone();
        ...
        let timestep_slider_seconds =
            egui::Slider::new(&mut user_set_timestep_seconds, 0.00001..=1e20)
                .logarithmic(true)
        ...
        let label = if self
            ...
            .lock()
            .unwrap()
            .is_change_nuclide_button_pressed()
        {
            "Change Nuclide (pending...)"
        } else {
            "Change Nuclide (OFF)"
        };
        if ui.button(label).clicked() {
        ...
        ui.label(format!(
            ...
            simulator_state_guard
                .get_triso_uniform_temperature()
                .get::<degree_celsius>()
        ));
        ui.label(format!(
        ...
        let mut user_selected_temp_celsius = simulator_state_guard
            .get_user_selected_temperature()
            .get::<degree_celsius>();

        ui.add(
            egui::Slider::new(&mut user_selected_temp_celsius, 0.0..=2200.0)
        ...
        let new_user_selected_temp =
            ThermodynamicTemperature::new::<degree_celsius>(user_selected_temp_celsius);
        ...
        self.simulator_state
            .lock()
            .unwrap()
            .csv_display_interval_seconds = self.csv_simulator_state.csv_display_interval_seconds;
        self.simulator_state
            .lock()
            .unwrap()
        ...
        let csv_display_interval_seconds = self.csv_simulator_state.csv_display_interval_seconds;
        let graph_data_record_interval_seconds =
            self.csv_simulator_state.graph_data_record_interval_seconds;

        let csv_data_display_interval: i32 =
            (csv_display_interval_seconds / graph_data_record_interval_seconds).ceil() as i32;
        ...
        for (idx, (time, nuclide_fraction_vector)) in
            nuclide_fractions_over_time_csv.iter().enumerate()
        {
            ...
            let blank_data_row = time_seconds.round() as u32 != 0;
            let data_display_remainder = display_counter.rem_euclid(csv_data_display_interval);

            let data_display_modulus_zero: bool = data_display_remainder == 0;
