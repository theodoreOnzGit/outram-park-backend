...
use uom::si::{
    ...
    time::{day, hour, millisecond, second, year},
};
...
impl TRISOSimApp {
    pub fn side_panel(&mut self, ui: &mut Ui) {
        ...
        let simulator_state_clone: SimulatorState = self.simulator_state.lock().unwrap().clone();
        ...
        let mut user_set_timestep_seconds = simulator_state_clone.get_timestep().get::<second>();

        let timestep_slider_seconds =
            egui::Slider::new(&mut user_set_timestep_seconds, 0.00001..=1500.0)
                .logarithmic(false)
        ...
        ui.label(format!(
            "Simulated Time (billion years): {}",
            fmt5(sim_years / 1.0e9)
        ));
        ...
        nuclide_fraction_remaining = (nuclide_fraction_remaining * 1e5_f64).round() / 1e5_f64;
        ...
        let (_, nuclide_library) = self.decay_sim_thread_1_ptr.lock().unwrap().clone();
        let nuclide_data: NuclideReactionAndDecayData = nuclide_library
            .try_match_nuclides_to_decay_data(nuclide)
            .unwrap();
        ...
        fn ui_fraction_list(ui: &mut Ui, fractions: &[(Nuclide, f64)]) {
            ...
            for (n, frac) in items.iter() {
                ui.label(format!("{} fraction : {:.5}", nuclide_to_string(*n), frac));
            }
        }
        ...
        let release_fraction: f64 = simulator_state_clone.get_release_fraction().get::<ratio>();
