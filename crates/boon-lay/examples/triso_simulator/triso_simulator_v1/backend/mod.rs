...
use boon_lay::{
    lagrangian_decay_simulator::lagrangian_diffusion::central_limit_theorem::oorandom_rng::OoRng64,
    prelude::{decay_library::DecayLibrary, SingleNuclideSimulatorMC},
    Nuclide,
};
use outram_mc_libs::rng::lcg::{prn, Lcg64};
use uom::{si::f64::Length, ConstZero};

use crate::triso_simulator_v1::{front_end::triso_particle::TrisoParticleUi, TRISOSimApp};

impl TRISOSimApp {
    pub fn construct_new_single_thread_multi_particle_simulation(
        num_of_nuclides: u64,
        nuclide: Nuclide,
        rng_seed: u64,
        ...
        decay_library.random_number_generator = Lcg64::new(rng_seed as u128);

        let mut rng_for_position = OoRng64::from_u64(rng_seed);

        let new_simulation =
            SingleNuclideSimulatorMC::new_decay_chain_simulation(nuclide, &mut decay_library);
        let mut v: Vec<SingleNuclideSimulatorMC> =
            vec![new_simulation; num_of_nuclides.try_into().unwrap()];
        ...
        return Arc::new(Mutex::new((v, decay_library)));
    }
    ...
    pub fn random_point_in_spherical_shell(
        ...
        assert!(
            r_in >= Length::ZERO && r_in < r_out,
            "Require 0 <= r_in < r_out"
        );
    ...
    pub fn random_point_in_triso(
        ...
        let coordinate =
            Self::random_point_in_spherical_shell(r_fuel, r_opyc, &mut rng_for_position.0);
