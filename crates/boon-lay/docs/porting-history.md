# boon-lay — porting/migration history (completed 2026-06)

Reference material: the one-time source-copy checklist, the RNG/uom/egui/edition
migration steps, and the per-file example-porting status. This work is **done**
(both examples ported; `cargo check --workspace --all-targets` green). Kept for
historical reference only — not per-turn guidance.
