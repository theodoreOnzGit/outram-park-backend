...
def case(fn: str, args: list, expected) -> None:
    ROWS.append((fn, [float(a) for a in args], float(expected)))
...
def gen_transient(calc):
    ...
    for val in [0.0, 1e-12, 1e-9, 1e-7, 1e-5, 1e-3]:
        for a in [1e-3, 4.5e-3, 1e-2]:
            case("rf_graph", [val, a], calc.RF_Graph(val, a))
...
def gen_nuclides(calc):
    ...
    for name in sorted(calc.nuclides):
        ...
        ROWS.append((f"nuclide.lam:{case_name}", [], float(nuc.lam)))
...
def main() -> int:
    ...
    seen: set[str] = set()
    dupes = 0
    for fn, a, exp in ROWS:
        line = f"{fn},{';'.join(fmt(x) for x in a)},{fmt(exp)}"
        if line in seen:
            dupes += 1
            continue
        seen.add(line)
