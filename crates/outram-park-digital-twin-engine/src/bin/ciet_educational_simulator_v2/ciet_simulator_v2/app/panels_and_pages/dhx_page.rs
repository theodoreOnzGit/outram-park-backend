...
use uom::si::{
    f64::*, mass_rate::kilogram_per_second, thermodynamic_temperature::degree_celsius, time::second,
};
...
impl CIETApp {
    ...
    pub fn ciet_sim_dhx_branch_page_graph(&mut self, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            let local_ciet_state = self.ciet_state.read().unwrap().clone();
            let current_dhx_br_blocked_state: bool = local_ciet_state.is_dhx_branch_blocked;
            if ui
                .button("Toggle DHX Branch Flow Blocking Mechanism")
                .clicked()
            {
                ...
                self.ciet_state.write().unwrap().is_dhx_branch_blocked =
        ...
        // panel for graphs
        egui::ScrollArea::both().show(ui, |ui| {
            let mut dhx_temp_plot =
                Plot::new("dhx inlet and outlet temp for shell and tube side degC")
            ...
            dhx_temp_plot =
                dhx_temp_plot.x_axis_label("time (seconds), current time (seconds): ".to_owned());
            dhx_temp_plot = dhx_temp_plot.y_axis_label("temperature degree_celsius".to_owned());
            let latest_ciet_plot_data: PagePlotData = self
                .ciet_plot_data_mutex_ptr_for_parallel_data_transfer
                .lock()
                .unwrap()
            ...
            ui.heading(
                "DHX Shell and Tube Heat Exchanger (STHE) Inlet and Outlet Temperature vs Time",
            );
            dhx_temp_plot.show(ui, |plot_ui| {
                plot_ui.line(Line::new(
                    "Pri Loop DHX Branch (dhx shell inlet) temperature deg C",
                    PlotPoints::from(time_shell_inlet_vec.clone()),
                ));
                plot_ui.line(Line::new(
                    "Pri Loop DHX Branch (dhx shell outlet) temperature deg C",
                    PlotPoints::from(time_shell_outlet_vec.clone()),
                ));
                plot_ui.line(Line::new(
                    "DRACS Loop (dhx tube inlet) temperature deg C",
                    PlotPoints::from(time_tube_inlet_vec.clone()),
                ));
                plot_ui.line(Line::new(
                    "DRACS Loop (dhx tube outlet) temperature deg C",
                    PlotPoints::from(time_tube_outlet_vec.clone()),
                ));
            ...
            power_plot = power_plot.x_axis_label("time (seconds)");
            power_plot = power_plot.y_axis_label("mass flowrate (kg/s)".to_owned());

            ui.heading("DHX STHE mass flowrate  vs Time");
            power_plot.show(ui, |plot_ui| {
                plot_ui.line(Line::new(
                    "Shell Side (Pri loop DHX Branch)",
                    PlotPoints::from(time_dhx_br_flowrate_dhx_shell),
                ));
                plot_ui.line(Line::new(
                    "Tube Side (DRACS Loop)",
                    PlotPoints::from(time_dracs_loop_flowrate_dhx_tube),
                ));
            });
