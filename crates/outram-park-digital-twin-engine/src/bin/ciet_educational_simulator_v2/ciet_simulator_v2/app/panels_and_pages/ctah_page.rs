...
use uom::si::{
    f64::*, heat_transfer::watt_per_square_meter_kelvin, thermodynamic_temperature::degree_celsius,
    time::second,
};
...
impl CIETApp {
    ...
    pub fn ciet_sim_ctah_page_graph(&mut self, ui: &mut Ui) {
        ...
        egui::ScrollArea::both().show(ui, |ui| {
            ...
            bt43_bt41_temp_plot = bt43_bt41_temp_plot
                .x_axis_label("time (seconds), current time (seconds): ".to_owned());
            bt43_bt41_temp_plot =
                bt43_bt41_temp_plot.y_axis_label("temperature degree_celsius".to_owned());
            let latest_ciet_plot_data: PagePlotData = self
                .ciet_plot_data_mutex_ptr_for_parallel_data_transfer
                .lock()
                .unwrap()
            ...
            bt43_bt41_temp_plot.show(ui, |plot_ui| {
                plot_ui.line(Line::new(
                    "bt43 (ctah inlet in fwd flow) temperature deg C",
                    PlotPoints::from(time_bt43_vec.clone()),
                ));
                plot_ui.line(Line::new(
                    "bt41 (ctah outlet in fwd flow) temperature deg C",
                    PlotPoints::from(time_bt41_vec.clone()),
                ));

                plot_ui.line(Line::new(
                    "bt41 (ctah outlet in fwd flow) set pt deg C",
                    PlotPoints::from(time_bt41_setpt_vec.clone()),
                ));
            ...
            power_plot = power_plot.y_axis_label("htc (watts per m2 kelvin)".to_owned());

            ui.heading("CTAH Heat Transfer Coefficient (HTC) vs Time");
            power_plot.show(ui, |plot_ui| {
                plot_ui.line(Line::new("CTAH HTC", PlotPoints::from(time_ctah_htc_vec)));
            });
