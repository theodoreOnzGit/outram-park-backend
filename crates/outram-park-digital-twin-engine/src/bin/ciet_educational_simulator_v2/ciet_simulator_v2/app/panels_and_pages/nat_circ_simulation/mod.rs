//! Natural-circulation-only variant of the CIET loop (no CTAH pump).
//!
//! ## Provenance
//!
//! Ported from the CIET Educational Simulator **v1**
//! (`crates/tuas_boussinesq_solver/examples/ciet_educational_simulator/ciet_simulator_v1/app/panels_and_pages/nat_circ_simulation/mod.rs`),
//! GPL-3.0, same licence. Physics unchanged. `egui`-free, so it builds on
//! Android/Termux.
//!
//! **What v2 changed here:** only the shared-state plumbing —
//! `Arc<Mutex<CIETState>>` became the library's `Arc<RwLock<CietState>>`, and the
//! accesses became `read()` / `write()`. Like `full_simulation::archive`, this
//! loop is superseded (the leading underscore means nothing calls it) and does
//! **not** get v2's per-timestep frequency-response driver; the live loop is
//! `super::full_simulation::educational_ciet_loop_version_4`, which handles both
...
pub fn _coupled_dracs_loop_version_7(global_ciet_state_ptr: SharedCietState) {
    use uom::si::f64::*;
    ...
    let local_ciet_state: CIETState = global_ciet_state_ptr.read().unwrap().clone();
    let tchx_outlet_temperature_set_point_degc = local_ciet_state.bt_66_tchx_outlet_set_pt_deg_c;
    ...
    loop {
        ...
        // primary loop DHX plus heater branch update
        {
            ...
            local_ciet_state.fm20_dhx_branch_kg_per_s =
                ((absolute_mass_flowrate_pri_loop.get::<kilogram_per_second>() * 1000.0).round()
                    / 1000.0) as f32;
        ...
        // DRACS loop update
        {
            ...
            local_ciet_state.fm_60_dracs_kg_per_s =
                (absolute_mass_flowrate_dracs.get::<kilogram_per_second>() * 1000.0).round()
                    / 1000.0;
            ...
            && fast_forward_button_on
        {
            ...
            let time_to_sleep: Duration = Duration::from_millis(30);
            thread::sleep(time_to_sleep);
        } else {
