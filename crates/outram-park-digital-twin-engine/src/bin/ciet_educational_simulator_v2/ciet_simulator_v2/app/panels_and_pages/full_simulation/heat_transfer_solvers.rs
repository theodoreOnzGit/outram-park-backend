//! Heat-transfer solvers for the three-branch CIET primary loop: it links up the
//! control volumes of every component (advection and lateral conduction) and then
//! advances them one timestep, excluding the DHX, which the caller handles
//! separately because it couples two loops.
//!
//! ## Provenance
//!
//! Ported from the CIET Educational Simulator **v1**
//! (`crates/tuas_boussinesq_solver/examples/ciet_educational_simulator/ciet_simulator_v1/app/panels_and_pages/full_simulation/heat_transfer_solvers.rs`),
//! GPL-3.0, same licence. **The physics is unchanged** -- same connection order,
//! same Nusselt correlations, same ambient-loss paths, same Prandtl wall
//! correction settings.
//!
//! **What v2 changed here: nothing but this header.** `egui`-free, so this builds
//! on Android/Termux.
//!
//! ## Verification status
//!
