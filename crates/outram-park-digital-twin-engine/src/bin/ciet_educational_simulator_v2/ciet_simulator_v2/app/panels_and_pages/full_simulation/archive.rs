//! Superseded versions of the CIET educational simulator physics loop, kept for
//! reference.
//!
//! ## Provenance
//!
//! Ported from the CIET Educational Simulator **v1**
//! (`crates/tuas_boussinesq_solver/examples/ciet_educational_simulator/ciet_simulator_v1/app/panels_and_pages/full_simulation/archive.rs`),
//! GPL-3.0, same licence. Physics unchanged.
//!
//! **What v2 changed here:** only the shared-state plumbing —
//! `Arc<Mutex<CIETState>>` became the library's `Arc<RwLock<CietState>>`, and the
//! two accesses became `read()` / `write()`. Nothing else. In particular this
//! loop does **not** get v2's per-timestep frequency-response driver: it is
//! superseded (note the leading underscore on the function name, meaning nothing
//! calls it) and adding features to dead code would only invite someone to
//! trust it.
//!
//! The live loop is `super::educational_ciet_loop_version_4`.
...
use tuas_boussinesq_solver::{
    ...
    pre_built_components::ciet_three_branch_plus_dracs::{
        components::{new_active_ctah_horizontal, new_active_ctah_vertical},
        solver_functions::{
    ...
    prelude::beta_testing::HeatTransferEntity,
...
use crate::ciet_simulator_v2::app::panels_and_pages::ciet_data::CIETState;
...
///
pub fn _educational_ciet_loop_version_3(global_ciet_state_ptr: SharedCietState) {
    use uom::si::f64::*;
    ...
    use tuas_boussinesq_solver::pre_built_components::ciet_isothermal_test_components::*;
        ...
        use tuas_boussinesq_solver::pre_built_components::ciet_nat_circ_tests::coupled_dracs_loop_tests::pri_loop_calc_functions::{pri_loop_dhx_shell_temperature_diagnostics, pri_loop_heater_temperature_diagnostics};
    ...
    heater_bottom_head_1b.calibrate_insulation_thickness(pri_loop_cold_leg_insulation_thickness);

    // pri loop hot leg
    //
    heater_top_head_1a.calibrate_insulation_thickness(pri_loop_hot_leg_insulation_thickness);
    ...
    loop {
        ...
        let heater_temp_vec_degc: Vec<f64> = heater_ver_1
        ...
        let heater_surf_temp_vec_degc: Vec<f64> = heater_ver_1
            .pipe_shell_temperature()
            .unwrap()
            .iter()
            .map(|temp| temp.get::<degree_celsius>())
            .collect();
        ...
        tchx_heat_transfer_coeff = {
            ...
            let tchx_max_heat_transfer = HeatTransfer::new::<watt_per_square_meter_kelvin>(10000.0);
        ...
        let ctah_heat_transfer_coeff: HeatTransfer = {
            ...
            let ctah_max_heat_transfer = HeatTransfer::new::<watt_per_square_meter_kelvin>(10000.0);
        ...
        let pri_flowrate_join_handle = thread::spawn(move || {
            ...
            // this was tested ok
            let (dhx_flow, heater_flow, ctah_flow) = three_branch_pri_loop_flowrates_parallel(
                pump_pressure,
                ctah_branch_blocked,
                dhx_branch_blocked,
                &cloned_pipe_4,
        ...
        heater_ver_1.calibrate_heat_transfer_to_ambient(HeatTransfer::ZERO);
        ...
        let ((bt_11, _wt_10), (bt_12, _wt_13)) = pri_loop_heater_temperature_diagnostics(
            &mut heater_bottom_head_1b,
            &mut static_mixer_10_label_2,
