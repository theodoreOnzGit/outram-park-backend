//! Fluid-mechanics solvers for the three-branch CIET primary loop: it solves for
//! the branch mass flowrates given the pump pressure rise and the branch
//! hydrostatic/frictional pressure changes, in parallel across branches.
//!
//! ## Provenance
//!
//! Ported from the CIET Educational Simulator **v1**
//! (`crates/tuas_boussinesq_solver/examples/ciet_educational_simulator/ciet_simulator_v1/app/panels_and_pages/full_simulation/fluid_solvers.rs`),
//! GPL-3.0, same licence. **The physics is unchanged** -- same TUAS
//! `FluidComponentCollection` calls, same branch topology, same handling of the
//! blocked-branch cases.
//!
//! **What v2 changed here: nothing but this header.** The `Arc<Mutex<MassRate>>`
//! handles below are *internal* scratch space for the parallel branch solve, not
//! the shared plant state, so they stay `Mutex` -- there is one writer and one
//! reader per handle and nothing for an `RwLock` to relieve. `egui`-free, so this
//! builds on Android/Termux.
//!
//! ## Verification status
//!
//! The maintainer has done validation work on v1's physics. This is a faithful
//! port; **the port equivalence has not been verified.**
