...
//! (`crates/tuas_boussinesq_solver/examples/ciet_educational_simulator/ciet_simulator_v1/app/useful_functions.rs`),
//! GPL-3.0, same licence.
//!
//! **What v2 changed here:** the plant-state handle passed to
//! [`update_ciet_plot_from_ciet_state`] is now `Arc<RwLock<CietState>>` and the
//! recorder takes a **read** lock, so it no longer serialises against the GUI or
//! the OPC-UA read callbacks. The plot-history handle stays an
//! `Arc<Mutex<PagePlotData>>`: this thread is its only writer.
//!
//! Desktop only -- everything here is `egui`-dependent.
...
use super::{
    panels_and_pages::ciet_data::{CIETState, PagePlotData},
    CIETApp,
};
...
/// A button whose fill colour encodes a temperature.
///
/// `button_temp_degc` is mapped onto the `min_temp_degc..=max_temp_degc` range
/// (all degrees Celsius) and passed through [`hot_to_cold_colour`]: blue at or
/// below the minimum, red at or above the maximum. `name` is the button's label,
/// and the returned button borrows it.
///
/// v2 note: the `<'_>` on the return type is a warning fix only (v1 wrote a bare
/// `egui::Button`, which now elides a lifetime confusingly). Behaviour unchanged.
pub fn new_temp_sensitive_button(
    ...
    let hotness: f32 = (button_temp_degc - min_temp_degc) / (max_temp_degc - min_temp_degc);

    let colour_temp = hot_to_cold_colour(hotness);
    let temp_sensitive_button = egui::Button::new(name).fill(colour_temp);
...
use uom::si::{
    f64::*, heat_transfer::watt_per_square_meter_kelvin, mass_rate::kilogram_per_second,
    power::kilowatt, pressure::pascal, thermodynamic_temperature::degree_celsius, time::second,
};
/// Sample the shared plant state into the plot/CSV history, for ever.
///
/// Runs on its own thread (spawned by `CIETApp::new`). Each pass takes a
/// **read** lock on `ciet_state_ptr`, appends one sample to every trend, and
/// sleeps for `PagePlotData::graph_data_record_interval_seconds` -- so the
/// recording interval is whatever the user set on the page they were last on.
///
/// Never returns; intended to be spawned and forgotten.
pub fn update_ciet_plot_from_ciet_state(
    ciet_state_ptr: SharedCietState,
    ciet_plot_ptr: Arc<Mutex<PagePlotData>>,
) {
    // get current ciet state first
    loop {
        let local_ciet_state: CIETState = ciet_state_ptr.read().unwrap().clone();

        // get the current plot object
        let mut local_ciet_plot: PagePlotData = ciet_plot_ptr.lock().unwrap().clone();

        // let's get the heater data

        let current_time: Time = Time::new::<second>(local_ciet_state.simulation_time_seconds);
        {
            let heater_power: Power =
                Power::new::<kilowatt>(local_ciet_state.heater_power_kilowatts);
        ...
        // ctah pump data
        {
            let ctah_pump_pressure: Pressure =
                Pressure::new::<pascal>(local_ciet_state.get_ctah_pump_pressure_f64());

            let ctah_br_mass_flowrate: MassRate =
                MassRate::new::<kilogram_per_second>(local_ciet_state.fm40_ctah_branch_kg_per_s);
        ...
        // dhx data
        {
            let dhx_shell_side_mass_flowrate = MassRate::new::<kilogram_per_second>(
                local_ciet_state.fm20_dhx_branch_kg_per_s as f64,
            );
            let dhx_tube_side_mass_flowrate =
                MassRate::new::<kilogram_per_second>(local_ciet_state.fm_60_dracs_kg_per_s);
            let dhx_shell_side_inlet_temp = ThermodynamicTemperature::new::<degree_celsius>(
                local_ciet_state.get_dhx_shell_inlet_temp_degc(),
            );
            let dhx_shell_side_outlet_temp = ThermodynamicTemperature::new::<degree_celsius>(
                local_ciet_state.get_dhx_shell_outlet_temp_degc(),
            );
            let dhx_tube_side_inlet_temp = ThermodynamicTemperature::new::<degree_celsius>(
                local_ciet_state.get_dhx_tube_inlet_temp_degc(),
            );
            let dhx_tube_side_outlet_temp = ThermodynamicTemperature::new::<degree_celsius>(
        ...
        let data_record_interval_seconds = local_ciet_plot.graph_data_record_interval_seconds;

        let data_record_interval_ms = data_record_interval_seconds * 1000.0;

        // update the plot
        *ciet_plot_ptr.lock().unwrap().deref_mut() = local_ciet_plot;
