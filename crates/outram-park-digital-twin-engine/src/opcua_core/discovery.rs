...
pub struct DiscoveredSimulator {
    ...
    /// `"ciet-bench3.local."`.
    pub host: String,
    /// TCP port the OPC-UA server listens on. Normally 4840.
    pub port: u16,
...
pub struct MdnsAdvertisement {
    ...
    fullname: String,
    /// Instance name as announced, for logging and for a GUI to display.
    instance_name: String,
    /// The announcing simulator's console prefix, so a withdrawal failure is
    /// reported under the same banner as the rest of its output.
...
impl Drop for MdnsAdvertisement {
    fn drop(&mut self) {
        ...
        if let Err(error) = self.daemon.unregister(&self.fullname) {
            eprintln!(
                "{prefix} mDNS: could not unregister {} ({error})",
                self.fullname
            );
        }
        if let Err(error) = self.daemon.shutdown() {
            eprintln!("{prefix} mDNS: could not shut down the daemon ({error})");
        }
    }
}
...
/// type the URL.
pub fn advertise_simulator<S: OpcuaSimulator>(
    port: u16,
    instance_name: &str,
) -> Result<MdnsAdvertisement, DiscoveryError> {
    let profile = S::PROFILE;
    let instance_name = sanitise_instance_name(instance_name, profile.mdns_instance_prefix);
    let host_name = format!("{instance_name}.local.");

    let mut properties: BTreeMap<String, String> = BTreeMap::new();
    properties.insert(PATH_TXT_KEY.to_owned(), profile.endpoint_path.to_owned());
    properties.insert(
        PRODUCT_TXT_KEY.to_owned(),
        profile.mdns_product_marker.to_owned(),
    );
...
pub struct MdnsBrowser<S: OpcuaSimulator> {
    ...
    /// Which simulator this browser filters for. Zero-sized; carries no data.
    simulator: PhantomData<fn() -> S>,
}

impl<S: OpcuaSimulator> MdnsBrowser<S> {
    ...
    pub fn start() -> Result<Self, DiscoveryError> {
        ...
        let events = daemon
            .browse(OPCUA_MDNS_SERVICE_TYPE)
        ...
        while let Ok(event) = self.events.try_recv() {
            match event {
                ServiceEvent::ServiceResolved(resolved) => {
                    if let Some(simulator) = simulator_from_resolved(
                        &resolved,
...
impl<S: OpcuaSimulator> std::fmt::Debug for MdnsBrowser<S> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let count = self.found.read().map(|f| f.len()).unwrap_or(0);
        f.debug_struct("MdnsBrowser")
            .field("service_type", &OPCUA_MDNS_SERVICE_TYPE)
            .field("product", &S::PROFILE.mdns_product_marker)
            .field("known_simulators", &count)
            .finish_non_exhaustive()
    }
}

impl<S: OpcuaSimulator> Drop for MdnsBrowser<S> {
    fn drop(&mut self) {
        let prefix = S::PROFILE.log_prefix;
        if let Err(error) = self.daemon.stop_browse(OPCUA_MDNS_SERVICE_TYPE) {
            eprintln!("{prefix} mDNS: could not stop browsing ({error})");
        }
        if let Err(error) = self.daemon.shutdown() {
            eprintln!("{prefix} mDNS: could not shut down the browse daemon ({error})");
        }
    }
}

/// Turn a resolved DNS-SD service into a [`DiscoveredSimulator`], or `None` if
/// it is not the simulator we are looking for.
///
/// The filter is the `product` TXT record: an OPC-UA server that does not
/// announce `product=<product_marker>` is not ours and is ignored.
/// `default_endpoint_path` is used when the announcement carries no `path`
/// record.
fn simulator_from_resolved(
    resolved: &ResolvedService,
    product_marker: &str,
    default_endpoint_path: &str,
) -> Option<DiscoveredSimulator> {
    let product = resolved
        .txt_properties
        .get(PRODUCT_TXT_KEY)
        .map(|p| p.val_str().to_owned())?;
    if product != product_marker {
        return None;
    }
    ...
    let path = resolved
        ...
        .get(PATH_TXT_KEY)
        .map(|p| p.val_str().to_owned())
        .unwrap_or_else(|| default_endpoint_path.to_owned());
...
/// trailing `-`. An empty result falls back to `fallback`, and the result is
/// truncated to 63 characters — the DNS label limit.
fn sanitise_instance_name(raw: &str, fallback: &str) -> String {
...
pub enum DiscoveryError {
    ...
    #[error("could not start browsing for {OPCUA_MDNS_SERVICE_TYPE} over mDNS: {0}")]
    Browse(String),
}
...
mod tests {
    ...
    /// Instance-name prefix used by the sanitisation test as its fallback. Any
    /// simulator's prefix would do; this one belongs to no simulator, so the
    /// test cannot pass by accident on a CIET-specific string.
    const TEST_FALLBACK_PREFIX: &str = "OPCUA-Core-Test-Simulator";

    /// Verifies that [`OPCUA_MDNS_SERVICE_TYPE`] is a well-formed DNS-SD service
    ...
    /// network.
    ///
    /// **Results (2026-07-28, unchanged 2026-08-12).** `_opcua-tcp._tcp.local.`
    /// — ends with `.local.` = true; labels = `["_opcua-tcp", "_tcp", "local"]`,
    /// of which 2 are underscore-prefixed; protocol label = `_tcp`; service
    /// label `opcua-tcp` is 9 characters (≤ 15) and uses only letters and a
    /// hyphen. Interpretation: the constant is the IANA-registered
    /// OPC-UA-over-TCP service type, correctly formed, so tooling that already
    /// browses for OPC-UA servers will see an OUTRAM PARK announcement.
    #[test]
    fn mdns_service_type_is_a_valid_dns_sd_type() {
        let service_type = OPCUA_MDNS_SERVICE_TYPE;
        assert!(
    ...
    /// limit), and is never empty. Then check the empty input falls back to the
    /// caller-supplied prefix. No network is used.
    ///
    /// **Results (2026-07-28, unchanged 2026-08-12).** `"bench 3"` →
    /// `"bench-3"`; `"CIET (teaching)"` → `"CIET-teaching"`; `"...."` → the
    /// supplied fallback; `""` → the same fallback; a 200-character input → 63
    /// characters. All four constraints held for every case. Interpretation: a
    /// user may type anything into the instance-name field without producing an
    /// announcement mDNS would reject.
    #[test]
    fn instance_names_are_sanitised_to_dns_safe_labels() {
        ...
        for raw in cases {
            let cleaned = sanitise_instance_name(raw, TEST_FALLBACK_PREFIX);
        ...
        assert_eq!(
            sanitise_instance_name("bench 3", TEST_FALLBACK_PREFIX),
            "bench-3"
        );
        assert_eq!(
            sanitise_instance_name("", TEST_FALLBACK_PREFIX),
            TEST_FALLBACK_PREFIX
        );
    }

    /// Verifies that the DNS-SD full name is reduced to the instance name a user
    /// sees in a client's simulator list.
    ///
    ...
    ///
    /// **Results (2026-07-28, unchanged 2026-08-12).**
