...
impl FHRSimulatorApp {
    ...
    pub(crate) fn four_branch_pri_and_intermediate_loop_single_time_step(
        ...
        let steam_generator_duty: SteamGeneratorDuty;
        {
            ...
            let shell_mean_temperature = sg_fluid_array_clone.try_get_bulk_temperature().unwrap();

            // `T_hot_in` for the pinch: the hottest shell node, which is
            // the salt inlet end of the counter-flow exchanger.
            let shell_inlet_temperature: ThermodynamicTemperature = sg_fluid_array_clone
                .get_temperature_vector()
                .unwrap()
                .into_iter()
                .fold(shell_mean_temperature, |hottest, node_temperature| {
                    if node_temperature.get::<degree_celsius>() > hottest.get::<degree_celsius>() {
                        node_temperature
                    } else {
                        hottest
                    }
            ...
            let salt_specific_heat_capacity: SpecificHeatCapacity = LiquidMaterial::HITEC
        ...
        loop {
            ...
            if let Some(active_freeze) = salt_freeze_monitor.event() {
                if salt_freeze_monitor.take_melt_request() {
                    ...
                    let melt_temperature = ThermodynamicTemperature::new::<degree_celsius>(
            ...
            let pump_outlet_pressure_for_pinch: Pressure = Pressure::new::<bar>(
                fhr_state_clone
                    .lock()
                    .unwrap()
...
mod steam_generator_pinch_at_the_nominal_operating_point {
    ...
    fn the_ua_slider_crossed_partway_up_at_the_nominal_operating_point() {
        ...
        while ua <= 7.0e5 {
            ...
            let outlet_enthalpy = feedwater.enthalpy + Power::new::<watt>(legacy_w) / feed_flow;
            ...
            let flashed = std::panic::catch_unwind(|| {
                ph_flash_eqm::t_ph_eqm(feedwater.pressure, outlet_enthalpy).get::<degree_celsius>()
            });
