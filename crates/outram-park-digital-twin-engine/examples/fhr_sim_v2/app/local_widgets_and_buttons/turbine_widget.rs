...
impl TurbineWidget {
    ...
    pub fn set_theta(&mut self, omega: AngularVelocity, simulation_time: Time) {
...
impl Widget for TurbineWidget {
    fn ui(self, ui: &mut egui::Ui) -> egui::Response {
        ...
        let turbine_blade_stroke = Stroke::new(turbine_blade_axial_thickness, turbine_blade_colour);
        ...
        let turbine_min_radius_pixels: f32 = turbine_max_radius / turbine_num_axial_sides as f32;

        let paint_turbine_blade_set = |set_number: isize| {
            let turbine_radius: f32 = turbine_min_radius_pixels * set_number.abs() as f32;
            ...
            painter.line_segment(
                [
                    turbine_center - vec2(-turbine_x_offset, turbine_radius),
                    turbine_center + vec2(turbine_x_offset, turbine_radius),
                ],
            ...
            if set_number == 0 {
                let axel_radius: f32 = turbine_min_radius_pixels * 0.5 as f32;
                painter.line_segment(
                    [
                        turbine_center - vec2(-turbine_x_offset, axel_radius),
                        turbine_center + vec2(turbine_x_offset, axel_radius),
                    ],
                    turbine_blade_stroke,
                );
            ...
            for i in 0..turbine_num_blades {
                //
                let theta_plus_phase_shift = self.get_theta()
                    + Angle::new::<radian>(i as f64 * (2.0 * PI) / (turbine_num_blades as f64));

                let rotor_blade_y_offset: f64 =
                    0.9 * turbine_radius as f64 * theta_plus_phase_shift.cos().get::<ratio>();

                let rotor_blade_center: Pos2 = Pos2 {
                ...
                if theta_plus_phase_shift.sin() > Ratio::ZERO && set_number > 0 {
                    ...
                    painter.line_segment(
                        [
                            rotor_blade_center
                                - vec2(0.5 * turbine_blade_axial_thickness, 0.1 * turbine_radius),
                            rotor_blade_center
                                + vec2(0.5 * turbine_blade_axial_thickness, 0.1 * turbine_radius),
                        ],
                        turbine_rotor_stroke,
