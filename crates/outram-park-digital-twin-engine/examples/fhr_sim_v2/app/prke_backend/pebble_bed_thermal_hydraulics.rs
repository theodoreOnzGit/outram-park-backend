...
impl PebbleBedThermalHydraulics {
        ...
        let heat_rate_transferred_from_pebble_bed_to_coolant: Power =
            -(pebble_bed_overall_htc * pebble_bed_heat_transfer_area * pebble_bed_coolant_temp
                - pebble_bed_overall_htc
                    ...
                    * self.get_temperature_from_enthalpy_uo2_heuristic());

        let net_power_gain = fission_power_adjusted_for_decay_heat
            - heat_rate_transferred_from_pebble_bed_to_coolant;

        let new_pebble_bed_enthalpy =
            self.current_fuel_specific_enthalpy * pebble_bed_mass + net_power_gain * timestep;

        let new_pebble_bed_specific_entahlpy = new_pebble_bed_enthalpy / pebble_bed_mass;
    ...
    pub fn new() -> Self {
        ...
        return Self {
            current_fuel_specific_enthalpy: start_enthalpy,
        };
    }
    ...
    /// based on assumed constant cp of 340 J/(kg K)
    /// enthalpy at 298K is deemd to be zero J/kg
    pub fn get_temperature_from_enthalpy_uo2_heuristic(&self) -> ThermodynamicTemperature {
        // h = cp (T - 298)
        let ref_temp_298 = TemperatureInterval::new::<uom::si::temperature_interval::kelvin>(298.0);
        let cp = SpecificHeatCapacity::new::<joule_per_kilogram_kelvin>(340.0);
        let cp_times298: AvailableEnergy = cp * ref_temp_298;

        let h_plus_cp_times_298: AvailableEnergy =
            self.current_fuel_specific_enthalpy + cp_times298;

        let fuel_temp_estimate: ThermodynamicTemperature = ThermodynamicTemperature::new::<kelvin>(
    ...
    pub fn get_enthalpy_from_temperature_uo2(
        ...
        let c1 = SpecificHeatCapacity::new::<joule_per_kilogram_kelvin>(302.27);
        let c2 = SpecificHeatCapacity::new::<joule_per_kilogram_kelvin>(8.463e-3)
            / TemperatureInterval::new::<uom::si::temperature_interval::kelvin>(1.0);
        let c3 = AvailableEnergy::new::<joule_per_kilogram>(8.741e7);

        // einstein temp
        let theta = ThermodynamicTemperature::new::<kelvin>(548.68);

        // electron activation energy divide by boltzmann constant
        let e_a = ThermodynamicTemperature::new::<kelvin>(18_531.7);
        ...
        // calc term by term
        let term1: AvailableEnergy = c1
            * theta
            * ((theta_by_t.get::<ratio>().exp() - 1.0).recip()
                - (theta_by_298.get::<ratio>().exp() - 1.0).recip());

        let term2: AvailableEnergy = c2 * (fuel_temp * fuel_temp - ref_temp_298 * ref_temp_298);

        let term3: AvailableEnergy = c3 * (-e_a_by_t).exp();

        // enthalpy increment over 298K
        let enthalpy_increment_over_298_kelvin = term1 + term2 + term3;
