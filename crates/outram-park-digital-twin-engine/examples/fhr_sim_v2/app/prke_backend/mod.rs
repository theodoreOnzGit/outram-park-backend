...
impl FHRSimulatorApp {
    ...
    pub fn calculate_prke_for_one_timestep(
        ...
        // Prompt term scaled by `prompt_power_fraction()` (derived from the
        // tabulated decay energy per fission, not a tuned 0.9) so decay energy
        // is not double-counted; the two terms sum to full power at equilibrium.
        let mut fission_power_corrected_for_decay_heat =
