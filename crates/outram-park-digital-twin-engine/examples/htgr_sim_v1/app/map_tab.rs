//! **Map** -- the live thermal and fission-product map of the core.
//!
//! # What this tab is, and how it differs from the Geometry tab
//!
//! [`super::geometry_tab`] draws the HTR-10 R-Z benchmark zones as a **static
//! reference**: it reads nothing from [`HtgrSnapshot`] and does not change
//! across a frame. This tab draws the *same published geometry* and colours it
//! from the **live plant state**, then drills down through the two length
//! scales the geometry tab cannot show:
//!
...
//! # NOT VALIDATED
//!
//! Everything the Geometry tab's caveat covers applies here unchanged -- the
//! R-Z reconstruction is not validated -- and the release table carries its
//! own, stronger caveat: it is **per curie of core inventory**, not curies.
...
use super::geometry_tab::zone_render_polygons;
...
fn draw_live_cross_section(ui: &mut Ui, s: &HtgrSnapshot) {
    ...
    // measured off, so the Geometry tab keeps the zoom controls and this one
    // scales to the panel.
...
fn draw_profile_table(ui: &mut Ui, s: &HtgrSnapshot) {
    egui::Grid::new("htgr_map_profile_grid")
        ...
        .show(ui, |ui| {
            ...
            let mut row = |ui: &mut Ui, name: &str, value: f64, previous: &mut f64| {
                ui.label(name);
...
/// Draw the whole Map tab.
pub fn draw_map(ui: &mut Ui, s: &HtgrSnapshot) {
    ui.heading("Core map -- live thermal state and fission-product release");
    ui.label(
        "The same published R-Z benchmark geometry the Geometry tab draws, coloured from the \
         live plant state, then resolved down to the fuel kernel that the Doppler feedback and \
         the TRISO release model are both driven from. Grey zones are structure this one-node \
         model carries no temperature for -- not missing data, not modelled.",
    );
    ui.separator();
