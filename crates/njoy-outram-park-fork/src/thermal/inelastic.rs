...
impl IncoherentInelastic {
    ...
    pub fn cross_section(&self, e: f64, temp_k: f64, natom: f64) -> f64 {
        ...
        let tev = BK_EV_PER_K * temp_k;
        // Each table β maps to an energy transfer |E'−E| = β·D (D = kT₀ for LAT=1,
        // else kT). Using the table's own β grid for both scatter directions puts
        // the E' quadrature points exactly where S(α,β) has structure — the wide
        // uniform grid a naive ∫dE' would use wastes all its resolution.
        let d = if self.lat == 1 { TEVZ } else { tev };
        let mut eps: Vec<f64> = Vec::with_capacity(2 * self.beta.len() + 1);
        eps.push(e); // the quasi-elastic point β = 0
        for &bj in &self.beta {
            let down = e - bj * d; // downscatter (E' < E)
            let up = e + bj * d; // upscatter   (E' > E)
            if down > 1.0e-5 {
                eps.push(down);
            }
            eps.push(up);
        ...
        // Trapezoid over that grid. Cache σ(E→E') at each node.
        let sig: Vec<f64> = eps.iter().map(|&ep| self.sigma_e_to_ep(e, ep, temp_k, natom)).collect();
        let mut sum = 0.0;
        for k in 1..eps.len() {
