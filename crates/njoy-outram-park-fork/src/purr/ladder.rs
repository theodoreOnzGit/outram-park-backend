//! Resonance-ladder generation and the analytic infinite-dilution reference —
//! `rann`, `ladr2`, `unresx`/`gnrx`, `rdheat` (`purr.f90`).
//!
//! Split out of `purr/mod.rs` (see the module doc there for the overall
//! `PURR` port map and status); this is the "ladder generation" half of the
//! plan-named boundary, the Monte Carlo core (`unrest`) lives in
//! [`super::unrest`].
...
/// freedom (1..=4).
pub(super) const CHISQ: [[f64; 4]; 20] = [
...
pub(super) fn chisq_quantile_index(rng: &mut Rng, scale: f64) -> usize {
