...
pub struct SequenceLadderParams {
    ...
    pub dbar: f64,
    /// Mean energy-scaled neutron width `Γn(E) = GNO·√E·V_l(E)·AMUN` \[eV\]
    /// — the sequence's *total* mean neutron width over its `AMUN` channels
    /// (`unresx`'s `cgn`, see the `vl_amun` note in
    /// [`infinite_dilution_reference`]). [`generate_ladder`] divides it by
    /// the degrees of freedom again before applying the χ² multiplier.
    pub gn_mean: f64,
...
pub fn infinite_dilution_reference(
    ...
    for range in ranges {
        ...
        for seq in range_sequences(range, e)? {
            ...
            // unresx calls `unfac2(ll,rho,rhoc,amun,vl,ps)` with the REAL
            // `amun`, and unfac2 folds it into the penetrability
            // (`vl=amun*r2/(1+r2)`, purr.f90:1497-1507); unresx then
            // multiplies by `amun` again (`gnx=gnox*vl*e2*amun`, :1428) and
            // divides by `nint(amun)` (:1442). Net: `GNO·V_l·√E·AMUN²/nint(AMUN)`
            // = `GNO·V_l·√E·AMUN` for integer AMUN — the sequence's mean
            // *total* neutron width, the same value UNRESR's unresl reaches
            // via `uunfac(..,one,..)` then `×amun` (unresr.f90:1021, 1068).
            // Until 2026-09-10 this port used unresl's un-folded `vl` with
            // unresx's `/nu`, halving every AMUN=2 (p-wave) sequence's
            // neutron width in both the analytic reference and the ladders:
            // U-235 capture/fission 1.7 % low at 2.25 keV, 6 % low at 10 keV
            // against NJOY (tests/purr_u235_urr.rs).
            let vl_amun = vl * seq.amun;
            let gnx_full = seq.gno * vl_amun * e2 * seq.amun;
...
mod tests {
    ...
    fn ladder_spacing_has_the_wigner_mean_and_width() {
        ...
        let spacings: Vec<f64> = ladder.windows(2).map(|w| w[1].energy - w[0].energy).collect();
        ...
        assert!((mean - dbar).abs() < 4.0 * want_sd / n.sqrt(), "mean {mean} vs {dbar}");
        assert!((var.sqrt() - want_sd).abs() < 0.02 * want_sd, "sd {} vs {want_sd}", var.sqrt());
        // First resonance uniform in [elow, elow + D√(4/π)).
        assert!(ladder[0].energy >= 0.0 && ladder[0].energy < dbar * (4.0 / std::f64::consts::PI).sqrt());
        ...
        assert!(ladder[..ladder.len() - 1].iter().all(|r| r.energy <= 400_000.0));
        for r in &ladder {
    ...
    fn ladder_neutron_widths_average_to_the_mean_width() {
        ...
        let mean_gn = ladder.iter().map(|r| r.gn_frac * r.total_width).sum::<f64>() / n;
        let table_mean: f64 = CHISQ.iter().map(|row| row[0]).sum::<f64>() / 20.0;
        assert!((mean_gn / seq.gn_mean - table_mean).abs() < 0.03, "{mean_gn} vs {table_mean}");
    ...
    fn line_shape_tiers_reproduce_uw2_to_each_approximations_accuracy() {
        ...
        for &(x, y, tol) in cases {
            ...
            assert!(e <= tol, "line_shape({x},{y}) = {got:?} vs uw2 {want:?}: rel err {e:e} > {tol:e}");
        }
    }
    ...
    fn line_shape_selects_the_same_tier_as_upstreams_index_ranges() {
        ...
        let two_term = |x: f64, y: f64| {
            ...
            let _ = yy;
            (f1 * (temp1 - a4 * y) + f2 * (temp1 - a5 * y), f1 * (a4 * x + temp2) + f2 * (a5 * x + temp2))
        };
        // Just past 100 → asymptotic; just under → 2-term.
        assert_eq!(line_shape(100.001, 1.0, 1.0, &table), asym(100.001, 1.0));
        assert_eq!(line_shape(1.0, 100.001, 100.001f64.powi(2), &table), asym(1.0, 100.001));
        ...
        assert_eq!(line_shape(0.2, 6.001, 6.001f64.powi(2), &table), two_term(0.2, 6.001));
        ...
        assert_eq!(line_shape(3.85, 2.95, 2.95f64.powi(2), &table), table.lookup_coarse(3.85, 2.95));
        assert_eq!(line_shape(3.85, 0.5, 0.25, &table), table.lookup_coarse(3.85, 0.5));
        assert_eq!(line_shape(3.85, 0.499, 0.499f64.powi(2), &table), table.lookup_fine(3.85, 0.499));
        assert_eq!(line_shape(-1.0, 0.1, 0.01, &table), table.lookup_fine(-1.0, 0.1));
    }
}
