...
fn draw_geometry_picker(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    for (row, idx) in (start..(start + visible).min(len)).enumerate() {
        ...
        let selected =
            std::mem::discriminant(&card_preset) == std::mem::discriminant(&app.selected_preset);
        ...
        tap_target(
            ...
            app,
            rect,
            UiAction::SelectPreset(card_preset),
            block,
            body,
        );
        ...
        if matches!(card_preset, GeometryPreset::BareSphere(_)) && rect.width > 14 {
            let toggle_rect = Rect {
                x: rect.x + rect.width.saturating_sub(12),
                y: rect.y,
                width: 11,
                height: 1,
            };
            app.hit_regions
                .push((toggle_rect, UiAction::ToggleSphereVariant));
        }
    }
    ...
    if len > visible {
        ...
        frame.render_widget(
            Paragraph::new(format!("({}/{}) scroll for more", start + 1, len))
...
fn draw_settings(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let compute_rect = Rect {
        x: area.x,
        y: area.y + ROW_HEIGHT,
        width: area.width,
        height: COMPUTE_PANEL_HEIGHT,
    };
    ...
    let compute_text = format!(
        ...
        s.compute_label(),
        app.gpu_status_line(),
        app.compute_dispatch_note()
    );
    tap_target(
        frame,
        app,
        compute_rect,
        UiAction::CycleCompute,
        Block::default()
            .borders(Borders::ALL)
            .title(" tap to cycle "),
    ...
    let rows_area = Rect {
        ...
        height: area
            .height
            .saturating_sub(ROW_HEIGHT + COMPUTE_PANEL_HEIGHT + 3),
    };
    ...
    let rows: Vec<(String, UiAction, UiAction)> = vec![
        (
            format!("Histories/gen: {}", s.n_particles),
            UiAction::BumpParticles(-1),
            UiAction::BumpParticles(1),
        ),
        (
            format!("Inactive gens: {}", s.n_inactive),
            UiAction::BumpInactive(-1),
            UiAction::BumpInactive(1),
        ),
        (
            format!("Active gens (batches): {}", s.n_active),
            UiAction::BumpActive(-1),
            UiAction::BumpActive(1),
        ),
        (
            format!("Seed: {}", s.seed),
    ...
    for (row, i) in (start..(start + visible).min(N_ROWS)).enumerate() {
        let rect = Rect {
            x: rows_area.x,
            y: rows_area.y + row as u16 * ROW_HEIGHT,
        ...
        let [minus_r, label_r, plus_r] = Layout::horizontal([
            Constraint::Length(5),
            Constraint::Min(6),
            Constraint::Length(5),
        ])
        ...
        frame.render_widget(
            Block::default()
                .borders(Borders::ALL)
                .title(Line::from(label.as_str()).centered()),
            label_r,
        );
        tap_target(
            frame,
    ...
    tap_target(
        ...
        Block::default()
            .borders(Borders::ALL)
            .border_style(Style::default().fg(GOOD).add_modifier(Modifier::BOLD)),
        Paragraph::new("RUN")
            .centered()
...
fn draw_run(frame: &mut Frame, app: &mut App, area: Rect) {
    let [status_area, chart_area, nav_area] = Layout::vertical([
        Constraint::Length(3),
        Constraint::Min(5),
        Constraint::Length(3),
    ])
    ...
    match &app.last_outcome {
        Some(Ok(outcome)) => {
            ...
            frame.render_widget(
                Paragraph::new(text).style(Style::default().fg(GOOD)),
                status_area,
            );
        ...
        Some(Err(e)) => {
            frame.render_widget(
                Paragraph::new(format!("run failed: {e}")).style(Style::default().fg(BAD)),
                status_area,
            );
        }
        None => match app.run_handle.snapshot() {
            RunPhase::Running { started } => {
                ...
                frame.render_widget(
                    Paragraph::new(text).style(Style::default().fg(WARN)),
                    status_area,
                );
            }
    ...
    tap_target(
        ...
        UiAction::StartRun,
        Block::default().borders(Borders::ALL),
        Paragraph::new("Rerun").centered(),
    );
    let spectrum_style = if can_spectrum {
        Style::default().fg(ACCENT)
    } else {
        Style::default().add_modifier(Modifier::DIM)
    };
    tap_target(
        ...
        Block::default()
            .borders(Borders::ALL)
            .border_style(spectrum_style),
        Paragraph::new(if can_spectrum {
            "Spectrum >"
        } else {
            "Spectrum (n/a)"
        })
...
fn draw_convergence_chart(frame: &mut Frame, area: Rect, k_by_generation: &[f64], revealed: usize) {
    ...
    let points: Vec<(f64, f64)> = k_by_generation[..n]
        .iter()
        .enumerate()
        .map(|(i, &k)| (i as f64, k))
        .collect();
    ...
    let chart = Chart::new(vec![dataset])
        ...
        .x_axis(
            Axis::default()
                .title("generation")
                .bounds([0.0, x_hi])
                .labels(["0".to_string(), format!("{x_hi:.0}")]),
        )
        .y_axis(
            Axis::default()
                .title("k")
                .bounds([y_lo, y_hi])
                .labels([format!("{y_lo:.3}"), format!("{y_hi:.3}")]),
        );
...
fn draw_spectrum(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let [back_r, _spacer] =
        Layout::horizontal([Constraint::Length(14), Constraint::Min(1)]).areas(nav_area);
    tap_target(
        frame,
        app,
        back_r,
...
fn draw_overlay_chart(frame: &mut Frame, area: Rect, overlay: &crate::spectrum::SpectrumOverlay) {
    ...
    let flux_points: Vec<(f64, f64)> = log_e
        .iter()
        .zip(&overlay.flux_mean)
        .map(|(&x, &y)| (x, y / flux_max))
        .collect();
    let xs_points: Vec<(f64, f64)> = log_e
        .iter()
        .zip(&overlay.xs_mid)
        .map(|(&x, &y)| (x, y / xs_max))
        .collect();
    ...
    let chart = Chart::new(vec![flux_ds, xs_ds])
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(" flux vs Sigma_t (log10 E/eV) "),
        )
        .x_axis(
            Axis::default()
                .title("log10(E/eV)")
        ...
        .y_axis(
            Axis::default()
                .title("normalized")
                .bounds([0.0, 1.05])
                .labels(["0".to_string(), "1".to_string()]),
        );
    frame.render_widget(chart, area);
}
