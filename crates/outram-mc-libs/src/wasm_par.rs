...
//!
//! For **this crate** the degradation is numerically exact, not merely
//! acceptable. The Monte Carlo drivers seed every history individually and
//! their own docs state the eigenvalue is identical regardless of
//! `n_threads` — only wall time changes. A pool of one is just the slowest
//! member of that family. If a future kernel is ever added whose result *does*
//! depend on the worker count, it must not use this shim without saying so.
//!
//! # Scope
//!
