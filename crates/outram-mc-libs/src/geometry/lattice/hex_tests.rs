...
fn notebook_lattice() -> HexLattice {
    let outer_ring: Vec<usize> = std::iter::once(1)
        .chain(std::iter::repeat(0).take(17))
        .collect();
    let ring_1: Vec<usize> = std::iter::once(1)
        .chain(std::iter::repeat(0).take(11))
        .collect();
    let ring_2: Vec<usize> = std::iter::once(1)
        .chain(std::iter::repeat(0).take(5))
        .collect();
...
fn valid_tile_count_matches_hexagon_formula() {
    ...
    assert_eq!(
        valid,
        (3 * nr * (nr - 1) + 1) as i32,
        "4-ring hexagon should have 37 tiles"
    );
    assert_eq!(valid, 37);
}
...
fn distance_from_center_matches_hex_face_geometry() {
    ...
    assert!(
        (dy - half).abs() < 1e-9,
        "+y distance {dy} should be half-pitch {half}"
    );
...
fn x_orientation_round_trip() {
    ...
    for (p, expect_i, expect_u) in cases {
        ...
        assert_eq!(
            gi, expect_i,
            "get_indices({p:?}) = {gi:?}, expected {expect_i:?}"
        );
...
fn from_rings_3d_stacks_axial_levels() {
    ...
    assert_eq!(
        set0,
        vec![10, 11, 12, 13, 14, 15, 42],
        "bottom level holds level-0 universes"
    );
    assert_eq!(
        set1,
        vec![20, 21, 22, 23, 24, 25, 84],
        "top level holds level-1 universes"
    );
    ...
    assert!(
        set0.contains(&u_bot),
        "bottom-centre universe {u_bot} in level-0 set"
    );
    assert!(
        set1.contains(&u_top),
        "top-centre universe {u_top} in level-1 set"
    );
    assert_ne!(
        u_bot, u_top,
        "same planar tile at different axial levels differs"
    );
}
...
fn assert_ring_fill_round_trip(orientation: HexOrientation, n_rings: usize) {
    ...
    for iy in 0..nside {
        for ix in 0..nside {
            ...
            if lat.are_valid_indices(i) {
                ...
                assert!(
                    !seen[stored as usize],
                    "universe {stored} placed on two tiles"
                );
    ...
    assert_eq!(
        valid_count, n_tiles,
        "valid tile count == N for {n_rings} rings"
    );
    assert!(
        seen.iter().all(|&s| s),
        "every universe 0..{n_tiles} placed exactly once"
    );
