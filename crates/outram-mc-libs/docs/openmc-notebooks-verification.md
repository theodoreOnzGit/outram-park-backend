...
| `depletion` | outram-mc | `deplete.CoupledOperator`, `deplete.PredictorIntegrator`, `deplete.Results`, `deplete.Chain`, `model.pin` | Depletion is out of scope in outram-mc (`src/chain.cpp` not ported) | no | op-6tz.18 | May land in a future dedicated crate; transmutation matrix absent. |
