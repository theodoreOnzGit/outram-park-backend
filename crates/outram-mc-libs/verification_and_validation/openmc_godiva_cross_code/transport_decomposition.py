...
def mubar(dist, e):
    ...
    try:
        i = min(max(np.searchsorted(dist.energy, e), 0), len(dist.mu) - 1)
        d = dist.mu[i]
        if isinstance(d, openmc.stats.Tabular):
            x, p = np.array(d.x), np.array(d.p)
            return float(np.trapezoid(x * p, x) / np.trapezoid(p, x)) if p.sum() > 0 else 0.0
    except Exception:
        pass
    return 0.0
...
print(f"  inelastic  <mu_lab> = {np.sum(w*num_in)/np.sum(w*sig_in):.4f}"
      f"   ({100*f_in:.1f}% of scattering)   <-- this crate samples it as 0")

Str_true = fw(sig_tot - (num_el + num_in))
Str_ours = fw(sig_tot - num_el)
rel = Str_ours / Str_true - 1.0
print(f"\n  Sigma_tr [1/cm]   should be {Str_true:.5f}   this crate {Str_ours:.5f}"
      f"   ({100*rel:+.2f}%)")
