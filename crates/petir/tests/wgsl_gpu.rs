//! **GPU verification of [`petir::wgsl`]** — the shaders are compiled,
//! dispatched on a real device, and compared against the `f32` CPU mirror and
//! the `f64` reference.
//!
//! # Why this is a test file and not a library module
//!
//! The library is `#![no_std]` unconditionally. A `wgpu` feature inside it
//! would repeat the `std`/`platform-libm` feature removed on 2026-09-17
//! (`bn:op-7kwt`) for being uncompilable in every configuration. A test binary
//! links `std` already, so the device handling belongs here and the shader
//! source stays `no_std` where it is useful to everyone.
//!
//! # What each comparison establishes
//!
//! Three layers, and they are not interchangeable:
//!
//! | comparison | establishes | expected |
//! |---|---|---|
//! | GPU vs `f32` mirror | the WGSL transcription is faithful | at or near **zero** |
//! | mirror vs `f64` PETIR | what `f32` costs | ~1e-7, measured in `wgsl::mirror` |
//! | naga validation | the shader text is well-formed | no diagnostics |
//!
//! A GPU-vs-`f64` comparison alone would conflate the first two and could not
//! tell a transcription bug from single-precision rounding. That is the whole
//! reason the mirror exists.
//!
//! # No adapter is not a pass
//!
//! On a host with no GPU these tests print `SKIP` and return, following the
//! convention already used by `outram-mc-libs`'s GPU tests. **A skipped run
//! proves nothing**, so the naga validation tests below are written to run
//! without a device: they are the floor of what CI always checks, and they
//! catch every syntax and type error even where nothing can be dispatched.
//!
//! This container runs Mesa's **lavapipe** software Vulkan device
//! (`llvmpipe (LLVM 20.1.2, 256 bits)`), so the dispatch tests do execute
//! here; every number in a `# Results` block below was measured on it. A
//! different device may round differently — see the ULP note at the end.
//!
//! # WGSL is not required to be correctly rounded
//!
//! The WGSL specification bounds builtin accuracy in ULP rather than requiring
//! correct rounding, and permits `a * b + c` to be contracted into an FMA at
//! the implementation's discretion. So GPU-vs-mirror is asserted at a small
//! tolerance rather than at bit-identity, and the measured number is recorded
//! so a regression is visible even while staying inside that tolerance.
...
/// Block on a future without pulling in an async runtime.
///
/// A `Wake`-based thread park, the same shape as `pollster::block_on`. Taken
/// from the pattern already established in `outram-mc-libs::gpu`, which exists
/// for the same reason: `wgpu`'s `request_adapter` / `request_device` return
/// futures, and neither crate wants an executor as a dependency for two calls.
...
/// Acquire a headless compute device, or `None` when the host has no adapter.
fn probe() -> Option<Gpu> {
    ...
    let (device, queue) = block_on(adapter.request_device(&wgpu::DeviceDescriptor {
        label: Some("petir wgsl verification device"),
...
/// Uniform block matching `struct Params` in [`petir::wgsl::test_kernel`].
#[repr(C)]
#[derive(Clone, Copy)]
struct Params {
    n: u32,
...
fn as_bytes<T: Copy>(v: &[T]) -> &[u8] {
    // SAFETY: `T` is `Copy` and we only ever pass `f32` / `Params`, both of
    // which are plain old data with no padding that matters to the GPU. The
    // slice is read-only and the lifetime is tied to `v`.
    unsafe { std::slice::from_raw_parts(v.as_ptr().cast::<u8>(), std::mem::size_of_val(v)) }
}

/// Run one of this crate's WGSL functions over `probe_values` and read back.
///
/// `sources` are the `petir::wgsl` shader constants to include, `call` is the
/// WGSL expression to evaluate per element (with `x` bound to the probe value
/// and `src` to `data`).
fn dispatch(
    gpu: &Gpu,
    sources: &[&str],
    ...
    // A zero-length storage buffer is invalid; pad to one element.
    ...
    let probe_buf = gpu
        ...
        .create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("probe"),
            contents: as_bytes(probe_values),
            usage: wgpu::BufferUsages::STORAGE,
        });
    let out_size = (probe_values.len() * std::mem::size_of::<f32>()) as u64;
        ...
        .create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
    {
        ...
        let groups = probe_values.len().div_ceil(64) as u32;
