...
// Only the test module below needs a growable buffer; the recurrence blocks
// themselves are fixed-state and allocation-free apart from their VecDeque.
#[cfg(test)]
...
// no_std uom has no `sqrt`/`exp` on Quantity (it falls back to FloatCore);
// RatioExt restores them for dimensionless Ratio. See transfer_fn::ratio_ext.
#[allow(unused_imports)]
use crate::transfer_fn::ratio_ext::RatioExt;
#[allow(unused_imports)]
use crate::real::Real;
...
use crate::transfer_fn::ChemEngProcessControlSimulatorError;
...
impl FirstOrderStableTransferFnForZeroes {
    ...
    pub(crate) fn reference_response(
        ...
        use crate::transfer_fn::step_fn::StepFunction;
