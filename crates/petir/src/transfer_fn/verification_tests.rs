...
use crate::transfer_fn::first_order_transfer_fn::FirstOrderStableTransferFnNoZeroes;
use crate::transfer_fn::second_order_transfer_fn::SecondOrderStableStepResponse;
...
fn zoh_first_order_reproduces_o1_recurrence_block() {
    ...
    std::println!("zoh vs recurrence block: max |diff| = {max_diff:e}");
...
fn zoh_second_order_step_response_exact_at_samples() {
    ...
    for zeta_val in [0.45, 1.0, 1.2] {
        ...
        std::println!("zoh second order, zeta = {zeta_val}: max_dev = {max_dev:e}");
...
fn tustin_round_trip_recovers_continuous_coefficients() {
    ...
    std::println!("tustin round trip: max_dev = {max_dev:e}");
...
fn matched_maps_poles_zeros_and_matches_dc_gain() {
    ...
    std::println!("matched G2: added zero dev = {zero2_dev:e}, dc_dev = {dc_dev2:e}");
...
fn matched_round_trip_recovers_continuous_coefficients() {
    ...
    std::println!("matched round trip: max_dev = {max_dev:e}");
...
fn filt_docstring_example_impulse_response() {
    ...
    std::println!("filt example: first samples = {first_four:?}, max_dev = {max_dev:e}");
...
fn discrete_block_state_size_does_not_grow() {
    ...
    std::println!("state size constant at 2 over {} steps", inputs.len());
}
...
fn error_paths_return_documented_variants() {
    ...
    std::println!("all six error paths returned their documented variants");
}
