...
fn minimaxs(p: f64) -> f64 {
    ...
    let s =
        t - (A0 + t * (A1 + t * (A2 + t * A3))) / (1.0 + t * (B0 + t * (B1 + t * (B2 + t * B3))));
...
pub fn inv_inc_gamma(a: f64, p: f64) -> f64 {
    ...
    if a < 1.0 {
        ...
        let w = a + s * sqrta + (s2 - 1.0) / 3.0 + (s3 - 7.0 * s) / (36.0 * sqrta)
...
mod tests {
    ...
    fn round_trip(a: f64, p: f64, tol: f64) {
        ...
        assert!(
            (p2 - p).abs() < tol,
            "round-trip failed a={a} p={p}: got P={p2} (diff={})",
            (p2 - p).abs()
        );
    }
    ...
    fn a_eq_1_explicit() {
        ...
        assert!((x - core::f64::consts::LN_2).abs() < 1e-10, "x={x}");
    }
