...
//
// PROVENANCE — LIFTED VERBATIM, DO NOT DIVERGE SILENTLY.
//
// This file is a verbatim copy of
//     crates/outram-foam-basic-lib/src/matrix/square_matrix.rs
// (itself a translation of OpenFOAM's `scalarSquareMatrix` / `LUDecompose`),
// with exactly three mechanical edits, made only to reach `no_std`:
//
//   1. `std::fmt::Display`  -> `core::fmt::Display`
//   2. `std::fmt::Formatter`/`std::fmt::Result` -> the `core::fmt` equivalents
//   3. `std::error::Error`  -> `core::error::Error`
//
// plus the two `alloc` imports below, which `std`'s prelude supplied for free.
// The numerics — Crout LU with scaled partial pivoting, the back-substitution,
// the singular-pivot detection — are byte-identical to the source, and the test
// module is unchanged. Keep it that way: if a defect is found here it is a
// defect there too, and the fix belongs in both. `tests/verbatim_provenance.rs`
// re-checks this mechanically.

//! Dense square-matrix linear algebra (`no_std`).
//!
//! [`SquareMatrix`] is a row-major `n x n` matrix of `f64` with in-place LU
//! decomposition (Crout's algorithm, scaled partial pivoting) and
//! back-substitution — the direct dense solver the rest of PETIR builds on, and
//! the counterpart of GSL's `gsl_linalg_LU_decomp` / `gsl_linalg_LU_solve`.
//!
//! Entries are bare `f64` and carry no unit information: this is a numerical
//! kernel, not a dimensioned physical quantity. A `uom`-typed façade, where one
//! makes sense, belongs in [`crate::units`].
//!
//! # Relation to GSL
//!
//! GSL pivots on the largest absolute value in the column
//! (`gsl_linalg_LU_decomp`); this routine pivots on the largest value *relative
//! to its row's largest element* (implicit scaling, the "Crout with scaled
//! partial pivoting" of Numerical Recipes §2.3, which is what OpenFOAM uses).
//! The two agree on well-scaled matrices and the scaled variant is the more
//! robust of the two when rows differ wildly in magnitude, so this port keeps
//! OpenFOAM's choice rather than substituting GSL's. Stated here because it is
//! a real numerical difference, not a stylistic one.
