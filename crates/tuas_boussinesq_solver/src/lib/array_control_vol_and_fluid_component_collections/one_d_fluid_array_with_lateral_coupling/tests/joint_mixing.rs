...
pub fn fluid_array_adiabatic_mixing_joint_test() {
    ...
    while simulation_time < max_time {
        inlet_pipe_1
            .link_constant_temperature_to_back_of_this_cv(
                hot_temp,
                advection_heat_transfer_interaction_pre_joint,
            )
            .unwrap();
        ...
        outlet_pipe
            .link_heat_addition_to_front_of_this_cv(
                Power::ZERO,
                advection_heat_transfer_interaction_post_joint,
            )
            .unwrap();
        ...
        inlet_pipe_1
            .advance_timestep_with_mass_flowrate(timestep, mass_flowrate_inlets)
            .unwrap();
        ...
        mixing_node.advance_timestep(timestep).unwrap();

        outlet_pipe
            .advance_timestep_with_mass_flowrate(timestep, mass_flowrate_outlet)
            .unwrap();

        simulation_time += timestep;
    }

    let inlet_pipe_1_temp = inlet_pipe_1.back_single_cv.temperature;

    let inlet_pipe_2_temp = inlet_pipe_2.back_single_cv.temperature;

    let mixing_joint_temp = mixing_node.temperature;

    let mixing_joint_temp_degc = mixing_joint_temp.get::<degree_celsius>();

    let outlet_pipe_temp = outlet_pipe.front_single_cv.temperature;
    ...
    approx::assert_abs_diff_eq!(mixing_joint_temp_degc, 75.0, epsilon = 0.5);
    approx::assert_abs_diff_eq!(
