...
impl CartesianConduction1DArray {
    ...
    pub fn get_max_timestep(
        ...
        let thermal_diffusivity_array: Array1<DiffusionCoefficient> = control_vol_temperature_array
            .map(|temperature_reference| {
                let alpha = try_get_alpha_thermal_diffusivity(
        ...
        let number_of_temperature_nodes: usize = self.inner_nodes + 2;

        let delta_x: Length = self.total_length / number_of_temperature_nodes as f64;
    ...
    pub(crate) fn get_current_timestep_rho_cp(
        material: Material,
    ...
    ///
    ///
    pub(crate) fn get_current_timestep_conductance_array(
        material: Material,
            ...
            temperature_array_curnent_timestep_reference.map(
                |temperature_reference: &ThermodynamicTemperature| {
                    ...
                    let k: ThermalConductivity =
                        try_get_kappa_thermal_conductivity(material, temperature, pressure)
                            .unwrap();
        ...
        let number_of_temperature_nodes: usize = thermal_conductivity_array.len();

        let number_of_thermal_resistors: usize = number_of_temperature_nodes - 1;

        let delta_x: Length = total_length / number_of_temperature_nodes as f64;

        let mut thermal_conductance_vector: Vec<ThermalConductance> = vec![];
        ...
        for index in 0..number_of_thermal_resistors {
            ...
            let thermal_conductance_idx: ThermalConductance =
                thermal_conductivity_array[index] * basis_area / (0.5 * delta_x);

            let thermal_conductance_idx_plus_one: ThermalConductance =
                thermal_conductivity_array[index + 1] * basis_area / (0.5 * delta_x);

            // sum both thermal resistance

            let total_thermal_resistance =
                1.0 / thermal_conductance_idx + 1.0 / thermal_conductance_idx_plus_one;

            // final thermal conductance

            let thermal_conductance_idx_and_idx_plus_one = 1.0 / total_thermal_resistance;

            // push to conductance vector
            thermal_conductance_vector.push(thermal_conductance_idx_and_idx_plus_one);
        }
        ...
        for (idx, conductance_reference) in thermal_conductance_vector.iter().enumerate() {
            // load conductance array at index plus 1

            thermal_conductance_array[idx + 1] = *conductance_reference;
        }
        // ok done!
