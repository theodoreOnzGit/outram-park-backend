...
impl SimpleShellAndTubeHeatExchanger {
    ...
    /// fluid component
    pub fn get_clone_of_shell_side_fluid_component(&self) -> FluidComponent {
        // first clone the heat transfer entity
        let shell_side_fluid_hte_clone: HeatTransferEntity = self.shell_side_fluid_array.clone();

        // convert it into a fluid array
        let shell_side_fluid_array: FluidArray = shell_side_fluid_hte_clone.try_into().unwrap();
    ...
    /// a fluid component
    pub fn get_clone_of_tube_side_parallel_tube_fluid_component(&self) -> FluidComponent {
    ...
    pub fn set_tube_side_total_mass_flowrate(&mut self, mass_flowrate_over_all_tubes: MassRate) {
        let mut tube_side_fluid_array: FluidArray = self
            .tube_side_fluid_array_for_single_tube
            .clone()
            .try_into()
            .unwrap();

        let single_tube_mass_rate = mass_flowrate_over_all_tubes / (self.number_of_tubes as f64);
        ...
        self.tube_side_fluid_array_for_single_tube
            .set(tube_side_fluid_array.into())
            .unwrap();
    }

    /// sets the tube side mass flowrate
    pub fn set_shell_side_total_mass_flowrate(&mut self, mass_flowrate_through_shell: MassRate) {
        ...
        self.shell_side_fluid_array
            .set(shell_side_fluid_array.into())
            .unwrap();
    }
    ...
    pub fn get_shell_side_hydraulic_diameter(&self) -> Length {
        ...
        let hydraulic_diameter: Length = shell_side_fluid_array.get_hydraulic_diameter_immutable();
    ...
    pub fn get_effective_length(&self) -> Length {
        ...
        let effective_length: Length = shell_side_fluid_array.get_component_length_immutable();
    ...
    pub fn get_shell_side_cross_sectional_area(&self) -> Area {
        ...
        let shell_side_xs_area: Area = shell_side_fluid_array.get_cross_sectional_area_immutable();
