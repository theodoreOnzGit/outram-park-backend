...
pub fn du_test_shell_and_tube_heat_exchanger_set_c() {
    ...
    use crate::fluid_mechanics_correlations::churchill_friction_factor::darcy;
    ...
    let tube_side_flow_area: Area = PI * 0.25 * tube_side_id * tube_side_id;

    let shell_side_flow_area: Area = PI * 0.25 * shell_side_id * shell_side_id
        - number_of_tubes as f64 * PI * 0.25 * tube_side_od * tube_side_od;

    let shell_side_fluid_hydraulic_diameter: Length = (shell_side_id * shell_side_id
        - number_of_tubes as f64 * tube_side_od * tube_side_od)
    ...
    let inlet_temp_salt = ThermodynamicTemperature::new::<degree_celsius>(214.93);
    let inlet_temp_oil = ThermodynamicTemperature::new::<degree_celsius>(74.49);
    // ambient temperature is 25C
    let ambient_temperature = ThermodynamicTemperature::new::<degree_celsius>(25.0);
    ...
    let tube_side_fluid_array: FluidArray = FluidArray::new_odd_shaped_pipe(
        pipe_length,
        tube_side_id,
        tube_side_flow_area,
        inlet_temp_oil,
    ...
    // the nusselt correlation here is a standard pipe correlation
    // but I don't use that
    let shell_side_fluid_array: FluidArray = FluidArray::new_odd_shaped_pipe(
        pipe_length,
    ...
    let outer_shell: SolidColumn = SolidColumn::new_cylindrical_shell(
        pipe_length,
        shell_side_id,
        shell_side_od,
        inlet_temp_salt,
    ...
    let inner_shell: SolidColumn = SolidColumn::new_cylindrical_shell(
        pipe_length,
        tube_side_id,
        tube_side_od,
        inlet_temp_salt,
    ...
    let dummy_insulation_array = inner_shell.clone();
    ...
    let shell_side_length_to_diameter: Ratio = pipe_length / shell_side_fluid_hydraulic_diameter;
    ...
    let shell_side_gnielinski_data: GnielinskiData = GnielinskiData {
    ...
    let shell_side_nusselt_correlation_to_tubes =
        NusseltCorrelation::CustomGnielinskiGenericPrandtlBulk(shell_side_gnielinski_data, c, m);

    let tube_side_length_to_diameter: Ratio = pipe_length / tube_side_id;
    ...
    let dummy_insulation_thickness = Length::new::<meter>(1.0);

    let sthe_one_shell_one_tube = SimpleShellAndTubeHeatExchanger {
    ...
    let vol_flowrate_salt = VolumeRate::new::<cubic_meter_per_hour>(16.63);
    let vol_flowrate_oil = VolumeRate::new::<cubic_meter_per_hour>(15.635);

    let inlet_rho_salt = LiquidMaterial::HITEC
        .try_get_density(inlet_temp_salt)
        .unwrap();
    let inlet_rho_oil = LiquidMaterial::YD325
        .try_get_density(inlet_temp_oil)
        .unwrap();
    ...
    fn obtain_outlet_steady_state_temp(
        ...
        let average_temp_hitec = ThermodynamicTemperature::new::<kelvin>(average_temp_kelvin_hitec);
        let average_hitec_density = LiquidMaterial::HITEC
            .try_get_density(average_temp_hitec)
            .unwrap();

        let average_temp_kelvin_oil = tube_inlet_temperature.value;
        let average_temp_oil = ThermodynamicTemperature::new::<kelvin>(average_temp_kelvin_oil);
        let average_oil_density = LiquidMaterial::YD325
            .try_get_density(average_temp_oil)
            .unwrap();
        ...
        while max_time > simulation_time {
            ...
            let tube_outlet_interaction = HeatTransferInteractionType::new_advection_interaction(
                tube_mass_flowrate,
            ...
            let shell_outlet_interaction = HeatTransferInteractionType::new_advection_interaction(
                shell_mass_flowrate,
            ...
            sthe.tube_side_fluid_array_for_single_tube
                .link_to_back(&mut tube_inlet_bc, tube_inlet_interaction)
                .unwrap();

            sthe.tube_side_fluid_array_for_single_tube
                .link_to_front(&mut outlet_bc, tube_outlet_interaction)
                .unwrap();

            // shell side goes front -> back
            sthe.shell_side_fluid_array
                .link_to_front(&mut shell_inlet_bc, shell_inlet_interaction)
                .unwrap();

            sthe.shell_side_fluid_array
                .link_to_back(&mut outlet_bc, shell_outlet_interaction)
                .unwrap();
        ...
        // collect information
        let temperature_vec_tube_side = sthe
            .tube_side_fluid_array_for_single_tube
    ...
    let test_thread = move |mut sthe: SimpleShellAndTubeHeatExchanger,
        ...
        let _ua: ThermalConductance = sthe
            .overall_heat_transfer_coeff_u_shell_side(
                correct_for_prandtl_wall_temperatures_u_and_ua,
            )
            .unwrap()
            * sthe.circular_tube_bundle_heat_transfer_area_shell_side();

        let u_calc_from_postprocess: HeatTransfer = sthe
            .overall_heat_transfer_coeff_u_shell_side(
                correct_for_prandtl_wall_temperatures_u_and_ua,
            )
            .unwrap();
        ...
        let (_shell_side_inlet_cv_temperature, shell_side_outlet_temperature): (
            ThermodynamicTemperature,
            ThermodynamicTemperature,
        ) = {
            let temperature_vec_shell_side = sthe
                .shell_side_fluid_array
                .get_temperature_vector()
                .unwrap();

            let shell_side_outlet_temperature = *temperature_vec_shell_side.first().unwrap();

            let shell_side_inlet_cv_temperature = *temperature_vec_shell_side.last().unwrap();
        ...
        let _tube_side_inlet_cv_temperature: ThermodynamicTemperature = {
            ...
            let tube_side_inlet_cv_temperature = *temperature_vec_tube_side.first().unwrap();
        ...
        let average_shell_side_temp_kelvin: f64 = 0.5
        ...
        let average_shell_side_temp: ThermodynamicTemperature =
            ThermodynamicTemperature::new::<kelvin>(average_shell_side_temp_kelvin);

        let average_cp_shell_side = LiquidMaterial::HITEC
            .try_get_cp(average_shell_side_temp)
            .unwrap();

        let shell_side_delta_t: TemperatureInterval = TemperatureInterval::new::<
            temperature_interval::kelvin,
        >(
            shell_side_outlet_temperature.get::<kelvin>() - shell_inlet_temperature.get::<kelvin>(),
        );

        let q_shell: Power = m_s * average_cp_shell_side * shell_side_delta_t;

        let average_tube_side_temp_kelvin: f64 = 0.5
        ...
        let average_tube_side_temp: ThermodynamicTemperature =
            ThermodynamicTemperature::new::<kelvin>(average_tube_side_temp_kelvin);

        let average_cp_tube_side = LiquidMaterial::YD325
            .try_get_cp(average_tube_side_temp)
            .unwrap();

        let tube_side_delta_t: TemperatureInterval = TemperatureInterval::new::<
            temperature_interval::kelvin,
        >(
            tube_side_outlet_temperature.get::<kelvin>() - tube_inlet_temperature.get::<kelvin>(),
        );

        let q_tube: Power = m_t * average_cp_tube_side * tube_side_delta_t;

        let q_avg: Power = 0.5 * (q_shell + q_tube);

        let heat_balance_error: Ratio = (q_tube - q_shell).abs() / q_avg;

        let max_heat_balance_error = Ratio::new::<ratio>(0.05);

        let heat_balance_valid: bool = heat_balance_error < max_heat_balance_error;
        ...
        // U = Q_av/(A_shell LMTD)
        let delta_t_max: TemperatureInterval = TemperatureInterval::new::<
            temperature_interval::kelvin,
        >(
            shell_side_outlet_temperature.get::<kelvin>() - tube_inlet_temperature.get::<kelvin>(),
        );

        let delta_t_min: TemperatureInterval = TemperatureInterval::new::<
            temperature_interval::kelvin,
        >(
            shell_inlet_temperature.get::<kelvin>() - tube_side_outlet_temperature.get::<kelvin>(),
        );

        let delta_t_max_to_delta_t_min: Ratio = delta_t_max / delta_t_min;

        let lmtd: TemperatureInterval =
            (delta_t_max - delta_t_min) / (delta_t_max_to_delta_t_min.get::<ratio>().ln());

        let shell_side_area: Area = number_of_tubes as f64 * PI * tube_side_od * pipe_length;
        ...
        let wall_side_bulk_temp: ThermodynamicTemperature = sthe
            .inner_pipe_shell_array_for_single_tube
        ...
        let tube_side_dynamic_viscosity: DynamicViscosity = LiquidMaterial::YD325
            .try_get_dynamic_viscosity(average_tube_side_temp)
            .unwrap();

        let tube_side_prandtl: Ratio = LiquidMaterial::YD325
            .try_get_prandtl_liquid(average_tube_side_temp, fluid_pressure)
            .unwrap();

        let tube_wall_side_prandtl: Ratio = LiquidMaterial::YD325
            .try_get_prandtl_liquid(wall_side_bulk_temp, fluid_pressure)
            .unwrap();

        let reynolds_tube_side: Ratio = m_t * tube_side_id
            / tube_side_flow_area
            / tube_side_dynamic_viscosity
            / number_of_tubes as f64;

        let darcy_friction_factor: f64 = darcy(
            reynolds_tube_side.get::<ratio>(),
            (SolidMaterial::SteelSS304L.surface_roughness().unwrap() / tube_side_id).get::<ratio>(),
        )
        .unwrap();

        let darcy_friction_factor: Ratio = Ratio::new::<ratio>(darcy_friction_factor);

        let gnielinski_data = match tube_side_nusselt_correlation {
            NusseltCorrelation::PipeGnielinskiGeneric(mut data) => {
                data.darcy_friction_factor = darcy_friction_factor.into();
        ...
        let lambda_tube: ThermalConductivity = LiquidMaterial::YD325
            .try_get_thermal_conductivity(average_tube_side_temp)
            .unwrap();

        let h_t: HeatTransfer = nusselt_tube_side * lambda_tube / tube_side_id;
        ...
        // 1/h_t d_o/d_i
        let reciprocal_tube_side_fluid_term = h_t.recip() * tube_side_od / tube_side_id;

        // d_o/(2 lambda_w) ln (d_o/d_i)
        //
        // lambda_w, based on 16.3 W/(m K) in Du's paper
        let _lambda_wall_du_paper: ThermalConductivity =
            ThermalConductivity::new::<watt_per_meter_kelvin>(16.3);
        let lambda_wall: ThermalConductivity = SolidMaterial::SteelSS304L
            .try_get_thermal_conductivity(wall_side_bulk_temp)
            .unwrap();

        let reciprocal_tube_side_solid_term = tube_side_od / (2.0 as f64 * lambda_wall)
        ...
        let one_over_hs =
            one_over_u - reciprocal_tube_side_fluid_term - reciprocal_tube_side_solid_term;
        ...
        let lambda_shell_fluid: ThermalConductivity = LiquidMaterial::HITEC
            .try_get_thermal_conductivity(average_shell_side_temp)
            .unwrap();

        let nusselt_number_shell_calculated: Ratio =
            h_s * shell_side_fluid_hydraulic_diameter / lambda_shell_fluid;

        // then shell side reynolds number

        let mu_shell_side: DynamicViscosity = LiquidMaterial::HITEC
            .try_get_dynamic_viscosity(average_shell_side_temp)
            .unwrap();

        let reynolds_shell_side: Ratio =
            m_s.abs() * shell_side_fluid_hydraulic_diameter / shell_side_flow_area / mu_shell_side;

        // for debugging, cos Nusselt is underestimated by about 1.5x
        // consistently

        let shell_side_fluid_bulk_temp: ThermodynamicTemperature = sthe
            .shell_side_fluid_array
            .try_get_bulk_temperature()
            .unwrap();

        let shell_side_fluid_bulk_prandtl = LiquidMaterial::HITEC
            .try_get_prandtl_liquid(shell_side_fluid_bulk_temp, fluid_pressure)
            .unwrap();
        ...
        let shell_side_fluid_wall_prandtl = LiquidMaterial::HITEC
            .try_get_prandtl_liquid(est_wall_temp_hitec, fluid_pressure)
            .unwrap();

        let shell_side_fluid_film_prandtl_estimate = shell_side_fluid_bulk_prandtl;
        ...
        let _u_from_conductance = sthe.overall_htc_based_on_conductance(
            correct_for_prandtl_wall_temperatures_u_and_ua,
            m_t.abs(),
            m_s.abs(),
        );
    ...
    let clone_for_test_one: SimpleShellAndTubeHeatExchanger = sthe_one_shell_one_tube.clone();

    let (reynolds_num_c1, bulk_prandtl_c1, wall_prandtl_c1, nusselt_number_c1) = test_thread(
        clone_for_test_one,
        tube_inlet_temperature,
        shell_inlet_temperature,
        m_t,
    ...
    assert_relative_eq!(reynolds_num_c1.get::<ratio>(), 4623.0, max_relative = 0.01,);

    assert_relative_eq!(bulk_prandtl_c1.get::<ratio>(), 24.05, max_relative = 0.01,);

    assert_relative_eq!(wall_prandtl_c1.get::<ratio>(), 37.89, max_relative = 0.01,);

    assert_relative_eq!(nusselt_number_c1.get::<ratio>(), 62.40, max_relative = 0.01,);
    ...
    if !one_test_only {
        let clone_for_test_two: SimpleShellAndTubeHeatExchanger = sthe_one_shell_one_tube.clone();
        let test_two = thread::spawn(move || {
            let inlet_temp_salt = ThermodynamicTemperature::new::<degree_celsius>(224.93);
            let inlet_temp_oil = ThermodynamicTemperature::new::<degree_celsius>(74.49);
            let vol_flowrate_salt = VolumeRate::new::<cubic_meter_per_hour>(16.63);
            let vol_flowrate_oil = VolumeRate::new::<cubic_meter_per_hour>(15.635);
            ...
            let (reynolds_num_c2, bulk_prandtl_c2, wall_prandtl_c2, nusselt_number_c2) =
                test_thread(
                    clone_for_test_two,
                    tube_inlet_temperature,
                    shell_inlet_temperature,
            ...
            assert_relative_eq!(reynolds_num_c2.get::<ratio>(), 5041.0, max_relative = 0.01,);

            assert_relative_eq!(bulk_prandtl_c2.get::<ratio>(), 22.06, max_relative = 0.01,);

            assert_relative_eq!(wall_prandtl_c2.get::<ratio>(), 37.89, max_relative = 0.01,);

            assert_relative_eq!(nusselt_number_c2.get::<ratio>(), 65.75, max_relative = 0.01,);
        });

        let clone_for_test_three: SimpleShellAndTubeHeatExchanger = sthe_one_shell_one_tube.clone();
        let test_three = thread::spawn(move || {
            let inlet_temp_salt = ThermodynamicTemperature::new::<degree_celsius>(236.91);
            let inlet_temp_oil = ThermodynamicTemperature::new::<degree_celsius>(74.49);
            let vol_flowrate_salt = VolumeRate::new::<cubic_meter_per_hour>(16.63);
            let vol_flowrate_oil = VolumeRate::new::<cubic_meter_per_hour>(15.635);
            ...
            let (reynolds_num_c3, bulk_prandtl_c3, wall_prandtl_c3, nusselt_number_c3) =
                test_thread(
            ...
            assert_relative_eq!(reynolds_num_c3.get::<ratio>(), 5567.0, max_relative = 0.01,);

            assert_relative_eq!(bulk_prandtl_c3.get::<ratio>(), 19.99, max_relative = 0.01,);

            assert_relative_eq!(wall_prandtl_c3.get::<ratio>(), 37.89, max_relative = 0.01,);

            assert_relative_eq!(nusselt_number_c3.get::<ratio>(), 69.54, max_relative = 0.01,);
        });

        let clone_for_test_four: SimpleShellAndTubeHeatExchanger = sthe_one_shell_one_tube.clone();
        let test_four = thread::spawn(move || {
            let inlet_temp_salt = ThermodynamicTemperature::new::<degree_celsius>(236.91);
            let inlet_temp_oil = ThermodynamicTemperature::new::<degree_celsius>(80.41);
            let vol_flowrate_salt = VolumeRate::new::<cubic_meter_per_hour>(16.63);
            let vol_flowrate_oil = VolumeRate::new::<cubic_meter_per_hour>(15.635);
            ...
            let (reynolds_num_c4, bulk_prandtl_c4, wall_prandtl_c4, nusselt_number_c4) =
                test_thread(
                    clone_for_test_four,
                    tube_inlet_temperature,
                    shell_inlet_temperature,
            ...
            assert_relative_eq!(reynolds_num_c4.get::<ratio>(), 5568.0, max_relative = 0.01,);

            assert_relative_eq!(bulk_prandtl_c4.get::<ratio>(), 19.99, max_relative = 0.01,);

            assert_relative_eq!(wall_prandtl_c4.get::<ratio>(), 37.89, max_relative = 0.01,);

            assert_relative_eq!(nusselt_number_c4.get::<ratio>(), 69.45, max_relative = 0.01,);
        });

        let clone_for_test_five: SimpleShellAndTubeHeatExchanger = sthe_one_shell_one_tube.clone();
        let test_five = thread::spawn(move || {
            let inlet_temp_salt = ThermodynamicTemperature::new::<degree_celsius>(214.93);
            let inlet_temp_oil = ThermodynamicTemperature::new::<degree_celsius>(90.41);
            let vol_flowrate_salt = VolumeRate::new::<cubic_meter_per_hour>(16.63);
            let vol_flowrate_oil = VolumeRate::new::<cubic_meter_per_hour>(15.635);
            ...
            let (reynolds_num_c5, bulk_prandtl_c5, wall_prandtl_c5, nusselt_number_c5) =
                test_thread(
                    clone_for_test_five,
                    tube_inlet_temperature,
                    shell_inlet_temperature,
            ...
            assert_relative_eq!(reynolds_num_c5.get::<ratio>(), 4633.0, max_relative = 0.01,);

            assert_relative_eq!(bulk_prandtl_c5.get::<ratio>(), 24.05, max_relative = 0.01,);

            assert_relative_eq!(wall_prandtl_c5.get::<ratio>(), 37.89, max_relative = 0.01,);

            assert_relative_eq!(nusselt_number_c5.get::<ratio>(), 62.31, max_relative = 0.01,);
        });

        let clone_for_test_six: SimpleShellAndTubeHeatExchanger = sthe_one_shell_one_tube.clone();
        let test_six = thread::spawn(move || {
            let inlet_temp_salt = ThermodynamicTemperature::new::<degree_celsius>(224.93);
            let inlet_temp_oil = ThermodynamicTemperature::new::<degree_celsius>(80.49);
            let vol_flowrate_salt = VolumeRate::new::<cubic_meter_per_hour>(16.63);
            let vol_flowrate_oil = VolumeRate::new::<cubic_meter_per_hour>(15.635);
            ...
            let (reynolds_num_c6, bulk_prandtl_c6, wall_prandtl_c6, nusselt_number_c6) =
                test_thread(
                    clone_for_test_six,
                    tube_inlet_temperature,
                    shell_inlet_temperature,
            ...
            assert_relative_eq!(reynolds_num_c6.get::<ratio>(), 5043.0, max_relative = 0.01,);

            assert_relative_eq!(bulk_prandtl_c6.get::<ratio>(), 22.05, max_relative = 0.01,);

            assert_relative_eq!(wall_prandtl_c6.get::<ratio>(), 37.89, max_relative = 0.01,);

            assert_relative_eq!(nusselt_number_c6.get::<ratio>(), 65.69, max_relative = 0.01,);
        });

        let clone_for_test_seven: SimpleShellAndTubeHeatExchanger = sthe_one_shell_one_tube.clone();
        let test_seven = thread::spawn(move || {
            let inlet_temp_salt = ThermodynamicTemperature::new::<degree_celsius>(236.91);
            let inlet_temp_oil = ThermodynamicTemperature::new::<degree_celsius>(90.49);
            let vol_flowrate_salt = VolumeRate::new::<cubic_meter_per_hour>(16.63);
            let vol_flowrate_oil = VolumeRate::new::<cubic_meter_per_hour>(15.635);
            ...
            let (reynolds_num_c7, bulk_prandtl_c7, wall_prandtl_c7, nusselt_number_c7) =
                test_thread(
            ...
            assert_relative_eq!(reynolds_num_c7.get::<ratio>(), 5575.0, max_relative = 0.01,);

            assert_relative_eq!(bulk_prandtl_c7.get::<ratio>(), 19.97, max_relative = 0.01,);

            assert_relative_eq!(wall_prandtl_c7.get::<ratio>(), 37.89, max_relative = 0.01,);

            assert_relative_eq!(nusselt_number_c7.get::<ratio>(), 69.32, max_relative = 0.01,);
        });
