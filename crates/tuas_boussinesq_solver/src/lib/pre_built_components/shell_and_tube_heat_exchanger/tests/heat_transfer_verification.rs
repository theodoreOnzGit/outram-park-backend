...
pub fn basic_test_shell_and_tube_heat_exchanger() {
    ...
    let shell_side_flow_area: Area =
        PI * 0.25 * shell_side_id * shell_side_id - PI * 0.25 * tube_side_od * tube_side_od;
    ...
    let initial_temperature = ThermodynamicTemperature::new::<degree_celsius>(250.0);
    ...
    let tube_side_nusselt_correlation = NusseltCorrelation::PipeConstantHeatFluxFullyDeveloped;
    ...
    let tube_inlet_temperature = ThermodynamicTemperature::new::<degree_celsius>(230.0);
    let shell_inlet_temperature = ThermodynamicTemperature::new::<degree_celsius>(270.0);
    ...
    fn obtain_outlet_steady_state_temp(
        ...
        let average_temp_kelvin =
            0.5 * tube_inlet_temperature.value + 0.5 * shell_inlet_temperature.value;
        let average_temp = ThermodynamicTemperature::new::<kelvin>(average_temp_kelvin);
        let average_hitec_density = LiquidMaterial::HITEC.try_get_density(average_temp).unwrap();
    ...
    let test_thread = move |mut sthe: SimpleShellAndTubeHeatExchanger,
                            ...
                            m_t: MassRate,
                            m_s: MassRate| {
        let tube_outlet_temperature: ThermodynamicTemperature = obtain_outlet_steady_state_temp(
            &mut sthe,
        ...
        let ua: ThermalConductance = sthe
            .overall_heat_transfer_coeff_u_shell_side(correct_for_prandtl_wall_temperatures)
            .unwrap()
    ...
    let test_one = thread::spawn(move || {
        test_thread(
            clone_for_test_one,
            tube_inlet_temperature,
            shell_inlet_temperature,
    ...
    let m_t = MassRate::new::<kilogram_per_second>(0.05);
    let m_s = -m_t * 0.5;
    let tube_inlet_temperature = ThermodynamicTemperature::new::<degree_celsius>(180_f64);
    let shell_inlet_temperature = ThermodynamicTemperature::new::<degree_celsius>(280_f64);
    ...
    let m_t = MassRate::new::<kilogram_per_second>(0.05);
    let m_s = -m_t * 0.5;
    let tube_inlet_temperature = ThermodynamicTemperature::new::<degree_celsius>(280_f64);
    let shell_inlet_temperature = ThermodynamicTemperature::new::<degree_celsius>(180_f64);
    ...
    let m_s = -m_t * 0.5;
    let tube_inlet_temperature = ThermodynamicTemperature::new::<degree_celsius>(220_f64);
    let shell_inlet_temperature = ThermodynamicTemperature::new::<degree_celsius>(270_f64);
...
pub fn basic_test_shell_and_tube_heat_exchanger_set_three() {
    ...
    let test_thread = move |mut sthe: SimpleShellAndTubeHeatExchanger,
        ...
        let (shell_side_inlet_cv_temperature, shell_side_outlet_temperature): (
            ThermodynamicTemperature,
            ThermodynamicTemperature,
        ) = {
            let temperature_vec_shell_side = sthe
