...
pub fn overall_htc_postprocessing_basic_test_shell_and_tube_heat_exchanger() {
    ...
    let overall_heat_exchg_coeff = sthe_one_shell_one_tube
        .overall_heat_transfer_coeff_u_shell_side(correct_for_prandtl_wall_temperatures)
        .unwrap();

    let u_val_watts_per_sqm_kelvin = overall_heat_exchg_coeff.get::<watt_per_square_meter_kelvin>();
    ...
    approx::assert_relative_eq!(18.48, u_val_watts_per_sqm_kelvin, max_relative = 0.04);
}
...
pub fn shell_side_tube_bundle_area_basic_test_shell_and_tube_heat_exchanger() {
    ...
    let shell_side_tube_area_m2 = shell_side_tube_area.get::<square_meter>();

    approx::assert_relative_eq!(0.085765, shell_side_tube_area_m2, max_relative = 1e-5);
}
