...
pub fn check_wall_side_htc() {
    ...
    use uom::si::{
        heat_transfer::watt_per_square_meter_kelvin, thermal_conductivity::watt_per_meter_kelvin,
    };
    ...
    let reciprocal_tube_side_solid_term = tube_side_od / (2.0 as f64 * lambda_wall_du_paper)
        * (tube_side_od / tube_side_id).get::<ratio>().ln();

    let wall_heat_transfer_expected = HeatTransfer::new::<watt_per_square_meter_kelvin>(6920.0);
...
pub fn check_high_bound_and_low_bound_shell_heat_trf_coeff() {
    ...
    let high_bound_thermal_cond: ThermalConductivity = LiquidMaterial::HITEC
        .try_get_thermal_conductivity(low_bound_temp)
        .unwrap();

    let low_bound_thermal_cond: ThermalConductivity = LiquidMaterial::HITEC
        .try_get_thermal_conductivity(high_bound_temp)
        .unwrap();

    let high_bound_htc: HeatTransfer =
        high_bound_nusselt_shell * high_bound_thermal_cond / hydraulic_diameter_shell;
    let low_bound_htc: HeatTransfer =
        low_bound_nusselt_shell * low_bound_thermal_cond / hydraulic_diameter_shell;
