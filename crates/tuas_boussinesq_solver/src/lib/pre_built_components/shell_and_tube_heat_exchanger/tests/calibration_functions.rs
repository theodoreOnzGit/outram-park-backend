...
pub fn dhx_calibration_regression_unit_test_shell_to_tube_only(
    ...
    let experimental_pri_mass_flowrate = MassRate::new::<kilogram_per_second>(
        experimental_pri_shell_side_mass_flowrate_kg_per_s_abs,
    );
    // dracs flowrate (tube side)
    // tube side flows in reverse direction
    // (need to negative)
    let experimental_dracs_mass_flowrate = MassRate::new::<kilogram_per_second>(
        -experimental_dracs_tube_side_mass_flowrate_kg_per_s_abs,
    );

    let dhx_shell_side_inlet_temperature =
        ThermodynamicTemperature::new::<degree_celsius>(dhx_shell_side_inlet_temp_degc);

    let dhx_tube_side_inlet_temperature =
        ThermodynamicTemperature::new::<degree_celsius>(dhx_tube_side_inlet_temp_degc);

    let dhx_shell_side_outlet_temperature_set_point =
        ThermodynamicTemperature::new::<degree_celsius>(dhx_shell_side_outlet_temp_set_point_degc);
    ...
    let initial_temperature: ThermodynamicTemperature = dhx_shell_side_outlet_temperature_set_point;
    ...
    let average_therminol_density = LiquidMaterial::TherminolVP1
        .try_get_density(average_temperature_for_advection_mass_flowrate_calcs)
        .unwrap();
    ...
    let mut dhx_shell_inlet_bc =
        HeatTransferEntity::new_const_temperature_bc(dhx_shell_side_inlet_temperature);

    let mut dhx_tube_inlet_bc =
        HeatTransferEntity::new_const_temperature_bc(dhx_tube_side_inlet_temperature);

    let mut dhx_shell_outlet_bc = HeatTransferEntity::new_adiabatic_bc();
    let mut dhx_tube_outlet_bc = HeatTransferEntity::new_adiabatic_bc();

    let mut dhx_shell_outlet_actual_temperature: ThermodynamicTemperature = initial_temperature;
    let mut dhx_tube_outlet_actual_temperature: ThermodynamicTemperature = initial_temperature;
    let mut dhx_shell_entrance_cv_actual_temperature: ThermodynamicTemperature =
        initial_temperature;
    let mut dhx_tube_entrance_cv_actual_temperature: ThermodynamicTemperature = initial_temperature;
    ...
    while current_simulation_time < max_simulation_time {
        ...
        dhx_tube_outlet_actual_temperature = {
            ...
            let dhx_tube_fluid_arr_back_single_cv_temperature: ThermodynamicTemperature =
                dhx_tube_fluid_arr_clone.back_single_cv.temperature;

            dhx_tube_entrance_cv_actual_temperature =
                dhx_tube_fluid_arr_clone.front_single_cv.temperature;
    ...
    let overall_htc_postprocessing: HeatTransfer = dhx_sthe.overall_htc_based_on_conductance(
        correct_prandtl_for_wall_temperatures,
        experimental_dracs_mass_flowrate,
        experimental_pri_mass_flowrate,
    );
    ...
    let thermal_resistance_overall_actual: ThermalResistance = dhx_sthe
        .get_shell_side_convective_thermal_resistance_cylindrical()
    ...
    dbg!(&(
        thermal_resistance_overall_expected,
    ...
    let thermal_resistance_based_on_expt_data: ThermalResistance = dhx_sthe
    ...
    let heat_transferred_in_sthe = dhx_sthe.get_tube_side_heat_rate_based_on_mass_flowrate(
        tube_inlet_temperature,
        tube_outlet_temeprature,
        tube_mass_flowrate,
    );
    ...
    let nusselt_number_shell_side_to_tubes_actual = dhx_sthe
        .obtain_shell_side_nusselt_number_based_on_expt_data(
            tube_inlet_temperature,
            tube_outlet_temeprature,
            tube_mass_flowrate,
            shell_inlet_temperature,
    ...
    let nusselt_number_shell_side_to_tubes_based_on_entrance_cv_temp = dhx_sthe
        .obtain_shell_side_nusselt_number_based_on_expt_data_and_heat_rate(
            heat_transferred_in_sthe,
            dhx_tube_entrance_cv_actual_temperature,
            tube_outlet_temeprature,
            dhx_shell_entrance_cv_actual_temperature,
...
pub fn dhx_calibration_regression_unit_test_parasitic_heat_loss_only(
    ...
    let insulation_cylindrical_thermal_resistance = dhx_sthe
        .try_get_insulation_cylindrical_thermal_resistance()
        .unwrap();

    // i'm not 100% sure what is wrong here.

    thermal_resistance_for_parasitic_heat_loss = convective_thermal_resistance_to_ambient
    ...
    let thermal_resistance_for_parasitic_heat_loss_experimental = dhx_sthe
    ...
    let parasitic_loss_rate: Power = dhx_sthe.obtain_parasitic_heat_loss_rate_based_on_expt_data(
        tube_inlet_temperature,
        tube_outlet_temeprature,
        tube_mass_flowrate,
        shell_inlet_temperature,
    ...
    let tube_side_heat_transfer_rate: Power = dhx_sthe
        .get_tube_side_heat_rate_based_on_mass_flowrate(
            tube_inlet_temperature,
            tube_outlet_temeprature,
            tube_mass_flowrate,
        );

    let shell_side_heat_transfer_rate: Power = dhx_sthe
        .get_shell_side_heat_rate_based_on_mass_flowrate(
            shell_inlet_temperature,
            shell_outlet_temeprature,
            shell_mass_flowrate,
        );
    ...
    let nusselt_number_shell_side_parasitic_actual = dhx_sthe
        .obtain_parasitic_nusselt_number_based_on_expt_data(
            tube_inlet_temperature,
            tube_outlet_temeprature,
            tube_mass_flowrate,
            shell_inlet_temperature,
