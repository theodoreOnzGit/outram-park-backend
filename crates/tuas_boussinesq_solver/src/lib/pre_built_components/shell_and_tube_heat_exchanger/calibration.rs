...
impl SimpleShellAndTubeHeatExchanger {
    ...
    pub fn try_get_insulation_cylindrical_thermal_resistance(
        &self,
        ...
        let insulation_array_temp: ThermodynamicTemperature =
            insulation_array_clone.try_get_bulk_temperature().unwrap();

        let insulation_array_material: SolidMaterial = insulation_array_clone
            .material_control_volume
            .clone()
            .try_into()
            .unwrap();

        lambda_insulation = insulation_array_material
            .try_get_thermal_conductivity(insulation_array_temp)
            .unwrap();

        let log_numerator_term: f64 = (insulation_od / insulation_id).get::<ratio>().ln();

        let denominator_term: ThermalConductance = 2.0 * PI * lambda_insulation * l;
    ...
    ///
    /// ln (d_o/d_i) * 1/(2 pi L lambda_insulation)
    pub fn get_outer_shell_cylindrical_thermal_resistance(&self) -> ThermalResistance {
        ...
        let mut outer_shell_array_clone: SolidColumn = self.outer_shell.clone().try_into().unwrap();

        let outer_shell_array_temp: ThermodynamicTemperature =
            outer_shell_array_clone.try_get_bulk_temperature().unwrap();

        let outer_shell_array_material: SolidMaterial = outer_shell_array_clone
            .material_control_volume
            .clone()
            .try_into()
            .unwrap();

        lambda_outer_shell = outer_shell_array_material
            .try_get_thermal_conductivity(outer_shell_array_temp)
            .unwrap();

        let log_numerator_term: f64 = (outer_shell_od / outer_shell_id).get::<ratio>().ln();

        let denominator_term: ThermalConductance = 2.0 * PI * lambda_outer_shell * l;
    ...
    pub fn get_inner_tubes_cylindrical_thermal_resistance(&self) -> ThermalResistance {
        ...
        let mut inner_single_tube_array_clone: SolidColumn = self
            .inner_pipe_shell_array_for_single_tube
            .clone()
            .try_into()
            .unwrap();

        let inner_tube_array_temp: ThermodynamicTemperature = inner_single_tube_array_clone
            .try_get_bulk_temperature()
            .unwrap();

        let inner_tube_array_material: SolidMaterial = inner_single_tube_array_clone
            .material_control_volume
            .clone()
            .try_into()
            .unwrap();

        lambda_inner_tubes = inner_tube_array_material
            .try_get_thermal_conductivity(inner_tube_array_temp)
            .unwrap();

        let log_numerator_term: f64 = (inner_tube_od / inner_tube_id).get::<ratio>().ln();
    ...
    pub fn get_convective_thermal_resistance_to_ambient(&self) -> ThermalResistance {
        ...
        let outermost_heat_transfer_area: Area = PI * od * self.get_effective_length();
    ...
    pub fn get_inner_tubes_convective_thermal_resistance_based_on_wetted_perimeter(
        ...
        let mut fluid_arr_clone_for_single_inner_tube: FluidArray = self
        ...
        let hydraulic_diameter: Length =
            fluid_arr_clone_for_single_inner_tube.get_hydraulic_diameter_immutable();

        let flow_area: Area =
            fluid_arr_clone_for_single_inner_tube.get_cross_sectional_area_immutable();

        let tube_length: Length = fluid_arr_clone_for_single_inner_tube.get_component_length();

        // d_h = 4A/P_w
        let wetted_perimeter: Length = (4.0 * flow_area) / hydraulic_diameter;

        let single_tube_heat_trf_area: Area = wetted_perimeter * tube_length;

        let area_tube_side_total: Area = single_tube_heat_trf_area * self.number_of_tubes as f64;

        let nusselt_tube_side: Ratio = self.nusselt_tube_side();

        // get h_t
        // Nu_t = h_t * d_t/lambda_t
        // lambda_t = thermal conductivity
        let tube_side_temperature: ThermodynamicTemperature = fluid_arr_clone_for_single_inner_tube
            .try_get_bulk_temperature()
            .unwrap();

        let tube_fluid_material: LiquidMaterial = fluid_arr_clone_for_single_inner_tube
            .material_control_volume
            .try_into()
            .unwrap();

        let thermal_conductivity_tube_side_fluid: ThermalConductivity = tube_fluid_material
            .try_get_thermal_conductivity(tube_side_temperature)
            .unwrap();

        let h_t: HeatTransfer =
            nusselt_tube_side * thermal_conductivity_tube_side_fluid / hydraulic_diameter;
    ...
    #[inline]
    pub fn get_shell_side_convective_thermal_resistance_cylindrical(&self) -> ThermalResistance {
        let mut shell_side_fluid_arr_clone: FluidArray =
            self.shell_side_fluid_array.clone().try_into().unwrap();

        let shell_length: Length = shell_side_fluid_arr_clone.get_component_length();

        let wetted_perimeter: Length = PI * self.tube_side_od;

        let single_tube_heat_trf_area: Area = wetted_perimeter * shell_length;

        let area_shell_side_total: Area = single_tube_heat_trf_area * self.number_of_tubes as f64;

        let nusselt_shell_side: Ratio = self.nusselt_number_shell_side_to_tubes();

        // get h_s
        // Nu_s = h_s * D_e/lambda_s
        // lambda_s = thermal conductivity
        let shell_side_temperature: ThermodynamicTemperature = shell_side_fluid_arr_clone
            .try_get_bulk_temperature()
            .unwrap();

        let shell_fluid_material: LiquidMaterial = shell_side_fluid_arr_clone
            .material_control_volume
            .try_into()
            .unwrap();

        let thermal_conductivity_shell_side_fluid: ThermalConductivity = shell_fluid_material
            .try_get_thermal_conductivity(shell_side_temperature)
            .unwrap();

        let hydraulic_diameter_shell_side: Length =
            shell_side_fluid_arr_clone.get_hydraulic_diameter_immutable();

        let h_s: HeatTransfer = nusselt_shell_side * thermal_conductivity_shell_side_fluid
    ...
    pub fn get_shell_side_parasitic_convective_thermal_resistance_cylindrical(
        ...
        let parasitic_nusselt_number: Ratio = self.nusselt_number_shell_side_parasitic();
        ...
        let h_parasitic: HeatTransfer = parasitic_nusselt_number
    ...
    pub fn get_shell_side_heat_rate_based_on_mass_flowrate(
        ...
        let cp_shell_side_fluid: SpecificHeatCapacity = shell_fluid_material
            .try_get_cp(shell_side_temperature)
            .unwrap();

        let delta_t: TemperatureInterval =
            TemperatureInterval::new::<uom::si::temperature_interval::kelvin>(
                shell_outlet_temeprature.get::<kelvin>() - shell_inlet_temperature.get::<kelvin>(),
            );
    ...
    pub fn get_shell_side_heat_rate_based_on_vol_flowrate(
        ...
        let rho_shell_side_fluid: MassDensity = shell_fluid_material
            .try_get_density(shell_side_temperature)
            .unwrap();
    ...
    pub fn get_tube_side_heat_rate_based_on_mass_flowrate(
        ...
        let mut tube_side_fluid_arr_clone: FluidArray = self
            .tube_side_fluid_array_for_single_tube
            .clone()
            .try_into()
            .unwrap();

        let tube_fluid_material: LiquidMaterial = tube_side_fluid_arr_clone
            .material_control_volume
            .try_into()
            .unwrap();

        let tube_side_temperature: ThermodynamicTemperature = tube_side_fluid_arr_clone
            .try_get_bulk_temperature()
            .unwrap();

        let cp_tube_side_fluid: SpecificHeatCapacity = tube_fluid_material
            .try_get_cp(tube_side_temperature)
            .unwrap();

        let delta_t: TemperatureInterval =
            TemperatureInterval::new::<uom::si::temperature_interval::kelvin>(
                tube_outlet_temeprature.get::<kelvin>() - tube_inlet_temperature.get::<kelvin>(),
            );
    ...
    pub fn get_tube_side_heat_rate_based_on_vol_flowrate(
        ...
        let rho_tube_side_fluid: MassDensity = tube_fluid_material
            .try_get_density(tube_side_temperature)
            .unwrap();
    ...
    /// manually
    #[inline]
    pub fn get_ua_based_on_mass_flowrates_and_temperature_differences(
        &self,
        tube_inlet_temperature: ThermodynamicTemperature,
        tube_outlet_temeprature: ThermodynamicTemperature,
        tube_mass_flowrate: MassRate,
        ...
        let tube_side_heat_rate: Power = self.get_tube_side_heat_rate_based_on_mass_flowrate(
            tube_inlet_temperature,
            tube_outlet_temeprature,
            tube_mass_flowrate,
        );

        let shell_side_heat_rate: Power = self.get_shell_side_heat_rate_based_on_mass_flowrate(
        ...
        if tube_side_heat_rate.abs() > shell_side_heat_rate.abs() {
            heat_transfer_rate_through_sthe_no_parasitic_losses = shell_side_heat_rate;
        } else {
            heat_transfer_rate_through_sthe_no_parasitic_losses = tube_side_heat_rate;
        }
    ...
    pub fn get_ua_based_on_heat_transfer_and_temperature_differences(
        ...
        if is_counter_current {
            delta_t_min = TemperatureInterval::new::<uom::si::temperature_interval::kelvin>(
                shell_inlet_temperature.get::<kelvin>() - tube_outlet_temeprature.get::<kelvin>(),
            );
            delta_t_max = TemperatureInterval::new::<uom::si::temperature_interval::kelvin>(
                shell_outlet_temeprature.get::<kelvin>() - tube_inlet_temperature.get::<kelvin>(),
            );
        } else {
            delta_t_min = TemperatureInterval::new::<uom::si::temperature_interval::kelvin>(
                shell_outlet_temeprature.get::<kelvin>() - tube_outlet_temeprature.get::<kelvin>(),
            );
            delta_t_max = TemperatureInterval::new::<uom::si::temperature_interval::kelvin>(
                shell_inlet_temperature.get::<kelvin>() - tube_inlet_temperature.get::<kelvin>(),
            );
        }

        let log_mean_temperature_difference: TemperatureInterval;

        let numerator_term_lmtd: TemperatureInterval = delta_t_max - delta_t_min;

        let denominator_term_lmtd: Ratio = (delta_t_max / delta_t_min).get::<ratio>().ln().into();

        log_mean_temperature_difference = numerator_term_lmtd / denominator_term_lmtd;
    ...
    pub fn obtain_shell_side_nusselt_number_based_on_expt_data(
        ...
        let overall_ua: ThermalConductance = self
            .get_ua_based_on_mass_flowrates_and_temperature_differences(
                tube_inlet_temperature,
                tube_outlet_temeprature,
                tube_mass_flowrate,
                shell_inlet_temperature,
        ...
        //
        // 1/(h_s A_s)
        let shell_side_thermal_resistance_expt_data = overall_thermal_resistance
        ...
        // A_s
        let total_area_shell_side = self.circular_tube_bundle_heat_transfer_area_shell_side();
        ...
        let shell_side_fluid_hydraulic_diameter = self.get_shell_side_hydraulic_diameter();

        let expt_nusselt_number_shell_side: Ratio = h_s * shell_side_fluid_hydraulic_diameter
    ...
    pub fn obtain_tube_side_nusselt_number_based_on_expt_data(
        ...
        // 1/(h_t A_t)
        let tube_side_thermal_resistance_expt_data = overall_thermal_resistance
            - self.get_shell_side_convective_thermal_resistance_cylindrical()
            - self.get_inner_tubes_cylindrical_thermal_resistance();

        // A_s
        let total_area_tube_side = self.circular_tube_bundle_heat_transfer_area_shell_side();
        ...
        let tube_side_fluid_hydraulic_diameter = self.tube_side_id;

        let expt_nusselt_number_tube_side: Ratio = h_t * tube_side_fluid_hydraulic_diameter
            / self.get_shell_side_fluid_thermal_conductivity();
    ...
    pub fn obtain_parasitic_nusselt_number_based_on_expt_data(
        ...
        let overall_thermal_resistance = self
            .obtain_parasitic_thermal_resistance_based_on_expt_data(
                tube_inlet_temperature,
                tube_outlet_temeprature,
                tube_mass_flowrate,
                shell_inlet_temperature,
        ...
        let mut shell_side_parasitic_thermal_resistance_expt_data = overall_thermal_resistance
            - self.get_convective_thermal_resistance_to_ambient()
            - self.get_outer_shell_cylindrical_thermal_resistance();

        if self.heat_exchanger_has_insulation {
            shell_side_parasitic_thermal_resistance_expt_data -= self
                .try_get_insulation_cylindrical_thermal_resistance()
                .unwrap();
        }

        // A_parasitic
        let total_area_shell_side_parasitic = self.parasitic_heat_transfer_area_shell_side();

        // 1/(h_parasitic) = A_parasitic * 1/(h_parasitic_A_parasitic)
        // take reciprocal then
        // h_parasitic
        let h_parasitic: HeatTransfer = (shell_side_parasitic_thermal_resistance_expt_data
            * total_area_shell_side_parasitic)
            .recip();

        let shell_side_fluid_hydraulic_diameter = self.get_shell_side_hydraulic_diameter();

        let expt_nusselt_number_parasitic_shell_side: Ratio = h_parasitic
    ...
    pub fn obtain_parasitic_thermal_resistance_based_on_expt_data(
        ...
        let parasitic_heat_transfer_rate = self.obtain_parasitic_heat_loss_rate_based_on_expt_data(
            tube_inlet_temperature,
            tube_outlet_temeprature,
            tube_mass_flowrate,
            shell_inlet_temperature,
