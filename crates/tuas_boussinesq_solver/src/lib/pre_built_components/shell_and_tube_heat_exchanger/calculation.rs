...
impl SimpleShellAndTubeHeatExchanger {
    ...
    #[inline]
    fn advance_timestep_for_parallel_tube_side_fluid_array_bundle(
        &mut self,
        timestep: Time,
    ) -> Result<(), TuasLibError> {
        ...
        let mut tube_side_fluid_array_for_single_tube_clone: FluidArray = self
        ...
        let forward_flow: bool = mass_flowrate_for_single_tube.ge(&MassRate::zero());
        ...
        let bulk_temperature =
            tube_side_fluid_array_for_single_tube_clone.try_get_bulk_temperature()?;
        ...
        // timestep
        let volume_fraction_array: Array1<f64> = tube_side_fluid_array_for_single_tube_clone
            .volume_fraction_array
            .iter()
            .map(|&vol_frac| vol_frac)
            .collect();
        // rho_cp is determined by the temperature array
        let rho_cp: Array1<VolumetricHeatCapacity> = tube_side_fluid_array_for_single_tube_clone
            .temperature_array_current_timestep
            .iter()
            .map(|&temperature| try_get_rho_cp(material, temperature, pressure).unwrap())
            .collect();
        // energy balance is:
        ...
        let mut sum_of_lateral_conductance_times_lateral_temperatures: Array1<Power> =
            Array1::zeros(number_of_nodes);
        ...
        let lateral_temperature_arary_connected: bool = tube_side_fluid_array_for_single_tube_clone
        ...
        if lateral_temperature_arary_connected {
            ...
            for conductance_array in tube_side_fluid_array_for_single_tube_clone
                .lateral_adjacent_array_conductance_vector
                .iter()
            {
            ...
            for (lateral_idx, conductance_arr) in tube_side_fluid_array_for_single_tube_clone
                .lateral_adjacent_array_conductance_vector
                .iter()
                .enumerate()
            {
                ...
                let temperature_arr: Array1<ThermodynamicTemperature> =
                    tube_side_fluid_array_for_single_tube_clone
                        ...
                        .clone();
                let mut power_arr: Array1<Power> = Array::zeros(number_of_nodes);

                for (node_idx, power) in power_arr.iter_mut().enumerate() {
                    *power = conductance_arr[node_idx] * temperature_arr[node_idx];
                }

                sum_of_lateral_conductance_times_lateral_temperatures += &power_arr;
            }
        }
        ...
        let mut sum_of_lateral_power_sources: Array1<Power> = Array::zeros(number_of_nodes);
        ...
        if lateral_power_sources_connected {
            ...
            let mut power_ndarray_vector: Vec<Array1<Power>> = vec![];

            for (lateral_idx, q_reference) in tube_side_fluid_array_for_single_tube_clone
                .q_vector
                .iter()
                .enumerate()
            {
                ...
                let power_frac_array: Array1<f64> = tube_side_fluid_array_for_single_tube_clone
                    .q_fraction_vector[lateral_idx]
                    .clone();

                let power_ndarray: Array1<Power> =
                    power_frac_array.map(|&power_frac| power_frac * (*q_reference));
            ...
            for (node_idx, node_power_source) in sum_of_lateral_power_sources.iter_mut().enumerate()
            {
                ...
                for power_vector in power_ndarray_vector.iter() {
                    ...
                    let node_power_contribution = power_vector[node_idx];

                    // then I add it to the power source

                    *node_power_source += node_power_contribution;
                }
            }
        }
        ...
        // back node calculation (first node)
        {
            ...
            power_source_vector[0] = sum_of_lateral_conductance_times_lateral_temperatures[0]
            ...
            if !forward_flow {
                ...
                let enthalpy_of_adjacent_node_to_the_front: AvailableEnergy = try_get_h(
                    ...
                    tube_side_fluid_array_for_single_tube_clone.temperature_array_current_timestep
                        [1],
                    tube_side_fluid_array_for_single_tube_clone
                        .back_single_cv
                ...
                power_source_vector[0] += mass_flowrate_for_single_tube * h_fluid_last_timestep;
            }
        }
        ...
        if number_of_nodes > 2 {
            ...
            for i in 1..number_of_nodes - 1 {
                ...
                power_source_vector[i] = sum_of_lateral_conductance_times_lateral_temperatures[i]
                    ...
                    + tube_side_fluid_array_for_single_tube_clone
                        .temperature_array_current_timestep[i]
                        * total_volume
                ...
                if forward_flow {
                    ...
                    let h_fluid_adjacent_node: AvailableEnergy = try_get_h(
                        tube_side_fluid_array_for_single_tube_clone
                            ...
                            .material_control_volume,
                        tube_side_fluid_array_for_single_tube_clone
                            .temperature_array_current_timestep[i - 1],
                        tube_side_fluid_array_for_single_tube_clone
                            .back_single_cv
                            .pressure_control_volume,
                    )?;
        ...
        let reynolds: Ratio = tube_side_fluid_array_for_single_tube_clone
            .get_reynolds(mass_flowrate_for_single_tube)?
            .abs();

        let prandtl_number: Ratio = try_get_prandtl(material, bulk_temperature, pressure)?;
        ...
        if low_peclet_number_flow {
            ...
            let average_fluid_conductivity =
                try_get_kappa_thermal_conductivity(material, bulk_temperature, pressure)?;

            // note that conductance axially is done only ONCE
            // per timestep to expedite the speed of calculation

            average_axial_conductance = average_fluid_conductivity
                * tube_side_fluid_array_for_single_tube_clone.xs_area
                / node_length;
            ...
            for node_idx in 0..number_of_nodes {
                ...
                if bulk_node {
                    ...
                    // coefficient matrix
                    coefficient_matrix[[node_idx, node_idx]] += 2.0 * average_axial_conductance;
                    ...
                    coefficient_matrix[[node_idx, node_idx + 1]] -= average_axial_conductance;
                ...
                if first_node {
                    ...
                    coefficient_matrix[[node_idx, node_idx]] += average_axial_conductance;
                    coefficient_matrix[[node_idx, node_idx + 1]] -= average_axial_conductance;
                }
                // last node
                if last_node {
        ...
        tube_side_fluid_array_for_single_tube_clone
            .back_single_cv
            .current_timestep_control_volume_specific_enthalpy = back_node_enthalpy_next_timestep;
        ...
        tube_side_fluid_array_for_single_tube_clone
            .front_single_cv
            .current_timestep_control_volume_specific_enthalpy = front_node_enthalpy_next_timestep;
        // let's also update the previous temperature vector

        tube_side_fluid_array_for_single_tube_clone
            .set_temperature_array(new_temperature_array.clone())?;
        ...
        //
        let back_cv_temperature: ThermodynamicTemperature = *new_temperature_array.first().unwrap();

        let front_cv_temperature: ThermodynamicTemperature = *new_temperature_array.last().unwrap();

        tube_side_fluid_array_for_single_tube_clone
            .back_single_cv
            .temperature = back_cv_temperature;
        tube_side_fluid_array_for_single_tube_clone
            .front_single_cv
            .temperature = front_cv_temperature;

        // set liquid cv mass after the temperature
        tube_side_fluid_array_for_single_tube_clone
            .back_single_cv
            .set_liquid_cv_mass_from_temperature()?;
        tube_side_fluid_array_for_single_tube_clone
            .front_single_cv
            .set_liquid_cv_mass_from_temperature()?;
    ...
    fn advance_timestep_for_parallel_tube_side_solid_column_bundle(
        ...
        let mut inner_pipe_shell_for_single_tube_clone: SolidColumn = self
        ...
        let back_cv_rate_enthalpy_change_vector: Vec<Power> =
            inner_pipe_shell_for_single_tube_clone
                .back_single_cv
                .rate_enthalpy_change_vector
                .clone();
        ...
        let dt = timestep;
        let node_length =
            inner_pipe_shell_for_single_tube_clone.total_length / number_of_nodes as f64;

        // for the fluid array, we do not keep track of node enthalpies,
        // instead, we keep track of temperatures and then calculate
        // enthalpy changes using rho_cp calculated at the current
        // timestep
        let volume_fraction_array: Array1<f64> = inner_pipe_shell_for_single_tube_clone
            .volume_fraction_array
            .iter()
            .map(|&vol_frac| vol_frac)
            .collect();
        // rho_cp is determined by the temperature array
        let rho_cp: Array1<VolumetricHeatCapacity> = inner_pipe_shell_for_single_tube_clone
            .temperature_array_current_timestep
            .iter()
            .map(|&temperature| try_get_rho_cp(material, temperature, pressure).unwrap())
            .collect();
        ...
        let lateral_temperature_arary_connected: bool = inner_pipe_shell_for_single_tube_clone
            .lateral_adjacent_array_conductance_vector
            .len()
            > 0;

        let _axial_conduction_only: bool =
            !lateral_power_sources_connected && !lateral_temperature_arary_connected;
        ...
        if lateral_temperature_arary_connected {
            ...
            for conductance_array in inner_pipe_shell_for_single_tube_clone
                .lateral_adjacent_array_conductance_vector
                .iter()
            {
                // now I'm inside the each conductance array,
            ...
            for (lateral_idx, conductance_arr) in inner_pipe_shell_for_single_tube_clone
                .lateral_adjacent_array_conductance_vector
                .iter()
                .enumerate()
            {
        ...
        if lateral_power_sources_connected {
            ...
            for (lateral_idx, q_reference) in inner_pipe_shell_for_single_tube_clone
                .q_vector
                .iter()
                .enumerate()
            {
                ...
                let power_ndarray: Array1<Power> = power_frac_array.map(|&power_frac| {
        ...
        // back node calculation (first node)
        {
            ...
            // Now I'm ready to construct the M matrix
            // belong in the M matrix, the rest belong in S
            coefficient_matrix[[0, 0]] = volume_fraction_array[0] * rho_cp[0] * single_shell_volume
                / dt
                + sum_of_lateral_conductances[0];
        ...
        if number_of_nodes > 2 {
            ...
            for i in 1..number_of_nodes - 1 {
                ...
                coefficient_matrix[[i, i]] =
                    volume_fraction_array[i] * rho_cp[i] * single_shell_volume / dt
                        + sum_of_lateral_conductances[i];
        ...
        // front node (last node) calculation
        {
            ...
            coefficient_matrix[[i, i]] = volume_fraction_array[i] * rho_cp[i] * single_shell_volume
                / dt
                + sum_of_lateral_conductances[i];
            // the first part of the source term deals with
            // the flow direction independent terms
        ...
        let average_axial_conductance: ThermalConductance = average_thermal_conductivity
            * inner_pipe_shell_for_single_tube_clone.get_component_xs_area()
            / node_length;
        ...
        self.inner_pipe_shell_array_for_single_tube = inner_pipe_shell_for_single_tube_clone.into();
    ...
    ///
    pub fn advance_timestep_thread_spawn(&self, timestep: Time) -> JoinHandle<Self> {
        // make a clone
