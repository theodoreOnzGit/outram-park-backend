...
impl SimpleShellAndTubeHeatExchanger {
    ...
    pub fn lateral_and_miscellaneous_connections(
        ...
        let outer_node_layer_to_air_conductance =
            self.get_air_to_outer_sthe_layer_nodal_conductance(heat_transfer_to_ambient)?;
        ...
        let outer_shell_to_shell_side_fluid_conductance: ThermalConductance = self
            .get_shell_side_fluid_to_outer_pipe_shell_nodal_conductance(
        ...
        let single_tube_to_shell_side_fluid_conductance: ThermalConductance = self
            .get_shell_side_fluid_to_single_inner_pipe_shell_nodal_conductance(
                prandtl_wall_correction_setting,
            )?;
        let single_tube_to_tube_side_fluid_conductance: ThermalConductance = self
            .get_single_tube_side_fluid_array_node_to_inner_pipe_shell_nodal_conductance(
                prandtl_wall_correction_setting,
            )?;

        let tube_bundle_to_shell_side_fluid_conductance: ThermalConductance =
            single_tube_to_shell_side_fluid_conductance * self.number_of_tubes as f64;
        ...
        // now for the lateral linkages
        {
            ...
            let mut single_inner_tube_fluid_arr_clone: FluidArray = self
                .tube_side_fluid_array_for_single_tube
                .clone()
                .try_into()?;

            let mut single_inner_pipe_shell_clone: SolidColumn = self
            ...
            let mut outer_shell_clone: SolidColumn = self.outer_shell.clone().try_into()?;
            ...
            single_inner_tube_fluid_arr_clone.lateral_link_new_temperature_vector_avg_conductance(
                single_tube_to_tube_side_fluid_conductance,
                single_inner_pipe_shell_arr_temp_vec.clone(),
            )?;

            single_inner_pipe_shell_clone.lateral_link_new_temperature_vector_avg_conductance(
                single_tube_to_tube_side_fluid_conductance,
            ...
            shell_side_fluid_arr_clone.lateral_link_new_temperature_vector_avg_conductance(
            ...
            outer_shell_clone.lateral_link_new_temperature_vector_avg_conductance(
                outer_shell_to_shell_side_fluid_conductance,
            ...
            self.shell_side_fluid_array
                .set(shell_side_fluid_arr_clone.into())?;
    ...
    #[inline]
    pub fn get_air_to_outer_sthe_layer_nodal_conductance(
        &mut self,
        ...
        if self.heat_exchanger_has_insulation {
            ...
            heated_length = shell_side_fluid_component_clone.get_component_length_immutable();

            // surface temperature is the insulation bulk temperature
            // (estimated)

            // the outer node clone is insulation if it is switched on
            outer_solid_array_clone = self.insulation_array.clone().try_into()?;

            outer_node_temperature = outer_solid_array_clone.try_get_bulk_temperature()?;
        } else {
            ...
            outer_solid_array_clone = self.outer_shell.clone().try_into()?;
        ...
        let outer_node_air_conductance_interaction: HeatTransferInteractionType =
            HeatTransferInteractionType::CylindricalConductionConvectionLiquidOutside(
                ...
                (h_air_to_pipe_surf, od.into(), node_length.into()),
            );
    ...
    pub fn get_single_tube_side_fluid_array_node_to_inner_pipe_shell_nodal_conductance(
        ...
        let mut tube_side_single_fluid_array_clone: FluidArray = self
            .tube_side_fluid_array_for_single_tube
            .clone()
            .try_into()?;

        let mut pipe_shell_clone: SolidColumn = self
            .inner_pipe_shell_array_for_single_tube
            .clone()
            .try_into()?;
    ...
    pub fn get_shell_side_fluid_to_single_inner_pipe_shell_nodal_conductance(
        ...
        let shell_side_cross_sectional_flow_area: Area = self.get_shell_side_cross_sectional_area();
    ...
    pub fn get_shell_side_fluid_to_outer_pipe_shell_nodal_conductance(
        ...
        let solid_material: SolidMaterial = outer_shell_clone.material_control_volume.try_into()?;
        ...
        let pipe_darcy_correlation = &self.shell_side_custom_component_loss_correlation;
    ...
    pub fn get_outer_pipe_shell_to_insulation_conductance(
        ...
        let mut pipe_shell_clone: SolidColumn = self.outer_shell.clone().try_into()?;
        ...
        let pipe_shell_mid_section_diameter = 0.5 * (self.shell_side_od + self.shell_side_id);

        let insulation_material_mid_section_diameter =
            self.insulation_thickness + self.shell_side_od;
        ...
        let solid_pipe_material: SolidMaterial =
            pipe_shell_clone.material_control_volume.try_into()?;

        let solid_pipe_material_conductivity: ThermalConductivity = solid_pipe_material
            .try_get_thermal_conductivity(solid_pipe_material_shell_temperature)?;
