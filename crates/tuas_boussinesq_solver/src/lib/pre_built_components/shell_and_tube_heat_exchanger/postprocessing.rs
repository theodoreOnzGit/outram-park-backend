...
impl SimpleShellAndTubeHeatExchanger {
    ...
    pub fn inner_pipe_shell_temperature(
        &mut self,
    ...
    pub fn inner_tube_fluid_array_temperature(
        &mut self,
    ) -> Result<Vec<ThermodynamicTemperature>, TuasLibError> {
    ...
    pub fn shell_side_fluid_array_temperature(
        &mut self,
    ) -> Result<Vec<ThermodynamicTemperature>, TuasLibError> {
    ...
    pub fn shell_side_outer_tube_array_temperature(
        &mut self,
    ) -> Result<Vec<ThermodynamicTemperature>, TuasLibError> {
    ...
    pub fn insulation_array_temperature(
        &mut self,
    ) -> Result<Vec<ThermodynamicTemperature>, TuasLibError> {
    ...
    pub fn overall_htc_based_on_conductance(
        &mut self,
        ...
        let one_over_ua_shell_node: ThermalResistance = single_tube_to_shell_side_fluid_conductance
            .recip()
            + single_tube_to_tube_side_fluid_conductance.recip();

        let ua_shell_node: ThermalConductance = one_over_ua_shell_node.recip();
        ...
        let mut single_inner_tube_fluid_arr_clone: FluidArray = self
            ...
            .try_into()
            .unwrap();
        let heated_length = single_inner_tube_fluid_arr_clone.get_component_length();
        ...
        let u_from_conductance: HeatTransfer = ua_shell_node / area_shell_for_one_node;
    ...
    /// + 1/h_s
    pub fn overall_heat_transfer_coeff_u_shell_side(
        &self,
        correct_for_prandtl_wall_temperatures: bool,
        ...
        let tube_fluid_material: LiquidMaterial = tube_side_single_fluid_array_clone
        ...
        let reynolds_number_single_tube_abs_for_nusselt_estimate: Ratio =
            (single_tube_mass_flowrate / single_tube_flow_area * single_tube_hydraulic_diameter
                / viscosity)
                .abs();

        // next, bulk prandtl number

        let bulk_prandtl_number_tube_side: Ratio = tube_fluid_material
            .try_get_prandtl_liquid(tube_fluid_temperature, atmospheric_pressure)?;
        ...
        if correct_for_prandtl_wall_temperatures {
            ...
            wall_prandtl_number_tube_side = tube_fluid_material
                .try_get_prandtl_liquid(wall_temperature_estimate, atmospheric_pressure)?;

            nusselt_estimate_tube_side = self
        ...
        let k_fluid_average_tube_side: ThermalConductivity =
            tube_fluid_material.try_get_thermal_conductivity(tube_fluid_temperature)?;

        let tube_side_h =
            nusselt_estimate_tube_side * k_fluid_average_tube_side / single_tube_hydraulic_diameter;

        let d_o_by_d_i: Ratio = self.tube_side_od / self.tube_side_id;

        // 1/h_t * d_o/d_i
        let one_over_ht_times_do_by_di = 1.0 as f64 / tube_side_h * d_o_by_d_i;

        // now for the shell conductivity part
        let shell_conductivity_average: ThermalConductivity =
            solid_material.try_get_thermal_conductivity(pipe_shell_surf_temperature)?;

        let ln_d_o_by_d_i: f64 = d_o_by_d_i.get::<ratio>().ln();

        // d_o/(2 lambda_w) * ln (d_o_by_d_i)

        let do_by_2_lambda_w_times_ln_do_by_di =
            self.tube_side_od * 0.5 / (shell_conductivity_average) * ln_d_o_by_d_i;
        ...
        let shell_fluid_material: LiquidMaterial = shell_side_fluid_array_clone
            .material_control_volume
            .try_into()?;
        ...
        // and live with it for the time being
        //
        let reynolds_number_shell_side_abs_for_nusselt_estimate: Ratio = (shell_side_mass_flowrate
        ...
        let bulk_prandtl_number_shell_side: Ratio = shell_fluid_material
            .try_get_prandtl_liquid(shell_side_fluid_temperature, atmospheric_pressure)?;
        ...
        let fldk_shell_side: Ratio = self
            ...
            .fldk_based_on_darcy_friction_factor(
        ...
        if correct_for_prandtl_wall_temperatures {
            ...
            let wall_prandtl_number_estimate: Ratio = shell_fluid_material
                .try_get_prandtl_liquid(wall_temperature_estimate, atmospheric_pressure)?;
        ...
        let k_fluid_average_shell_side: ThermalConductivity =
            shell_fluid_material.try_get_thermal_conductivity(shell_side_fluid_temperature)?;

        shell_side_h_to_fluid = nusselt_estimate_shell_side * k_fluid_average_shell_side
            / shell_side_fluid_hydraulic_diameter;
        ...
        let one_over_u =
            one_over_ht_times_do_by_di + do_by_2_lambda_w_times_ln_do_by_di + one_over_hs;
        ...
        let u_based_on_shell_side_area: HeatTransfer = 1.0 as f64 / one_over_u;
    ...
    pub fn nusselt_tube_side(&self) -> Ratio {
        ...
        let tube_fluid_temperature: ThermodynamicTemperature = tube_side_single_fluid_array_clone
        ...
        let viscosity: DynamicViscosity = tube_fluid_material
            .try_get_dynamic_viscosity(tube_fluid_temperature)
            .unwrap();
    ...
    pub fn nusselt_number_shell_side_to_tubes(&self) -> Ratio {
        ...
        let shell_side_fluid_temperature: ThermodynamicTemperature = shell_side_fluid_array_clone
            .try_get_bulk_temperature()
            .unwrap();
        ...
        let viscosity: DynamicViscosity = shell_fluid_material
            .try_get_dynamic_viscosity(shell_side_fluid_temperature)
            .unwrap();
    ...
    pub fn wall_prandtl_number_shell_side_fluid_for_inner_tube(&self) -> Ratio {
        ...
        let wall_prandtl_number_estimate_shell_side_to_tubes: Ratio = shell_fluid_material
            .try_get_prandtl_liquid(wall_temperature_estimate, atmospheric_pressure)
            .unwrap();
    ...
    pub fn wall_prandtl_number_shell_side_fluid_for_outer_tube(&self) -> Ratio {
        ...
        let mut outer_pipe_shell_clone: SolidColumn = self.outer_shell.clone().try_into().unwrap();
    ...
    pub fn nusselt_number_shell_side_parasitic(&self) -> Ratio {
        let mut outer_shell_clone: SolidColumn = self.outer_shell.clone().try_into().unwrap();
        ...
        nusselt_estimate_shell_side_to_outer_shell = shell_side_fluid_parasitic_nusselt_correlation
            .estimate_based_on_prandtl_darcy_and_reynolds_wall_correction(
                bulk_prandtl_number_shell_side,
                wall_prandtl_number_estimate,
                modifed_darcy_friction_factor_shell_side,
    ...
    pub fn circular_tube_bundle_heat_transfer_area_shell_side(&self) -> Area {
        ...
        let pipe_shell_clone: SolidColumn = self
            .inner_pipe_shell_array_for_single_tube
            .clone()
            .try_into()
            .unwrap();
        ...
        let shell_area: Area = n_t as f64 * PI * d_o * l;
    ...
    pub fn circular_tube_bundle_heat_transfer_area_tube_side(&self) -> Area {
        ...
        let tube_side_area: Area = n_t as f64 * PI * d_i * l;
