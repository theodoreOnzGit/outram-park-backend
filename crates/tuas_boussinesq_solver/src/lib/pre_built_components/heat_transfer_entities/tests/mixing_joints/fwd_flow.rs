...
#[cfg(test)]
#[test]
pub fn adiabatic_mixing_joint_test_link_to_front_and_back() {
    ...
    let mut inlet_bc_hot = HeatTransferEntity::new_const_temperature_bc(hot_temp);
    let mut inlet_bc_cold = HeatTransferEntity::new_const_temperature_bc(cold_temp);
    ...
    let mut inlet_pipe_1: HeatTransferEntity = FluidArray::new_cylinder(
        length,
        hydraulic_diameter,
        initial_temperature,
        initial_pressure,
    ...
    let mut inlet_pipe_2: HeatTransferEntity = FluidArray::new_cylinder(
        length,
        hydraulic_diameter,
        initial_temperature,
        initial_pressure,
    ...
    let mut outlet_pipe: HeatTransferEntity = FluidArray::new_cylinder(
        length,
        hydraulic_diameter,
        initial_temperature,
        initial_pressure,
    ...
    let advection_heat_transfer_interaction_pre_joint: HeatTransferInteractionType;

    let mass_flowrate_inlets = MassRate::new::<kilogram_per_second>(0.05);
    let mass_flowrate_outlet = MassRate::new::<kilogram_per_second>(0.1);

    let average_therminol_density = LiquidMaterial::TherminolVP1
        .try_get_density(cold_temp)
        .unwrap();
    ...
    let advection_heat_transfer_interaction_post_joint: HeatTransferInteractionType;
    ...
    while simulation_time < max_time {
        ...
        inlet_pipe_1
            .try_set_flowrate_for_fluid_array(mass_flowrate_inlets)
            .unwrap();
        inlet_pipe_2
            .try_set_flowrate_for_fluid_array(mass_flowrate_inlets)
            .unwrap();
        ...
        // you MUST set mass flowrate before advancing timestep
        outlet_pipe
            .try_set_flowrate_for_fluid_array(mass_flowrate_outlet)
            .unwrap();
    ...
    let inlet_pipe_1_temp = inlet_pipe_1.try_get_bulk_temperature().unwrap();
    let inlet_pipe_2_temp = inlet_pipe_2.try_get_bulk_temperature().unwrap();
    let mixing_joint_temp = mixing_joint_cv.try_get_bulk_temperature().unwrap();

    let outlet_pipe_temp = outlet_pipe.try_get_bulk_temperature().unwrap();
...
pub fn adiabatic_mixing_joint_test_link_to_front_only() {
    ...
    use crate::prelude::beta_testing::{
        link_heat_transfer_entity, FluidArray, HeatTransferEntity, HeatTransferInteractionType,
    };
    use crate::single_control_vol::SingleCVNode;
...
pub fn adiabatic_mixing_joint_test_hte_single_cv_only() {
    ...
    use crate::prelude::beta_testing::{
        link_heat_transfer_entity, HeatTransferEntity, HeatTransferInteractionType,
    };
    use crate::single_control_vol::SingleCVNode;
    ...
    let mut inlet_pipe_1: HeatTransferEntity = mixing_node.clone().into();

    let mut inlet_pipe_2: HeatTransferEntity = mixing_node.clone().into();

    let mut outlet_pipe: HeatTransferEntity = mixing_node.clone().into();

    let advection_heat_transfer_interaction_pre_joint_cold: HeatTransferInteractionType;

    let mass_flowrate_inlets = MassRate::new::<kilogram_per_second>(0.05);
    let mass_flowrate_outlet = MassRate::new::<kilogram_per_second>(0.1);

    let average_therminol_density_cold = LiquidMaterial::TherminolVP1
        .try_get_density(cold_temp)
        .unwrap();
    let average_therminol_density_hot = LiquidMaterial::TherminolVP1
        .try_get_density(hot_temp)
        .unwrap();
    let average_therminol_density_mid = LiquidMaterial::TherminolVP1
        .try_get_density(ThermodynamicTemperature::new::<degree_celsius>(75.0))
        .unwrap();
    ...
    let mixing_joint_temp_degc = mixing_joint_temp.get::<degree_celsius>();

    approx::assert_abs_diff_eq!(mixing_joint_temp_degc, 75.0, epsilon = 0.6);

    let outlet_temp = outlet_pipe.try_get_bulk_temperature().unwrap();

    let outlet_temp_degc = outlet_temp.get::<degree_celsius>();

    approx::assert_abs_diff_eq!(outlet_temp_degc, 75.0, epsilon = 0.6);
}
