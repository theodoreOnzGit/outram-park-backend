...
/// two metallic shell nodes in the radial direction using FluidArray and
/// SolidColumn objects wrapped in HeatTransferEntity Enums
///
/// This test checks the impact of
/// using the matrix solvers on calculation speed in comparison to using
/// only SingleCV objects,
///
/// However, it also doubles as an example for which to construct
/// a component using HeatTransferEntity enums
...
pub fn ciet_heater_v2_speedup_using_fluid_array_solid_column_hte() {
    ...
    let therminol_array: HeatTransferEntity = FluidArray::new_odd_shaped_pipe(
        heated_length,
        hydraulic_diameter,
        flow_area,
        initial_temperature,
    ...
    let steel_shell_array: HeatTransferEntity = SolidColumn::new_cylindrical_shell(
        heated_length,
        id,
        od,
        initial_temperature,
    ...
    let fluid_array_ptr = Arc::new(Mutex::new(therminol_array));

    let solid_array_ptr = Arc::new(Mutex::new(steel_shell_array));
    ...
    let calculation_loop = move || {
        ...
        let mut wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "array_cv_test_ciet_heater_v_2_0_steady_state.csv",
        ))
        .unwrap();
        ...
        let mut time_wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "array_cv_test_ciet_heater_v_2_0_calc_time_profile.csv",
        ))
        .unwrap();
        ...
        while current_time_simulation_time <= *max_time_ptr.deref() {
            ...
            let mut therminol_entity_ptr_in_loop = fluid_array_ptr_for_loop.lock().unwrap();

            let mut steel_entity_ptr_in_loop = solid_array_ptr_for_loop.lock().unwrap();
            ...
            let mut therminol_array_clone: FluidArray = therminol_entity_ptr_in_loop
                .deref_mut()
                .clone()
                .try_into()
                .unwrap();
            let mut steel_array_clone: SolidColumn = steel_entity_ptr_in_loop
                .deref_mut()
                .clone()
                .try_into()
                .unwrap();
            ...
            // test
            // heater 2.0
            let h_to_therminol: HeatTransfer = _heat_transfer_coefficient_ciet_v_2_0(
            ...
            // lateral connections
            {
                ...
                steel_array_clone
                    .lateral_link_new_power_vector(heater_steady_state_power, q_frac_arr)
                    .unwrap();
                ...
                therminol_array_clone.set_mass_flowrate(therminol_mass_flowrate);

                // now that lateral connections are done,
                // modify the heat transfer entity
                therminol_entity_ptr_in_loop
                    .deref_mut()
                    .set(therminol_array_clone.clone().into())
                    .unwrap();

                steel_entity_ptr_in_loop
                    .deref_mut()
                    .set(steel_array_clone.clone().into())
                    .unwrap();
            }
            ...
            // axial connection
            {
                ...
                // bcs
                let mut adiabatic_bc: HeatTransferEntity = BCType::new_adiabatic_bc().into();
                let mut inlet_temperature_bc: HeatTransferEntity =
                    BCType::new_const_temperature(inlet_temperature).into();
                ...
                let outlet_advection_interaction: HeatTransferInteractionType =
                    DataAdvection::new_from_temperature_and_liquid_material(
                        ...
                        therminol_array_clone
                            .front_single_cv
                            .get_temperature_from_enthalpy_and_set()
                            .unwrap(),
                        therminol_array_clone
                            .front_single_cv
                            .get_temperature_from_enthalpy_and_set()
                ...
                therminol_entity_ptr_in_loop
                    .deref_mut()
                    .link_to_back(&mut inlet_temperature_bc, inlet_advection_interaction)
                    .unwrap();

                therminol_entity_ptr_in_loop
                    .deref_mut()
                    .link_to_front(&mut adiabatic_bc, outlet_advection_interaction)
                    .unwrap();

                steel_entity_ptr_in_loop
                    .deref_mut()
                    .link_to_front(&mut adiabatic_bc, constant_heat_addition)
                    .unwrap();

                steel_entity_ptr_in_loop
                    .deref_mut()
                    .link_to_back(&mut adiabatic_bc, constant_heat_addition)
                    .unwrap();
            }
            ...
            // record current fluid temperature profile
            {
                ...
                let steel_column: SolidColumn = steel_entity_ptr_in_loop
                    .deref_mut()
                    .clone()
                    .try_into()
                    .unwrap();

                let steel_temperature_array = steel_column.get_temperature_vector().unwrap();
            ...
            // outlet temperature profile
            {
                ...
                let mut therminol_fluid_arr: FluidArray = therminol_entity_ptr_in_loop
                    .deref_mut()
                    .clone()
                    .try_into()
                    .unwrap();

                let therminol_outlet_temp: ThermodynamicTemperature = therminol_fluid_arr
                    .front_single_cv
                    .get_temperature_from_enthalpy_and_set()
                    .unwrap();
                ...
                let shell_celsius_string = st_11_node_temp.get::<degree_celsius>().to_string();

                // timestep in seconds
                //

                let timestep_string = timestep.get::<second>().to_string();
...
#[inline]
fn _ciet_heater_v_2_0_nusselt_number(reynolds: Ratio, prandtl: Ratio) -> Ratio {
...
#[inline]
fn _ciet_heater_v_2_0_reynolds_nunber(mass_flowrate: MassRate, mu: DynamicViscosity) -> Ratio {
    // Re = m* D_H/ A_{XS}/mu
    let hydraulic_diameter = Length::new::<meter>(0.01467);
    let flow_area = Area::new::<square_meter>(0.00105);
...
fn _heat_transfer_coefficient_ciet_v_2_0(
    ...
    let reynolds: Ratio = _ciet_heater_v_2_0_reynolds_nunber(mass_flowrate, mu);

    let prandtl: Ratio = try_get_prandtl(therminol, therminol_temperature, pressure).unwrap();

    let nusselt: Ratio = _ciet_heater_v_2_0_nusselt_number(reynolds, prandtl);
