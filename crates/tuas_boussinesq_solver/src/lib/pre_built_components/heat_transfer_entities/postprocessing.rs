...
impl HeatTransferEntity {
    ...
    pub fn temperature(
        ...
        let cv_temperature_result = match entity {
            Self::ControlVolume(cv_type) => match cv_type {
                CVType::SingleCV(single_cv) => single_cv.get_temperature_from_enthalpy_and_set(),
                CVType::FluidArrayCV(fluid_array_cv) => fluid_array_cv.try_get_bulk_temperature(),
                CVType::SolidArrayCV(solid_array_cv) => solid_array_cv.try_get_bulk_temperature(),
            },
            Self::BoundaryConditions(bc_type) => match bc_type {
                BCType::UserSpecifiedTemperature(temperature) => Ok(*temperature),
                BCType::UserSpecifiedHeatFlux(_) | BCType::UserSpecifiedHeatAddition(_) => {
                    return Err(TuasLibError::NotImplementedForBoundaryConditions(
                        "getting temperature not \n 
                                implemented for BoundaryConditions"
                            .to_owned(),
                    ));
                }
            },
    ...
    pub fn try_get_bulk_temperature(&mut self) -> Result<ThermodynamicTemperature, TuasLibError> {
        let bulk_temp: ThermodynamicTemperature = HeatTransferEntity::temperature(self)?;
    ...
    #[inline]
    pub fn temperature_vector(
        entity: &mut HeatTransferEntity,
    ) -> Result<Vec<ThermodynamicTemperature>, TuasLibError> {
        // oof nested matching, kind of ugly but I'll live with it for now
        let cv_temperature_vector_result: Result<Vec<ThermodynamicTemperature>, TuasLibError> =
            match entity {
                HeatTransferEntity::ControlVolume(cv_type) => {
                    let temp_vector_result: Result<Vec<ThermodynamicTemperature>, TuasLibError> =
                        match cv_type {
    ...
    pub fn density_vector(
        ...
        let material = match entity {
            HeatTransferEntity::ControlVolume(cv) => match cv {
                CVType::SingleCV(single_cv) => single_cv.material_control_volume.clone(),
                CVType::FluidArrayCV(fluid_array_cv) => fluid_array_cv.material_control_volume,
                CVType::SolidArrayCV(solid_array_cv) => solid_array_cv.material_control_volume,
            },
        ...
        let pressure = match entity {
            HeatTransferEntity::ControlVolume(cv) => match cv {
                CVType::SingleCV(single_cv) => single_cv.pressure_control_volume,
                CVType::FluidArrayCV(fluid_array_cv) => fluid_array_cv.pressure_control_volume,
                CVType::SolidArrayCV(solid_array_cv) => solid_array_cv.pressure_control_volume,
            },
            HeatTransferEntity::BoundaryConditions(_) => {
                return Err(TuasLibError::NotImplementedForBoundaryConditions(
                    "density not implemented for BC".to_string(),
        ...
        for temperature in temperature_vector {
            let density = try_get_rho(material, temperature, pressure)?;
