...
use crate::heat_transfer_correlations::heat_transfer_interactions::heat_transfer_geometry::CylindricalAndSphericalSolidFluidArrangement;
...
impl HeatTransferEntity {
    ...
    #[inline]
    pub fn link_to_front(
        &mut self,
        other_hte: &mut HeatTransferEntity,
        interaction: HeatTransferInteractionType,
    ) -> Result<(), TuasLibError> {
        link_heat_transfer_entity(self, other_hte, interaction)
    }
    ...
    pub fn link_to_back(
        ...
        link_heat_transfer_entity(other_hte, self, interaction)
    }
    ...
    pub fn link(
        ...
        link_heat_transfer_entity(entity, other_hte, interaction)
    }
    ...
    pub fn try_set_flowrate_for_fluid_array(
        ...
        match self {
            HeatTransferEntity::ControlVolume(CVType::FluidArrayCV(fluid_arr)) => {
...
#[inline]
pub fn link_heat_transfer_entity(
    ...
    let heat_transfer_entity_result = match (entity_1, entity_2) {
        ...
        ) => calculate_control_volume_serial(cv_type_1, cv_type_2, interaction),
        (
            HeatTransferEntity::BoundaryConditions(bc_type),
            HeatTransferEntity::ControlVolume(cv_type),
        ) => attach_boundary_condition_to_control_volume_back_serial(bc_type, cv_type, interaction),
        (
            HeatTransferEntity::ControlVolume(cv_type),
            HeatTransferEntity::BoundaryConditions(bc_type),
        ) => {
            attach_boundary_condition_to_control_volume_front_serial(cv_type, bc_type, interaction)
        }
        (
...
pub(crate) fn calculate_control_volume_serial(
    ...
    let cv_result = match (control_vol_1, control_vol_2) {
        (CVType::SingleCV(single_cv_1), CVType::SingleCV(single_cv_2)) => {
            calculate_between_two_singular_cv_nodes(single_cv_1, single_cv_2, interaction)
        }
        // basically like this
        //
        // (array_cv_back -----array_cv --- array_cv_front) --- (single_cv)
        (CVType::SolidArrayCV(solid_array_cv), CVType::SingleCV(single_cv)) => {
            solid_array_cv.link_single_cv_to_higher_side(single_cv, interaction)
        }
        // basically like this
        //
        // (array_cv_back -----array_cv --- array_cv_front) --- (single_cv)
        (CVType::FluidArrayCV(fluid_array_cv), CVType::SingleCV(single_cv)) => {
            fluid_array_cv.link_single_cv_to_higher_side(single_cv, interaction)
        }
        // basically like this
        //
        // (single_cv) --- (array_cv_back -----array_cv --- array_cv_front)
        (CVType::SingleCV(single_cv), CVType::SolidArrayCV(solid_array_cv)) => {
            solid_array_cv.link_single_cv_to_lower_side(single_cv, interaction)
        }
        // basically like this
        //
        // (single_cv) --- (array_cv_back -----array_cv --- array_cv_front)
        (CVType::SingleCV(single_cv), CVType::FluidArrayCV(fluid_array_cv)) => {
            fluid_array_cv.link_single_cv_to_lower_side(single_cv, interaction)
        }
        // basically like this
        //
        ...
        (CVType::SolidArrayCV(solid_array_cv_1), CVType::SolidArrayCV(solid_array_cv_2)) => {
            solid_array_cv_1
                .link_solid_column_to_the_front_of_this_solid_column(solid_array_cv_2, interaction)
        }
        // basically like this
        //
        // (back --- cv_1 --- front) ---- (back --- cv_2 --- front)
        (CVType::SolidArrayCV(solid_array_cv_1), CVType::FluidArrayCV(fluid_array_cv_2)) => {
            solid_array_cv_1
                .link_fluid_array_to_the_front_of_this_solid_column(fluid_array_cv_2, interaction)
        }
        // basically like this
        //
        // (back --- cv_1 --- front) ---- (back --- cv_2 --- front)
        (CVType::FluidArrayCV(fluid_array_cv_1), CVType::SolidArrayCV(solid_array_cv_2)) => {
            fluid_array_cv_1
                .link_solid_column_to_the_front_of_this_fluid_array(solid_array_cv_2, interaction)
        }
        // basically like this
        //
        // (back --- cv_1 --- front) ---- (back --- cv_2 --- front)
        (CVType::FluidArrayCV(fluid_array_cv_1), CVType::FluidArrayCV(fluid_array_cv_2)) => {
            fluid_array_cv_1
                .link_fluid_array_to_the_front_of_this_fluid_array(fluid_array_cv_2, interaction)
        }
    };
...
pub fn calculate_timescales_for_heat_transfer_entity(
    ...
    let heat_transfer_entity_timestep_result = match (entity_1, entity_2) {
        (
            HeatTransferEntity::ControlVolume(cv_type_1),
            HeatTransferEntity::ControlVolume(cv_type_2),
        ) => calculate_timestep_control_volume_serial(cv_type_1, cv_type_2, interaction),
        (
            HeatTransferEntity::BoundaryConditions(bc_type),
            HeatTransferEntity::ControlVolume(cv_type),
...
pub(crate) fn calculate_timestep_control_volume_front_to_boundary_condition_serial(
    ...
    let cv_bc_result = match (control_vol, boundary_condition) {
        ...
        (CVType::SolidArrayCV(solid_array_cv), BCType::UserSpecifiedHeatFlux(_heat_flux)) => {
            ...
            // condition
            calculate_mesh_stability_conduction_timestep_for_single_node_and_bc(
                &mut solid_array_cv.front_single_cv,
                interaction,
            )
        }
        ...
        (
            ...
            BCType::UserSpecifiedTemperature(_bc_temperature),
        ) => {
            // match the cv to the cv type, extract the back boundary
            // condition
            calculate_mesh_stability_conduction_timestep_for_single_node_and_bc(
        ...
        (CVType::FluidArrayCV(fluid_array_cv), BCType::UserSpecifiedHeatFlux(_heat_flux)) => {
            ...
            // condition
            calculate_mesh_stability_conduction_timestep_for_single_node_and_bc(
                &mut fluid_array_cv.front_single_cv,
                interaction,
            )
        }
...
pub(crate) fn calculate_timestep_control_volume_serial(
    ...
    let cv_result = match (control_vol_1, control_vol_2) {
        (CVType::SingleCV(single_cv_1), CVType::SingleCV(single_cv_2)) => single_cv_1
            .calculate_mesh_stability_timestep_for_two_single_cv_nodes(single_cv_2, interaction),
        (CVType::FluidArrayCV(fluid_array_cv), CVType::SingleCV(single_cv_node)) => fluid_array_cv
            .calculate_timestep_for_single_cv_to_front_of_array_cv(single_cv_node, interaction),
        (CVType::SolidArrayCV(solid_array_cv), CVType::SingleCV(single_cv_node)) => solid_array_cv
            .calculate_timestep_for_single_cv_to_front_of_array_cv(single_cv_node, interaction),
        (CVType::SingleCV(single_cv_node), CVType::SolidArrayCV(solid_array_cv)) => solid_array_cv
            .calculate_timestep_for_single_cv_to_back_of_array_cv(single_cv_node, interaction),
        (CVType::SingleCV(single_cv_node), CVType::FluidArrayCV(fluid_array_cv)) => fluid_array_cv
            .calculate_timestep_for_single_cv_to_back_of_array_cv(single_cv_node, interaction),
        (CVType::FluidArrayCV(fluid_array_cv_1), CVType::FluidArrayCV(fluid_array_cv_2)) => {
            fluid_array_cv_2.calculate_timestep_for_single_cv_to_back_of_array_cv(
                &mut fluid_array_cv_1.front_single_cv,
                interaction,
            )
        }
        (CVType::SolidArrayCV(solid_array_cv_1), CVType::SolidArrayCV(solid_array_cv_2)) => {
            solid_array_cv_2.calculate_timestep_for_single_cv_to_back_of_array_cv(
...
pub(crate) fn calculate_timestep_control_volume_back_to_boundary_condition_serial(
    ...
    let cv_bc_result = match (control_vol, boundary_condition) {
        ...
        (CVType::SolidArrayCV(solid_array_cv), BCType::UserSpecifiedHeatFlux(_heat_flux)) => {
            ...
            // condition
            calculate_mesh_stability_conduction_timestep_for_single_node_and_bc(
                &mut solid_array_cv.back_single_cv,
                interaction,
            )
        }
        (CVType::SolidArrayCV(solid_array_cv), BCType::UserSpecifiedHeatAddition(_heat_rate)) => {
        ...
        (CVType::FluidArrayCV(fluid_array_cv), BCType::UserSpecifiedHeatFlux(_heat_flux)) => {
            ...
            // condition
            calculate_mesh_stability_conduction_timestep_for_single_node_and_bc(
                &mut fluid_array_cv.back_single_cv,
                interaction,
            )
        }
        (CVType::FluidArrayCV(fluid_array_cv), BCType::UserSpecifiedHeatAddition(_heat_rate)) => {
...
///
pub(crate) fn try_get_thermal_conductance_based_on_interaction(
    ...
    let conductance: ThermalConductance = match interaction {
        ...
        HeatTransferInteractionType::CylindricalConductionConvectionLiquidInside(
            (solid_material, shell_thickness, solid_temperature, solid_pressure),
        ...
        HeatTransferInteractionType::DualCylindricalThermalConductance(
            ...
            (inner_diameter, outer_diameter, cylinder_length),
        ) => {
            ...
            expected_outer_diameter = id + inner_thickness + outer_thickness;
        ...
        HeatTransferInteractionType::DualCartesianThermalConductance(
            ...
            let overall_resistance = 1.0 / conductnace_layer_2 + 1.0 / conductnace_layer_1;
        ...
        HeatTransferInteractionType::DualCartesianThermalConductanceThreeDimension(
            ...
            let material_1 = data_dual_cartesian_conduction.material_1;

            let material_2 = data_dual_cartesian_conduction.material_2;

            let thickness_1 = data_dual_cartesian_conduction.thickness_1;

            let thickness_2 = data_dual_cartesian_conduction.thickness_2;

            let xs_area = data_dual_cartesian_conduction.xs_area;
        ...
        HeatTransferInteractionType::UserSpecifiedConvectionResistance(
            ...
            let heat_transfer_coeff: HeatTransfer = data_convection_resistance.heat_transfer_coeff;
            let surf_area: Area = data_convection_resistance.surf_area.into();
        ...
        HeatTransferInteractionType::SimpleRadiation(area_coeff) => {
            simple_radiation_conductance(area_coeff, temperature_1, temperature_2)
        }
    };
...
pub(crate) fn attach_boundary_condition_to_control_volume_front_serial(
    ...
    let cv_bc_result = match (control_vol, boundary_condition) {
        ...
        (CVType::SingleCV(single_cv), BCType::UserSpecifiedHeatFlux(heat_flux)) => {
            calculate_constant_heat_flux_front_single_cv_back(single_cv, *heat_flux, interaction)
        }
        ...
        (CVType::SolidArrayCV(solid_array_cv), BCType::UserSpecifiedHeatFlux(heat_flux)) => {
            solid_array_cv.link_heat_flux_bc_to_front_of_this_cv(*heat_flux, interaction)
        }
        (CVType::SolidArrayCV(solid_array_cv), BCType::UserSpecifiedHeatAddition(heat_rate)) => {
            solid_array_cv.link_heat_addition_to_front_of_this_cv(*heat_rate, interaction)
        }
        (
            CVType::SolidArrayCV(solid_array_cv),
            BCType::UserSpecifiedTemperature(bc_temperature),
        ) => solid_array_cv
            .link_constant_temperature_to_front_of_this_cv(*bc_temperature, interaction),
        (CVType::FluidArrayCV(fluid_array_cv), BCType::UserSpecifiedHeatFlux(heat_flux)) => {
            fluid_array_cv.link_heat_flux_bc_to_front_of_this_cv(*heat_flux, interaction)
        }
        (CVType::FluidArrayCV(fluid_array_cv), BCType::UserSpecifiedHeatAddition(heat_rate)) => {
            fluid_array_cv.link_heat_addition_to_front_of_this_cv(*heat_rate, interaction)
        }
        (
            CVType::FluidArrayCV(fluid_array_cv),
            BCType::UserSpecifiedTemperature(bc_temperature),
...
pub(crate) fn attach_boundary_condition_to_control_volume_back_serial(
    ...
    let cv_bc_result = match (control_vol, boundary_condition) {
        ...
        (CVType::SingleCV(single_cv), BCType::UserSpecifiedHeatFlux(heat_flux)) => {
            calculate_single_cv_front_heat_flux_back(*heat_flux, single_cv, interaction)
        }
        (CVType::SingleCV(single_cv), BCType::UserSpecifiedTemperature(bc_temperature)) => {
        ...
        (CVType::FluidArrayCV(cv), BCType::UserSpecifiedHeatFlux(heat_flux)) => {
            cv.link_heat_flux_bc_to_back_of_this_cv(*heat_flux, interaction)
        }
        (CVType::FluidArrayCV(cv), BCType::UserSpecifiedHeatAddition(heat_rate)) => {
            cv.link_heat_addition_to_back_of_this_cv(*heat_rate, interaction)
        }
        (CVType::FluidArrayCV(cv), BCType::UserSpecifiedTemperature(bc_temperature)) => {
            cv.link_constant_temperature_to_back_of_this_cv(*bc_temperature, interaction)
        }
...
pub fn calculate_constant_temperature_front_single_cv_back(
    ...
    match interaction {
        HeatTransferInteractionType::Advection(advection_dataset) => {
            //
...
pub fn calculate_constant_heat_flux_front_single_cv_back(
    ...
    let heat_flowrate_into_control_vol: Power = heat_flux_into_control_vol * heat_transfer_area;
