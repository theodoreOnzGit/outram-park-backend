...
pub fn calculate_constant_heat_addition_front_single_cv_back(
    ...
    match interaction {
        ...
        _ => {
            println!(
                "you need to specify that the interaction type \n 
...
pub fn calculate_mesh_stability_conduction_timestep_for_single_node_and_bc(
    ...
    // control volume
    let mut cv_timestep: Time = control_vol.calculate_conduction_timestep()?;
    ...
    let cv_alpha: DiffusionCoefficient =
        try_get_alpha_thermal_diffusivity(cv_material, cv_temperature, cv_pressure)?;
    ...
    match interaction {
        ...
        HeatTransferInteractionType::UserSpecifiedHeatFluxCustomArea(area) => {
            ...
            let time_step_max_based_on_volume_to_area: Time =
                max_mesh_fourier_number * volume_to_area_ratio * volume_to_area_ratio / cv_alpha;
        ...
        HeatTransferInteractionType::SingleCartesianThermalConductanceOneDimension(
            ...
            // material
            let cv_alpha: DiffusionCoefficient =
                try_get_alpha_thermal_diffusivity(material, cv_temperature, cv_pressure)?;
            ...
            let time_step_max_based_on_x_thickness: Time =
                max_mesh_fourier_number * lengthscale * lengthscale / cv_alpha;
        ...
        HeatTransferInteractionType::DualCartesianThermalConductance(
            ...
            let alpha_1: DiffusionCoefficient =
                try_get_alpha_thermal_diffusivity(material_1, cv_temperature, cv_pressure)?;

            let alpha_2: DiffusionCoefficient =
                try_get_alpha_thermal_diffusivity(material_2, cv_temperature, cv_pressure)?;

            let length_1: Length = length_1.into();
            let length_2: Length = length_2.into();

            let timestep_1: Time = max_mesh_fourier_number * length_1 * length_1 / alpha_1;

            let timestep_2: Time = max_mesh_fourier_number * length_2 * length_2 / alpha_2;
        ...
        HeatTransferInteractionType::DualCylindricalThermalConductance(
            ...
            let timestep_1: Time = max_mesh_fourier_number * radius_1 * radius_1 / alpha_1;

            let timestep_2: Time = max_mesh_fourier_number * radius_2 * radius_2 / alpha_2;
        ...
        HeatTransferInteractionType::DualCartesianThermalConductanceThreeDimension(
            ...
            let material_1 = data_dual_cartesian_conduction_data.material_1.clone();

            let material_2 = data_dual_cartesian_conduction_data.material_2.clone();
        ...
        HeatTransferInteractionType::CylindricalConductionConvectionLiquidInside(
            (material, radius, temperature, pressure),
            _,
        ) => {
            // for a single node connected to a BC, you're
            // not really supposed to have a timescale
            ...
            let alpha_1: DiffusionCoefficient =
                try_get_alpha_thermal_diffusivity(material, temperature, pressure)?;
        ...
        HeatTransferInteractionType::UserSpecifiedHeatFluxCylindricalOuterArea(l, od) => {
        ...
        HeatTransferInteractionType::UserSpecifiedHeatFluxCylindricalInnerArea(l, id) => {
            // this is treated like a custom area kind of thing
            // so we calculate the area first
...
pub(crate) fn calculate_cv_front_bc_back_advection_for_heat_flux_or_heat_addition(
    ...
    control_vol.calculate_cv_front_bc_back_advection_non_set_temperature(advection_data)
}
/// for advection calculations with heat flux or heat addition BC,
/// the temperature of flows flowing in and out of the BC will be
/// determined by that of the control volume
...
pub(crate) fn calculate_bc_front_cv_back_advection_for_heat_flux_or_heat_addition(
    ...
    // call the method from single_cv_node
    // makes testing easier
    control_vol.calculate_bc_front_cv_back_advection_non_set_temperature(advection_data)
}
