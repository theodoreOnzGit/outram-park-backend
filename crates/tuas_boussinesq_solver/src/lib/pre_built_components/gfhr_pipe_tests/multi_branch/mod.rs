...
use fluid_mechanics_solvers::{
    four_branch_pri_and_intermediate_loop_single_time_step, FHRThermalHydraulicsState,
};
...
pub(crate) fn test_fhr_four_branch_full_th_long_regression() {
    let initial_temperature_pri_loop = ThermodynamicTemperature::new::<degree_celsius>(500.0);
    ...
    let initial_temperature_intrmd_loop = ThermodynamicTemperature::new::<degree_celsius>(500.0);
    ...
    let mut fhr_steam_generator_shell_side_14 =
        new_fhr_intermediate_loop_steam_generator_shell_side_14(initial_temperature_intrmd_loop);
    ...
    let mut top_mixing_node_pri_loop = gfhr_top_mixing_node_pri_loop(initial_temperature_pri_loop);
    ...
    intermediate_heat_exchanger_branch_flow = fhr_state.intermediate_heat_exchanger_branch_flow;
    intrmd_loop_ihx_br_flow = fhr_state.intrmd_loop_ihx_br_flow;
    intrmd_loop_steam_gen_br_flow = fhr_state.intrmd_loop_steam_gen_br_flow;
    ...
    let downcomer_2_temp_profile_degc = fhr_state.downcomer_2_temp_profile_degc;
    let downcomer_3_temp_profile_degc = fhr_state.downcomer_3_temp_profile_degc;
    ...
    let pipe_4_temp_profile_degc = fhr_state.pipe_4_temp_profile_degc;
    let pipe_5_temp_profile_degc = fhr_state.pipe_5_temp_profile_degc;
    let pipe_7_temp_profile_degc = fhr_state.pipe_7_temp_profile_degc;
    let pipe_8_temp_profile_degc = fhr_state.pipe_8_temp_profile_degc;
    let pump_9_temp_profile_degc = fhr_state.pump_9_temp_profile_degc;
    let pipe_10_temp_profile_degc = fhr_state.pipe_10_temp_profile_degc;
    let pipe_11_temp_profile_degc = fhr_state.pipe_11_temp_profile_degc;

    // temp profile of pipes in intrmd loop
    let pipe_12_temp_profile_degc = fhr_state.pipe_12_temp_profile_degc;
    let pipe_13_temp_profile_degc = fhr_state.pipe_13_temp_profile_degc;
    let pipe_15_temp_profile_degc = fhr_state.pipe_15_temp_profile_degc;
    let pump_16_temp_profile_degc = fhr_state.pump_16_temp_profile_degc;
    let pipe_17_temp_profile_degc = fhr_state.pipe_17_temp_profile_degc;
    ...
    assert_eq!(pipe_4_temp_profile_degc, vec![503.18, 503.18],);
    assert_eq!(pipe_5_temp_profile_degc, vec![503.13, 503.16],);
    assert_eq!(ihx_shell_side_temp_profile_degc, vec![485.64, 494.52],);
    assert_eq!(
        pipe_7_temp_profile_degc,
        vec![485.63, 485.63, 485.64, 485.64, 485.64],
    );
    assert_eq!(pipe_8_temp_profile_degc, vec![485.63, 485.63],);
    assert_eq!(pump_9_temp_profile_degc, vec![485.63, 485.63],);
    assert_eq!(
        pipe_10_temp_profile_degc,
        vec![485.60, 485.61, 485.61, 485.61, 485.62, 485.62, 485.62, 485.63],
    );
    assert_eq!(pipe_11_temp_profile_degc, vec![485.60, 485.60],);

    // intermediate loop
    assert_eq!(ihx_tube_side_temp_profile_degc, vec![326.05, 339.55],);
    assert_eq!(pipe_12_temp_profile_degc, vec![339.55, 339.56],);
    assert_eq!(pipe_13_temp_profile_degc, vec![339.58, 339.57],);
    assert_eq!(sg_shell_side_temp_profile_degc, vec![312.11, 325.84],);
    assert_eq!(pipe_15_temp_profile_degc, vec![312.12, 312.12],);
    assert_eq!(pump_16_temp_profile_degc, vec![312.13, 312.12],);
    assert_eq!(pipe_17_temp_profile_degc, vec![312.14, 312.15],);
