...
pub fn heater_branch_with_heater_v2_test() {
    ...
    let branch_5_component: FluidComponent =
        FluidComponent::FluidArray(branch_5.pipe_fluid_array.clone().try_into().unwrap());

    let pipe_4_component: FluidComponent =
        FluidComponent::FluidArray(pipe_4.pipe_fluid_array.clone().try_into().unwrap());

    let pipe_3_component: FluidComponent =
        FluidComponent::FluidArray(pipe_3.pipe_fluid_array.clone().try_into().unwrap());

    let static_mixer_2_component: FluidComponent =
        FluidComponent::FluidArray(static_mixer_2.pipe_fluid_array.try_into().unwrap());

    let static_mixer_pipe_2a_component: FluidComponent =
        FluidComponent::FluidArray(static_mixer_pipe_2a.pipe_fluid_array.try_into().unwrap());

    let heater_top_head_1a_component: FluidComponent =
        FluidComponent::FluidArray(heater_top_head_1a.pipe_fluid_array.try_into().unwrap());

    let heater_1_component: FluidComponent =
        FluidComponent::FluidArray(heated_section_1.pipe_fluid_array.try_into().unwrap());

    let heater_bottom_head_1b_component: FluidComponent =
        FluidComponent::FluidArray(heater_bottom_head_1b.pipe_fluid_array.try_into().unwrap());

    let pipe_18_component: FluidComponent =
        FluidComponent::FluidArray(pipe_18.pipe_fluid_array.try_into().unwrap());

    let heater_branch = FluidComponentCollection {
        components: vec![
            branch_5_component,
            pipe_4_component,
            pipe_3_component,
    ...
    // let's check the hydrostatic pressure, 0.0 kg/s fluid flow
    {
        ...
        let series_pipe_pressure_change = heater_branch.get_pressure_change(pipe_fluid_flow);
...
pub fn ctah_branch_test() {
    ...
    // let's check the hydrostatic pressure, 0.0 kg/s fluid flow
    {
        ...
        let series_pipe_pressure_change = ctah_branch.get_pressure_change(pipe_fluid_flow);
...
pub fn isothermal_ctah_and_heater_branch_validation_test() {
    ...
    fn validate_mass_flowrate_given_pressure_change(
        ...
        let validation_temperature = ThermodynamicTemperature::new::<degree_celsius>(20.0);
        let heater_branch = heater_branch_builder_isothermal_test(validation_temperature);
        let ctah_branch =
            ctah_branch_builder_isothermal_test(test_pressure, validation_temperature);

        // expected flowrate

        // you'll now need to add this into a super collection
        let mut ctah_and_heater_branches = FluidComponentSuperCollection::default();
        ...
        let mass_flowrate_across_each_branch: Vec<MassRate> = ctah_and_heater_branches
        ...
        let mut mass_flowrate_test: MassRate = *mass_flowrate_across_each_branch.first().unwrap();
...
pub fn isothermal_ctah_and_heater_branch_code_to_code_verification_test() {
    ...
    fn verify_mass_flowrate_given_pressure_change(
        ...
        let verification_temperature = ThermodynamicTemperature::new::<degree_celsius>(20.0);
        let heater_branch = heater_branch_builder_isothermal_test(verification_temperature);
        let ctah_branch =
            ctah_branch_builder_isothermal_test(test_pressure, verification_temperature);
...
pub trait IsothermalCIETSolvers {
    ...
    fn isothermal_ciet_solve_pressure_change_across_each_branch_for_ctah_and_heater_branch(
        ctah_and_heater_branch: &FluidComponentSuperCollection,
    ) -> Pressure {
        let pressure_change_root = |pressure_change_pascals: f64| -> f64 {
            ...
            let mass_flowrate_vector_kg_per_s: Vec<f64> = ctah_and_heater_branch
                .fluid_component_super_vector
                .iter()
                .map(|branch: &FluidComponentCollection| {
                    ...
                    let branch_mass_flowrate =
                        branch.get_mass_flowrate_from_pressure_change(pressure_change);

                    branch_mass_flowrate.get::<kilogram_per_second>()
                })
                .collect();

            let total_mass_flowrate_ks_per_s: f64 = mass_flowrate_vector_kg_per_s.into_iter().sum();
        ...
        let hydrostatic_pressure: Pressure = ctah_and_heater_branch
            .fluid_component_super_vector
            .first()
            .unwrap()
            .get_pressure_change(MassRate::ZERO); //

        let upper_bound: Pressure = hydrostatic_pressure + Pressure::new::<pascal>(50000_f64);

        let lower_bound: Pressure = hydrostatic_pressure + Pressure::new::<pascal>(-50000_f64);
        ...
        let pressure_change_pascals = find_root_brent(
