...
impl StructuralSupport {
    ...
    pub fn lateral_and_miscellaneous_connections(&mut self) {
        ...
        let mut steel_shell_clone: SolidColumn = self.support_array.clone().try_into().unwrap();

        // first let's get all the conductances

        let support_to_air_conductance: ThermalConductance =
            self.get_air_to_steel_array_conductance(h_air_to_steel_surf);
    ...
    #[inline]
    pub fn get_air_to_steel_array_conductance(
        &mut self,
        ...
        let node_area: Area = total_lateral_surface_area / number_of_temperature_nodes as f64;
    ...
    pub fn get_axial_node_to_bc_conductance(&mut self) -> ThermalConductance {
        let mut steel_support_clone: SolidColumn = self.support_array.clone().try_into().unwrap();
        let number_of_temperature_nodes = self.inner_nodes + 2;
        let component_length: Length = steel_support_clone.get_component_length();
        let steel_bulk_temperature = steel_support_clone.try_get_bulk_temperature().unwrap();

        let steel: SolidMaterial = steel_support_clone
            .material_control_volume
            .try_into()
            .unwrap();

        let steel_conductivity: ThermalConductivity = steel
            .try_get_thermal_conductivity(steel_bulk_temperature)
            .unwrap();

        let node_length: Length = component_length / number_of_temperature_nodes as f64;

        let node_xs_area = steel_support_clone.get_component_xs_area();

        let conductance: ThermalConductance = steel_conductivity * node_xs_area / node_length * 0.5;
