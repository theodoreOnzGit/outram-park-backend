...
impl NonInsulatedFluidComponent {
    ...
    pub fn lateral_and_miscellaneous_connections(
        ...
        let pipe_shell_to_air_nodal_conductance: ThermalConductance =
            self.get_air_shell_nodal_shell_conductance(heat_transfer_to_ambient)?;

        self.set_mass_flowrate(mass_flowrate);

        let pipe_shell_surf_to_fluid_conductance: ThermalConductance = self
            .get_fluid_array_node_pipe_shell_conductance(correct_prandtl_for_wall_temperatures)?;
    ...
    #[inline]
    pub fn get_air_shell_nodal_shell_conductance(
        &mut self,
