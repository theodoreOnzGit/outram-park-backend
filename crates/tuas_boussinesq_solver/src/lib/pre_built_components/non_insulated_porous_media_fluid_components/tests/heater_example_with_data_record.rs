...
pub fn example_heater_with_struct_supports_and_mx10() {
    ...
    if calibration_mode {
        let h_to_air = HeatTransfer::new::<watt_per_square_meter_kelvin>(20.0);
        ...
        heater_top_head_bare = HeaterTopBottomHead::_new_user_callibrated_top_head(
            initial_temperature,
            ambient_air_temp,
            h_to_air,
        );
        heater_bottom_head_bare = HeaterTopBottomHead::_new_user_callibrated_bottom_head(
            initial_temperature,
            ambient_air_temp,
            h_to_air,
        );
    ...
    let mut structural_support_heater_top_head = StructuralSupport::new_steel_support_cylinder(
    ...
    let mut structural_support_heater_bottom_head = structural_support_heater_top_head.clone();

    let mut structural_support_mx_10 = structural_support_heater_top_head.clone();

    let approx_support_conductance: ThermalConductance =
        structural_support_heater_top_head.get_axial_node_to_bc_conductance();

    let support_conductance_interaction =
        HeatTransferInteractionType::UserSpecifiedThermalConductance(approx_support_conductance);
    ...
    let mut ambient_air_temp_bc: HeatTransferEntity = inlet_bc.clone();
    ...
    let mut wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
        "lib_heater_example_steady_state.csv",
    ))
    .unwrap();
    wtr.write_record(&[
        "time_seconds",
    ...
    let node_length_heated_section =
        heater_v2_bare.get_component_length() / number_of_nodes_heated_section as f64;
    ...
    for index in 0..number_of_nodes_heated_section {
        ...
        let mid_node_length: Length = index as f64 * node_length_heated_section + half_node_length;
    ...
    let main_loop = thread::spawn(move || {
        ...
        while max_time > simulation_time {
            ...
            // record current fluid temperature profile
            {
                ...
                let current_time_string = simulation_time.get::<second>().to_string();
                ...
                let steel_temperature_vec = heater_v2_bare.pipe_shell_temperature();
            ...
            // outlet temperature profile
            {
                ...
                let mut therminol_fluid_arr: FluidArray = therminol_array_clone;
                ...
                let static_mixer_outlet_temp_string = static_mixer_exit_temperature
                    .get::<degree_celsius>()
                    .to_string();
                ...
                let node_length: Length =
                    heater_v2_bare.get_component_length() / number_of_nodes as f64;
                ...
                let steel_temperature_array = steel_shell_clone.get_temperature_vector().unwrap();
            ...
            let static_mixer_join_handle =
                static_mixer_mx_10_object.lateral_connection_thread_spawn_mx10(mass_flowrate);
            ...
            if support_structures_enabled == true {
                structural_support_heater_bottom_head
                    .support_array
                    .link_to_front(&mut ambient_air_temp_bc, support_conductance_interaction)
                    .unwrap();
            ...
            structural_support_mx_10.lateral_and_miscellaneous_connections();

            structural_support_heater_top_head =
                struct_support_top_head_join_handle.join().unwrap();
            structural_support_heater_bottom_head =
                structural_support_heater_bottom_head_join_handle
                    .join()
                    .unwrap();
            ...
            let structural_support_heater_bottom_head_join_handle =
                structural_support_heater_bottom_head.advance_timestep_thread_spawn(timestep);
            let structural_support_heater_top_head_join_handle =
                structural_support_heater_top_head.advance_timestep_thread_spawn(timestep);

            structural_support_mx_10._advance_timestep(timestep);

            structural_support_heater_bottom_head =
                structural_support_heater_bottom_head_join_handle
                    .join()
                    .unwrap();
            structural_support_heater_top_head = structural_support_heater_top_head_join_handle
                .join()
                .unwrap();
