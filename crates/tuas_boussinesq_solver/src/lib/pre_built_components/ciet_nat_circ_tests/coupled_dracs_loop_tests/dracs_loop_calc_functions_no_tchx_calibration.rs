...
pub fn dracs_loop_tchx_temperature_diagnostics(
    ...
    let (tchx_bulk_outlet_temperature, tchx_wall_outlet_temperature): (
        ...
        let tchx_outlet_wall_temperature = static_mixer_60_label_36
            .pipe_shell
            .try_get_bulk_temperature()
            .unwrap();

        (tchx_outlet_fluid_temperature, tchx_outlet_wall_temperature)
    };

    let (tchx_bulk_inlet_temperature, tchx_wall_inlet_temperature): (
        ThermodynamicTemperature,
        ThermodynamicTemperature,
    ) = {
        ...
        let tchx_inlet_fluid_temperature =
            pipe_34.pipe_fluid_array.try_get_bulk_temperature().unwrap();

        let tchx_inlet_wall_temperature = pipe_34.pipe_shell.try_get_bulk_temperature().unwrap();

        (tchx_inlet_fluid_temperature, tchx_inlet_wall_temperature)
    };
