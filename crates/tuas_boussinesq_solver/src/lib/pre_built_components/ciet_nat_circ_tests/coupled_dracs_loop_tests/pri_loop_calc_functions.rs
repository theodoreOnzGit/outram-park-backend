...
pub fn get_abs_mass_flowrate_across_two_branches(
    dhx_and_heater_branches: &FluidComponentSuperCollection,
) -> MassRate {
    let pressure_change_across_each_branch =
        dhx_and_heater_branches.get_pressure_change(MassRate::ZERO);

    let mass_flowrate_across_each_branch: Vec<MassRate> = dhx_and_heater_branches
...
pub fn pri_loop_dhx_shell_temperature_diagnostics(
    ...
    let bt_21 = pipe_25a
        ...
        .try_get_bulk_temperature()
        .unwrap();
    let wt_20 = pipe_25a.pipe_shell.try_get_bulk_temperature().unwrap();
