...
impl InsulatedFluidComponent {
    ...
    /// to some value
    pub fn calibrate_heat_transfer_to_ambient(&mut self, ambient_htc: HeatTransfer) {
    ...
    pub fn try_calibrate_gnielinski_nusselt(
        &mut self,
        calibration_ratio: Ratio,
    ) -> Result<(), TuasLibError> {
        let mut fluid_arr_clone: FluidArray = self.pipe_fluid_array.clone().try_into().unwrap();
