...
pub fn gfhr_pipe_with_custom_graphite_material() {
    ...
    let mut pipe_1 = InsulatedFluidComponent::new_insulated_pipe(
        initial_temperature,
        ambient_temperature,
        fluid_pressure,
        solid_pressure,
    ...
    let test_pressure = Pressure::new::<megapascal>(0.4);

    let dummy_therminol_density = MassDensity::new::<kilogram_per_cubic_meter>(1.0);
    ...
    while current_simulation_time < simulation_endtime {
        ...
        let test_mass_flowrate =
            pipe_1.get_mass_flowrate_from_pressure_loss_immutable(test_pressure);
        ...
        pipe_1
            .pipe_fluid_array
            .link_to_back(&mut inlet_bc_entity, advection_heat_transfer_interaction)
            .unwrap();

        pipe_1
            .pipe_fluid_array
            .link_to_front(&mut outlet_bc_entity, advection_heat_transfer_interaction)
            .unwrap();
    ...
    let outlet_temperature: ThermodynamicTemperature = *temperature_vector.iter().last().unwrap();

    // now, the outlet temperature is about 134.835C,
    // well not quite, but similar:

    let inlet_temperature: ThermodynamicTemperature = temperature_vector[0];
    // this is also a sensing of how much mass flowrate you get
    let test_mass_flowrate = pipe_1.get_mass_flowrate_from_pressure_loss_immutable(test_pressure);
    dbg!(&(outlet_temperature, inlet_temperature, test_mass_flowrate));
