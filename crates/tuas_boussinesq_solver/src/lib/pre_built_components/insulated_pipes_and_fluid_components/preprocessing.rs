...
impl InsulatedFluidComponent {
    ...
    #[inline]
    pub fn lateral_and_miscellaneous_connections_no_wall_correction(
        &mut self,
    ...
    #[inline]
    pub fn lateral_and_miscellaneous_connections_wall_correction(
        &mut self,
        mass_flowrate: MassRate,
        heater_power: Power,
    ) -> Result<(), TuasLibError> {
    ...
    pub fn lateral_and_miscellaneous_connections(
        ...
        let insulation_to_air_nodal_conductance: ThermalConductance = self
        ...
        let pipe_shell_node_to_fluid_array_conductance: ThermalConductance = self
        ...
        // lateral connections
        {
            ...
            let mut insulation_array_clone: SolidColumn = self.insulation.clone().try_into()?;

            // note, must set mass flowrate first
            // otherwise there is by default zero flow through
            // the array

            fluid_array_clone.set_mass_flowrate(mass_flowrate);
            ...
            pipe_shell_clone.lateral_link_new_power_vector(heater_power, q_frac_arr)?;
    ...
    #[inline]
    pub fn get_ambient_surroundings_to_insulation_nodalised_thermal_conductance(
        &mut self,
        ...
        let mut insulation_clone: SolidColumn = self.insulation.clone().try_into()?;
        ...
        let pipe_air_nodal_thermal_conductance: ThermalConductance =
    ...
    pub fn get_fluid_array_node_to_pipe_shell_conductance(
        ...
        let mass_flowrate: MassRate = fluid_array_clone.get_mass_flowrate();
        ...
        //
        let reynolds_number: Ratio = mass_flowrate / flow_area * hydraulic_diameter / viscosity;
        ...
        let mut fluid_array: FluidArray = self.pipe_fluid_array.clone().try_into()?;
        ...
        if !correct_prandtl_for_wall_temperatures {
            ...
            let mut wall_temperature_estimate = pipe_shell_clone.try_get_bulk_temperature()?;
        ...
        let fluid_array_pipe_shell_conductance_interaction: HeatTransferInteractionType =
            HeatTransferInteractionType::CylindricalConductionConvectionLiquidInside(
                ...
                (h_to_fluid_array, id.into(), node_length.into()),
            );
    ...
    pub fn get_reynolds_based_on_hydraulic_diameter_and_flow_area(
        ...
        let fluid_array_clone: FluidArray = self.pipe_fluid_array.clone().try_into()?;

        let fluid_material: LiquidMaterial =
            fluid_array_clone.material_control_volume.try_into()?;

        let viscosity: DynamicViscosity = fluid_material.try_get_dynamic_viscosity(temperature)?;
    ...
    pub fn get_pipe_shell_to_insulation_nodal_conductance(
        ...
        let pipe_shell_mid_section_diameter = 0.5 * (self.tube_od + self.tube_id);

        let insulation_material_mid_section_diameter =
            0.5 * (self.insulation_id + self.insulation_od);
