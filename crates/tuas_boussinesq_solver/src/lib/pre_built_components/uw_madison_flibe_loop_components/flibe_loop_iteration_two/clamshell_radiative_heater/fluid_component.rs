...
impl ClamshellRadiativeHeater {
    ...
    /// fluid component
    pub fn get_clone_of_annular_air_array(&self) -> FluidComponent {
        // first clone the heat transfer entity
        let annular_air_array_hte_clone: HeatTransferEntity = self.annular_air_array.clone();

        // convert it into a fluid array
        let annular_air_fluid_array: FluidArray = annular_air_array_hte_clone.try_into().unwrap();
    ...
    pub fn get_annular_air_hydraulic_diameter(&self) -> Length {
        ...
        //
        let shell_side_fluid_array: FluidArray = self.annular_air_array.clone().try_into().unwrap();
    ...
    pub fn set_tube_side_total_mass_flowrate(&mut self, mass_flowrate: MassRate) {
    ...
    /// in kg/s
    pub fn set_annular_side_total_mass_flowrate(
        &mut self,
...
impl FluidComponentTrait for ClamshellRadiativeHeater {
    fn get_mass_flowrate(&mut self) -> MassRate {
        let mut pipe_fluid_array: FluidArray = self.pipe_fluid_array.clone().try_into().unwrap();
    ...
    fn get_mass_flowrate_from_pressure_loss_immutable(&self, pressure_loss: Pressure) -> MassRate {
        let pipe_fluid_array: FluidArray = self.pipe_fluid_array.clone().try_into().unwrap();

        pipe_fluid_array.get_mass_flowrate_from_pressure_loss_immutable(pressure_loss)
    }
