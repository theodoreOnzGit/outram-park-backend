...
use crate::heat_transfer_correlations::view_factors::cocentric_cylinders::{
    inner_cylinder_to_annular_end_ring_view_factor, outer_cylinder_to_annular_end_ring_view_factor,
...
use crate::{
    ...
    pre_built_components::heat_transfer_entities::preprocessing::try_get_thermal_conductance_based_on_interaction,
};
...
impl ClamshellRadiativeHeater {
    ...
    #[inline]
    pub fn lateral_and_miscellaneous_connections(
        &mut self,
        ...
        let outer_node_layer_to_air_conductance =
            self.get_ambient_air_to_insulation_nodal_conductance(heat_transfer_to_ambient)?;

        let insulation_to_outer_shell_conductance: ThermalConductance;

        let outer_shell_to_shell_side_fluid_conductance: ThermalConductance = self
            .get_heating_element_to_annular_air_nodal_conductance(
                prandtl_wall_correction_setting,
            )?;

        let single_tube_to_shell_side_fluid_conductance: ThermalConductance = self
            .get_annular_air_inner_tube_shell_nodal_conductance(prandtl_wall_correction_setting)?;
        let single_tube_to_tube_side_fluid_conductance: ThermalConductance = self
            .get_tube_fluid_array_to_pipe_shell_nodal_conductance(
                prandtl_wall_correction_setting,
            )?;
        ...
        // now for the lateral linkages
        {
            ...
            let mut tube_fluid_arr_clone: FluidArray = self.pipe_fluid_array.clone().try_into()?;

            let mut pipe_shell_clone: SolidColumn = self.pipe_shell_array.clone().try_into()?;
            ...
            tube_fluid_arr_clone.lateral_link_new_temperature_vector_avg_conductance(
            ...
            annular_air_arr_clone.lateral_link_new_temperature_vector_avg_conductance(
                single_tube_to_shell_side_fluid_conductance,
                pipe_shell_arr_temp_vec.clone(),
            )?;
            ...
            heating_element_shell_clone.lateral_link_new_temperature_vector_avg_conductance(
                outer_shell_to_shell_side_fluid_conductance,
            ...
            self.heating_element_shell
                .set(heating_element_shell_clone.into())?;

            self.annular_air_array.set(annular_air_arr_clone.into())?;

            self.pipe_fluid_array.set(tube_fluid_arr_clone.into())?;

            self.pipe_shell_array.set(pipe_shell_clone.into())?;
        }
    ...
    fn zero_power_bc_axial_connection(&mut self) -> Result<(), TuasLibError> {
        ...
        let mut zero_power_bc: HeatTransferEntity =
            HeatTransferEntity::BoundaryConditions(BCType::UserSpecifiedHeatAddition(zero_power));
        ...
        self.pipe_fluid_array
            .link_to_front(&mut zero_power_bc, interaction)?;

        self.pipe_fluid_array
            .link_to_back(&mut zero_power_bc, interaction)?;
    ...
    #[inline]
    pub fn get_ambient_air_to_insulation_nodal_conductance(
        &mut self,
        ...
        heated_length = annular_air_fluid_component_clone.get_component_length_immutable();

        // surface temperature is the insulation bulk temperature
        // (estimated)

        insulation_array_clone = self.insulation_array.clone().try_into()?;

        insulation_node_temperature = insulation_array_clone.try_get_bulk_temperature()?;

        // the outer node clone is insulation if it is switched on

        let cylinder_mid_diameter: Length = 0.5 * (insulation_id + insulation_od);

        let node_length = heated_length / number_of_temperature_nodes as f64;
        ...
        let outer_node_air_conductance_interaction: HeatTransferInteractionType =
            HeatTransferInteractionType::CylindricalConductionConvectionLiquidOutside(
                ...
                (h_air_to_pipe_surf, insulation_od.into(), node_length.into()),
            );
    ...
    pub fn get_heating_element_to_insulation_conductance(
        ...
        let mut insulation_array_clone: SolidColumn = self.insulation_array.clone().try_into()?;

        let mut pipe_shell_clone: SolidColumn = self.heating_element_shell.clone().try_into()?;
        ...
        let node_length = array_length / number_of_temperature_nodes as f64;
        ...
        let heating_element_mid_section_diameter =
            0.5 * (self.heating_element_od + self.heating_element_id);

        let insulation_material_mid_section_diameter =
            self.insulation_thickness + self.heating_element_od;
        ...
        let heating_element_material: SolidMaterial =
            pipe_shell_clone.material_control_volume.try_into()?;

        let heating_element_material_conductivity: ThermalConductivity =
            heating_element_material
                .try_get_thermal_conductivity(heating_element_material_temperature)?;

        let insulation_material_shell_temperature =
            insulation_array_clone.try_get_bulk_temperature()?;

        let insulation_material: SolidMaterial =
            insulation_array_clone.material_control_volume.try_into()?;

        let insulation_material_conductivity: ThermalConductivity = insulation_material
            .try_get_thermal_conductivity(insulation_material_shell_temperature)?;
    ...
    pub fn get_heating_element_to_annular_air_nodal_conductance(
        ...
        let mut annular_air_array_clone: FluidArray = self.annular_air_array.clone().try_into()?;
        ...
        let annular_air_mass_flowrate: MassRate = annular_air_array_clone.get_mass_flowrate();
        ...
        let shell_side_fluid_hydraulic_diameter = self.get_annular_air_hydraulic_diameter();
        ...
        //
        let reynolds_number_shell_side: Ratio = annular_air_mass_flowrate
        ...
        let reynolds_number_abs_for_nusselt_estimate: Ratio = reynolds_number_shell_side.abs();
        // next, bulk prandtl number

        let bulk_prandtl_number: Ratio =
            fluid_material.try_get_prandtl_liquid(annular_air_temperature, atmospheric_pressure)?;
        ...
        // (f_darcy L/D + K)
        let pipe_darcy_correlation = &self.annular_air_loss_correlation;

        let fldk: Ratio = pipe_darcy_correlation
            .fldk_based_on_darcy_friction_factor(reynolds_number_abs_for_nusselt_estimate)
            .unwrap();

        let length_to_diameter: Ratio = annular_air_array_clone.get_component_length_immutable()
            / shell_side_fluid_hydraulic_diameter;

        // (f_darcy + D/L  K)
        // then let's scale it by length to diameter
        let modified_darcy_friction_factor: Ratio = fldk / length_to_diameter;
        ...
        if correct_prandtl_for_wall_temperatures {
            ...
            let wall_prandtl_number: Ratio = fluid_material
                .try_get_prandtl_liquid(wall_temperature_estimate, atmospheric_pressure)?;
        ...
        let k_fluid_average: ThermalConductivity =
            fluid_material.try_get_thermal_conductivity(annular_air_temperature)?;
        ...
        let shell_fluid_to_outer_tube_conductance_interaction: HeatTransferInteractionType =
            HeatTransferInteractionType::CylindricalConductionConvectionLiquidInside(
                ...
                (h_to_fluid, id.into(), node_length.into()),
            );

        let shell_fluid_to_outer_tube_surf_nodal_thermal_conductance: ThermalConductance =
            try_get_thermal_conductance_based_on_interaction(
    ...
    pub fn get_annular_air_inner_tube_shell_nodal_conductance(
        ...
        let shell_side_mass_flowrate: MassRate = shell_side_fluid_array_clone.get_mass_flowrate();
        ...
        let fluid_material: LiquidMaterial = shell_side_fluid_array_clone
            .material_control_volume
            .try_into()?;

        let solid_material: SolidMaterial = pipe_shell_clone.material_control_volume.try_into()?;
        ...
        // and live with it for the time being
        //
        let reynolds_number_shell_side: Ratio = shell_side_mass_flowrate
            / shell_side_cross_sectional_flow_area
            * shell_side_fluid_hydraulic_diameter
            / viscosity;
        ...
        let bulk_prandtl_number: Ratio =
            fluid_material.try_get_prandtl_liquid(fluid_temperature, atmospheric_pressure)?;
        ...
        let length_to_diameter: Ratio = shell_side_fluid_array_clone
        ...
        let k_fluid_average: ThermalConductivity =
            fluid_material.try_get_thermal_conductivity(fluid_temperature)?;

        shell_h_to_fluid =
            nusselt_estimate_shell * k_fluid_average / shell_side_fluid_hydraulic_diameter;
        ...
        let shell_fluid_to_inner_tube_surf_conductance_interaction: HeatTransferInteractionType =
            HeatTransferInteractionType::CylindricalConductionConvectionLiquidOutside(
                ...
                (shell_h_to_fluid, od.into(), node_length.into()),
            );
    ...
    pub fn get_tube_fluid_array_to_pipe_shell_nodal_conductance(
        ...
        let pipe_shell_surf_temperature: ThermodynamicTemperature = wall_temperature;

        let single_tube_hydraulic_diameter = self.get_tube_side_hydraulic_diameter_circular_tube();
        let single_tube_flow_area: Area =
            tube_side_single_fluid_array_clone.get_cross_sectional_area_immutable();

        // flow area and hydraulic diameter are ok

        let fluid_material: LiquidMaterial = tube_side_single_fluid_array_clone
            .material_control_volume
            .try_into()?;
        ...
        // and live with it for the time being
        //
        let reynolds_number_single_tube: Ratio = single_tube_mass_flowrate / single_tube_flow_area
            * single_tube_hydraulic_diameter
            / viscosity;
        ...
        let reynolds_number_abs_for_nusselt: Ratio = reynolds_number_single_tube.abs();
        ...
        let mut pipe_prandtl_reynolds_data: GnielinskiData = GnielinskiData::default();
        ...
        pipe_prandtl_reynolds_data.length_to_diameter = tube_side_single_fluid_array_clone
            .get_component_length_immutable()
        ...
        let nusselt_estimate_tube_side = self
        ...
        tube_h_to_fluid =
            nusselt_estimate_tube_side * k_fluid_average / single_tube_hydraulic_diameter;
        ...
        let fluid_pipe_shell_conductance_interaction: HeatTransferInteractionType =
            HeatTransferInteractionType::CylindricalConductionConvectionLiquidInside(
                ...
                (tube_h_to_fluid, id.into(), node_length.into()),
            );
    ...
    pub fn get_nodal_radiative_conductance_between_cylinders(
        ...
        // then i need to get the component length
        let l = annular_air_fluid_component_clone.get_component_length_immutable();

        // area is PI * D * L
        let heating_element_area: Area = PI * heating_element_id * l;
        ...
        let area_factor: Area = area_coefficient_times_view_factor * emissivity * absorptivity;
        ...
        let rht_interaction_between_shells =
            HeatTransferInteractionType::SimpleRadiation(area_factor);

        let atmospheric_pressure = Pressure::new::<atmosphere>(1.0);

        let total_cylinder_conductance: ThermalConductance = rht_interaction_between_shells
        ...
        let nodal_cylinder_conductance = total_cylinder_conductance / number_of_nodes;
    ...
    pub fn get_nodal_radiative_conductance_between_heating_element_and_axial_exit(
        ...
        let total_cylinder_conductance_to_axial_exit: ThermalConductance =
            rht_interaction_between_shells.get_thermal_conductance_based_on_interaction(
                inner_cylinder_temp,
                outer_cylinder_temp,
                atmospheric_pressure,
                atmospheric_pressure,
    ...
    pub fn lateral_connection_thread_spawn(
        &self,
        prandtl_wall_correction_setting: bool,
        ...
        let join_handle = thread::spawn(move || -> Self {
    ...
    ///
    pub fn calibrate_insulation_thickness(&mut self, insulation_thickness: Length) {
