...
pub fn get_abs_mass_flowrate_across_two_branches(
    flibe_loop_branches: &FluidComponentSuperCollection,
) -> MassRate {
    let pressure_change_across_each_branch =
        flibe_loop_branches.get_pressure_change(MassRate::ZERO);

    let mass_flowrate_across_each_branch: Vec<MassRate> = flibe_loop_branches
...
pub fn uw_madison_flibe_fluid_mechanics_calc_abs_mass_rate(
    ...
    let mut flibe_hot_branch = FluidComponentCollection::new_series_component_collection();
    ...
    let mut flibe_cold_branch = FluidComponentCollection::new_series_component_collection();
    ...
    let mut flibe_branches = FluidComponentSuperCollection::default();

    flibe_branches.set_orientation_to_parallel();
    flibe_branches
        .fluid_component_super_vector
        .push(flibe_hot_branch);
    flibe_branches
        .fluid_component_super_vector
        .push(flibe_cold_branch);

    let abs_mass_rate = get_abs_mass_flowrate_across_two_branches(&flibe_branches);
...
pub fn uw_madison_flibe_loop_link_up_components(
    ...
    // set by the controller
    {
        ...
        pipe_1.heat_transfer_to_ambient = ambient_htc;

        // hot leg to cold leg opening to tank (top left)
        pipe_2.heat_transfer_to_ambient = ambient_htc;
        ...
        pipe_5.heat_transfer_to_ambient = cold_leg_diagonal_heat_transfer_coeff;
        // cold leg bend (y joint) top right
        pipe_6.heat_transfer_to_ambient = ambient_htc;
        // cold leg vertical part
        pipe_7.heat_transfer_to_ambient = cold_leg_vertical_heat_transfer_coeff;

        // cold leg bend circular segment
        // approximated as three pipes
        // (bottom right)
        pipe_8.heat_transfer_to_ambient = ambient_htc;
    ...
    // add lateral heat losses, lateral connections for dhx not done here
    {
        ...
        pipe_5.ambient_temperature = ThermodynamicTemperature::new::<degree_celsius>(20.0);
        pipe_7.ambient_temperature = ThermodynamicTemperature::new::<degree_celsius>(20.0);
...
pub fn uw_madison_flibe_loop_advance_timestep(
    ...
    // cold branch
    pipe_2.advance_timestep(timestep).unwrap();

    pipe_3.advance_timestep(timestep).unwrap();
    pipe_4.advance_timestep(timestep).unwrap();

    pipe_5.advance_timestep(timestep).unwrap();
    pipe_6.advance_timestep(timestep).unwrap();
    pipe_7.advance_timestep(timestep).unwrap();

    pipe_8.advance_timestep(timestep).unwrap();
...
pub fn uw_madison_flibe_loop_iteration_1_temperature_diagnostics(
    ...
    let tc_21 = pipe_4.pipe_fluid_array.try_get_bulk_temperature().unwrap();
    let tc_24 = pipe_6.pipe_fluid_array.try_get_bulk_temperature().unwrap();

    // TC32 is somewhere in the middle of the vertical cooling part,
    // I'll just use the bulk temperature as a proxy for that
    let tc_32_estimate = pipe_7.pipe_fluid_array.try_get_bulk_temperature().unwrap();

    // TC35 is just before entering bottom heater section
    let tc_35 = pipe_10.pipe_fluid_array.try_get_bulk_temperature().unwrap();

    // TC11 is just before heater 3
    let tc_11 = pipe_13.pipe_fluid_array.try_get_bulk_temperature().unwrap();

    // TC12 is in the middle of the vertical heater section
    let tc_12_estimate = pipe_1.pipe_fluid_array.try_get_bulk_temperature().unwrap();

    // TC 14 is the temperature just after exiting pipe 1
    // TC 18 is the temperature of the opening to tank (pipe 2)

    let tc_18 = pipe_2.pipe_fluid_array.try_get_bulk_temperature().unwrap();
