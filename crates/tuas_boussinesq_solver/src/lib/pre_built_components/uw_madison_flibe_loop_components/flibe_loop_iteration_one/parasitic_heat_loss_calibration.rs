...
pub fn heater_check_if_four_heater_test1() {
    ...
    let (
        expt_individual_heater_power_watts,
        regression_flibe_heat_addition_watts,
        flibe_velocity_cm_per_s,
...
pub fn parasitic_heat_loss_calibration_dry_run_1() {
    let (tc_11_degc, tc_12_degc, tc_14_degc, tc_21_degc, tc_24_degc, tc_32_degc, tc_35_degc) =
    ...
    let (bottom_cross_insulation_thickness_cm, riser_insulation_thickness_cm) = (0.17, 0.035);
...
pub fn calibrate_uw_madison_parasitic_heat_loss_fixed_flowrate(
    ...
    let top_cross_exit_temp_set_point =
        ThermodynamicTemperature::new::<degree_celsius>(tc_24_degc_expt);

    // vertical cooler (downcomer) exit temp
    let downcomer_exit_temp_set_point =
        ThermodynamicTemperature::new::<degree_celsius>(tc_35_degc_expt);

    let _downcomer_rough_midpoint_temp_expt =
        ThermodynamicTemperature::new::<degree_celsius>(tc_32_degc);

    // horizontal-ish heater (bottom cross) exit temp
    let _bottom_cross_exit_temp_expt =
        ThermodynamicTemperature::new::<degree_celsius>(tc_11_degc_expt);

    // vertical heater (riser) exit temp
    let _riser_exit_temp_expt = ThermodynamicTemperature::new::<degree_celsius>(tc_14_degc_expt);

    let _riser_midpoint_temp_expt = ThermodynamicTemperature::new::<degree_celsius>(tc_12_degc);

    // horizontal-ish cooler (top cross) entrance temp
    let _top_cross_entrance_temp_expt =
        ThermodynamicTemperature::new::<degree_celsius>(tc_21_degc_expt);
    ...
    let riser_insulation_thickness = Length::new::<centimeter>(riser_insulation_thickness_cm);

    // other settings
    let (mass_flowrate_clockwise, hot_leg_diagonal_heater_power, hot_leg_vertical_heater_power) = (
        MassRate::new::<kilogram_per_second>(flibe_mass_flowrate_kg_per_s),
        input_power_per_two_heaters,
        input_power_per_two_heaters,
    );

    let reference_cooler_htc = HeatTransfer::new::<watt_per_square_meter_kelvin>(10.0);
    let average_temperature_for_density_calcs =
        ThermodynamicTemperature::new::<degree_celsius>(0.5 * (tc_21_degc_expt + tc_35_degc_expt));
    ...
    let mut pid_controller_top_cross: AnalogController =
        AnalogController::new_filtered_pid_controller(
            controller_gain,
            integral_time,
            derivative_time,
            alpha,
    ...
    let mut pid_controller_downcomer: AnalogController = pid_controller_top_cross.clone();
    ...
    // hot leg
    let mut pipe_11_bottom_cross_heater =
        new_uw_flibe_pipe_11_bottom_cross_heater(initial_temperature);
    ...
    let (
        (mut tc_21_estimate, mut tc_24_estimate, mut tc_35_estimate),
    ...
    while current_simulation_time < max_simulation_time {
        ...
        let cold_leg_diagonal_heat_transfer_coeff = {
            ...
            let set_point_abs_error_deg_celsius =
                -top_cross_exit_temp_set_point.get::<kelvin>() + tc_24_estimate.get::<kelvin>();
            ...
            let dimensionless_heat_trf_input: Ratio = pid_controller_top_cross
                .set_user_input_and_calc(nondimensional_error, current_simulation_time)
                .unwrap();
            let mut top_cross_heat_trf_output =
                dimensionless_heat_trf_input * reference_cooler_htc + reference_cooler_htc;
        ...
        let cold_leg_vertical_heat_transfer_coeff = {
            ...
            // error = y_sp - y_measured
            let set_point_abs_error_deg_celsius =
                -downcomer_exit_temp_set_point.get::<kelvin>() + tc_35_estimate.get::<kelvin>();
            ...
            let dimensionless_heat_trf_input: Ratio = pid_controller_downcomer
                .set_user_input_and_calc(nondimensional_error, current_simulation_time)
                .unwrap();
        ...
        pipe_1_riser_heater.calibrate_insulation_thickness(riser_insulation_thickness);
        pipe_11_bottom_cross_heater
            .calibrate_insulation_thickness(bottom_cross_insulation_thickness);
        ...
        (
            (tc_21_estimate, tc_24_estimate, tc_35_estimate),
            (tc_11_estimate, tc_14_estimate),
    ...
    dbg!(&(top_cross_heat_transfer_coeff, downcomer_heat_transfer_coeff));
