...
impl InsulatedPorousMediaFluidComponent {
    ...
    #[inline]
    pub fn lateral_and_miscellaneous_connections_mx10(&mut self, mass_flowrate: MassRate) {
        ...
        let mut steel_shell_clone: SolidColumn = self.pipe_shell.clone().try_into().unwrap();
        ...
        let insulation_to_air_conductance: ThermalConductance =
            self.get_air_insulation_shell_conductance_mx10(h_air_to_steel_surf);
    ...
    /// series
    #[inline]
    pub fn get_air_insulation_shell_conductance_mx10(
        &mut self,
        h_air_to_insulation_surf: HeatTransfer,
    ) -> ThermalConductance {
        // first, make a clone of steel

        let mut fiberglass_clone: SolidColumn = self.insulation_array.clone().try_into().unwrap();

        // find parameters for fiberglass conductance

        let number_of_temperature_nodes = self.inner_nodes + 2;

        let fiberglass_shell_temperature = fiberglass_clone.try_get_bulk_temperature().unwrap();

        let fiberglass: SolidMaterial =
            fiberglass_clone.material_control_volume.try_into().unwrap();

        let fiberglass_conductivity: ThermalConductivity = fiberglass
            .try_get_thermal_conductivity(fiberglass_shell_temperature)
            .unwrap();

        let fiberglass_layer_conductance: ThermalConductance = fiberglass_conductivity
        ...
        let air_convection_conductance: ThermalConductance = node_area * h_air_to_insulation_surf;

        let total_resistance =
            1.0 / air_convection_conductance + 1.0 / fiberglass_layer_conductance;
    ...
    #[inline]
    pub fn get_therminol_node_steel_shell_conductance_mx10(&mut self) -> ThermalConductance {
        ...
        let mass_flowrate: MassRate = therminol_fluid_array_clone.get_mass_flowrate();

        let bulk_temperature: ThermodynamicTemperature = therminol_fluid_array_clone
        ...
        let reynolds_number: Ratio =
            self.mx10_hydraulic_diameter_reynolds(mass_flowrate, bulk_temperature);

        // next, bulk prandtl number

        let bulk_prandtl_number: Ratio = LiquidMaterial::TherminolVP1
            .try_get_prandtl_liquid(bulk_temperature, atmospheric_pressure)
            .unwrap();

        // surface prandtl number
        //
        let surface_prandtl_number: Ratio = LiquidMaterial::TherminolVP1
            .try_get_prandtl_liquid(steel_surf_temperature, atmospheric_pressure)
            .unwrap();
        ...
        let mut mx10_prandtl_reynolds_data: GnielinskiData = GnielinskiData::default();
        ...
        mx10_prandtl_reynolds_data.darcy_friction_factor = self
            .darcy_loss_correlation
            .fldk_based_on_darcy_friction_factor(reynolds_number)
            .unwrap();

        mx10_prandtl_reynolds_data.length_to_diameter = length / hydraulic_diameter;

        let mixer_nusselt_correlation: NusseltCorrelation =
            NusseltCorrelation::PipeGnielinskiGeneric(mx10_prandtl_reynolds_data);

        let nusselt_estimate: Ratio = mixer_nusselt_correlation.try_get_nusselt().unwrap();

        // now we can get the heat transfer coeff,

        let h: HeatTransfer;

        let k_fluid_average: ThermalConductivity = LiquidMaterial::TherminolVP1
            .try_get_thermal_conductivity(bulk_temperature)
            .unwrap();
        ...
        let heat_transfer_area_total: Area = length * id * PI;

        let heat_transfer_area_per_node: Area =
            heat_transfer_area_total / number_of_temperature_nodes as f64;

        let node_length = length / number_of_temperature_nodes as f64;
        ...
        let steel_conductivity: ThermalConductivity = SolidMaterial::SteelSS304L
            .try_get_thermal_conductivity(steel_surf_temperature)
            .unwrap();
        ...
        let cylinder_node_resistance = 1.0 / cylinder_node_conductance;
    ...
    #[inline]
    pub fn mx10_hydraulic_diameter_reynolds(
        &mut self,
        ...
        let viscosity: DynamicViscosity = LiquidMaterial::TherminolVP1
            .try_get_dynamic_viscosity(temperature)
            .unwrap();
        ...
        //
        let reynolds: Ratio = mass_flowrate / flow_area * hydraulic_diameter / viscosity;
    ...
    #[inline]
    pub fn get_steel_to_fiberglass_conductance_mx10_nodal(&self) -> ThermalConductance {
        // first, make a clone of steel and fiberglass nodes

        let mut fiberglass_clone: SolidColumn = self.insulation_array.clone().try_into().unwrap();

        let mut steel_clone: SolidColumn = self.pipe_shell.clone().try_into().unwrap();

        let number_of_temperature_nodes = self.inner_nodes + 2;
        // next, thermal conductivities of both steel and fiberglass

        let steel_shell_temperature = steel_clone.try_get_bulk_temperature().unwrap();

        let steel: SolidMaterial = steel_clone.material_control_volume.try_into().unwrap();

        let steel_conductivity: ThermalConductivity = steel
            .try_get_thermal_conductivity(steel_shell_temperature)
            .unwrap();
        ...
        let fiberglass_layer_conductance_nodal: ThermalConductance = fiberglass_conductivity
            * self.thermal_conductance_lengthscale_insulation_to_insulation_pipe_interface
            / number_of_temperature_nodes as f64;

        let steel_layer_conductance_nodal: ThermalConductance = steel_conductivity
    ...
    pub fn lateral_connection_thread_spawn_mx10(
        &self,
        mass_flowrate: MassRate,
    ) -> JoinHandle<Self> {
        ...
        let join_handle = thread::spawn(move || -> Self {
            ...
            component_clone.lateral_and_miscellaneous_connections_mx10(mass_flowrate);
