...
impl InsulatedPorousMediaFluidComponent {
    ...
    pub fn _advance_timestep_parallel_buggy(&mut self, timestep: Time) {
        ...
        let therminol_array_mutex = Arc::new(Mutex::new(self.pipe_fluid_array.clone()));

        let steel_shell_mutex = Arc::new(Mutex::new(self.pipe_shell.clone()));

        let twisted_tape_mutex = Arc::new(Mutex::new(self.insulation_array.clone()));
