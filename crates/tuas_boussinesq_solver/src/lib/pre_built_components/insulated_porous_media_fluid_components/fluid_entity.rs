...
impl FluidComponentTrait for InsulatedPorousMediaFluidComponent {
    fn get_mass_flowrate(&mut self) -> MassRate {
        let mut therminol_array: FluidArray = self.pipe_fluid_array.clone().try_into().unwrap();
    ...
    fn get_mass_flowrate_from_pressure_loss_immutable(&self, pressure_loss: Pressure) -> MassRate {
        let therminol_array: FluidArray = self.pipe_fluid_array.clone().try_into().unwrap();

        therminol_array.get_mass_flowrate_from_pressure_loss_immutable(pressure_loss)
    }
