...
impl HeaterTopBottomHead {
    ...
    pub fn _advance_timestep(&mut self, timestep: Time) {
    ...
    pub fn _advance_timestep_parallel_buggy(&mut self, timestep: Time) {
        // advances timestep in parallel,
        // but must clone first
        let therminol_array_mutex = Arc::new(Mutex::new(self.therminol_array.clone()));

        let steel_shell_mutex = Arc::new(Mutex::new(self.steel_shell.clone()));

        let twisted_tape_mutex = Arc::new(Mutex::new(self.twisted_tape_interior.clone()));
        ...
        let handle_1: JoinHandle<()> = thread::spawn(move || {
            let mut therminol_arr_mutex_guard = therminol_array_mutex_clone.lock().unwrap();

            // now advance timestep

            therminol_arr_mutex_guard
                .deref_mut()
                .advance_timestep_mut_self(timestep)
                .unwrap();
        });
        let handle_2: JoinHandle<()> = thread::spawn(move || {
            let mut steel_shell_arr_mutex_guard = steel_shell_mutex_clone.lock().unwrap();

            // now advance timestep

            steel_shell_arr_mutex_guard
                .deref_mut()
                .advance_timestep_mut_self(timestep)
                .unwrap();
        });
        let handle_3: JoinHandle<()> = thread::spawn(move || {
            let mut twisted_tape_arr_mutex_guard = twisted_tape_mutex_clone.lock().unwrap();

            // now advance timestep

            twisted_tape_arr_mutex_guard
                .deref_mut()
                .advance_timestep_mut_self(timestep)
                .unwrap();
        });
        ...
        let _ = handle_vec.into_iter().map(|handle| handle.join().unwrap());
        ...
        self.therminol_array
            .set(therminol_array_mutex.lock().as_deref_mut().unwrap().clone())
            .unwrap();

        self.steel_shell
            .set(steel_shell_mutex.lock().as_deref_mut().unwrap().clone())
            .unwrap();

        self.twisted_tape_interior
            .set(twisted_tape_mutex.lock().as_deref_mut().unwrap().clone())
            .unwrap();
