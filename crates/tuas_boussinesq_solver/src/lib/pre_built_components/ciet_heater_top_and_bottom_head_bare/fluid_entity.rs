...
impl FluidComponentTrait for HeaterTopBottomHead {
    fn get_mass_flowrate(&mut self) -> MassRate {
        let mut therminol_array: FluidArray = self.therminol_array.clone().try_into().unwrap();
    ...
    fn get_mass_flowrate_from_pressure_loss_immutable(&self, pressure_loss: Pressure) -> MassRate {
        let therminol_array: FluidArray = self.therminol_array.clone().try_into().unwrap();
