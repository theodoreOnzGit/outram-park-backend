...
impl HeaterTopBottomHead {
    ...
    #[inline]
    pub fn lateral_and_miscellaneous_connections(&mut self, mass_flowrate: MassRate) {
        ...
        let steel_to_air_conductance: ThermalConductance =
            self.get_air_steel_shell_conductance(self.heat_transfer_to_air);
        ...
        // lateral connections
        {
            ...
            let mut steel_shell_clone: SolidColumn = self.steel_shell.clone().try_into().unwrap();
    ...
    #[inline]
    pub fn get_air_steel_shell_conductance(
        &mut self,
        ...
        let node_area: Area =
            PI * hydraulic_diameter * component_length / number_of_temperature_nodes as f64;

        let air_to_surface_node_conductance: ThermalConductance = h_air_to_steel_surf * node_area;
        ...
        let node_length = component_length / number_of_temperature_nodes as f64;
        ...
        let air_to_steel_resistance =
            1.0 / cylinder_node_conductance + 1.0 / air_to_surface_node_conductance;
    ...
    #[inline]
    pub fn get_therminol_node_steel_shell_conductance(&mut self) -> ThermalConductance {
        ...
        let reynolds_number: Ratio =
            self.heater_v2_hydraulic_diameter_reynolds(mass_flowrate, bulk_temperature);
        ...
        let nusselt_estimate: Ratio = heater_nusselt_correlation.try_get_nusselt().unwrap();
        ...
        let heat_transfer_area_heated_length_plus_heads: Area = heated_length_plus_heads * id * PI;

        let heat_transfer_area_heated_length_only: Area =
            heated_length / heated_length_plus_heads * heat_transfer_area_heated_length_plus_heads;

        let heat_transfer_area_per_node: Area =
            heat_transfer_area_heated_length_only / number_of_temperature_nodes as f64;
    ...
    #[inline]
    pub fn heater_v2_hydraulic_diameter_reynolds(
        &self,
    ...
    #[inline]
    pub fn get_therminol_node_twisted_tape_conductance(&self) -> ThermalConductance {
        ...
        let component_length = therminol_fluid_array_clone.get_component_length();
        ...
        let fluid_average_temperature: ThermodynamicTemperature = therminol_fluid_array_clone
            .try_get_bulk_temperature()
            .unwrap();

        let fluid_average_pressure: Pressure = therminol_fluid_array_clone.pressure_control_volume;

        let fluid_material: LiquidMaterial = therminol_fluid_array_clone
            .material_control_volume
            .try_into()
            .unwrap();

        let fluid_prandtl: Ratio = fluid_material
            .try_get_prandtl_liquid(fluid_average_temperature, fluid_average_pressure)
            .unwrap();

        let twisted_tape_temperature: ThermodynamicTemperature =
            twisted_tape_array_clone.try_get_bulk_temperature().unwrap();

        let twisted_tape_wall_prandtl: Ratio = fluid_material
            .try_get_prandtl_liquid(twisted_tape_temperature, fluid_average_pressure)
            .unwrap();

        // with Pr and Re, get nusselt estimate
        //

        let nusselt_estimate: Ratio = therminol_fluid_array_clone
            .get_nusselt(reynolds, fluid_prandtl, twisted_tape_wall_prandtl)
            .unwrap();
        ...
        let k_fluid_average: ThermalConductivity = fluid_material
            .try_get_thermal_conductivity(fluid_average_temperature)
            .unwrap();
        ...
        let average_node_conductance: ThermalConductance = h * heat_transfer_area_per_node;
    ...
    pub fn lateral_connection_thread_spawn(&self, mass_flowrate: MassRate) -> JoinHandle<Self> {
        // make an Arc Ptr
        ...
        let join_handle = thread::spawn(move || -> Self {
            ...
            heater_peripherals_clone.lateral_and_miscellaneous_connections(mass_flowrate);
