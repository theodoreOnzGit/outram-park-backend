...
impl FluidComponentTrait for NonInsulatedParallelFluidComponent {
    ...
    fn get_mass_flowrate(&mut self) -> MassRate {
        ...
        let single_tube_mass_rate = pipe_fluid_array.get_mass_flowrate();
    ...
    fn set_mass_flowrate(&mut self, mass_flowrate: MassRate) {
        ...
        let single_tube_mass_rate = mass_flowrate / (self.number_of_tubes as f64);
    ...
    fn get_pressure_loss_immutable(&self, mass_flowrate: MassRate) -> Pressure {
        ...
        let single_pipe_mass_flowrate: MassRate = mass_flowrate / (self.number_of_tubes as f64);

        pipe_fluid_array.get_pressure_loss_immutable(single_pipe_mass_flowrate)
    }

    fn get_cross_sectional_area(&mut self) -> Area {
        let pipe_fluid_array: FluidArray = self.pipe_fluid_array.clone().try_into().unwrap();

        let single_pipe_xs_area = pipe_fluid_array.get_cross_sectional_area_immutable();
