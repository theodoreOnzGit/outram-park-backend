...
impl NonInsulatedParallelFluidComponent {
    ...
    #[inline]
    fn _advance_timestep_single_tube(&mut self, timestep: Time) -> Result<(), TuasLibError> {
    ...
    #[inline]
    fn advance_timestep_for_parallel_fluid_array_bundle(
        &mut self,
        timestep: Time,
    ) -> Result<(), TuasLibError> {
        ...
        let mut fluid_array_clone: FluidArray = self.pipe_fluid_array.clone().try_into()?;

        // for mass flowrate, getting the mass flow property from the
        // fluid array clone is the single tube mass flowrate
        let mass_flowrate_for_single_tube = fluid_array_clone.get_mass_flowrate();
        ...
        let back_cv_rate_enthalpy_change_vector: Vec<Power> = fluid_array_clone
            .back_single_cv
            .rate_enthalpy_change_vector
            .clone();

        let front_cv_rate_enthalpy_change_vector: Vec<Power> = fluid_array_clone
            .front_single_cv
            .rate_enthalpy_change_vector
            .clone();
        ...
        let total_volume =
            fluid_array_clone.get_component_length() * fluid_array_clone.get_cross_sectional_area();
        let dt = timestep;
        let node_length = fluid_array_clone.get_component_length() / number_of_nodes as f64;
        ...
        // timestep
        let volume_fraction_array: Array1<f64> = fluid_array_clone
            .volume_fraction_array
            .iter()
            .map(|&vol_frac| vol_frac)
            .collect();
        // rho_cp is determined by the temperature array
        let rho_cp: Array1<VolumetricHeatCapacity> = fluid_array_clone
        ...
        let lateral_temperature_arary_connected: bool = fluid_array_clone
        ...
        if lateral_temperature_arary_connected {
            ...
            for conductance_array in fluid_array_clone
                .lateral_adjacent_array_conductance_vector
                .iter()
            {
            ...
            for (lateral_idx, conductance_arr) in fluid_array_clone
                .lateral_adjacent_array_conductance_vector
                .iter()
                .enumerate()
            {
                ...
                let temperature_arr: Array1<ThermodynamicTemperature> = fluid_array_clone
        ...
        let lateral_power_sources_connected: bool = fluid_array_clone.q_vector.len() > 0;
        ...
        // back node calculation (first node)
        {
            ...
            let h_fluid_last_timestep: AvailableEnergy = fluid_array_clone
                .back_single_cv
        ...
        let reynolds: Ratio = fluid_array_clone
        ...
        if low_peclet_number_flow {
            ...
            average_axial_conductance =
                average_fluid_conductivity * fluid_array_clone.xs_area / node_length;
    ...
    fn advance_timestep_for_parallel_solid_column_bundle(
        ...
        let mut pipe_shell_clone: SolidColumn = self.pipe_shell.clone().try_into()?;
        ...
        let back_cv_rate_enthalpy_change_vector: Vec<Power> = pipe_shell_clone
            .back_single_cv
            .rate_enthalpy_change_vector
            .clone();

        let front_cv_rate_enthalpy_change_vector: Vec<Power> = pipe_shell_clone
            .front_single_cv
            .rate_enthalpy_change_vector
            .clone();
        ...
        let single_shell_volume =
            pipe_shell_clone.total_length * pipe_shell_clone.get_component_xs_area();
        ...
        // enthalpy changes using rho_cp calculated at the current
        // timestep
        let volume_fraction_array: Array1<f64> = pipe_shell_clone
            .volume_fraction_array
            .iter()
            .map(|&vol_frac| vol_frac)
            .collect();
        // rho_cp is determined by the temperature array
        let rho_cp: Array1<VolumetricHeatCapacity> = pipe_shell_clone
            .temperature_array_current_timestep
            .iter()
            .map(|&temperature| try_get_rho_cp(material, temperature, pressure).unwrap())
            .collect();
        ...
        // neglected
        let lateral_power_sources_connected: bool = pipe_shell_clone.q_vector.len() > 0;
        let lateral_temperature_arary_connected: bool = pipe_shell_clone
            .lateral_adjacent_array_conductance_vector
            .len()
            > 0;
        ...
        if lateral_temperature_arary_connected {
            ...
            for conductance_array in pipe_shell_clone
                .lateral_adjacent_array_conductance_vector
                .iter()
            {
                // now I'm inside the each conductance array,
            ...
            for (lateral_idx, conductance_arr) in pipe_shell_clone
                .lateral_adjacent_array_conductance_vector
                .iter()
                .enumerate()
            {
        ...
        let average_axial_conductance: ThermalConductance =
            average_thermal_conductivity * pipe_shell_clone.get_component_xs_area() / node_length;
