...
impl NonInsulatedParallelFluidComponent {
    ...
    pub fn lateral_and_miscellaneous_connections(
        ...
        let single_pipe_shell_to_air_nodal_conductance: ThermalConductance =
            self.get_air_to_single_shell_nodal_shell_conductance(heat_transfer_to_ambient)?;

        self.set_mass_flowrate(mass_flowrate_over_all_tubes);

        let single_pipe_shell_surf_to_fluid_conductance: ThermalConductance = self
            .get_single_tube_fluid_array_node_pipe_shell_conductance(
                correct_prandtl_for_wall_temperatures,
            )?;
        ...
        // lateral connections
        {
            ...
            let heater_power_over_single_tube =
                heater_power_over_all_tubes / (self.number_of_tubes as f64);

            pipe_shell_clone
                .lateral_link_new_power_vector(heater_power_over_single_tube, q_frac_arr)?;
    ...
    #[inline]
    pub fn get_air_to_single_shell_nodal_shell_conductance(
        &mut self,
    ...
    #[inline]
    pub fn get_single_tube_fluid_array_node_pipe_shell_conductance(
        &mut self,
        correct_prandtl_for_wall_temperatures: bool,
    ) -> Result<ThermalConductance, TuasLibError> {
        ...
        let single_tube_mass_flowrate: MassRate = fluid_array_clone.get_mass_flowrate();
        ...
        let single_tube_hydraulic_diameter = self.get_hydraulic_diameter();
        let bundled_tubes_flow_area: Area = self.get_cross_sectional_area_immutable();

        let single_tube_flow_area: Area = bundled_tubes_flow_area / (self.number_of_tubes as f64);
