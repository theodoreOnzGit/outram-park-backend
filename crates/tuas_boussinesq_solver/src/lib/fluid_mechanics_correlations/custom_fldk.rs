...
pub fn custom_f_ldk(
    custom_darcy: &dyn Fn(f64, f64) -> Result<f64, TuasLibError>,
    reynolds_number: f64,
    roughness_ratio: f64,
...
pub fn custom_kpipe(
    ...
    let darcy_fn = crate::fluid_mechanics_correlations::darcy;

    let f_ldk_result = custom_f_ldk(
        &darcy_fn,
        reynolds_number,
        roughness_ratio,
...
pub fn get_reynolds(
    ...
    custom_k: &'static dyn Fn(f64) -> Result<f64, TuasLibError>,
) -> Result<f64, TuasLibError> {
...
impl RootFindingProblem<1, 1, (f64, f64)> for ReynoldsFromBejanDCustom {
    ...
    fn initial_guess(&self) -> (f64, f64) {
        let upper_bound = self.max_reynolds;
        let lower_bound = self.min_reynolds;
