...
#[inline]
pub fn try_get_prandtl(
    material: Material,
    temperature: ThermodynamicTemperature,
    pressure: Pressure,
) -> Result<Ratio, TuasLibError> {
    // get mu
    let mu: DynamicViscosity = try_get_mu_viscosity(material, temperature, pressure)?;

    // get cp
    let cp: SpecificHeatCapacity = try_get_cp(material, temperature, pressure)?;
