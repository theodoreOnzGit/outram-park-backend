...
pub fn fiberglass_specific_heat_capacity(
    ...
    return SpecificHeatCapacity::new::<joule_per_kilogram_kelvin>(844.0);
}
...
pub fn fiberglass_thermal_conductivity_zou_zweibaum_spline(
    ...
    let thermal_cond_temperature_values_kelvin = c!(250.0, 293.15, 350.0, 400.0, 500.0, 600.0);
    let thermal_conductivity_values_watt_per_meter_kelvin =
        c!(0.028616, 0.033060, 0.038916, 0.044066, 0.054366, 0.064666);
    ...
    let fiberglass_thermal_conductivity_value = s.unwrap().eval(temperature_value_kelvin);
...
pub fn fiberglass_specific_enthalpy(temperature: ThermodynamicTemperature) -> AvailableEnergy {
    let specific_enthalpy_value_j_per_kg = 844.0 * temperature.get::<degree_celsius>();

    return AvailableEnergy::new::<joule_per_kilogram>(specific_enthalpy_value_j_per_kg);
}
#[test]
fn fiberglass_enthalpy_test() {
    let fiberglass_temp = ThermodynamicTemperature::new::<kelvin>(373.0);

    let fiberglass_reference_enthalpy_value = 844.0 * (fiberglass_temp.get::<kelvin>() - 273.15);

    let fiberglass_enthalpy = fiberglass_specific_enthalpy(fiberglass_temp);
...
pub(crate) fn fiberglass_spline_temp_attempt_1_from_specific_enthalpy(
    ...
    for index_i in 0..temperature_vec_len {
        ...
        let fiberglass_temp = ThermodynamicTemperature::new::<kelvin>(temperature_value);
        let pressure = Pressure::new::<atmosphere>(1.0);

        let fiberglass_enthalpy_result = try_get_h(fiberglass, fiberglass_temp, pressure);
    ...
    let temperature_from_enthalpy_kelvin = enthalpy_to_temperature_spline
        .unwrap()
        .eval(h_fiberglass_joules_per_kg);
...
pub fn fiberglass_temperature_from_enthalpy_test_spline_1() {
    ...
    let enthalpy_spline_zweibaum_375k = try_get_h(fiberglass, fiberglass_temp, pressure).unwrap();
