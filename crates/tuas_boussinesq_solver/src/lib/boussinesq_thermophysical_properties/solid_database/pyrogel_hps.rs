...
pub fn pryogel_hps_specific_heat_capacity_rough_estimate(
    ...
    return SpecificHeatCapacity::new::<joule_per_kilogram_kelvin>(1700.0);
}
...
pub fn pyrogel_hps_specific_enthalpy(temperature: ThermodynamicTemperature) -> AvailableEnergy {
    let specific_enthalpy_value_j_per_kg = 1700.0 * temperature.get::<degree_celsius>();

    return AvailableEnergy::new::<joule_per_kilogram>(specific_enthalpy_value_j_per_kg);
}
...
pub fn pryogel_hps_specific_heat_capacity_spline_low_temp(
    ...
    let pyrogel_cp_temperature_values_degc = c!(9.883, 24.019, 39.819);
    let pyrogel_cp_values_joule_per_kg_kelvin = c!(883.261, 898.311, 983.814);
    let s = CubicSpline::from_nodes(
        &pyrogel_cp_temperature_values_degc,
        &pyrogel_cp_values_joule_per_kg_kelvin,
    );

    let temperature_value_degc: f64 = temperature.get::<degree_celsius>();
    let pyrogel_generic_cp_joule_per_kg_kelvin = s.unwrap().eval(temperature_value_degc);
...
///
#[inline]
pub fn ground_pyrogel_hps_dsc_spline_data(
    temperature: ThermodynamicTemperature,
    ...
    let specific_power_temperature_values_degc = c!(
        35.426, 42.085, 50.609, 59.132, 74.581, 88.432, 99.619, 123.592, 136.91, 151.294, 166.21,
        178.463, 190.183, 200.837, 215.221, 225.875, 234.399, 239.726, 243.455, 248.25, 251.446,
        253.577, 257.839, 259.97, 262.633, 265.83, 267.428, 271.157, 279.148, 304.186, 326.294
    );
    let specific_power_values_milliwatts_per_milligram = c!(
        0.261, 0.274, 0.285, 0.292, 0.293, 0.291, 0.287, 0.28, 0.278, 0.277, 0.282, 0.285, 0.285,
        0.285, 0.291, 0.302, 0.32, 0.338, 0.351, 0.365, 0.373, 0.374, 0.369, 0.361, 0.345, 0.322,
        0.303, 0.296, 0.29, 0.286, 0.283
    );

    let s = CubicSpline::from_nodes(
        &specific_power_temperature_values_degc,
        &specific_power_values_milliwatts_per_milligram,
    );

    let temperature_value_degc: f64 = temperature.get::<degree_celsius>();
    let pyrogel_generic_dsc_milliwatt_per_milligram = s.unwrap().eval(temperature_value_degc);
...
pub fn pyrogel_thermal_conductivity_commercial_factsheet_spline(
    ...
    let thermal_cond_temperature_values_degc =
        c!(0.0, 100.0, 200.0, 300.0, 400.0, 500.0, 600.0, 650.0);
    let thermal_conductivity_values_milliwatt_per_meter_kelvin =
        c!(20.0, 24.0, 28.0, 33.0, 40.0, 49.0, 62.0, 69.0);

    let s = CubicSpline::from_nodes(
        &thermal_cond_temperature_values_degc,
        &thermal_conductivity_values_milliwatt_per_meter_kelvin,
    );

    let pyrogel_hps_thermal_conductivity_value_milliwatt_per_meter_kelvin =
...
pub(crate) fn pyrogel_hps_spline_temp_attempt_1_from_specific_enthalpy(
    ...
    for index_i in 0..temperature_vec_len {
        ...
        let pyrogel_hps_temp = ThermodynamicTemperature::new::<kelvin>(temperature_value);
        let pressure = Pressure::new::<atmosphere>(1.0);

        let pyrogel_enthalpy_result = try_get_h(pyrogel_hps, pyrogel_hps_temp, pressure);
    ...
    let enthalpy_root = |temp_degrees_c_value: f64| -> f64 {
        ...
        let fiberglass_temp = ThermodynamicTemperature::new::<kelvin>(temp_degrees_c_value);
        let pressure = Pressure::new::<atmosphere>(1.0);

        let rhs = try_get_h(fiberglass, fiberglass_temp, pressure);
