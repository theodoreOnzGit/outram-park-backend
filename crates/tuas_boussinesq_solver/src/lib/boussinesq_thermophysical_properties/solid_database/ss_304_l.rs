...
pub fn steel_304_l_spline_specific_heat_capacity_ciet_zweibaum(
    ...
    let thermal_cond_temperature_values_kelvin =
        c!(250.0, 300.0, 350.0, 400.0, 450.0, 500.0, 700.0, 1000.0);
    let specific_heat_capacity_values_joule_per_kilogram_kelvin =
        c!(443.3375, 457.0361, 469.4894, 480.6974, 490.66, 500.6227, 526.7746, 551.6812);

    let s = CubicSpline::from_nodes(
        &thermal_cond_temperature_values_kelvin,
        &specific_heat_capacity_values_joule_per_kilogram_kelvin,
    );

    let steel_specific_heat_capacity_value = s.unwrap().eval(temperature_value_kelvin);
...
pub fn steel_304_l_libreoffice_spline_specific_heat_capacity_ciet_zweibaum(
    ...
    let steel_specific_heat_capacity_value = 3.494005840e2
...
pub fn steel_ss_304_l_ornl_specific_heat_capacity(
    ...
    let temperature_value_kelvin: f64 = temperature.get::<kelvin>();
    let specific_heat_capacity_val = 1000.0
        * (0.4267 + 1.700 * f64::powf(10.0, -4.0) * temperature_value_kelvin
...
pub fn specific_heat_capacity_test_steel() {
    let thermal_cond_spline =
        steel_304_l_spline_specific_heat_capacity_ciet_zweibaum(ThermodynamicTemperature::new::<
            kelvin,
        >(350.0));
    ...
    let specific_heat_graves_et_al_1991 =
        steel_ss_304_l_ornl_specific_heat_capacity(ThermodynamicTemperature::new::<kelvin>(350.0));
...
pub fn steel_ss_304_l_ornl_thermal_conductivity(
    ...
    let temperature_value_kelvin: f64 = temperature.get::<kelvin>();
    let thermal_conductivity_val =
        7.9318 + 0.023051 * temperature_value_kelvin - 6.4166 * f64::powf(10.0, -6.0);
...
pub fn steel_304_l_spline_thermal_conductivity(
    ...
    let thermal_cond_temperature_values_kelvin =
        c!(250.0, 300.0, 350.0, 400.0, 450.0, 500.0, 700.0, 1000.0);
    let thermal_conductivity_values_watt_per_meter_kelin =
        c!(14.31, 14.94, 15.58, 16.21, 16.85, 17.48, 20.02, 23.83);
    ...
    let steel_thermal_conductivity_value = s.unwrap().eval(temperature_value_kelvin);
...
pub fn steel_304_l_libreoffice_spline_thermal_conductivity_zweibaum(
    ...
    let steel_thermal_conductivity_value = 1.113e1 + 1.269e-2 * temperature_value_kelvin;
...
pub fn density_test_steel() {
    ...
    approx::assert_relative_eq!(8030_f64, density.unwrap().value, max_relative = 0.01);
}
...
pub fn steel_304_l_spline_specific_enthalpy_ciet_zweibaum(
    ...
    // here we use a cubic spline to interpolate the values
    // it's a little calculation heavy, but don't really care now
    let specific_enthalpy_temperature_values_kelvin =
        c!(250.0, 300.0, 350.0, 400.0, 450.0, 500.0, 700.0, 1000.0);
    let specific_heat_capacity_values_joule_per_kilogram_kelvin =
        c!(443.3375, 457.0361, 469.4894, 480.6974, 490.66, 500.6227, 526.7746, 551.6812);

    let s = CubicSpline::from_nodes(
        &specific_enthalpy_temperature_values_kelvin,
        &specific_heat_capacity_values_joule_per_kilogram_kelvin,
    );

    let steel_specific_enthalpy_value = s.unwrap().integrate((273.15, temperature_value_kelvin));

    return AvailableEnergy::new::<joule_per_kilogram>(steel_specific_enthalpy_value);
}
...
pub fn steel_ss_304_l_ornl_specific_enthalpy_graves(
    ...
    fn definite_integral_specific_enthalpy(
        ...
        temp_2: ThermodynamicTemperature,
    ) -> AvailableEnergy {
        ...
        let enthalpy_value_joule_per_gram_per_kelvin_temp_1 = 1.73333e-8
            * f64::powf(temp_1_value_kelvin, 3.0)
            + 0.000085 * f64::powf(temp_1_value_kelvin, 2.0)
            + 0.4267 * temp_1_value_kelvin;

        let enthalpy_value_joule_per_gram_per_kelvin_temp_2 = 1.73333e-8
            * f64::powf(temp_2_value_kelvin, 3.0)
            + 0.000085 * f64::powf(temp_2_value_kelvin, 2.0)
            + 0.4267 * temp_2_value_kelvin;

        let enthalpy_difference_joule_per_gram = enthalpy_value_joule_per_gram_per_kelvin_temp_2
            - enthalpy_value_joule_per_gram_per_kelvin_temp_1;

        AvailableEnergy::new::<joule_per_gram>(enthalpy_difference_joule_per_gram)
    }

    // reference temperature is zero degrees c,
    // enthalpy is zero j/kg at that point
    let refernce_temperature = ThermodynamicTemperature::new::<kelvin>(273.15);

    let steel_enthalpy = definite_integral_specific_enthalpy(refernce_temperature, temperature);
...
pub fn specific_enthalpy_test_steel_ornl() {
    ...
    let test_temperature = ThermodynamicTemperature::new::<kelvin>(350.0);
    ...
    let enthalpy_analytical_ornl = steel_ss_304_l_ornl_specific_enthalpy_graves(test_temperature);
...
pub fn specific_enthalpy_test_steel_ornl_and_zweibaum_spline() {
    ...
    let test_temperature_1 = ThermodynamicTemperature::new::<kelvin>(375.0);
    let test_temperature_2 = ThermodynamicTemperature::new::<kelvin>(425.0);
    ...
    let enthalpy_analytical_ornl = steel_ss_304_l_ornl_specific_enthalpy_graves(test_temperature_2)
...
pub(crate) fn steel_304_l_spline_temp_attempt_3_from_specific_enthalpy_ciet_zweibaum(
    ...
    for index_i in 0..temperature_vec_len {
        ...
        let steel = Material::Solid(SolidMaterial::SteelSS304L);
        let steel_temp = ThermodynamicTemperature::new::<kelvin>(temperature_value);
        let pressure = Pressure::new::<atmosphere>(1.0);

        let steel_enthalpy_result = try_get_h(steel, steel_temp, pressure);
    ...
    let enthalpy_root = |temp_degrees_c_value: f64| -> f64 {
        ...
        let steel = Material::Solid(SolidMaterial::SteelSS304L);
        let steel_temp = ThermodynamicTemperature::new::<kelvin>(temp_degrees_c_value);
        let pressure = Pressure::new::<atmosphere>(1.0);

        let rhs = try_get_h(steel, steel_temp, pressure);
    ...
    let temperature_from_enthalpy_kelvin: f64 = match fluid_temperature_degrees_c_result {
...
pub fn steel_temperature_from_enthalpy_test_spline_3() {
    ...
    let enthalpy_spline_zweibaum_375k = try_get_h(steel, steel_temp, atmospheric_pressure).unwrap();
    ...
    let temperature_vec_kelvin: Vec<f64> = vec![325.0, 425.0, 525.0, 625.0, 725.0, 825.0, 925.0];

    for temperature_val_kelvin in temperature_vec_kelvin.iter() {
        let steel_temp = ThermodynamicTemperature::new::<kelvin>(*temperature_val_kelvin);

        let enthalpy_spline_zweibaum = try_get_h(steel, steel_temp, atmospheric_pressure).unwrap();
