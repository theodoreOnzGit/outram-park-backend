...
pub fn get_custom_solid_density(
    ...
    // panic otherwise
    range_check_custom_solid(solid_temp, upper_bound_temperature, lower_bound_temperature)?;
...
impl CustomSolidTemperatureFromEnthalpy {
    ...
    fn enthalpy_root(&self, temp_degrees_c: Pt<1>) -> Result<Pt<1>> {
        ...
        let enthalpy_value_joule_per_kg = self.solid_enthalpy.get::<joule_per_kilogram>();
        ...
        let rhs = get_custom_solid_enthalpy(
            fluid_temperature,
            self.cp_function,
...
///
pub fn range_check_custom_solid(
    solid_temp: ThermodynamicTemperature,
    upper_bound_temperature: ThermodynamicTemperature,
    lower_bound_temperature: ThermodynamicTemperature,
    ...
    // celsius
    let temp_value_celsius = solid_temp.get::<degree_celsius>();
    ...
    // As with the custom-fluid checker, only the bounds are in scope here, not
    // the `SolidMaterial` itself, so the payload names the material class.
