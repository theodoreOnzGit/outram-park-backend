...
#[inline]
pub fn try_get_nu_momentum_diffusivity(
    material: Material,
    ...
    let material_dynamic_viscosity: DynamicViscosity =
        dynamic_viscosity::try_get_mu_viscosity(material, temperature, pressure)?;

    let nu: DiffusionCoefficient = material_dynamic_viscosity / material_density;
...
impl LiquidMaterial {
    ...
    pub fn try_get_nu_momentum_diffusivity(
        ...
        try_get_nu_momentum_diffusivity(self.clone().into(), fluid_temp, pressure)
    }
}
...
pub fn check_that_prandtl_is_nu_divide_by_alpha() {
    ...
    let prandtl = hitec.try_get_prandtl_liquid(temperature, pressure).unwrap();
