...
pub fn get_custom_fluid_density(
    ...
    // panic otherwise
    range_check_custom_fluid(fluid_temp, upper_bound_temperature, lower_bound_temperature)?;
...
pub fn get_custom_fluid_enthalpy(
    ...
    let specific_enthalpy_joule_per_kilogram = integrate(
        // cp function float
        cp_fn_float,
    ...
    let enthalpy = AvailableEnergy::new::<joule_per_kilogram>(specific_enthalpy_joule_per_kilogram);
...
pub fn test_custom_fluid_enthalpy() {
    ...
    let test_temperature_550_k = ThermodynamicTemperature::new::<kelvin>(550.0);
    ...
    let lower_bound_temperature = ThermodynamicTemperature::new::<kelvin>(440.0);
    let upper_bound_temperature = ThermodynamicTemperature::new::<kelvin>(800.0);
    ...
    let test_enthalpy = get_custom_fluid_enthalpy(
...
pub fn get_custom_fluid_temperature_from_enthalpy(
    ...
    return Ok(ThermodynamicTemperature::new::<degree_celsius>(
        fluid_temperature_degrees_c,
    ));
}
...
impl CustomFluidTemperatureFromEnthalpy {
    ...
    fn enthalpy_root(&self, temp_degrees_c: Pt<1>) -> Result<Pt<1>> {
        ...
        let enthalpy_value_joule_per_kg = self.fluid_enthalpy.get::<joule_per_kilogram>();
        let lhs_value = enthalpy_value_joule_per_kg;
        let temp_degrees_c_value_double: f64 = temp_degrees_c[0];

        let fluid_temperature =
            ThermodynamicTemperature::new::<degree_celsius>(temp_degrees_c_value_double);
        let rhs = get_custom_fluid_enthalpy(
            fluid_temperature,
            self.cp_function,
            self.upper_bound_temperature,
            self.lower_bound_temperature,
        )
        .unwrap();
...
impl RootFindingProblem<1, 1, (f64, f64)> for CustomFluidTemperatureFromEnthalpy {
    ...
    fn initial_guess(&self) -> (f64, f64) {
        ...
        let upper_bound_temp_degc = self.upper_bound_temperature.get::<degree_celsius>();
        let lower_bound_temp_degc = self.lower_bound_temperature.get::<degree_celsius>();
...
pub fn test_custom_fluid_temperature_from_enthalpy() {
    ...
    let test_temperature = get_custom_fluid_temperature_from_enthalpy(
        ref_enthalpy,
        cp_function,
        upper_bound_temperature,
        lower_bound_temperature,
...
/// (it does not panic).
pub fn range_check_custom_fluid(
    fluid_temp: ThermodynamicTemperature,
    upper_bound_temperature: ThermodynamicTemperature,
    lower_bound_temperature: ThermodynamicTemperature,
    ...
    // celsius
    let temp_value_celsius = fluid_temp.get::<degree_celsius>();
