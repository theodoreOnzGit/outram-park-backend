...
pub fn get_yd325_constant_pressure_specific_heat_capacity(
    ...
    let cp_value_joule_per_kg = a + b * fluid_temp_kelvin;
...
pub fn get_yd325_specific_enthalpy(
    ...
    let enthalpy_value_joule_per_kg = enthalpy_value_joule_per_kg_calc(temp_kelvin_value)
...
pub fn get_temperature_from_enthalpy(
    ...
    if fluid_enthalpy.value < 0_f64 {
        panic!(
            "yd_325_heat_transfer_oil : get_temperature_from_enthalpy \n
    ...
    let fluid_temperature_degrees_kelvin_result =
        bisection!(enthalpy_root, (323.0, 523.0), 100, 1e-8);
...
///
///
pub fn range_check_yd325_oil(fluid_temp: ThermodynamicTemperature) -> Result<bool, TuasLibError> {
    // first i convert the fluidTemp object into a degree
    // celsius

    // TBD with range checking
    range_check(
        &Material::Liquid(LiquidMaterial::YD325),
        fluid_temp,
        max_temp_yd325_oil(),
        min_temp_yd325_oil(),
    )?;
