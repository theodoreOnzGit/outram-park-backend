...
pub fn get_hitec_dynamic_viscosity(
    ...
    // generic correlation is:
    // a + bT + cT^2 + dT^3 + eT^4;
    let viscosity_value_pascal_second = a
        + b * fluid_temp_kelvin
...
pub fn hitec_nitrate_salt_test_viscosity() {
    ...
    let temperature_346_f = ThermodynamicTemperature::new::<degree_fahrenheit>(346.338);

    // let's get the viscosity, should be around 11 cP
    let viscosity_346_f = get_hitec_dynamic_viscosity(temperature_346_f).unwrap();

    let viscosity_value_centipoise_346_f = viscosity_346_f.get::<centipoise>();
    ...
    let temperature_654_f = ThermodynamicTemperature::new::<degree_fahrenheit>(653.944);

    // let's get the viscosity, should be around 2.5 cP
    let viscosity_654_f = get_hitec_dynamic_viscosity(temperature_654_f).unwrap();

    let viscosity_value_centipoise_654f = viscosity_654_f.get::<centipoise>();

    // we expect a dynamic viscosity of around 2.5 cP at this temperature
    // we have +/- 16% uncertainty
    approx::assert_relative_eq!(2.5, viscosity_value_centipoise_654f, max_relative = 0.16);
}
...
pub fn get_hitec_thermal_conductivity(
    ...
    // generic correlation is:
    // a + bT + cT^2 + dT^3 + eT^4;
    let thermal_conductivity_value = a
        + b * fluid_temp_kelvin
        + c * fluid_temp_kelvin.powf(2.0)
        + d * fluid_temp_kelvin.powf(3.0)
        + e * fluid_temp_kelvin.powf(4.0);
...
pub fn get_hitec_specific_enthalpy(
    ...
    let enthalpy_value_joule_per_kg = -686400_f64 + 1560.0 * temp_kelvin_value;
...
pub fn get_temperature_from_enthalpy(
    ...
    if fluid_enthalpy.value < 0_f64 {
        panic!(
            "HITEC : get_temperature_from_enthalpy \n
    ...
    let fluid_temperature_degrees_kelvin_result =
        bisection!(enthalpy_root, (440.0, 800.0), 100, 1e-8);
...
///
///
pub fn range_check_hitec_salt(fluid_temp: ThermodynamicTemperature) -> Result<bool, TuasLibError> {
    // first i convert the fluidTemp object into a degree
    // celsius

    range_check(
        &Material::Liquid(LiquidMaterial::HITEC),
        fluid_temp,
        max_temp_hitec(),
        min_temp_hitec(),
    )?;
