...
pub fn get_dowtherm_a_density(
    ...
    let density_value_kg_per_m3 = 1078.0 - 0.85 * fluid_temp.get::<degree_celsius>();
...
pub fn get_dowtherm_a_viscosity(
    ...
    let viscosity_value_pascal_second = 0.130 / temperature_degrees_c_value.powf(1.072);
...
pub fn get_dowtherm_a_thermal_conductivity(
    ...
    range_check_dowtherm_a(fluid_temp)?;
    let thermal_conductivity_value = 0.142 - 0.00016 * fluid_temp.get::<degree_celsius>();
...
pub fn get_dowtherm_a_enthalpy(
    ...
    let enthalpy_value_joule_per_kg =
        1518.0 * temp_c_value + 2.82 / 2.0 * temp_c_value.powf(2.0) - 30924.0;
...
pub fn get_temperature_from_enthalpy(
    ...
    let fluid_temperature_degrees_cresult = bisection!(enthalpy_root, (20.0, 180.0), 100, 1e-8);
...
/// of validity
pub fn range_check_dowtherm_a(fluid_temp: ThermodynamicTemperature) -> Result<bool, TuasLibError> {
    // first i convert the fluidTemp object into a degree
    // celsius

    range_check(
        &Material::Liquid(LiquidMaterial::DowthermA),
        fluid_temp,
...
pub fn dynamic_viscosity_test_dowtherm_a() {
    ...
    let dynamic_viscosity_mpa_sec = try_get_mu_viscosity(dowtherm_a, temperature, pressure)
        .unwrap()
        .get::<millipascal_second>();

    approx::assert_relative_eq!(0.91, dynamic_viscosity_mpa_sec, max_relative = 0.03);
}
