...
pub fn flibe_salt_test_viscosity() {
    ...
    let temperature_900_k = ThermodynamicTemperature::new::<kelvin>(900.0);

    // let's get the viscosity, should be around 7.5 cP
    let viscosity_900_k = get_flibe_dynamic_viscosity(temperature_900_k).unwrap();

    let viscosity_value_centipoise_900_k = viscosity_900_k.get::<centipoise>();

    // we expect a dynamic viscosity of around 7.5 cP at this temperature
    // we have +/- 2% uncertainty
    approx::assert_relative_eq!(7.5, viscosity_value_centipoise_900_k, max_relative = 0.02);
}
...
pub fn flibe_prandtl_number() {
    ...
    approx::assert_relative_eq!(prandtl_560c.get::<ratio>(), 24.0, max_relative = 0.02);

    // at 730c, Pr is around 10.5
    // accurate to within 2%
    let prandtl_730c = prandtl_fn(temp_2_730_c);

    approx::assert_relative_eq!(prandtl_730c.get::<ratio>(), 10.5, max_relative = 0.02);
}
...
pub fn get_flibe_specific_enthalpy(
    ...
    let enthalpy_value_joule_per_kg = -low_bound_temp_kelvin
        * cp_val_constant_joule_per_kilogram_kelvin
...
pub fn get_temperature_from_enthalpy(
    ...
    let fluid_temperature_degrees_kelvin_result =
        bisection!(enthalpy_root, (732.2, 1573.0), 100, 1e-8);
...
///
///
pub fn range_check_flibe_salt(fluid_temp: ThermodynamicTemperature) -> Result<bool, TuasLibError> {
    // first i convert the fluidTemp object into a degree
    // celsius

    range_check(
        &Material::Liquid(LiquidMaterial::FLiBe),
        fluid_temp,
