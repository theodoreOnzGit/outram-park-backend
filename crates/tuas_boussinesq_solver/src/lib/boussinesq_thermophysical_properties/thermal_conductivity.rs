...
/// ```
#[inline]
pub fn try_get_kappa_thermal_conductivity(
    material: Material,
...
impl Material {
    ...
    /// material
    #[inline]
    pub fn try_get_thermal_conductivity(
        &self,
        temperature: ThermodynamicTemperature,
    ) -> Result<ThermalConductivity, TuasLibError> {
        match self {
            Material::Solid(_) => solid_thermal_conductivity(*self, temperature),
            Material::Liquid(_) => liquid_thermal_conductivity(*self, temperature),
        }
    }
}

// should the material happen to be a solid, use this function
fn solid_thermal_conductivity(
    material: Material,
    temperature: ThermodynamicTemperature,
) -> Result<ThermalConductivity, TuasLibError> {
    ...
    pub fn try_get_thermal_conductivity(
        ...
        let thermal_conductivity: ThermalConductivity = match self {
            ...
            CustomLiquid((low_bound_temp, high_bound_temp), _cp, k_fn, _mu_fn, _rho_fn) => {
                liquid_database::custom_liquid_material::get_custom_fluid_thermal_conductivity(
                    fluid_temp,
                    *k_fn,
                    *high_bound_temp,
                    *low_bound_temp,
        ...
        let thermal_conductivity: ThermalConductivity = match self {
            ...
            SteelSS304L => {
                let conductivity_result =
                    steel_304_l_libreoffice_spline_thermal_conductivity_zweibaum(solid_temp);
...
fn liquid_thermal_conductivity(
    material: Material,
    fluid_temp: ThermodynamicTemperature,
) -> Result<ThermalConductivity, TuasLibError> {
...
pub fn thermal_conductivity_test_steel() {
    ...
    let thermal_cond_spline =
        steel_304_l_spline_thermal_conductivity(ThermodynamicTemperature::new::<kelvin>(350.0));
    ...
    let thermal_cond_graves_et_al_1991 =
        steel_ss_304_l_ornl_thermal_conductivity(ThermodynamicTemperature::new::<kelvin>(350.0));
    ...
    let thermal_cond_spline =
        steel_304_l_spline_thermal_conductivity(ThermodynamicTemperature::new::<kelvin>(1000.0));
