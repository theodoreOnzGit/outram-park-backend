...
use crate::heat_transfer_correlations::thermal_resistance::subtract_two_thermodynamic_temperatures;
...
pub fn log_mean_temperature_difference(
    ...
    if temp_hot_fluid_a.value < temp_cold_fluid_a.value {
        panic!(
            "LMTD: hot fluid temperature input colder than \n
    ...
    let a_temperature_interval =
        subtract_two_thermodynamic_temperatures(temp_hot_fluid_a, temp_cold_fluid_a);

    let b_temperature_interval =
        subtract_two_thermodynamic_temperatures(temp_hot_fluid_b, temp_cold_fluid_b);

    let numerator = a_temperature_interval - b_temperature_interval;

    let denominator = a_temperature_interval.value.ln() - b_temperature_interval.value.ln();
...
pub fn calculate_lmtd_heat_flux_based_on_ambient_temp(
    ...
    if fluid_temperature_in.value == fluid_temperature_out.value {
        return Ok(overall_heat_transfer_coeff
            * surface_area
            * subtract_two_thermodynamic_temperatures(ambient_temperature, fluid_temperature_in));
    }
    // note, i do this to calculate
    // delta T = ambient_temperature - fluid_temperature
    let log_mean_temp_diff = log_mean_temperature_difference(
        ambient_temperature,
        ambient_temperature,
    ...
    return Ok(overall_heat_transfer_coeff * (log_mean_temp_diff) * surface_area);
}
