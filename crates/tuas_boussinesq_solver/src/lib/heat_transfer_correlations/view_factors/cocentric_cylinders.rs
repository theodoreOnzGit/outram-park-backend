...
pub fn outer_cylinder_self_view_factor(
    ...
    let view_factor_value: f64 =
        1.0 - one_over_r - 0.25 * one_over_r * ((h_sq + 4.0 * r_sq).sqrt() - h_value)
            + PI.recip() * b;
...
pub fn outer_cylinder_to_inner_cylinder_view_factor(
    ...
    //
    // e1 = sqrt( hsq_plus_rsq_plus_one^2 - 4.0 * rsq)/(2H)
    let e1 = (hsq_plus_rsq_plus_one.powf(2.0) - 4.0 * r_sq).sqrt() * 0.5 * one_over_h;
    ...
    let view_factor_value: f64 = one_over_r * (1.0 - b - PI.recip() * c);
...
pub fn outer_cylinder_to_annular_end_ring_view_factor(
    ...
    // d1 = sqrt (1 + R^2 + H^2)^2 -  4 R^2) / (4H)
    // d2 = PI/2 + asin(Y)
    let d1 = ((1.0 + r_sq + h_sq).powf(2.0) - 4.0 * r_sq).sqrt() * 0.25 * one_over_h;
...
pub fn cocentric_cylinders_view_factor_shld_equal_one_for_outer_cyl() {
    ...
    let self_view_factor =
        outer_cylinder_self_view_factor(inner_diameter, outer_diameter, cylinder_height);

    let outer_to_inner_cyl_view_factor = outer_cylinder_to_inner_cylinder_view_factor(
        inner_diameter,
        outer_diameter,
        cylinder_height,
    );
    ...
    let outer_cyl_to_annular_end_ring_view_factor = outer_cylinder_to_annular_end_ring_view_factor(
        inner_diameter,
        outer_diameter,
        cylinder_height,
    );

    let total_view_factor = self_view_factor
        + outer_to_inner_cyl_view_factor
        + outer_cyl_to_annular_end_ring_view_factor
        + outer_cyl_to_annular_end_ring_view_factor;

    approx::assert_relative_eq!(total_view_factor.get::<ratio>(), 1.0, max_relative = 1e-5);
}
...
pub fn inner_cylinder_to_outer_cylinder_view_factor(
    ...
    let view_factor_outer_to_inner: Ratio = outer_cylinder_to_inner_cylinder_view_factor(
        inner_diameter,
        outer_diameter,
        cylinder_height,
    );
...
/// basically i'm asserting that the view factors for annular cylinders
/// for the inner cylinder should sum to one
///
/// This is basic view factor algebra for radiation heat transfer
#[cfg(test)]
#[test]
pub fn cocentric_cylinders_view_factor_shld_equal_one_for_inner_cyl() {
    ...
    let inner_to_outer_cyl_view_factor = inner_cylinder_to_outer_cylinder_view_factor(
        inner_diameter,
        outer_diameter,
        cylinder_height,
    );
    ...
    // this needs to be multiplied twice or added twice
    // as there are two ends
    let inner_cyl_to_annular_end_ring_view_factor = inner_cylinder_to_annular_end_ring_view_factor(
        inner_diameter,
        outer_diameter,
        cylinder_height,
    );

    let total_view_factor = inner_to_outer_cyl_view_factor
