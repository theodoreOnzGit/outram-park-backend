...
pub fn subtract_two_thermodynamic_temperatures(
    ...
    return TemperatureInterval::new::<temperature_interval::kelvin>(temperature_interval_value);
}
...
pub fn obtain_power_two_convection_two_conduction_thermal_resistance(
    ...
    let thermal_resistance_1 = 1.0_f64 / average_surface_area_1 / heat_transfer_coefficient_1;

    // convection thermal resistance 2
    let thermal_resistance_2 = 1.0_f64 / average_surface_area_2 / heat_transfer_coefficient_2;

    // (Delta x)/(kA) for first layer
    let thermal_resistance_3 =
        length_of_wall_1 / average_thermal_conductivity_layer_1 / average_wall_surface_area_1;

    // (Delta x)/(kA) for second layer
    let thermal_resistance_4 =
        length_of_wall_2 / average_thermal_conductivity_layer_2 / average_wall_surface_area_2;

    let thermal_resistance =
        thermal_resistance_1 + thermal_resistance_2 + thermal_resistance_3 + thermal_resistance_4;

    // -Delta T
    let temperature_interval = -subtract_two_thermodynamic_temperatures(
        temperature_of_heat_recipient,
        temperature_of_heat_source,
    );

    let heat_flow: Power = temperature_interval / thermal_resistance;
...
pub fn obtain_power_one_convection_one_conduction_thermal_resistance(
    ...
    // (Delta x)/(kA) for first layer
    let thermal_resistance_2 = length_of_wall / average_thermal_conductivity / average_surface_area;

    let thermal_resistance = thermal_resistance_1 + thermal_resistance_2;
...
pub fn obtain_power_two_convection_one_conduction_thermal_resistance(
    ...
    // (Delta x)/(kA) for first layer
    let thermal_resistance_3 =
        length_of_wall / average_thermal_conductivity / average_surface_area_thermal_cond;

    let thermal_resistance = thermal_resistance_1 + thermal_resistance_2 + thermal_resistance_3;
...
pub fn obtain_power_through_single_convection_thermal_resistance(
    ...
    let thermal_resistance = 1.0_f64 / average_surface_area / heat_transfer_coefficient;
...
pub fn obtain_power_through_two_layer_wall_thermal_resistance(
    ...
    let thermal_resistance_1 =
        length_of_wall_1 / average_thermal_conductivity_layer_1 / average_surface_area_1;

    // (Delta x)/(kA) for second layer
    let thermal_resistance_2 =
        length_of_wall_2 / average_thermal_conductivity_layer_2 / average_surface_area_2;
...
pub fn obtain_power_through_wall_thermal_resistance(
    ...
    // (Delta x)/(kA)
    let thermal_resistance = length_of_wall / average_thermal_conductivity / average_surface_area;
...
pub fn try_get_thermal_conductance_annular_cylinder(
    ...
    if inner_diameter.value <= 0.0 {
        return Err("inner diameter specified is less than or equal\n 
            to zero for annular cylinder thermal conductance"
            .to_string());
    }
    ...
    if k.value <= 0.0 {
        return Err("negative thermal \n 
            thermal_conductivity supplied"
            .to_string());
    }

    if outer_diameter <= inner_diameter {
        return Err("outer diameter specified is less than or equal\n 
            to inner_diameter for annular cylinder thermal conductance"
            .to_string());
    }
    ...
    let outer_radius_divide_by_inner_radius: Ratio = outer_diameter / inner_diameter;

    let denominator: f64 = outer_radius_divide_by_inner_radius.value.log(E);
