...
/// volume of sorts
///
pub fn get_conductance_single_cartesian_one_dimension(
    material: Material,
    ...
    let average_material_pressure: Pressure = (material_pressure_1 + material_pressure_2) / 2.0;

    let average_material_temperature_kelvin =
        material_temperature_1.value + material_temperature_2.value - 273.15;
    ...
    let cross_sectional_area: Area =
        Area::new::<uom::si::area::square_meter>(UNIT_AREA_SQ_METER_FOR_ONE_DIMENSIONAL_CALCS);

    let material_thickness: Length = thickness.into();

    let material_conductance: ThermalConductance =
        material_thermal_conductivity * cross_sectional_area / material_thickness;
...
pub fn get_conductance_dual_cartesian_three_dimensions(
    ...
    let material_1_conductance: ThermalConductance =
        material_1_thermal_conductivity * cross_sectional_area / material_thickness_1;

    let material_2_conductance: ThermalConductance =
        material_2_thermal_conductivity * cross_sectional_area / material_thickness_2;
    ...
    if material_1_conductance.value == 0.0 || material_2_conductance.value == 0.0 {
    ...
    let overall_thermal_resistance = 1.0 / material_2_conductance + 1.0 / material_1_conductance;

    let overall_conductance: ThermalConductance = 1.0 / overall_thermal_resistance;
...
pub fn get_conductance_cylindrical_radial_two_materials(
    ...
    // inner        interim             outer
    //
    let inner_layer_conductance: ThermalConductance = try_get_thermal_conductance_annular_cylinder(
        id,
        interim_diameter,
        l,
    ...
    let outer_layer_conductance: ThermalConductance = try_get_thermal_conductance_annular_cylinder(
        interim_diameter,
        od,
        l,
    ...
    let total_thermal_resistance = 1.0 / inner_layer_conductance + 1.0 / outer_layer_conductance;
...
pub fn get_conductance_single_cylindrical_radial_solid_liquid(
    ...
    let solid_thermal_conductivity: ThermalConductivity =
        try_get_kappa_thermal_conductivity(solid, solid_temperature, solid_pressure)?;

    let solid_layer_conductance: ThermalConductance =
        try_get_thermal_conductance_annular_cylinder(id, od, l, solid_thermal_conductivity)?;
    ...
    let surface_area_for_solid_liquid_boundary: Area = match solid_liquid_arrangement {
    ...
    let liquid_thermal_conductance: ThermalConductance = h * surface_area_for_solid_liquid_boundary;

    // now we do overall thermal resistance and conductance

    let overall_thermal_resistance =
        1.0 / liquid_thermal_conductance + 1.0 / solid_layer_conductance;
...
pub fn simple_radiation_conductance(
    ...
    let hot_temperature_interval: TemperatureInterval =
        TemperatureInterval::new::<temperature_interval::kelvin>(hot_temperature.get::<kelvin>());

    let cold_temperature_interval: TemperatureInterval =
        TemperatureInterval::new::<temperature_interval::kelvin>(cold_temperature.get::<kelvin>());
    ...
    let stefan_boltzmann_constant_watts_per_m2_kelvin = 5.670374419e-8_f64;
    ...
    let radiation_thermal_conductance: ThermalConductance =
        area_coeff * stefan_boltzmann_constant * square_term * linear_term;
