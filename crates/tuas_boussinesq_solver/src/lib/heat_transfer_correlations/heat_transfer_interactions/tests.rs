...
use uom::si::{
    heat_transfer::watt_per_square_meter_kelvin, length::inch, pressure::atmosphere,
    thermal_conductivity::watt_per_meter_kelvin, thermodynamic_temperature::degree_celsius,
};

use crate::{
    boussinesq_thermophysical_properties::SolidMaterial,
...
pub fn radiation_conductance_unit_test() {
    ...
    let radiation_thermal_conductance =
        simple_radiation_conductance(area_coeff, hot_temperature, cold_temperature);

    let temperature_interval: ThermodynamicTemperature = hot_temperature
        - TemperatureInterval::new::<temperature_interval::kelvin>(
            cold_temperature.get::<kelvin>(),
        );

    let power: Power = radiation_thermal_conductance * temperature_interval;

    // assert that it is
    // P = 20956.91616 W to within 0.01%,
    // the botlzmann constant used will change things.

    approx::assert_relative_eq!(power.get::<watt>(), 20956.91616, max_relative = 1e-4);
}
...
pub fn radial_conductance_solid_inside_liquid_inside_test() {
    ...
    let test_thermal_conductance_overall = get_conductance_single_cylindrical_radial_solid_liquid(
        steel.into(),
        room_temp,
    ...
    let thermal_conductance_lengthscale_test =
        test_thermal_conductance_overall / steel.try_get_thermal_conductivity(room_temp).unwrap();
...
pub fn radial_conductance_solid_test() {
    ...
    //
    let thermal_conductivity_k = ThermalConductivity::new::<watt_per_meter_kelvin>(5.0);

    // and then i try getting thermal conductance
    let thermal_conductance_h = try_get_thermal_conductance_annular_cylinder(
        r_inner,
        r_outer,
        pipe_length,
    ...
    let thermal_conductance_lengthscale_test = thermal_conductance_h / thermal_conductivity_k;

    assert_eq!(
        thermal_conductance_lengthscale_test,
