...
#[inline]
pub fn advection_heat_rate(
    mass_flow_from_a_to_b: MassRate,
    specific_enthalpy_of_a: AvailableEnergy,
    specific_enthalpy_of_b: AvailableEnergy,
...
impl HeatTransferInteractionType {
    ...
    pub fn new_advection_interaction(
        ...
        fluid_density_heat_transfer_entity_2: MassDensity,
    ) -> Self {
        let advection_interaction: HeatTransferInteractionType =
            HeatTransferInteractionType::Advection(DataAdvection {
                mass_flowrate,
...
pub fn test_advection_signs() {
    ...
    let forward_mass_flow: MassRate = MassRate::new::<kilogram_per_second>(5.0);
    let backwards_mass_flow: MassRate = MassRate::new::<kilogram_per_second>(-5.0);

    let enthalpy_a: AvailableEnergy = AvailableEnergy::new::<joule_per_kilogram>(350.0);
    let enthalpy_b: AvailableEnergy = AvailableEnergy::new::<joule_per_kilogram>(550.0);

    // if forward flow from a to b, then expected power in forward
    // flow is
    let expected_forward_flow_enthalpy_a_to_b: Power = forward_mass_flow * enthalpy_a;

    let test_forward_flow_enthalpy_rate: Power =
        advection_heat_rate(forward_mass_flow, enthalpy_a, enthalpy_b).unwrap();

    assert_eq!(
        test_forward_flow_enthalpy_rate,
        expected_forward_flow_enthalpy_a_to_b
    );

    // if backward flow from b to a, expected power in backward flow is:
    let expected_forward_flow_enthalpy_a_to_b: Power = backwards_mass_flow * enthalpy_b;

    let test_backward_flow_enthalpy_rate: Power =
        advection_heat_rate(backwards_mass_flow, enthalpy_a, enthalpy_b).unwrap();
    ...
    assert_eq!(
        test_backward_flow_enthalpy_rate,
        expected_forward_flow_enthalpy_a_to_b
    );
}
