...
pub fn advance_timestep_fluid_node_array_pipe_high_peclet_number(
    ...
    let mut total_enthalpy_rate_change_back_node = Power::new::<watt>(0.0);

    for enthalpy_chg_rate in back_cv_rate_enthalpy_change_vector.iter() {
        total_enthalpy_rate_change_back_node += *enthalpy_chg_rate;
    }

    // then the front node,

    let mut total_enthalpy_rate_change_front_node = Power::new::<watt>(0.0);

    for enthalpy_chg_rate in front_cv_rate_enthalpy_change_vector.iter() {
    ...
    let mut temperature_vector: Array1<ThermodynamicTemperature> = last_timestep_temperature_fluid
        .map(|temp_kelvin_ptr: &ThermodynamicTemperature| {
    ...
    if number_of_nodes > 1 {
        ...
        let mut power_source_vector: Array1<Power> = Array::zeros(number_of_nodes);
        ...
        // back node calculation (first node)
        {
            ...
            coefficient_matrix[[0, 0]] = volume_fraction_array[0] * rho_cp[0] * total_volume / dt
                + solid_fluid_conductance_array[0];
            ...
            power_source_vector[0] = solid_fluid_conductance_array[0]
                * last_timestep_temperature_solid[0]
                ...
                + last_timestep_temperature_fluid[0]
                    * total_volume
                    * volume_fraction_array[0]
                    * rho_cp[0]
                    / dt
                + q * q_fraction[0]
            ...
            if !forward_flow {
                ...
                let enthalpy_of_adjacent_node_to_the_front: AvailableEnergy = try_get_h(
                ...
                power_source_vector[0] += mass_flowrate * h_fluid_last_timestep;
            }
        }
        ...
        if number_of_nodes > 2 {
            ...
            for i in 1..number_of_nodes - 1 {
                ...
                coefficient_matrix[[i, i]] = volume_fraction_array[i] * rho_cp[i] * total_volume
                    / dt
                    + solid_fluid_conductance_array[i];

                // we also consider outflow using previous timestep
                // temperature,
                // assume back cv and front cv material are the same

                let h_fluid_last_timestep: AvailableEnergy = try_get_h(
                    back_single_cv.material_control_volume,
                ...
                if forward_flow {
                    ...
                    let h_fluid_adjacent_node: AvailableEnergy = try_get_h(
                        back_single_cv.material_control_volume,
                        last_timestep_temperature_fluid[i - 1],
                        back_single_cv.pressure_control_volume,
                    )?;

                    power_source_vector[i] += h_fluid_adjacent_node * mass_flowrate.abs();
                } else {
        ...
        // front node (last node) calculation
        {
            ...
            coefficient_matrix[[i, i]] = volume_fraction_array[i] * rho_cp[i] * total_volume / dt
                + solid_fluid_conductance_array[i];
            // the first part of the source term deals with
            // the flow direction independent terms
            ...
            if forward_flow {
                ...
                let enthalpy_of_adjacent_node_to_the_rear: AvailableEnergy = try_get_h(
                    back_single_cv.material_control_volume,
                    last_timestep_temperature_fluid[i - 1],
                    back_single_cv.pressure_control_volume,
                )?;
                ...
                power_source_vector[i] -= mass_flowrate.abs() * h_fluid_last_timestep;
            }
        }
        ...
        temperature_vector =
            solve_conductance_matrix_power_vector(coefficient_matrix, power_source_vector)?;
    }
    // update the single cvs at the front and back with new enthalpies

    // Todo: probably need to synchronise error types in future
    let back_node_enthalpy_next_timestep: AvailableEnergy = try_get_h(
        back_single_cv.material_control_volume,
    ...
    let front_node_enthalpy_next_timestep: AvailableEnergy = try_get_h(
    ...
    *last_timestep_temperature_fluid = temperature_vector.mapv(|temperature_value| {
...
pub fn fluid_node_calculation_initial_test() {
    ...
    use crate::control_volume_dimensions::RadialCylindricalThicknessThermalConduction;
    use crate::control_volume_dimensions::InnerDiameterThermalConduction;
    ...
    let inlet_temperature = ThermodynamicTemperature::new::<degree_celsius>(79.12);
    ...
    let fluid_back_entity: SingleCVNode = SingleCVNode::new_odd_shaped_pipe(
        node_length,
        flow_area,
        therminol,
        initial_temperature,
    ...
    let back_cv_ptr = Arc::new(Mutex::new(fluid_back_cv));
    let front_cv_ptr = Arc::new(Mutex::new(fluid_front_cv));
    let fluid_temp_at_present_timestep_ptr = Arc::new(Mutex::new(fluid_temperature_array));
    let steel_temp_at_present_timestep_ptr = Arc::new(Mutex::new(steel_temperature_array));
    let q_fraction_ptr = Arc::new(Mutex::new(q_fraction));
    let fluid_vol_fraction_ptr = Arc::new(Mutex::new(volume_fraction_array.clone()));
    let solid_vol_fraction_ptr = Arc::new(Mutex::new(volume_fraction_array));
    ...
    let steel_temp_at_present_timestep_ptr_for_loop = steel_temp_at_present_timestep_ptr.clone();
    ...
    let calculation_loop = move || {
        ...
        let mut time_wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "fluid_node_calc_time_profile.csv",
        ))
        .unwrap();

        time_wtr
            .write_record(&[
                "loop_calculation_time_nanoseconds",
                "mutex_lock_time_ns",
        ...
        while current_time_simulation_time <= *max_time_ptr.deref() {
            ...
            let mut back_cv_ptr_in_loop = back_cv_ptr_clone_for_loop.lock().unwrap();
            let mut front_cv_ptr_in_loop = front_cv_ptr_clone_for_loop.lock().unwrap();
            let mut fluid_temp_vec_ptr_in_loop = fluid_temp_vec_ptr_for_loop.lock().unwrap();
            let mut steel_temp_at_present_timestep_ptr_in_loop =
                steel_temp_at_present_timestep_ptr_for_loop.lock().unwrap();
            let mut q_fraction_ptr_in_loop = q_fraction_ptr_for_loop.lock().unwrap();
            let mut fluid_vol_fraction_ptr_in_loop =
                fluid_vol_fraction_ptr_for_loop.lock().unwrap();
            let solid_vol_fraction_ptr_in_loop = solid_vol_fraction_ptr_for_loop.lock().unwrap();
            ...
            let arc_mutex_lock_elapsed_ns = arc_mutex_lock_start.elapsed().unwrap().as_nanos();
            ...
            let mut fluid_temp_kelvin_times_vol_frac_average: Vec<f64> = vec![];

            for (index, fluid_temp) in fluid_temp_vec_ptr_in_loop.iter().enumerate() {
                let fluid_temp_times_vol_frac =
                    fluid_vol_fraction_ptr_in_loop[index] * (*fluid_temp);

                fluid_temp_kelvin_times_vol_frac_average
                    .push(fluid_temp_times_vol_frac.get::<kelvin>());
            }

            // find average in kelvin, convert back to correct units

            let fluid_avg_temp_kelvin: f64 = fluid_temp_kelvin_times_vol_frac_average.iter().sum();

            let fluid_avg_temp = ThermodynamicTemperature::new::<kelvin>(fluid_avg_temp_kelvin);

            // we do the same for solid temp
            //
            //

            let mut steel_temp_kelvin_times_vol_frac_average: Vec<f64> = vec![];

            for (index, steel_temp) in steel_temp_at_present_timestep_ptr_in_loop
                .iter()
                .enumerate()
            {
                let steel_temp_times_vol_frac =
                    solid_vol_fraction_ptr_in_loop[index] * (*steel_temp);

                steel_temp_kelvin_times_vol_frac_average
                    .push(steel_temp_times_vol_frac.get::<kelvin>());
            }

            // find average in kelvin, convert back to correct units

            let steel_avg_temp_kelvin: f64 = steel_temp_kelvin_times_vol_frac_average.iter().sum();

            let steel_avg_temp = ThermodynamicTemperature::new::<kelvin>(steel_avg_temp_kelvin);
            ...
            let radial_thickness_thermal_conduction: RadialCylindricalThicknessThermalConduction =
                radial_thickness_thermal_conduction.into();

            let inner_diameter_thermal_conduction: InnerDiameterThermalConduction = id.into();
            ...
            let nodal_thermal_conductance: ThermalConductance = conductance_interaction
            ...
            let mut conductance_vector: Array1<ThermalConductance> = Array::zeros(number_of_nodes);
            ...
            for (index, fluid_temp) in fluid_temp_vec_ptr_in_loop.iter().enumerate() {
                let fluid_nodal_rho_cp: VolumetricHeatCapacity =
                    try_get_rho_cp(steel, *fluid_temp, atmospheric_pressure).unwrap();
            ...
            let enthalpy_inflow_in_back_cv: Power = therminol_mass_flowrate
                * try_get_h(therminol, inlet_temperature, atmospheric_pressure).unwrap();

            let enthalpy_outflow_in_front_cv: Power = therminol_mass_flowrate
                * front_cv_ptr_in_loop
                    .deref()
            ...
            let node_connection_end_ns = node_connection_start.elapsed().unwrap().as_nanos();
            ...
            let _new_temperature_vec = advance_timestep_fluid_node_array_pipe_high_peclet_number(
            ...
            let timestep_advance_end_ns = timestep_advance_start.elapsed().unwrap().as_nanos();

            // write timestep diagnostics

            let total_time_ns = arc_mutex_lock_elapsed_ns
...
pub fn fluid_node_backflow_calculation_initial_test() {
    ...
    let fluid_back_cv: SingleCVNode = SingleCVNode::new_odd_shaped_pipe(
        node_length,
        flow_area,
        therminol,
        initial_temperature,
    ...
    let calculation_loop = move || {
        ...
        let mut time_wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "fluid_node_backflow_calc_time_profile.csv",
        ))
        .unwrap();
        ...
        while current_time_simulation_time <= *max_time_ptr.deref() {
            ...
            let h: HeatTransfer = HeatTransfer::new::<watt_per_square_meter_kelvin>(35.0);
            ...
            let enthalpy_inflow_from_bc: Power = therminol_mass_flowrate.abs()
                * try_get_h(therminol, inlet_temperature, atmospheric_pressure).unwrap();
            ...
            let enthalpy_outflow_to_bc: Power = therminol_mass_flowrate.abs()
                * back_cv_ptr_in_loop
                    .deref()
                    .current_timestep_control_volume_specific_enthalpy;
