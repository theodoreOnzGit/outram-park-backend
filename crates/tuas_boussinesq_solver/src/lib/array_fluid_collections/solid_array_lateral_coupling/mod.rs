...
impl SolidColumn {
    ...
    /// form
    pub fn get_temperature_array(&self) -> Result<Array1<ThermodynamicTemperature>, TuasLibError> {
        // converts the fixed sized temperature array (at compile time)
        // into a dynamically sized ndarray type so we can use solve
        // methods
        let mut temperature_arr: Array1<ThermodynamicTemperature> = Array1::default(self.len());

        for (idx, temperature) in self.temperature_array_current_timestep.iter().enumerate() {
    ...
    pub fn set_temperature_vector(
        ...
        let back_cv_temperature: ThermodynamicTemperature = temperature_vec[0];

        let front_cv_temperature: ThermodynamicTemperature = *temperature_vec.last().unwrap();
        ...
        let back_cv_enthalpy = try_get_h(material, back_cv_temperature, pressure)?;

        let front_cv_enthalpy = try_get_h(material, front_cv_temperature, pressure)?;

        self.back_single_cv
            .current_timestep_control_volume_specific_enthalpy = back_cv_enthalpy;

        self.front_single_cv
            .current_timestep_control_volume_specific_enthalpy = front_cv_enthalpy;
    ...
    pub fn set_temperature_array(
        &mut self,
        temperature_arr: Array1<ThermodynamicTemperature>,
    ) -> Result<(), TuasLibError> {
