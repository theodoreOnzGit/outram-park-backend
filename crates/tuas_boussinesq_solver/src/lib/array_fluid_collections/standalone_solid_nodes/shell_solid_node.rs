...
pub fn advance_timestep_solid_cylindrical_shell_node_no_axial_conduction(
    ...
    let mut new_timestep_temperature_vector: Array1<ThermodynamicTemperature> =
        last_timestep_temperature_solid.map(|temp_kelvin_ptr: &ThermodynamicTemperature| {
    ...
    if number_of_nodes <= 1 {
        ...
        // back node calculation (first node)
        {
            ...
            power_source_vector[0] = last_timestep_temperature_solid[0]
                * total_volume
                * volume_fraction_array[0]
                * rho_cp[0]
                / dt
                + solid_inner_conductance_array[0] * last_timestep_temperature_inner_side[0]
                + solid_outer_conductance_array[0] * last_timestep_temperature_outer_side[0]
                + q * q_fraction[0];
        }
        ...
        if number_of_nodes > 2 {
            ...
            for i in 1..number_of_nodes - 1 {
                ...
                power_source_vector[i] = last_timestep_temperature_solid[i]
                    * total_volume
                    * volume_fraction_array[i]
                    * rho_cp[i]
                    / dt
                    + solid_inner_conductance_array[i] * last_timestep_temperature_inner_side[i]
                    + solid_outer_conductance_array[i] * last_timestep_temperature_outer_side[i]
                    + q * q_fraction[i];
            }
        }
    ...
    *last_timestep_temperature_solid = new_timestep_temperature_vector.mapv(|temperature_value| {
...
fn fluid_solid_node_calculation_initial_test() {
    ...
    let total_steel_volume = heated_length * (PI * 0.25 * od * od - PI * 0.25 * id * id);
    ...
    let ambient_air_temp_at_present_timestep_ptr =
        Arc::new(Mutex::new(ambient_air_temperature_array));
    ...
    let steel_temperature_array_present_timestep_ptr =
        Arc::new(Mutex::new(steel_temperature_array));
    ...
    let calculation_loop = move || {
        ...
        let mut time_wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "cht_nodes_calc_time_profile.csv",
        ))
        .unwrap();
        ...
        while current_time_simulation_time <= *max_time_ptr.deref() {
            ...
            let mut ambient_air_temp_at_present_timestep_ptr_in_loop =
                ambient_air_temp_at_present_timestep_ptr_for_loop
                    .lock()
                    .unwrap();
            ...
            let mut steel_temperature_array_present_timestep_ptr_in_loop =
                steel_temperature_array_present_timestep_ptr_for_loop
                    .lock()
                    .unwrap();
            ...
            for (index, steel_temp) in steel_temperature_array_present_timestep_ptr_in_loop
                .iter()
                .enumerate()
            {
            ...
            let outer_diameter_thermal_conduction: OuterDiameterThermalConduction = od.into();
            ...
            for (index, fluid_temp) in fluid_temp_vec_ptr_in_loop.iter().enumerate() {
                let fluid_nodal_rho_cp: VolumetricHeatCapacity =
                    try_get_rho_cp(therminol, *fluid_temp, atmospheric_pressure).unwrap();
            ...
            for (index, solid_temp) in steel_temperature_array_present_timestep_ptr_in_loop
                .iter()
                .enumerate()
            {
                let solid_nodal_rho_cp: VolumetricHeatCapacity =
                    try_get_rho_cp(steel, *solid_temp, atmospheric_pressure).unwrap();
            ...
            // record current fluid temperature profile
            {
                ...
                for node_temp in steel_temperature_array_present_timestep_ptr_in_loop
                    .deref_mut()
                    .iter()
                {
            ...
            let mut fluid_temp_vec_last_timestep = fluid_temp_vec_ptr_in_loop.deref().clone();
