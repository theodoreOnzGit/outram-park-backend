...
pub(crate) fn _advance_timestep_for_externally_cooled_array_cv_no_insulation(
    ...
    let mut total_enthalpy_rate_change_innermost_node = Power::new::<watt>(0.0);

    for enthalpy_chg_rate in rate_enthalpy_change_vector.iter() {
    ...
    let mut temperature_array: Array1<ThermodynamicTemperature> = last_timestep_temperature_array
    ...
    if nodes_number > 1 {
        ...
        // note that Hs[0] is never used, it may as well be 0
        {
            ...
            conductance_matrix[[0, 0]] = volume_fraction_array[0] * rho_cp[0] * total_volume / dt
                + thermal_conductance_array[1];
            power_source_array[0] = q * power_distribution_array[0]
                + total_enthalpy_rate_change_innermost_node
                + last_timestep_temperature_array[0]
                    * total_volume
        ...
        if nodes_number > 2 {
            for i in 1..nodes_number - 1 {
                ...
                conductance_matrix[[i, i]] = volume_fraction_array[i] * rho_cp[i] * total_volume
                    / dt
                    + thermal_conductance_array[i + 1]
                    + thermal_conductance_array[i];
                power_source_array[i] = q * power_distribution_array[i]
                    + last_timestep_temperature_array[i]
...

        {
            ...
            let conductance_to_coolant_fraction: ThermalConductance =
                thermal_conductance_array[i + 1] * thermal_conductance_to_coolant
                    / (thermal_conductance_array[i + 1] + thermal_conductance_to_coolant);
            conductance_matrix[[i, i - 1]] = volume_fraction_array[i] * rho_cp[i] * total_volume
                / dt
                + thermal_conductance_array[i]
                + conductance_to_coolant_fraction;
        ...
        temperature_array =
            solve_conductance_matrix_power_vector(conductance_matrix, power_source_array)?;
    } else {
        let conductance_to_coolant_fraction: ThermalConductance = thermal_conductance_array[1]
            * thermal_conductance_to_coolant
            / (thermal_conductance_array[1] + thermal_conductance_to_coolant);
        let cv_conductance = volume_fraction_array[0] * total_volume * rho_cp[0] / dt
            + conductance_to_coolant_fraction;
        ...
        temperature_array[0] = ThermodynamicTemperature::new::<kelvin>(temperature.value);
    }

    // Todo: probably need to synchronise error types in future
    let inner_node_enthalpy_next_timestep: AvailableEnergy = try_get_h(
    ...
    *last_timestep_temperature_array = temperature_array.mapv(|temperature_value| {
...
pub(crate) fn advance_timestep_for_specified_conductance_array_cv(
    ...
    for enthalpy_chg_rate in rate_enthalpy_change_vector_inner_node.iter() {
        total_enthalpy_rate_change_innermost_node += *enthalpy_chg_rate;
    }
    ...
    let mut total_enthalpy_rate_change_outermost_node = Power::new::<watt>(0.0);

    for enthalpy_chg_rate in rate_enthalpy_change_vector_outer_node.iter() {
    ...
    if nodes_number > 1 {
        ...
        // note that Hs[0] is never used, it may as well be 0
        {
            ...
            conductance_matrix[[0, 0]] =
                vol_fraction[0] * rho_cp[0] * total_volume / dt + conductance_array_ref[1];
            power_source_array[0] = q * power_distribution_array_ref[0]
                + total_enthalpy_rate_change_innermost_node
                + last_timetep_temperature_array_ref[0]
                    * total_volume
                    * vol_fraction[0]
                    * rho_cp[0]
                    / dt;
        ...
        if nodes_number > 2 {
            for i in 1..nodes_number - 1 {
                ...
                conductance_matrix[[i, i]] = vol_fraction[i] * rho_cp[i] * total_volume / dt
                    + conductance_array_ref[i + 1]
                    + conductance_array_ref[i];
                power_source_array[i] = q * power_distribution_array_ref[i]
                    + last_timetep_temperature_array_ref[i]
                        * vol_fraction[i]
                        * total_volume
                        * rho_cp[i]
                        / dt;
...

        {
            ...
            let dummy_coolant_temperature_at_boundary =
            ...
            conductance_matrix[[i, i]] = vol_fraction[i] * total_volume * rho_cp[i] / dt
                + conductance_array_ref[i]
                + dummy_conductance_to_coolant_fraction;
            power_source_array[i] = q * power_distribution_array_ref[i]
