...
impl FluidArray {
    ...
    pub fn get_max_timestep(
        ...
        let max_timestep_front_cv: Time = self
            .front_single_cv
            .get_max_timestep(max_temperature_change)?;

        let max_timestep_back_cv: Time = self
            .back_single_cv
            .get_max_timestep(max_temperature_change)?;
        ...
        let thermal_diffusivity_array: Array1<DiffusionCoefficient> = control_vol_temperature_array
            .map(|temperature_reference| {
        ...
        let number_of_temperature_nodes: usize = self.len();

        let fluid_volume_in_one_node: Volume = self.front_single_cv.volume;

        // now, delta x is based on the cross sectional area
        // of the control volume, not the node length
        //

        let node_length: Length = self.total_length / number_of_temperature_nodes as f64;

        let cross_sectional_area: Area = fluid_volume_in_one_node / node_length;
        ...
        let thermal_diffusivity_coeff_opt =
            thermal_diffusivity_array.iter().max_by(|alpha_1, alpha_2| {
        ...
        let thermal_diffusivity_coeff: DiffusionCoefficient = match thermal_diffusivity_coeff_opt {
        ...
        let max_conduction_timescale: Time =
            max_mesh_fourier_number * delta_x * delta_x / thermal_diffusivity_coeff;
        ...
        let fluid_prandtl_number =
            try_get_prandtl(control_vol_material, bulk_temp, control_vol_pressure)?;

        let fluid_reynolds_number = self.get_reynolds(mass_flowrate)?;

        // obtain a nusselt number estimate ignoring wall prandtl
        // number
        let nusselt_estimate_ignoring_wall_prandtl = self
            .nusselt_correlation
        ...
        let max_radial_condition_timescale: Time =
            max_mesh_fourier_number * self.xs_area / thermal_diffusivity_coeff;

        let max_solid_fluid_convection_timescale: Time =
            max_radial_condition_timescale / nusselt_estimate_ignoring_wall_prandtl;
        ...
        let maximum_timestep: Time = *max_timestep_vector
            .iter()
            .min_by(|time_1, time_2| {
                // a and b represent two typical items in the array
                let a = &time_1.value;
                let b = &time_2.value;
    ...
    ///
    #[inline]
    pub fn try_get_bulk_temperature(&mut self) -> Result<ThermodynamicTemperature, TuasLibError> {
        ...
        let temperature_array_reference = &self.temperature_array_current_timestep;

        let mut vol_averaged_temperature_array_values: Array1<f64> =
            Array::default(temperature_array_reference.len());

        for (idx, temperature_reference) in temperature_array_reference.iter().enumerate() {
            //get the vol fraction

            let vol_fraction: f64 = volume_fraction_array_reference[idx];

            let vol_avg_temperature_component: f64 =
                vol_fraction * (temperature_reference.get::<kelvin>());

            vol_averaged_temperature_array_values[idx] = vol_avg_temperature_component;
        }

        // sum it all up (these are float values)

        let vol_averaged_temperature_kelvin: f64 = vol_averaged_temperature_array_values.sum();

        return Ok(ThermodynamicTemperature::new::<kelvin>(
            vol_averaged_temperature_kelvin,
        ));
    }

    /// gets the reynolds number for this fluid array
    #[inline]
    pub fn get_reynolds(&mut self, mass_flowrate: MassRate) -> Result<Ratio, TuasLibError> {
        let xs_area = self.xs_area;
        let hydraulic_diameter = self.get_hydraulic_diameter();
        let fluid_viscosity = self.get_fluid_viscosity();

        let reynolds = mass_flowrate / xs_area * hydraulic_diameter / fluid_viscosity;
    ...
    pub fn get_nusselt(
        ...
        let length_to_diameter: Ratio = self.get_component_length() / self.get_hydraulic_diameter();

        // f + D/L K
        // this is the darcy plus a bit a little bit of form loss

        let darcy_plus_normalised_form_loss: Ratio = fldk / length_to_diameter;
