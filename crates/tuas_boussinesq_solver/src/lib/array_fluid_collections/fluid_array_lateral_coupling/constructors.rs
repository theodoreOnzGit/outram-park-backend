...
impl FluidArray {
    ...
    pub fn new_cylinder(
        ...
        let node_length: Length = default_length / number_of_temperature_nodes as f64;

        let vol_frac_default: f64 = 1.0 / number_of_temperature_nodes as f64;
        ...
        let mut vol_frac_array: Array1<f64> = Array::zeros(number_of_temperature_nodes);
        vol_frac_array.fill(vol_frac_default);

        let liquid_material: Material = Material::Liquid(liquid_material);

        let cross_sectional_area = PI * hydraulic_diameter * hydraulic_diameter * 0.25;

        let surface_roughness = adjacent_solid_material.surface_roughness().unwrap();
        ...
        let pipe_prandtl =
            try_get_prandtl(liquid_material, default_temp, default_pressure).unwrap();
    ...
    pub fn new_annular_cylinder(
        ...
        let cross_sectional_area = PI * outer_diameter * outer_diameter * 0.25
        ...
        let calculated_wetted_perimeter: Length = PI * inner_diameter + PI * outer_diameter;

        let hydraulic_diameter: Length = 4.0 * cross_sectional_area / calculated_wetted_perimeter;
