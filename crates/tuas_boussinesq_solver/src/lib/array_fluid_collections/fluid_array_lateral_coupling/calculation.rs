...
impl FluidArray {
    ...
    pub fn advance_timestep(&mut self, timestep: Time) -> Result<(), TuasLibError> {
        self.advance_timestep_with_mass_flowrate(timestep, self.mass_flowrate)
    }

    /// advances timestep for the fluid array
    /// given a fixed timestep
    #[inline]
    pub fn advance_timestep_with_mass_flowrate(
        &mut self,
        timestep: Time,
        ...
        if lateral_temperature_arary_connected {
            ...
            for conductance_array in self.lateral_adjacent_array_conductance_vector.iter() {
            ...
            for (lateral_idx, conductance_arr) in self
        ...
        let lateral_power_sources_connected: bool = self.q_vector.len() > 0;
        ...
        if lateral_power_sources_connected {
            ...
            for (lateral_idx, q_reference) in self.q_vector.iter().enumerate() {
                ...
                let power_frac_array: Array1<f64> = self.q_fraction_vector[lateral_idx].clone();
        ...
        // back node calculation (first node)
        {
            ...
            let h_fluid_last_timestep: AvailableEnergy = self
                .back_single_cv
            ...
            if !forward_flow {
                ...
                let enthalpy_of_adjacent_node_to_the_front: AvailableEnergy = try_get_h(
                    ...
                    self.back_single_cv.pressure_control_volume,
                )
                .unwrap();
        ...
        if low_peclet_number_flow {
            ...
            average_axial_conductance = average_fluid_conductivity * self.xs_area / node_length;
        ...
        let front_node_enthalpy_next_timestep: AvailableEnergy = try_get_h(
            ...
            self.front_single_cv.pressure_control_volume,
        )
        .unwrap();

        self.front_single_cv
    ...
    pub fn clear_vectors(&mut self) -> Result<(), TuasLibError> {
