...
impl FluidComponentTrait for FluidComponent {
    fn get_mass_flowrate(&mut self) -> MassRate {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_mass_flowrate(),
            ...
            //
            //
            FluidComponent::ParallelUniformFluidArray(fluid_array, number_of_tubes) => {
    ...
    fn set_mass_flowrate(&mut self, mass_flowrate: MassRate) {
        match self {
            ...
            FluidComponent::ParallelUniformFluidArray(fluid_array, number_of_tubes) => {
                ...
                let flow_through_one_tube = flow_through_all_tubes / (*number_of_tubes as f64);
    ...
    fn get_mass_flowrate_from_pressure_loss_immutable(&self, pressure_loss: Pressure) -> MassRate {
        match self {
            ...
            FluidComponent::ParallelUniformFluidArray(fluid_array, number_of_tubes) => {
                ...
                let flow_through_all_tubes = flow_through_one_tube * (*number_of_tubes as f64);
    ...
    fn get_pressure_loss(&mut self) -> Pressure {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_pressure_loss(),
    ...
    fn set_pressure_loss(&mut self, pressure_loss: Pressure) {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.set_pressure_loss(pressure_loss),
            FluidComponent::ParallelUniformFluidArray(fluid_array, _) => {
                // since components are in parallel, pressure loss is the
                // same for each tube
    ...
    fn get_cross_sectional_area(&mut self) -> Area {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_cross_sectional_area(),
            ...
            //
            FluidComponent::ParallelUniformFluidArray(fluid_array, number_of_tubes) => {
                let xs_area_one_tube = fluid_array.get_cross_sectional_area();
    ...
    fn get_cross_sectional_area_immutable(&self) -> Area {
        match self {
            ...
            // cross sectional area is on a total basis
            FluidComponent::ParallelUniformFluidArray(fluid_array, number_of_tubes) => {
                let xs_area_one_tube = fluid_array.get_cross_sectional_area_immutable();
    ...
    fn get_hydraulic_diameter(&mut self) -> Length {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_hydraulic_diameter(),
            ...
            //
            FluidComponent::ParallelUniformFluidArray(fluid_array, number_of_tubes) => {
                let hydraulic_diameter_one_tube = fluid_array.get_hydraulic_diameter();

                let cross_sectional_area_one_tube: Area = fluid_array.get_cross_sectional_area();
                // D_H = 4A/P
                // where P is the wetted perimeter of one tube
                let wetted_perimeter_one_tube: Length =
                    4.0 * fluid_array.get_cross_sectional_area() / hydraulic_diameter_one_tube;

                let total_xs_area: Area = cross_sectional_area_one_tube * (*number_of_tubes as f64);

                let total_wetted_perimeter: Length =
                    wetted_perimeter_one_tube * (*number_of_tubes as f64);

                let hydraulic_diameter_overall: Length = total_xs_area / total_wetted_perimeter;
    ...
    fn get_hydraulic_diameter_immutable(&self) -> Length {
        match self {
            ...
            //
            FluidComponent::ParallelUniformFluidArray(fluid_array, number_of_tubes) => {
                let hydraulic_diameter_one_tube = fluid_array.get_hydraulic_diameter_immutable();

                let cross_sectional_area_one_tube: Area =
                    fluid_array.get_cross_sectional_area_immutable();
                // D_H = 4A/P
                // where P is the wetted perimeter of one tube
                let wetted_perimeter_one_tube: Length = 4.0
                    * fluid_array.get_cross_sectional_area_immutable()
    ...
    fn get_fluid_viscosity_at_ref_temperature(&mut self) -> DynamicViscosity {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_fluid_viscosity(),
            FluidComponent::ParallelUniformFluidArray(fluid_array, _) => {
    ...
    fn get_fluid_viscosity_immutable_at_ref_temperature(&self) -> DynamicViscosity {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_fluid_viscosity_immutable(),
            FluidComponent::ParallelUniformFluidArray(fluid_array, _) => {
    ...
    fn get_fluid_density_at_ref_temperature(&mut self) -> MassDensity {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_fluid_density(),
            FluidComponent::ParallelUniformFluidArray(fluid_array, _) => {
    ...
    fn get_fluid_density_immutable_at_ref_temperature(&self) -> MassDensity {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_fluid_density_immutable(),
            FluidComponent::ParallelUniformFluidArray(fluid_array, _) => {
    ...
    fn get_component_length(&mut self) -> Length {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_component_length(),
    ...
    fn get_component_length_immutable(&self) -> Length {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_component_length_immutable(),
            // component lengths are same whether overall or individual
            FluidComponent::ParallelUniformFluidArray(fluid_array, _) => {
    ...
    fn get_incline_angle(&mut self) -> Angle {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_incline_angle(),
            FluidComponent::ParallelUniformFluidArray(fluid_array, _) => {
    ...
    fn get_incline_angle_immutable(&self) -> Angle {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_incline_angle_immutable(),
            FluidComponent::ParallelUniformFluidArray(fluid_array, _) => {
    ...
    fn get_internal_pressure_source(&mut self) -> Pressure {
        match self {
            FluidComponent::FluidArray(fluid_array) => fluid_array.get_internal_pressure_source(),
            // getting or
