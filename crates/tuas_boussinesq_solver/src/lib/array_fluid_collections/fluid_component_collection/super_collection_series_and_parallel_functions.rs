...
pub trait FluidComponentSuperCollectionSeriesAssociatedFunctions {
    ...
    fn calculate_pressure_change_from_mass_flowrate(
        ...
        fluid_component_collection_vector: &Vec<FluidComponentCollection>,
    ) -> Pressure {
            ...
            fluid_component_collection_vector.iter().enumerate()
        {
            ...
            pressure_vector[index] = fluid_component_collection_pressure_change;
        }
    ...
    fn calculate_mass_flowrate_from_pressure_change(
        ...
        let user_specified_pressure_loss_pascals =
            get_pressure_loss_from_pressure_change(pressure_change)
                .unwrap()
                .value;
        ...
        // mass flowrate value:
        let check_flow_pressure_loss_pascals = |mass_flowrate: MassRate| -> Pressure {
            let zero_mass_flow: MassRate = MassRate::new::<kilogram_per_second>(0.0);
        ...
        let check_for_check_valve_behaviour = |mass_flowrate: MassRate| -> Result<bool, String> {
            ...
            let max_pressure_loss_at_specified_flow = f64::max(
                pressure_loss_at_specified_forward_flow.value.abs(),
                pressure_loss_at_specified_backward_flow.value.abs(),
            );

            let min_pressure_loss_at_specified_flow = f64::min(
                pressure_loss_at_specified_forward_flow.value.abs(),
                pressure_loss_at_specified_backward_flow.value.abs(),
            );
            ...
            if max_pressure_loss_at_specified_flow > min_pressure_loss_at_specified_flow * 1000.0 {
                return Ok(true);
            }
        ...
        let check_valve_behaviour_result =
            check_for_check_valve_behaviour(MassRate::new::<kilogram_per_second>(0.01));

        // now i should try to unwrap this result, if i cannot, then panic

        let check_valve_behaviour_exists: bool = check_valve_behaviour_result.unwrap();
        ...
        let forward_flow_true: bool = user_specified_pressure_loss_pascals > 0.0;
        ...
        if check_valve_behaviour_exists {
            ...
            let forward_bias: bool = {
                ...
                if pressure_loss_at_specified_backward_flow
                    .value
                    .abs()
                    .is_nan()
                {
                ...
                if pressure_loss_at_specified_backward_flow.value.abs() == f64::MAX {
                    forward_bias = true;
                }

                if pressure_loss_at_specified_backward_flow.value.abs() == f64::INFINITY {
                    forward_bias = true;
                }
...
pub trait FluidComponentSuperCollectionParallelAssociatedFunctions {
    ...
    fn calculate_mass_flowrate_from_pressure_change(
        ...
        for (index, fluid_component_pointer) in fluid_component_collection_vector.iter().enumerate()
        {
            ...
            let fluid_component_mass_flowrate =
                fluid_component.get_mass_flowrate_from_pressure_change(pressure_change);
    ...
    fn calculate_pressure_change_from_mass_flowrate(
        ...
        if external_circulation_dominant {
            ...
            let number_of_branches: f64 = fluid_component_collection_vector.len() as f64;
    ...
    fn calculate_maximum_pressure_difference_between_branches(
        ...
        pressure_changes_for_all_branches_pascals = fluid_component_collection_vector
            ...
            .map(|branch: &FluidComponentCollection| {
                ...
                branch
                    .get_pressure_change(individual_branch_guess_average_mass_flowrate)
                    .get::<pascal>()
            })
            .collect();

        let max_pressure_change_pascals: f64 = pressure_changes_for_all_branches_pascals.max();

        let min_pressure_change_pascals: f64 = pressure_changes_for_all_branches_pascals.min();
    ...
    fn calculate_maximum_mass_flowrate_given_pressure_drop_across_each_branch(
        ...
        abs_mass_flowrate_for_all_branches_kg_per_s = fluid_component_collection_vector
            ...
            .map(|branch: &FluidComponentCollection| {
                branch
                    .get_mass_flowrate_from_pressure_loss(pressure_drop)
        ...
        let max_mass_flowrate_magnitude_kilogram_per_s: f64 =
            abs_mass_flowrate_for_all_branches_kg_per_s.max();

        // return mass flowrate as a dimensioned quantity

        MassRate::new::<kilogram_per_second>(max_mass_flowrate_magnitude_kilogram_per_s)
    }
    ...
    fn calculate_pressure_change_using_guessed_branch_mass_flowrate(
        ...
        let pressure_change_forward_and_backward_est_vector: Vec<Pressure> = [
            pressure_change_est_vector_forward_direction,
            pressure_change_est_vector_backward_direction,
        ]
        .concat();
        ...
        let mut pressure_change_pascals_result_user_specified_flow = find_root_brent(
        ...
        pressure_change_pascals_result_user_specified_flow = find_root_regula_falsi(
            user_specified_pressure_upper_bound.value,
            user_specified_pressure_lower_bound.value,
            &pressure_change_from_mass_flowrate_root,
            &mut convergency,
    ...
    fn obtain_pressure_estimate_vector(
            ...
            fluid_component_collection_vector.iter().enumerate()
        {
            ...
            let fluid_component_collection = &*fluid_component_collection_pointer;
    ...
    fn obtain_pressure_loss_estimate_vector(
        ...
        for (index, fluid_component_pointer) in fluid_component_collection_vector.iter().enumerate()
        {
            ...
            let fluid_component_pressure_loss = fluid_component.get_pressure_loss(mass_flowrate);
