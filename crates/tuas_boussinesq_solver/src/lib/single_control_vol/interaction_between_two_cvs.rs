...
impl SingleCVNode {
    ...
    pub fn calculate_conductance_interaction_to_front_singular_cv_node(
        ...
        let single_cv_1_enthalpy = self.current_timestep_control_volume_specific_enthalpy;
        let single_cv_2_enthalpy = single_cv_2.current_timestep_control_volume_specific_enthalpy;
        ...
        let cv_2_temp_minus_cv_1_temp_kelvin: f64 =
            single_cv_2_temperature.get::<kelvin>() - single_cv_1_temperature.get::<kelvin>();

        let cv_2_temp_minus_cv_1: TemperatureInterval = TemperatureInterval::new::<
            uom::si::temperature_interval::kelvin,
        >(cv_2_temp_minus_cv_1_temp_kelvin);

        let heat_flowrate_from_cv_1_to_cv_2: Power = -thermal_conductance * cv_2_temp_minus_cv_1;
    ...
    pub fn calculate_advection_interaction_to_front_singular_cv_node(
        ...
        let heat_flowrate_from_cv_1_to_cv_2: Power = advection_heat_rate(
            mass_flow_from_cv_1_to_cv_2,
        ...
        self.volumetric_flowrate_vector.push(-volumetric_flowrate);
        single_cv_2
            .volumetric_flowrate_vector
            .push(volumetric_flowrate);
    ...
    pub fn calculate_mesh_stability_timestep_for_two_single_cv_nodes(
        ...
        let temperature_1: ThermodynamicTemperature = self.temperature;

        let temperature_2: ThermodynamicTemperature = single_cv_2.temperature;

        let pressure_1: Pressure = self.pressure_control_volume;

        let pressure_2: Pressure = single_cv_2.pressure_control_volume;
        ...
        let alpha_1: DiffusionCoefficient =
            try_get_alpha_thermal_diffusivity(material_1, temperature_1, pressure_1)?;

        let alpha_2: DiffusionCoefficient =
            try_get_alpha_thermal_diffusivity(material_2, temperature_2, pressure_2)?;

        // we also want to get the ratios of the two thermal inertias

        let cp_1: SpecificHeatCapacity = try_get_cp(material_1, temperature_1, pressure_1)?;
        let cp_2: SpecificHeatCapacity = try_get_cp(material_2, temperature_2, pressure_2)?;
        ...
        let mut cv_1_timestep: Time = self.calculate_conduction_timestep()?;

        let mut cv_2_timestep: Time = single_cv_2.calculate_conduction_timestep()?;
        ...
        match interaction {
            HeatTransferInteractionType::UserSpecifiedThermalConductance(
                ...
                let lengthscale_stability_vec_1 = self.mesh_stability_lengthscale_vector.clone();
                ...
                let approx_thermal_resistance_1 = total_resistance * heat_capacity_ratio_cv_1;

                let approx_thermal_cond_1: ThermalConductance = 1.0 / approx_thermal_resistance_1;
                ...
                let approx_thermal_resistance_2 = total_resistance * heat_capacity_ratio_cv_2;

                let approx_thermal_cond_2: ThermalConductance = 1.0 / approx_thermal_resistance_2;
                ...
                let density_1: MassDensity = try_get_rho(material_1, temperature_1, pressure_1)?;

                let density_2: MassDensity = try_get_rho(material_2, temperature_2, pressure_2)?;

                let rho_cp_1: VolumetricHeatCapacity = density_1 * cp_1;
                let rho_cp_2: VolumetricHeatCapacity = density_2 * cp_2;

                let approx_alpha_1: DiffusionCoefficient = approx_thermal_conductivity_1 / rho_cp_1;

                let approx_alpha_2: DiffusionCoefficient = approx_thermal_conductivity_2 / rho_cp_2;
                ...
                let timescale_1: Time =
                    max_mesh_fourier_number * min_lengthscale * min_lengthscale / approx_alpha_1;

                let timescale_2: Time =
                    max_mesh_fourier_number * min_lengthscale * min_lengthscale / approx_alpha_2;
            ...
            HeatTransferInteractionType::SingleCartesianThermalConductanceOneDimension(
                ...
                let thickness_cv_1: Length = heat_capacity_ratio_cv_1 * conduction_thickness;

                let thickness_cv_2: Length = heat_capacity_ratio_cv_2 * conduction_thickness;
                ...
                let timescale_1: Time =
                    max_mesh_fourier_number * thickness_cv_1 * thickness_cv_1 / alpha_1;

                let timescale_2: Time =
                    max_mesh_fourier_number * thickness_cv_2 * thickness_cv_2 / alpha_2;
            ...
            HeatTransferInteractionType::CylindricalConductionConvectionLiquidInside(
                (_solid_material, shell_thickness, _solid_temperature, _solid_pressure),
                ...
                let mut liquid_thermal_conductivity: ThermalConductivity =
                    try_get_kappa_thermal_conductivity(material_1, temperature_1, pressure_1)?;
                ...
                let nusselt_number_diameter: Ratio = h * id / liquid_thermal_conductivity;

                let nusselt_number_diameter: f64 = nusselt_number_diameter.value;
                ...
                let liquid_timescale: Time =
                    max_mesh_fourier_number * liquid_lengthscale * liquid_lengthscale
                        / alpha_liquid
                ...
                let solid_timescale: Time =
                    max_mesh_fourier_number * solid_lengthscale * solid_lengthscale / alpha_solid;
            ...
            HeatTransferInteractionType::CylindricalConductionConvectionLiquidOutside(
                ...
                let liquid_lengthscale: Length = solid_lengthscale * liquid_mcp / solid_mcp;
                ...
                let nusselt_number: Ratio = h * liquid_lengthscale / liquid_thermal_conductivity;
            ...
            HeatTransferInteractionType::DualCylindricalThermalConductance(
                ...
                (inner_diameter, outer_diameter, _cylinder_length),
            ) => {
                ...
                let alpha_inner: DiffusionCoefficient = try_get_alpha_thermal_diffusivity(
                    inner_shell_material,
                    inner_shell_temperature,
                    inner_shell_pressure,
                )?;

                let alpha_outer: DiffusionCoefficient = try_get_alpha_thermal_diffusivity(
                ...
                let inner_shell_timescale: Time =
                    max_mesh_fourier_number * inner_thickness * inner_thickness / alpha_inner;

                let outer_shell_timescale: Time =
                    max_mesh_fourier_number * outer_thickness * outer_thickness / alpha_outer;
            ...
            HeatTransferInteractionType::DualCartesianThermalConductance(
                ...
                let mut larger_alpha: DiffusionCoefficient = alpha_1;
                ...
                let interaction_minimum_timescale: Time =
                    max_mesh_fourier_number * smaller_lengthscale * smaller_lengthscale
                        / larger_alpha;
            ...
            HeatTransferInteractionType::SimpleRadiation(_area_coeff) => {
