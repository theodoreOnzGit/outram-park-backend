...
impl SingleCVNode {
    ...
    pub fn calculate_conduction_timestep(&self) -> Result<Time, TuasLibError> {
        ...
        let lengthscale_stability_vec = self.mesh_stability_lengthscale_vector.clone();
        ...
        let thermal_diffusivity_coeff: DiffusionCoefficient = try_get_alpha_thermal_diffusivity(
        ...
        let min_conduction_timescale =
            max_mesh_fourier_number * min_lengthscale * min_lengthscale / thermal_diffusivity_coeff;
    ...
    #[inline]
    pub fn calculate_courant_number_timestep(
        &mut self,
        ...
        let cv_density: MassDensity = try_get_rho(
            *cv_material_reference,
            cv_temperature,
        ...
        let timestep: Time =
            max_courant_number * 2.0 * cv_volume / absolute_sum_of_volumetric_flowrate;
    ...
    #[inline]
    pub fn append_conduction_mesh_stability_timestep(
        &mut self,
        lengthscale: Length,
    ) -> Result<Time, TuasLibError> {
        // now we can determine the conduction time scale regardless
        ...
        let min_conduction_timescale =
            max_mesh_fourier_number * lengthscale * lengthscale / thermal_diffusivity_coeff;
    ...
    fn get_max_timestep_based_on_max_temperature_change(
        ...
        for enthalpy_chg_rate in self.rate_enthalpy_change_vector.clone().iter() {
        ...
        let cv_heat_capacity = try_get_cp(cv_material, cv_temperature, cv_pressure)?;

        // now we can calculate time step

        let time_step_based_on_max_temperature_change =
            cv_mass_clone * cv_heat_capacity * max_temperature_change / total_enthalpy_rate_change;
    ...
    pub fn get_max_timestep(
        ...
        let max_temperature_change_timestep: Time =
            self.get_max_timestep_based_on_max_temperature_change(max_temperature_change)?;

        self.max_timestep_vector
            .push(max_temperature_change_timestep);
