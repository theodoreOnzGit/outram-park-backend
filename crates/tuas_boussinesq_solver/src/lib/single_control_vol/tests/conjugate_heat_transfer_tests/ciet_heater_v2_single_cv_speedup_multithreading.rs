...
pub fn ciet_heater_v_2_0_test_steady_state_v_1_1_speedup_threads() {
    ...
    let initial_temperature = ThermodynamicTemperature::new::<degree_celsius>(80.0);
    let atmospheric_pressure = Pressure::new::<atmosphere>(1.0);

    let flow_area = Area::new::<square_meter>(0.00105);
    let number_of_nodes: usize = 8;
    let ambient_air_temp = ThermodynamicTemperature::new::<degree_celsius>(21.67);
    ...
    fn construct_heated_section_fluid_nodes(
        therminol: Material,
        ...
        for _index in 0..number_of_nodes {
            let therminol_node: HeatTransferEntity = SingleCVNode::new_odd_shaped_pipe(
                node_length,
    ...
    let fluid_node_vec: Vec<HeatTransferEntity> = construct_heated_section_fluid_nodes(
        therminol,
        flow_area,
        heated_length,
        initial_temperature,
    ...
    fn construct_steel_shell_nodes(
        steel: Material,
        id: Length,
        od: Length,
        heated_length: Length,
    ...
    let midway_point_steel_shell: Length = Length::new::<meter>(0.0392);

    let steel_shell_inner_node_vec: Vec<HeatTransferEntity> = construct_steel_shell_nodes(
        steel,
        id,
    ...
    let steel_shell_outer_node_vec: Vec<HeatTransferEntity> = construct_steel_shell_nodes(
        steel,
        midway_point_steel_shell,
        od,
        heated_length,
    ...
    let fluid_node_vec_ptr = Arc::new(Mutex::new(fluid_node_vec));

    let steel_shell_inner_node_vec_ptr = Arc::new(Mutex::new(steel_shell_inner_node_vec));

    let steel_shell_outer_node_vec_ptr = Arc::new(Mutex::new(steel_shell_outer_node_vec));

    // need two boundary conditions

    let inlet_const_temp = HeatTransferEntity::BoundaryConditions(
        BCType::UserSpecifiedTemperature(ThermodynamicTemperature::new::<degree_celsius>(79.12)),
    );
    ...
    let ambient_temperature_bc =
        HeatTransferEntity::BoundaryConditions(BCType::UserSpecifiedTemperature(ambient_air_temp));

    let inlet_const_temp_ptr = Arc::new(Mutex::new(inlet_const_temp));

    let outlet_zero_heat_flux_ptr = Arc::new(Mutex::new(outlet_zero_heat_flux));

    let ambient_air_temp_bc_ptr = Arc::new(Mutex::new(ambient_temperature_bc));
    ...
    let calculation_loop = move || {
        ...
        let mut wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "par_trial_ciet_heater_v_2_0_steady_state.csv",
        ))
        .unwrap();
        ...
        let mut time_wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "par_trial_ciet_heater_v_2_0_steady_state_time.csv",
        ))
        .unwrap();

        time_wtr
            .write_record(&[
                "loop_calculation_time_ms",
                "mutex_lock_frac",
        ...
        let mut temp_profile_wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "par_trial_ciet_heater_v_2_0_steady_state_temp_profile.csv",
        ))
        .unwrap();
        ...
        // this is code for writing the array of required temperatures
        {
            ...
            for index in 0..number_of_nodes {
                let mid_node_length: Length = index as f64 * node_length + half_node_length;

                let prefix: String = "heater_temp_celsius_at_".to_string();

                let suffix: String = "_cm".to_string();

                let mid_node_length_cm: f64 = mid_node_length.get::<centimeter>();

                let mid_node_length_string: String = mid_node_length_cm.to_string();
        ...
        while current_time_simulation_time <= *max_time_ptr_in_loop {
            ...
            let mut inlet_const_temp_in_loop = inlet_const_temp_ptr.lock().unwrap();
            let mut outlet_zero_heat_flux_in_loop = outlet_zero_heat_flux_ptr.lock().unwrap();

            let mut fluid_vec_in_loop = fluid_node_vec_ptr.lock().unwrap();
            ...
            let arc_mutex_lock_elapsed_ms = arc_mutex_lock_start.elapsed().unwrap().as_millis();
            ...
            let fluid_node_0_ref_parallel = Arc::new(Mutex::new(fluid_node_0_clone));
            let fluid_node_1_ref_parallel = Arc::new(Mutex::new(fluid_node_1_clone));
            let fluid_node_2_ref_parallel = Arc::new(Mutex::new(fluid_node_2_clone));
            let fluid_node_3_ref_parallel = Arc::new(Mutex::new(fluid_node_3_clone));
            let fluid_node_4_ref_parallel = Arc::new(Mutex::new(fluid_node_4_clone));
            let fluid_node_5_ref_parallel = Arc::new(Mutex::new(fluid_node_5_clone));
            let fluid_node_6_ref_parallel = Arc::new(Mutex::new(fluid_node_6_clone));
            let fluid_node_7_ref_parallel = Arc::new(Mutex::new(fluid_node_7_clone));

            // ensure that there is a second pointer to the same
            // data
            let fluid_node_0_ref_to_obtain_data = fluid_node_0_ref_parallel.clone();

            let fluid_node_1_ref_to_obtain_data = fluid_node_1_ref_parallel.clone();

            let fluid_node_2_ref_to_obtain_data = fluid_node_2_ref_parallel.clone();

            let fluid_node_3_ref_to_obtain_data = fluid_node_3_ref_parallel.clone();

            let fluid_node_4_ref_to_obtain_data = fluid_node_4_ref_parallel.clone();

            let fluid_node_5_ref_to_obtain_data = fluid_node_5_ref_parallel.clone();

            let fluid_node_6_ref_to_obtain_data = fluid_node_6_ref_parallel.clone();

            let fluid_node_7_ref_to_obtain_data = fluid_node_7_ref_parallel.clone();

            // repeat for the steel inner nodes

            let steel_inner_node_0_clone = steel_shell_inner_node_vec_in_loop[0].clone();
            let steel_inner_node_1_clone = steel_shell_inner_node_vec_in_loop[1].clone();
            let steel_inner_node_2_clone = steel_shell_inner_node_vec_in_loop[2].clone();
            let steel_inner_node_3_clone = steel_shell_inner_node_vec_in_loop[3].clone();
            let steel_inner_node_4_clone = steel_shell_inner_node_vec_in_loop[4].clone();
            let steel_inner_node_5_clone = steel_shell_inner_node_vec_in_loop[5].clone();
            let steel_inner_node_6_clone = steel_shell_inner_node_vec_in_loop[6].clone();
            let steel_inner_node_7_clone = steel_shell_inner_node_vec_in_loop[7].clone();

            let steel_inner_node_0_ref = Arc::new(Mutex::new(steel_inner_node_0_clone));
            let steel_inner_node_1_ref = Arc::new(Mutex::new(steel_inner_node_1_clone));
            let steel_inner_node_2_ref = Arc::new(Mutex::new(steel_inner_node_2_clone));
            let steel_inner_node_3_ref = Arc::new(Mutex::new(steel_inner_node_3_clone));
            let steel_inner_node_4_ref = Arc::new(Mutex::new(steel_inner_node_4_clone));
            let steel_inner_node_5_ref = Arc::new(Mutex::new(steel_inner_node_5_clone));
            let steel_inner_node_6_ref = Arc::new(Mutex::new(steel_inner_node_6_clone));
            let steel_inner_node_7_ref = Arc::new(Mutex::new(steel_inner_node_7_clone));

            // create clone references for use in parallelism

            let steel_inner_node_0_ref_for_parallel = steel_inner_node_0_ref.clone();
            let steel_inner_node_1_ref_for_parallel = steel_inner_node_1_ref.clone();
            let steel_inner_node_2_ref_for_parallel = steel_inner_node_2_ref.clone();
            let steel_inner_node_3_ref_for_parallel = steel_inner_node_3_ref.clone();
            let steel_inner_node_4_ref_for_parallel = steel_inner_node_4_ref.clone();
            let steel_inner_node_5_ref_for_parallel = steel_inner_node_5_ref.clone();
            let steel_inner_node_6_ref_for_parallel = steel_inner_node_6_ref.clone();
            let steel_inner_node_7_ref_for_parallel = steel_inner_node_7_ref.clone();

            // and finally for the outer shell, do the same

            let steel_outer_node_0_ref =
                Arc::new(Mutex::new(steel_shell_outer_node_vec_in_loop[0].clone()));
            let steel_outer_node_1_ref =
                Arc::new(Mutex::new(steel_shell_outer_node_vec_in_loop[1].clone()));
            let steel_outer_node_2_ref =
                Arc::new(Mutex::new(steel_shell_outer_node_vec_in_loop[2].clone()));
            let steel_outer_node_3_ref =
                Arc::new(Mutex::new(steel_shell_outer_node_vec_in_loop[3].clone()));
            let steel_outer_node_4_ref =
                Arc::new(Mutex::new(steel_shell_outer_node_vec_in_loop[4].clone()));
            let steel_outer_node_5_ref =
                Arc::new(Mutex::new(steel_shell_outer_node_vec_in_loop[5].clone()));
            let steel_outer_node_6_ref =
                Arc::new(Mutex::new(steel_shell_outer_node_vec_in_loop[6].clone()));
            let steel_outer_node_7_ref =
                Arc::new(Mutex::new(steel_shell_outer_node_vec_in_loop[7].clone()));

            // now create references for the parallel threads

            let steel_outer_node_0_ref_for_parallel = steel_outer_node_0_ref.clone();
            let steel_outer_node_1_ref_for_parallel = steel_outer_node_1_ref.clone();
            let steel_outer_node_2_ref_for_parallel = steel_outer_node_2_ref.clone();
            let steel_outer_node_3_ref_for_parallel = steel_outer_node_3_ref.clone();
            let steel_outer_node_4_ref_for_parallel = steel_outer_node_4_ref.clone();
            let steel_outer_node_5_ref_for_parallel = steel_outer_node_5_ref.clone();
            let steel_outer_node_6_ref_for_parallel = steel_outer_node_6_ref.clone();
            let steel_outer_node_7_ref_for_parallel = steel_outer_node_7_ref.clone();

            // heater power

            let node_heater_power: Power;
            {
                node_heater_power = heater_power / 2 as f64 / fluid_vec_in_loop.len() as f64;
            }

            // ambient air bc, 4 clones to make
            // one needed for each thread
            let ambient_air_temp_bc_ptr_for_parallel_0 = ambient_air_temp_bc_ptr.clone();
            let ambient_air_temp_bc_ptr_for_parallel_1 = ambient_air_temp_bc_ptr.clone();
            let ambient_air_temp_bc_ptr_for_parallel_2 = ambient_air_temp_bc_ptr.clone();
            let ambient_air_temp_bc_ptr_for_parallel_3 = ambient_air_temp_bc_ptr.clone();
            let ambient_air_temp_bc_ptr_for_parallel_4 = ambient_air_temp_bc_ptr.clone();
            let ambient_air_temp_bc_ptr_for_parallel_5 = ambient_air_temp_bc_ptr.clone();
            let ambient_air_temp_bc_ptr_for_parallel_6 = ambient_air_temp_bc_ptr.clone();
            let ambient_air_temp_bc_ptr_for_parallel_7 = ambient_air_temp_bc_ptr.clone();

            let thread_0 = thread::spawn(move || {
                // thread 0 connects nodes at node 0
                let radial_thickness: Length = (midway_point_steel_shell - id) * 0.5;

                // fluid to inner shell
                connect_fluid_and_steel_inner_node(
                    fluid_node_0_ref_parallel.lock().unwrap().deref_mut(),
                    steel_inner_node_0_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    radial_thickness,
                ...
                link_inner_shell_to_outer_shell(
                    steel_outer_node_0_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    steel_inner_node_0_ref_for_parallel
                ...
                link_outer_shell_to_ambient_temperature(
                    steel_outer_node_0_ref_for_parallel
                        ...
                        .unwrap()
                        .deref_mut(),
                    ambient_air_temp_bc_ptr_for_parallel_0
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    od,
            ...
            let thread_1 = thread::spawn(move || {
                ...
                connect_fluid_and_steel_inner_node(
                    ...
                    steel_inner_node_1_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    radial_thickness,
                ...
                link_inner_shell_to_outer_shell(
                    steel_outer_node_1_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    steel_inner_node_1_ref_for_parallel
                ...
                link_outer_shell_to_ambient_temperature(
                    steel_outer_node_1_ref_for_parallel
                        ...
                        .unwrap()
                        .deref_mut(),
                    ambient_air_temp_bc_ptr_for_parallel_1
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    od,
            ...
            let thread_2 = thread::spawn(move || {
                ...
                connect_fluid_and_steel_inner_node(
                    ...
                    steel_inner_node_2_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    radial_thickness,
                ...
                // inner shell to outer shell
                link_inner_shell_to_outer_shell(
                    steel_outer_node_2_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    steel_inner_node_2_ref_for_parallel
                ...
                link_outer_shell_to_ambient_temperature(
                    steel_outer_node_2_ref_for_parallel
                        ...
                        .unwrap()
                        .deref_mut(),
                    ambient_air_temp_bc_ptr_for_parallel_2
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    od,
            ...
            let thread_3 = thread::spawn(move || {
                ...
                connect_fluid_and_steel_inner_node(
                    ...
                    steel_inner_node_3_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    radial_thickness,
                ...
                // inner shell to outer shell
                link_inner_shell_to_outer_shell(
                    steel_outer_node_3_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    steel_inner_node_3_ref_for_parallel
                ...
                link_outer_shell_to_ambient_temperature(
                    steel_outer_node_3_ref_for_parallel
                        ...
                        .unwrap()
                        .deref_mut(),
                    ambient_air_temp_bc_ptr_for_parallel_3
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    od,
            ...
            let thread_4 = thread::spawn(move || {
                ...
                connect_fluid_and_steel_inner_node(
                    ...
                    steel_inner_node_4_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    radial_thickness,
                ...
                // inner shell to outer shell
                link_inner_shell_to_outer_shell(
                    steel_outer_node_4_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    steel_inner_node_4_ref_for_parallel
                ...
                link_outer_shell_to_ambient_temperature(
                    steel_outer_node_4_ref_for_parallel
                        ...
                        .unwrap()
                        .deref_mut(),
                    ambient_air_temp_bc_ptr_for_parallel_4
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    od,
            ...
            let thread_5 = thread::spawn(move || {
                ...
                connect_fluid_and_steel_inner_node(
                    ...
                    steel_inner_node_5_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    radial_thickness,
                ...
                // inner shell to outer shell
                link_inner_shell_to_outer_shell(
                    steel_outer_node_5_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    steel_inner_node_5_ref_for_parallel
                ...
                link_outer_shell_to_ambient_temperature(
                    steel_outer_node_5_ref_for_parallel
                        ...
                        .unwrap()
                        .deref_mut(),
                    ambient_air_temp_bc_ptr_for_parallel_5
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    od,
            ...
            let thread_6 = thread::spawn(move || {
                ...
                connect_fluid_and_steel_inner_node(
                    ...
                    steel_inner_node_6_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    radial_thickness,
                ...
                // inner shell to outer shell
                link_inner_shell_to_outer_shell(
                    steel_outer_node_6_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    steel_inner_node_6_ref_for_parallel
                ...
                link_outer_shell_to_ambient_temperature(
                    steel_outer_node_6_ref_for_parallel
                        ...
                        .unwrap()
                        .deref_mut(),
                    ambient_air_temp_bc_ptr_for_parallel_6
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    od,
            ...
            let thread_7 = thread::spawn(move || {
                ...
                connect_fluid_and_steel_inner_node(
                    ...
                    steel_inner_node_7_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    radial_thickness,
                ...
                // inner shell to outer shell
                link_inner_shell_to_outer_shell(
                    steel_outer_node_7_ref_for_parallel
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    steel_inner_node_7_ref_for_parallel
                ...
                link_outer_shell_to_ambient_temperature(
                    steel_outer_node_7_ref_for_parallel
                        ...
                        .unwrap()
                        .deref_mut(),
                    ambient_air_temp_bc_ptr_for_parallel_7
                        .lock()
                        .unwrap()
                        .deref_mut(),
                    od,
...

            {
                ...
                let therminol_inlet_density =
                    try_get_rho(therminol, therminol_inlet_temperature, atmospheric_pressure)
                        .unwrap();

                // i need to borrow the first and last indexed vector
                // mutably, so I split again

                let (vec_slice_one, vec_slice_two) = fluid_vec_in_loop.split_at_mut(1);

                let mut inlet_node: &mut HeatTransferEntity = vec_slice_one.first_mut().unwrap();

                let mut outlet_node: &mut HeatTransferEntity = vec_slice_two.last_mut().unwrap();

                // we now need inlet and outlet node densities

                let inlet_node_density_vec =
                    HeatTransferEntity::density_vector(inlet_node.deref_mut()).unwrap();

                let inlet_node_density: MassDensity = inlet_node_density_vec[0];

                let outlet_node_density_vec =
                    HeatTransferEntity::density_vector(outlet_node.deref_mut()).unwrap();

                let outlet_node_density: MassDensity = outlet_node_density_vec[0];
                ...
                let inlet_interaction =
                    HeatTransferInteractionType::Advection(inlet_advection_dataset);
                let outlet_interaction =
                    HeatTransferInteractionType::Advection(outlet_advection_dataset);
            ...
            let node_connection_end_ms = node_connection_start.elapsed().unwrap().as_millis();
            ...
            // todo:
            {
                ...
                let current_time_string = current_time_simulation_time.get::<second>().to_string();

                // next simulation time string
                let elapsed_calc_time_seconds_string = calculation_time_elapsed
                    .elapsed()
                    .unwrap()
                    .as_secs()
                    .to_string();
                ...
                for outer_shell_ptr in steel_shell_outer_node_vec_in_loop.iter_mut() {
                    ...
                    let node_temp: ThermodynamicTemperature =
                        HeatTransferEntity::temperature(outer_shell_ptr).unwrap();

                    // get it in degc

                    let node_temp_deg_c: f64 = node_temp.get::<degree_celsius>();

                    // convert to string and push to vector

                    let node_temp_c_string: String = node_temp_deg_c.to_string();
                ...
                // now write
                temp_profile_wtr
                    .write_record(&temp_profile_data_vec)
                    .unwrap();
            }
            ...
            // todo:
            {
                ...
                let therminol_outlet_temp_string =
                    HeatTransferEntity::temperature(thermoinol_outlet_node)
                        .unwrap()
                        .get::<degree_celsius>()
                        .to_string();

                let shell_celsius_string = HeatTransferEntity::temperature(st_11_node)
                    .unwrap()
                    .get::<degree_celsius>()
                    .to_string();
                ...
                for therminol_node in fluid_vec_in_loop.iter_mut() {
                    ...
                    let local_timestep: Time = therminol_node_clone_single_cv
                        .get_max_timestep(TemperatureInterval::new::<
                ...
                let heater_power_kilowatt_string = heater_power.get::<kilowatt>().to_string();
            ...
            // or else some other parallelism here
            {
                ...
                for therminol_node in fluid_vec_in_loop.iter_mut() {
                    HeatTransferEntity::advance_timestep(therminol_node, auto_calculated_timestep)
                        .unwrap();
                }
            ...
            let timestep_advance_end_ms = timestep_advance_start.elapsed().unwrap().as_millis();

            // write timestep diagnostics

            let total_time_ms = arc_mutex_lock_elapsed_ms
                + node_connection_end_ms
                + data_recording_time_end_ms
                + timestep_advance_end_ms;

            let mutex_lock_frac: f64 = arc_mutex_lock_elapsed_ms as f64 / total_time_ms as f64;

            let node_connection_frac: f64 = node_connection_end_ms as f64 / total_time_ms as f64;

            let data_record_frac: f64 = data_recording_time_end_ms as f64 / total_time_ms as f64;

            let timestep_advance_frac: f64 = timestep_advance_end_ms as f64 / total_time_ms as f64;
...
pub fn ciet_heater_v_2_0_test_steady_state_functional_test_v_1_1() {
    ...
    let calculation_loop = move || {
        ...
        let mut wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "par_trial_functional_ciet_heater_v_2_0_steady_state.csv",
        ))
        .unwrap();
        ...
        let mut time_wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "par_trial_functional_ciet_heater_v_2_0_steady_state_time.csv",
        ))
        .unwrap();
        ...
        let mut temp_profile_wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "par_trial_functional_ciet_heater_v_2_0_steady_state_temp_profile.csv",
        ))
        .unwrap();
        ...
        while current_time_simulation_time <= *max_time_ptr_in_loop {
            ...
            let mut ambient_air_temp_bc_in_loop = ambient_air_temp_bc_ptr.lock().unwrap();
            ...
            fn connect_fluid_and_steel_inner_node(
                ...
                let steel_inner_cylindrical_node_temp: ThermodynamicTemperature =
                    HeatTransferEntity::temperature(steel_inner_node).unwrap();
                // now need to get heat transfer coeff

                let therminol_temp: ThermodynamicTemperature =
                    HeatTransferEntity::temperature(fluid_node).unwrap();

                let heat_trf_coeff: HeatTransfer = heat_transfer_coefficient_ciet_v_2_0(
                    therminol_mass_flowrate,
                    therminol_temp,
                    pressure,
                );

                let inner_diameter: InnerDiameterThermalConduction = id.clone().into();

                let node_length: Length = heated_length / (number_of_nodes as f64);

                let node_length: CylinderLengthThermalConduction = node_length.into();
                ...
                let interaction: HeatTransferInteractionType =
                    HeatTransferInteractionType::CylindricalConductionConvectionLiquidInside(
                        ...
                        (heat_trf_coeff, inner_diameter, node_length),
                    );

                // link the entities,
                // this is the fluid to the inner shell
                link_heat_transfer_entity(fluid_node, steel_inner_node, interaction).unwrap();
            }
                ...
                steel_shell_inner_node_vec_in_loop.iter_mut().enumerate()
            {
                ...
                let inner_radial_thickness: Length = 0.5 * (midway_point_steel_shell - id);

                let outer_radial_thickness: Length = 0.5 * (od - midway_point_steel_shell);
                ...
                let id_mid_inner_shell: Length = id + inner_radial_thickness.into();

                let id_mid_inner_shell: InnerDiameterThermalConduction = id_mid_inner_shell.into();

                let od_mid_inner_shell: Length = od - outer_radial_thickness.into();

                let od_mid_inner_shell: OuterDiameterThermalConduction = od_mid_inner_shell.into();
                ...
                let interaction = HeatTransferInteractionType::DualCylindricalThermalConductance(
                ...
                // link them together
                link_heat_transfer_entity(inner_shell_ptr, outer_shell_ptr, interaction).unwrap();
                ...
                steel_shell_outer_node_vec_in_loop.iter_mut().enumerate()
            {
                ...
                //
                let radial_thickness: Length = (od - midway_point_steel_shell) * 0.5;

                let radial_thickness: RadialCylindricalThicknessThermalConduction =
                    radial_thickness.into();

                let steel_outer_cylindrical_node_temp: ThermodynamicTemperature =
                    HeatTransferEntity::temperature(&mut steel_shell_inner_node_vec_in_loop[index])
                        .unwrap();
                ...
                let outer_diameter: OuterDiameterThermalConduction = od.clone().into();
                ...
                let interaction: HeatTransferInteractionType =
                    HeatTransferInteractionType::CylindricalConductionConvectionLiquidOutside(
                        ...
                        (heat_trf_coeff, outer_diameter, node_length),
                    );

                // link the entities,
                // this is the fluid to the inner shell
                link_heat_transfer_entity(
                    &mut ambient_air_temp_bc_in_loop,
                    outer_shell_ptr,
                    interaction,
                )
                .unwrap();
            ...
            for index in 0..fluid_vec_in_loop.len() - 1 {
                ...
                let (vec_slice_one, vec_slice_two) = fluid_vec_in_loop.split_at_mut(index + 1);

                let fluid_node_idx: &mut HeatTransferEntity = vec_slice_one.last_mut().unwrap();
                ...
                let fluid_node_idx_temperature: ThermodynamicTemperature =
                    HeatTransferEntity::temperature(fluid_node_idx).unwrap();

                let fluid_node_idx_plus_one_temperature: ThermodynamicTemperature =
                    HeatTransferEntity::temperature(fluid_node_idx_plus_one).unwrap();

                let fluid_node_idx_density =
                    try_get_rho(therminol, fluid_node_idx_temperature, atmospheric_pressure)
                        .unwrap();
                ...
                let mid_heater_advection_interaction =
                    HeatTransferInteractionType::Advection(mid_heater_advection_dataset);
                ...
                link_heat_transfer_entity(
                    ...
                    mid_heater_advection_interaction,
                )
                .unwrap();
            }
...
fn _analog_poresky_2017_power_signal(simulation_time: Time) -> Power {
    ...
    let mut angular_frequency_vector: Vec<AngularVelocity> = vec![];

    angular_frequency_vector.push(AngularVelocity::new::<radian_per_second>(0.002301));
    angular_frequency_vector.push(AngularVelocity::new::<radian_per_second>(0.02301));
    angular_frequency_vector.push(AngularVelocity::new::<radian_per_second>(0.2301));
    angular_frequency_vector.push(AngularVelocity::new::<radian_per_second>(2.301));
    angular_frequency_vector.push(AngularVelocity::new::<radian_per_second>(23.01));
    ...
    for angular_frequency_ptr in angular_frequency_vector.iter() {
        ...
        let power_component = steady_state_power + power_amplitude * phase_angle_sine;
...
fn _mfbs_power_signal_logspace_custom(simulation_time: Time) -> Power {
    ...
    let angular_frequency_vector_values: Vec<f64> = _logspace(0.001, 1_f64, 15);

    let mut angular_frequency_vector: Vec<AngularVelocity> = vec![];

    for angular_freq_val in angular_frequency_vector_values {
        angular_frequency_vector.push(AngularVelocity::new::<radian_per_second>(angular_freq_val));
    }
...
#[inline]
fn ciet_heater_v_2_0_nusselt_number(reynolds: Ratio, prandtl: Ratio) -> Ratio {
    let reynolds_power_0_836 = reynolds.value.powf(0.836);
    let prandtl_power_0_333 = prandtl.value.powf(0.333333333333333);

    Ratio::new::<ratio>(0.04179 * reynolds_power_0_836 * prandtl_power_0_333)
}

#[inline]
fn ciet_heater_v_2_0_reynolds_nunber(mass_flowrate: MassRate, mu: DynamicViscosity) -> Ratio {
    // Re = m* D_H/ A_{XS}/mu
...
fn heat_transfer_coefficient_ciet_v_2_0(
    ...
    let therminol = Material::Liquid(LiquidMaterial::TherminolVP1);
    let mu: DynamicViscosity =
        try_get_mu_viscosity(therminol, therminol_temperature, pressure).unwrap();

    let k: ThermalConductivity =
        try_get_kappa_thermal_conductivity(therminol, therminol_temperature, pressure).unwrap();

    let reynolds: Ratio = ciet_heater_v_2_0_reynolds_nunber(mass_flowrate, mu);

    let prandtl: Ratio = try_get_prandtl(therminol, therminol_temperature, pressure).unwrap();

    let nusselt: Ratio = ciet_heater_v_2_0_nusselt_number(reynolds, prandtl);

    let hydraulic_diameter = Length::new::<meter>(0.01467);

    let heat_transfer_coeff: HeatTransfer = nusselt * k / hydraulic_diameter;
...
fn add_heater_power_to_shell(
    ...
    let heat_addition_interaction = HeatTransferInteractionType::UserSpecifiedHeatAddition;
...
fn connect_fluid_and_steel_inner_node(
    ...
    let radial_thickness: RadialCylindricalThicknessThermalConduction = radial_thickness.into();
    ...
    let heat_trf_coeff: HeatTransfer =
        heat_transfer_coefficient_ciet_v_2_0(therminol_mass_flowrate, therminol_temp, pressure);
...
fn link_outer_shell_to_ambient_temperature(
    ...
    let heat_trf_coeff: HeatTransfer = HeatTransfer::new::<watt_per_square_meter_kelvin>(20.0);
    ...
    // link the entities,
    // this is the fluid to the inner shell
    link_heat_transfer_entity(ambient_air_temp_bc_in_loop, outer_shell_ptr, interaction).unwrap();
}
fn link_mid_heater_nodes_via_advection(
    ...
    let fluid_node_idx_temperature: ThermodynamicTemperature =
        HeatTransferEntity::temperature(fluid_node_left).unwrap();

    let fluid_node_idx_plus_one_temperature: ThermodynamicTemperature =
        HeatTransferEntity::temperature(fluid_node_right).unwrap();

    let fluid_node_idx_density =
        try_get_rho(therminol, fluid_node_idx_temperature, atmospheric_pressure).unwrap();
