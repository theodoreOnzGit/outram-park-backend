...
use crate::heat_transfer_correlations::heat_transfer_interactions::heat_transfer_interaction_enums::DataUserSpecifiedConvectionResistance;
...
pub fn one_dimension_ciet_heater_v_1_0_test_for_better_sampling_frequency() {
    ...
    let therminol_cylinder: HeatTransferEntity = SingleCVNode::new_cylindrical_shell(
        total_length,
        inner_tube_od.into(),
        id.into(),
        therminol,
    ...
    let steel_shell: HeatTransferEntity = SingleCVNode::new_cylindrical_shell(
        heated_length,
        id.into(),
        od.into(),
        steel,
    ...
    let therminol_cylinder_ptr = Arc::new(Mutex::new(therminol_cylinder));

    let steel_shell_ptr = Arc::new(Mutex::new(steel_shell));

    // need two boundary conditions

    let inlet_const_temp = HeatTransferEntity::BoundaryConditions(
        BCType::UserSpecifiedTemperature(ThermodynamicTemperature::new::<degree_celsius>(80.0)),
    );
    ...
    let convection_data = DataUserSpecifiedConvectionResistance {
        surf_area: SurfaceArea::from(Area::new::<square_centimeter>(2007_f64)),
        heat_transfer_coeff: HeatTransfer::new::<watt_per_square_meter_kelvin>(607_f64),
    };

    let convection_resistance =
        HeatTransferInteractionType::UserSpecifiedConvectionResistance(convection_data);
    ...
    //
    let test_temperature: ThermodynamicTemperature = ThermodynamicTemperature::ZERO;
    ...
    let calculation_loop = move || {
        ...
        while current_time_simulation_time <= *max_time_ptr_in_loop {
            ...
            let mut therminol_cylinder_in_loop = therminol_cylinder_ptr.lock().unwrap();
            let mut steel_shell_in_loop = steel_shell_ptr.lock().unwrap();
            ...
            let mut test_temperature_ptr_in_loop = test_temperature_ptr.lock().unwrap();

            // advection bc, so at boundary condition, therminol flows in at
            // 0.18 kg/s at 80C

            let therminol_inlet_temperature = ThermodynamicTemperature::new::<degree_celsius>(80.0);

            let therminol_inlet_density =
                try_get_rho(therminol, therminol_inlet_temperature, atmospheric_pressure).unwrap();

            // i can also calculate the densities of each cv

            let therminol_cv_density_vec =
                HeatTransferEntity::density_vector(therminol_cylinder_in_loop.deref_mut()).unwrap();

            let heater_fluid_cv_density: MassDensity = therminol_cv_density_vec[0];
            ...
            let inlet_interaction = HeatTransferInteractionType::Advection(inlet_advection_dataset);
            ...
            let _heater_power = mfbs_poresky_2017_power_signal(current_time_simulation_time);

            let heater_power_logspace =
                mfbs_power_signal_logspace_custom(current_time_simulation_time);

            let mut electrical_heat_bc: HeatTransferEntity =
                BCType::new_const_heat_addition(heater_power_logspace).into();
            ...
            let auto_calculated_timestep = therminol_cylinder_clone_cv
            ...
            *therminol_cylinder_in_loop.deref_mut() = therminol_cylinder_clone_cv.into();
            ...
            let heater_power_kilowatt_string = heater_power_logspace.get::<kilowatt>().to_string();

            let therminol_celsius_string =
                HeatTransferEntity::temperature(&mut therminol_cylinder_in_loop)
                    .unwrap()
                    .get::<degree_celsius>()
                    .to_string();

            let shell_celsius_string = HeatTransferEntity::temperature(&mut steel_shell_in_loop)
                .unwrap()
                .get::<degree_celsius>()
                .to_string();
            ...
            *test_temperature_ptr_in_loop.deref_mut() =
                HeatTransferEntity::temperature(&mut therminol_cylinder_in_loop).unwrap();

            // advance timestep for steel shell and therminol cylinder
            HeatTransferEntity::advance_timestep(&mut therminol_cylinder_in_loop, timestep_value)
                .unwrap();

            HeatTransferEntity::advance_timestep(&mut steel_shell_in_loop, timestep_value).unwrap();
    ...
    let last_timestep_temp: ThermodynamicTemperature = *test_temperature_ptr_clone.lock().unwrap();
...
pub fn one_dimension_ciet_heater_v_1_0_auto_timestep_test() {
    ...
    let calculation_loop = move || {
        ...
        let mut wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "one_dimension_ciet_cht_autotimestep_test.csv",
        ))
        .unwrap();
        ...
        while current_time_simulation_time <= *max_time_ptr_in_loop {
            ...
            let heater_power = mfbs_power_signal_logspace_custom(current_time_simulation_time);
...
pub fn one_dimension_ciet_heater_v_1_0_functional_test() {
    ...
    let calculation_loop = move || {
        ...
        let mut wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "one_dimension_ciet_cht_functional_test.csv",
        ))
        .unwrap();
        ...
        while current_time_simulation_time <= *max_time_ptr_in_loop {
            ...
            let heater_power = analog_poresky_2017_power_signal(current_time_simulation_time);
            ...
            *therminol_cylinder_in_loop.deref_mut() = therminol_cylinder_clone_cv.into();
            ...
            // used for debugging
            let _number_of_mass_flows = match &mut therminol_cylinder_in_loop.clone() {
                HeatTransferEntity::ControlVolume(single_cv) => match single_cv {
                    CVType::SingleCV(single_cv) => single_cv.volumetric_flowrate_vector.len(),
            ...
            let _courant_number_timestep = match &mut therminol_cylinder_in_loop.clone() {
                HeatTransferEntity::ControlVolume(single_cv) => match single_cv {
                    CVType::SingleCV(single_cv) => single_cv
                        .calculate_courant_number_timestep(Ratio::new::<ratio>(1.0))
                        .unwrap()
                        .get::<second>(),
                    CVType::FluidArrayCV(_) => todo!(),
                    CVType::SolidArrayCV(_) => todo!(),
...
fn mfbs_power_signal_logspace_custom(simulation_time: Time) -> Power {
    ...
    let angular_frequency_vector_values: Vec<f64> = logspace(0.001, 1_f64, 15);
