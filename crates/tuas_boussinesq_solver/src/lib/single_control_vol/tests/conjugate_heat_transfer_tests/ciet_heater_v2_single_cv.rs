...
use crate::boussinesq_thermophysical_properties::{LiquidMaterial, Material, SolidMaterial};
use crate::control_volume_dimensions::{
    CylinderLengthThermalConduction, InnerDiameterThermalConduction,
    OuterDiameterThermalConduction, RadialCylindricalThicknessThermalConduction,
};
use crate::heat_transfer_correlations::heat_transfer_interactions::heat_transfer_interaction_enums::{
    DataAdvection, HeatTransferInteractionType,
};
...
pub fn ciet_heater_v_2_0_test_steady_state_v_1_0() {
    ...
    let calculation_loop = move || {
        ...
        let mut wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "ciet_heater_v_2_0_steady_state.csv",
        ))
        .unwrap();
        ...
        let mut time_wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "ciet_heater_v_2_0_steady_state_time.csv",
        ))
        .unwrap();
        ...
        let mut temp_profile_wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "ciet_heater_v_2_0_steady_state_temp_profile.csv",
        ))
        .unwrap();
        ...
        while current_time_simulation_time <= *max_time_ptr_in_loop {
            ...
            for (index, fluid_node_heat_trf_entity_ptr) in fluid_vec_in_loop.iter_mut().enumerate()
            {
                ...
                let therminol_temp: ThermodynamicTemperature =
                    HeatTransferEntity::temperature(fluid_node_heat_trf_entity_ptr).unwrap();
                ...
                link_heat_transfer_entity(
                    fluid_node_heat_trf_entity_ptr,
