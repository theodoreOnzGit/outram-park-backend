...
fn lumped_heat_capacitance_steel_ball_in_air() -> Result<(), TuasLibError> {
    ...
    let steel_initial_enthalpy = try_get_h(steel, steel_initial_temperature, pressure)?;
    ...
    //
    // V = 4/3 pi R^3
    let steel_ball_volume: Volume =
        4.0 / 3.0 * PI * steel_ball_radius * steel_ball_radius * steel_ball_radius;

    let steel_ball_density: MassDensity = try_get_rho(steel, steel_initial_temperature, pressure)?;

    let steel_ball_mass: Mass = steel_ball_density * steel_ball_volume;

    let steel_control_vol = HeatTransferEntity::ControlVolume(CVType::SingleCV(SingleCVNode {
        current_timestep_control_volume_specific_enthalpy: steel_initial_enthalpy,
        next_timestep_specific_enthalpy: steel_initial_enthalpy,
        ...
        max_timestep_vector: vec![],
        mesh_stability_lengthscale_vector: vec![],
    ...
    let calculation_loop = move || {
        ...
        while current_time_simulation_time <= *max_time_ptr_in_loop {
            ...
            let temperature_for_export = try_get_temperature_from_h(
            ...
            let mut total_enthalpy_rate_change = Power::new::<watt>(0.0);

            for enthalpy_chg_rate in single_cv.rate_enthalpy_change_vector.clone().iter() {
            ...
            // change
            assert_ne!(total_enthalpy_rate_change, Power::new::<watt>(0.0));

            // now, add the enthalpy change to the next timestep
            //

            let enthalpy_next_timestep = total_enthalpy_rate_change * timestep_in_loop.clone()
                + single_cv
                    .current_timestep_control_volume_specific_enthalpy
                    .clone()
                    * single_cv.mass_control_volume.clone();

            let specific_enthalpy_next_timestep =
                enthalpy_next_timestep / single_cv.mass_control_volume.clone();

            single_cv.next_timestep_specific_enthalpy = specific_enthalpy_next_timestep;
...
#[test]
//#[ignore = "lumped lumped-capacitance-test takes about 1 min, too long"]
fn lumped_heat_capacitance_steel_ball_in_air_improved_api() -> Result<(), TuasLibError> {
    // first, a steel control vol
    // determine parameters first
    let steel = Material::Solid(SolidMaterial::SteelSS304L);
    let pressure = Pressure::new::<atmosphere>(1.0);
    ...
    let steel_control_vol =
        SingleCVNode::new_sphere(diameter, steel, steel_initial_temperature, pressure)?;

    let steel_control_vol: HeatTransferEntity = steel_control_vol.into();
    ...
    let calculation_loop = move || {
        ...
        use csv::Writer;
        let mut wtr = Writer::from_path(crate::vnv_test_support::vnv_csv_path(
            "air_cooled_steel_sphere_api_test.csv",
        ))
        .unwrap();
