...
use panels_and_pages::{
    ciet_data::{CIETState, PagePlotData},
    frequency_response_and_transients::FreqResponseAndTransientSettings,
    full_simulation::educational_ciet_loop_version_4,
    online_calibration::HeaterType,
    Panel,
};
...
impl CIETApp {
    ...
    pub fn new(_cc: &eframe::CreationContext<'_>) -> Self {
        ...
        let ciet_state_ptr: Arc<Mutex<CIETState>> = new_ciet_app.ciet_state.clone();

        // this is the current state of ciet for plotting
        // like the instantaneous temperature and such
        let ciet_state_ptr_for_plotting: Arc<Mutex<CIETState>> = new_ciet_app.ciet_state.clone();
        // for data recording,
        // I'll also clone the pointer and start a thread
        // this contains arrays with historical data
        let ciet_plot_ptr: Arc<Mutex<PagePlotData>> = new_ciet_app
...
impl eframe::App for CIETApp {
    ...
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        if self
            ...
            && self
                .frequency_response_settings
                .frequency_response_switched_on
        {
            // frequency response controls
            // first get current state
            let mut ciet_state_local: CIETState = self.ciet_state.lock().unwrap().clone();
            let current_sim_time = Time::new::<second>(ciet_state_local.simulation_time_seconds);

            let total_heater_power_kw = self
                .frequency_response_settings
                .get_frequency_response_signal(current_sim_time)
                .get::<kilowatt>();
            ciet_state_local.heater_power_kilowatts = total_heater_power_kw;
