...
impl CIETApp {
    pub fn ciet_sim_online_calibration_page(&mut self, ui: &mut Ui) {
        ...
        let heater_type_display: &String = &self.user_desired_heater_type.clone().to_string();
        egui::ComboBox::from_label("Choose Heater Type")
            .selected_text(format!("{heater_type_display}"))
            .show_ui(ui, |ui| {
                ui.selectable_value(
                    &mut self.user_desired_heater_type,
                ...
                ui.selectable_value(
                    ...
                    "Heater V1 Corase (8 Nodes)",
                );
            });
        // displays current heater type
        ui.label("current_heater_type:");

        let local_ciet_state: CIETState = self.ciet_state.lock().unwrap().deref().clone();
