...
impl CIETApp {
    pub fn ciet_sim_heater_page_csv(&mut self, ui: &mut Ui) {
        ...
        egui::ScrollArea::both().show(ui, |ui| {
            ...
            // ptr
            let mut ciet_data_global_ptr_lock = self
            ...
            latest_heater_data.iter().for_each(|data_tuple| {
                let (time, power, bt11, bt12) = data_tuple;

                let time_seconds: f64 = (time.get::<second>() * 1000.0).round() / 1000.0;

                let power_kw: f64 = (power.get::<kilowatt>() * 1000.0).round() / 1000.0;
                let bt11_degc: f64 = (bt11.get::<degree_celsius>() * 1000.0).round() / 1000.0;

                let bt12_degc: f64 = (bt12.get::<degree_celsius>() * 1000.0).round() / 1000.0;

                let heater_data_row: String = time_seconds.to_string()
                    + ","
                    + &power_kw.to_string()
                    + ","
                    + &bt11_degc.to_string()
    ...
    pub fn ciet_sim_heater_page_graph(&mut self, ui: &mut Ui) {
        ...
        egui::ScrollArea::both().show(ui, |ui| {
            let mut bt11_bt12_temp_plot =
                Plot::new("heater inlet and outlet temp degC").legend(Legend::default());
            ...
            bt11_bt12_temp_plot = bt11_bt12_temp_plot
                .x_axis_label("time (seconds), current time (seconds): ".to_owned());
            bt11_bt12_temp_plot =
                bt11_bt12_temp_plot.y_axis_label("temperature degree_celsius".to_owned());
            let latest_ciet_plot_data: PagePlotData = self
                .ciet_plot_data_mutex_ptr_for_parallel_data_transfer
                .lock()
                .unwrap()
            ...
            bt11_bt12_temp_plot.show(ui, |plot_ui| {
                plot_ui.line(Line::new(
                    "bt11 (heater inlet) temperature deg C",
                    PlotPoints::from(time_bt11_vec.clone()),
                ));
                plot_ui.line(Line::new(
                    "bt12 (heater outlet) temperature deg C",
                    PlotPoints::from(time_bt12_vec.clone()),
                ));
            ...
            power_plot = power_plot.y_axis_label("heater power (kW)".to_owned());

            ui.heading("Heater Power vs Time");
            power_plot.show(ui, |plot_ui| {
                plot_ui.line(Line::new(
                    "Heater Power (kW)",
                    PlotPoints::from(time_heater_power_vec),
                ));
            });
