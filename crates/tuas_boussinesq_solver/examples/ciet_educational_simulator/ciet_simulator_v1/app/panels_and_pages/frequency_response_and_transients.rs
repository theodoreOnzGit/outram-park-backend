...
impl FreqResponseAndTransientSettings {
    pub fn get_frequency_response_signal(&self, current_sim_time: Time) -> Power {
        ...
        let sinusoid_amplitude = self.total_amplitude_kw;

        let sinusoid_signal = sinusoid_amplitude * (angular_phase_f64.sin());

        // next I want to have some frequency response

        let total_heater_power = self.steady_state_power_kw + sinusoid_signal;
