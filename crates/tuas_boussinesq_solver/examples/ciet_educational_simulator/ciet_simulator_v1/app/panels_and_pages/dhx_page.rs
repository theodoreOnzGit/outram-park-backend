    ...
    pub fn ciet_sim_dhx_branch_page_graph(&mut self, ui: &mut Ui){
        ...
        egui::ScrollArea::both().show(ui, |ui| {
            ...
            dhx_temp_plot.show(ui, |plot_ui| {
                plot_ui.line(Line::new("Pri Loop DHX Branch (dhx shell inlet) temperature deg C", PlotPoints::from(time_shell_inlet_vec.clone())));
                plot_ui.line(Line::new("Pri Loop DHX Branch (dhx shell outlet) temperature deg C", PlotPoints::from(time_shell_outlet_vec.clone())));
                plot_ui.line(Line::new("DRACS Loop (dhx tube inlet) temperature deg C", PlotPoints::from(time_tube_inlet_vec.clone())));
                plot_ui.line(Line::new("DRACS Loop (dhx tube outlet) temperature deg C", PlotPoints::from(time_tube_outlet_vec.clone())));
            ...
            power_plot.show(ui, |plot_ui| {
                plot_ui.line(Line::new("Shell Side (Pri loop DHX Branch)", PlotPoints::from(time_dhx_br_flowrate_dhx_shell)));
                plot_ui.line(Line::new("Tube Side (DRACS Loop)", PlotPoints::from(time_dracs_loop_flowrate_dhx_tube)));
            });
