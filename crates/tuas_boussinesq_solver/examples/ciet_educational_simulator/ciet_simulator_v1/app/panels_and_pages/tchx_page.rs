...
impl CIETApp {
    ...
    pub fn ciet_sim_tchx_page_graph(&mut self, ui: &mut Ui) {
        ...
        egui::ScrollArea::both().show(ui, |ui| {
            ...
            bt65_bt66_temp_plot = bt65_bt66_temp_plot
                .x_axis_label("time (seconds), current time (seconds): ".to_owned());
            bt65_bt66_temp_plot =
                bt65_bt66_temp_plot.y_axis_label("temperature degree_celsius".to_owned());
            let latest_ciet_plot_data: PagePlotData = self
                .ciet_plot_data_mutex_ptr_for_parallel_data_transfer
                .lock()
                .unwrap()
            ...
            bt65_bt66_temp_plot.show(ui, |plot_ui| {
                plot_ui.line(Line::new(
                    "bt65 (tchx inlet) temperature deg C",
                    PlotPoints::from(time_bt65_vec.clone()),
                ));
                plot_ui.line(Line::new(
                    "bt66 (tchx outlet) temperature deg C",
                    PlotPoints::from(time_bt66_vec.clone()),
                ));

                plot_ui.line(Line::new(
                    "bt66 (tchx outlet) set pt deg C",
                    PlotPoints::from(time_bt66_setpt_vec.clone()),
                ));
            ...
            power_plot.show(ui, |plot_ui| {
                plot_ui.line(Line::new("TCHX HTC", PlotPoints::from(time_tchx_htc_vec)));
            });
